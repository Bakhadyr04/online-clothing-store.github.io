...
# so we don't have to differentiate between "I'm using a context manager"
# and "I'm not"
...
class AIORateLimiter(BaseRateLimiter[int]):
    ...
    def _get_group_limiter(
            self, group_id: Union[str, int, bool]) -> "AsyncLimiter":
        ...
        # or at least adapt the threshold dynamically depending on the number
        # of active limiters
    ...
    async def _run_request(
        ...
        base_context = self._base_limiter if (
            chat and self._base_limiter) else null_context()
        group_context = (
            self._get_group_limiter(
                group) if group and self._group_max_rate else null_context()
        )

        async with group_context:  # skipcq: PTC-W0062
            async with base_context:
                # In case a retry_after was hit, we wait with processing the
                # request
                await self._retry_after_event.wait()

                return await callback(*args, **kwargs)

    # mypy doesn't understand that the last run of the for loop raises an
    # exception
    async def process_request(  # type: ignore[return]
        ...
        for i in range(max_retries + 1):
            try:
                ...
                self._logger.info(
                    "Rate limit hit. Retrying after %f seconds", sleep)
