...
_UpdaterType = TypeVar(
    "_UpdaterType",
    bound="Updater")  # pylint: disable=invalid-name
...
class Updater(AsyncContextManager["Updater"]):
        ...
        async def polling_action_cb() -> bool:
            ...
            if updates:
                if not self.running:
                    ...
                    # Add one to 'confirm' it
                    self._last_update_id = updates[-1].update_id + 1

            # Keep fetching updates & don't quit. Polls with poll_interval.
            return True
    ...
    @staticmethod
    def _gen_webhook_url(protocol: str, listen: str,
                         port: int, url_path: str) -> str:
        ...
        while self.running:
            try:
                try:
                    ...
                    self._logger.error(
                        "Error while %s: %s", description, telegram_exc)
                    on_err_cb(telegram_exc)

                    # increase waiting times on subsequent errors up to 30secs
                    cur_interval = 1 if cur_interval == 0 else min(
                        30, 1.5 * cur_interval)
                else:
                    cur_interval = interval
                ...
                self._logger.debug(
                    "Network loop retry %s was cancelled", description)
                break
    ...
    async def _bootstrap(
        ...
        def bootstrap_on_err_cb(exc: Exception) -> None:
            ...
            if not isinstance(exc, InvalidToken) and (
                ...
                self._logger.error(
                    "Failed bootstrap phase after %s retries (%s)", retries, exc)
                raise exc
