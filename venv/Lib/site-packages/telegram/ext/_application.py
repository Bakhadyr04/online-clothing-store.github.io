...
_AppType = TypeVar(
    "_AppType",
    bound="Application")  # pylint: disable=invalid-name
_RT = TypeVar("_RT")
_STOP_SIGNAL = object()
...
class Application(Generic[BT, CCT, UD, CD, BD, JQ],
                  AsyncContextManager["Application"]):
    ...
    def __init__(
        ...
        *,
        ...
        post_init: Optional[
            Callable[["Application[BT, CCT, UD, CD, BD, JQ]"],
        ...
        # handlers so that we can extract the changed states during
        # `update_persistence`
        ...
        # Used for awaiting tasks upon exit
    ...
    async def initialize(self) -> None:
        ...
        * The :attr:`persistence`, by loading persistent conversations and data.
        ...
        # Initialize the persistent conversation handlers with the stored
        # states
        for handler in itertools.chain.from_iterable(self.handlers.values()):
            if isinstance(
                    handler, ConversationHandler) and handler.persistent and handler.name:
                await self._add_ch_to_persistence(handler)

        self._initialized = True

    async def _add_ch_to_persistence(
            self, handler: "ConversationHandler") -> None:
    ...
    async def _initialize_persistence(self) -> None:
        ...
        if self.persistence.store_data.callback_data and (
            ...
            self.bot.callback_data_cache is not None
        ):
            persistent_data = await self.persistence.get_callback_data()
            if persistent_data is not None:
                if not isinstance(persistent_data, tuple) or len(
                        persistent_data) != 2:
                    raise ValueError(
    ...
    def __run(
        ...
        try:
            ...
            # In case the coroutine wasn't awaited, we don't need to bother the
            # user with a warning
            updater_coroutine.close()
            raise exc
        finally:
            # We arrive here either by catching the exceptions above or if the
            # loop gets stopped
            try:
                # Mypy doesn't know that we already check if updater is None
                if self.updater.running:  # type: ignore[union-attr]
                    loop.run_until_complete(
                        self.updater.stop())  # type: ignore[union-attr]
                if self.running:
    ...
    def __create_task_done_callback(self, task: asyncio.Task) -> None:
        # Discard from our set since we are done with it
    ...
    async def __create_task_callback(
        ...
        try:
            ...
            # Raise exception so that it can be set on the task and retrieved
            # by task.exception()
            raise exception
        finally:
    ...
    async def _update_fetcher(self) -> None:
        # Continuously fetch updates from the queue. Exit only once the signal
        # object is found.
        while True:
            ...
            if self._concurrent_updates:
                # We don't await the below because it has to be run
                # concurrently
                self.create_task(
    ...
    async def process_update(self, update: object) -> None:
        ...
        """
        # Processing updates before initialize() is a problem e.g. if
        # persistence is used
        self._check_initialized()
        ...
        for handlers in self.handlers.values():
            try:
                for handler in handlers:
                    # Should the handler handle this update?
                    ...
                    if not (check is None or check is False):  # if yes,
                        ...
                        coroutine: Coroutine = handler.handle_update(
                            update, self, check, context)
            ...
            # Stop processing with any other handler.
                ...
                _logger.debug(
                    "Stopping further handlers due to ApplicationHandlerStop")
                break
    ...
    def add_handler(
            self, handler: BaseHandler[Any, CCT], group: int = DEFAULT_GROUP) -> None:
        """Register a handler.
        ...
        if not isinstance(handler, BaseHandler):
            raise TypeError(
                f"handler is not an instance of {BaseHandler.__name__}")
    ...
    def add_handlers(
        ...
        handlers: Union[
            ...
            Dict[int, Union[List[BaseHandler[Any, CCT]],
                            Tuple[BaseHandler[Any, CCT]]]],
        ],
        ...
        if isinstance(handlers, dict) and not isinstance(group, DefaultValue):
            raise ValueError(
                "The `group` argument can only be used with a sequence of handlers.")

        if isinstance(handlers, dict):
            for handler_group, grp_handlers in handlers.items():
                if not isinstance(grp_handlers, (list, tuple)):
                    raise ValueError(
                        f"Handlers for group {handler_group} must be a list or tuple")
    ...
    def _mark_for_persistence_update(
            self, *, update: object = None, job: "Job" = None) -> None:
        if isinstance(update, Update):
            if update.effective_chat:
                self._chat_ids_to_be_updated_in_persistence.add(
                    update.effective_chat.id)
            if update.effective_user:
                self._user_ids_to_be_updated_in_persistence.add(
    ...
    async def _persistence_updater(self) -> None:
        # Update the persistence in regular intervals. Exit only when the stop
        # event has been set
            ...
            for chat_id in update_ids:
                coroutines.add(
                    self.persistence.update_chat_data(
                        chat_id, deepcopy(self.chat_data[chat_id]))
                )
            ...
            for user_id in update_ids:
                coroutines.add(
                    self.persistence.update_user_data(
                        user_id, deepcopy(self.user_data[user_id]))
                )
        ...
        for name, (key, new_state) in itertools.chain.from_iterable(
            ...
            if isinstance(new_state, PendingState):
                ...
                if not new_state.done():
                    ...
                    # We need to check again on the next run if the state is
                    # done
                    self._conversation_handler_conversations[name].mark_as_accessed(
                        key)
                else:
    ...
    def remove_error_handler(
            self, callback: HandlerCallback[object, CCT, None]) -> None:
