...
class DictPersistence(
        BasePersistence[Dict[Any, Any], Dict[Any, Any], Dict[Any, Any]]):
    ...
    def __init__(
        ...
        if user_data_json:
            try:
                self._user_data = self._decode_user_chat_data_from_json(
                    user_data_json)
                self._user_data_json = user_data_json
            except (ValueError, AttributeError) as exc:
                raise TypeError(
                    "Unable to deserialize user_data_json. Not valid JSON") from exc
        if chat_data_json:
            try:
                self._chat_data = self._decode_user_chat_data_from_json(
                    chat_data_json)
                self._chat_data_json = chat_data_json
            except (ValueError, AttributeError) as exc:
                raise TypeError(
                    "Unable to deserialize chat_data_json. Not valid JSON") from exc
        if bot_data_json:
            try:
                ...
                raise TypeError(
                    "Unable to deserialize bot_data_json. Not valid JSON") from exc
        ...
        if callback_data_json:
            ...
            try:
                ...
                raise TypeError(
                    "callback_data_json is not in the required format") from exc
        ...
        if conversations_json:
            try:
                self._conversations = self._decode_conversations_from_json(
                    conversations_json)
    ...
    async def get_conversations(self, name: str) -> ConversationDict:
        ...
        return self.conversations.get(
            name, {}).copy()  # type: ignore[union-attr]
    ...
    async def update_user_data(
            self, user_id: int, data: Dict[Any, Any]) -> None:
    ...
    async def update_chat_data(
            self, chat_id: int, data: Dict[Any, Any]) -> None:
    ...
    async def refresh_user_data(
            self, user_id: int, user_data: Dict[Any, Any]) -> None:
        """Does nothing.
    ...
    async def refresh_chat_data(
            self, chat_id: int, chat_data: Dict[Any, Any]) -> None:
        """Does nothing.
    ...
    @staticmethod
    def _encode_conversations_to_json(
            conversations: Dict[str, ConversationDict]) -> str:
    ...
    @staticmethod
    def _decode_conversations_from_json(
            json_string: str) -> Dict[str, ConversationDict]:
    ...
    @staticmethod
    def _decode_user_chat_data_from_json(
            data: str) -> Dict[int, Dict[object, object]]:
