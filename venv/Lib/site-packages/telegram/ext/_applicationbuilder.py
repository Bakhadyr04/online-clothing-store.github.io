...
class ApplicationBuilder(Generic[BT, CCT, UD, CD, BD, JQ]):
    ...
    def __init__(self: "InitApplicationBuilder"):
        ...
        self._base_url: DVType[str] = DefaultValue(
            "https://api.telegram.org/bot")
        self._base_file_url: DVType[str] = DefaultValue(
        ...
        self._arbitrary_callback_data: Union[DefaultValue[bool],
                                             int] = DEFAULT_FALSE
        ...
        self._context_types: DVType[ContextTypes] = DefaultValue(
            ContextTypes())
        self._application_class: DVType[Type[Application]] = DefaultValue(
            Application)
        self._application_kwargs: Dict[str, object] = {}
        self._concurrent_updates: Union[int,
                                        DefaultValue[bool]] = DEFAULT_FALSE
        self._updater: ODVInput[Updater] = DEFAULT_NONE
        self._post_init: Optional[Callable[[
            Application], Coroutine[Any, Any, None]]] = None
        self._post_shutdown: Optional[Callable[[
            Application], Coroutine[Any, Any, None]]] = None
        self._post_stop: Optional[Callable[[
            Application], Coroutine[Any, Any, None]]] = None
    ...
    def _build_request(self, get_updates: bool) -> BaseRequest:
        ...
        if get_updates:
            ...
            connection_pool_size = (
                DefaultValue.get_value(
                    getattr(
                        self,
                        f"{prefix}connection_pool_size")) or 256
            )
        ...
        http_version = DefaultValue.get_value(
            getattr(self, f"{prefix}http_version")) or "2"
    ...
    def build(
        ...
        if persistence is not None:
            ...
            # but self.bot is not an instance of ExtBot - so no need to check
            # that later on
    ...
    def _request_check(self, get_updates: bool) -> None:
        ...
        for attr in ("connect_timeout", "read_timeout",
        ...
        if not isinstance(
                getattr(self, f"_{prefix}connection_pool_size"), DefaultValue):
            raise RuntimeError(
        ...
        if not isinstance(
                getattr(self, f"_{prefix}http_version"), DefaultValue):
    ...
    def _request_param_check(self, name: str, get_updates: bool) -> None:
        ...
        # disallow request args if bot is set (has Request)
        if self._bot is not DEFAULT_NONE:
            raise RuntimeError(
                _TWO_ARGS_REQ.format(
        ...
        if self._updater not in (
                DEFAULT_NONE, None):  # disallow request args for updater(has bot)
            raise RuntimeError(
                _TWO_ARGS_REQ.format(
                    f"get_updates_{name}" if get_updates else name, "updater")
            )
    ...
    def connection_pool_size(self: BuilderType,
                             connection_pool_size: int) -> BuilderType:
    ...
    def connect_timeout(self: BuilderType,
                        connect_timeout: Optional[float]) -> BuilderType:
    ...
    def read_timeout(self: BuilderType,
                     read_timeout: Optional[float]) -> BuilderType:
    ...
    def write_timeout(self: BuilderType,
                      write_timeout: Optional[float]) -> BuilderType:
    ...
    def pool_timeout(self: BuilderType,
                     pool_timeout: Optional[float]) -> BuilderType:
    ...
    def get_updates_request(self: BuilderType,
                            get_updates_request: BaseRequest) -> BuilderType:
        """Sets a :class:`telegram.request.BaseRequest` instance for the
    ...
    def get_updates_proxy_url(self: BuilderType,
                              get_updates_proxy_url: str) -> BuilderType:
        """Sets the proxy for the :paramref:`telegram.request.HTTPXRequest.proxy_url`
    ...
    def get_updates_http_version(
            self: BuilderType, get_updates_http_version: str) -> BuilderType:
        """Sets the HTTP protocol version which is used for the
    ...
    def private_key(
        ...
        self._private_key = (
            private_key if isinstance(
                private_key, bytes) else Path(private_key).read_bytes()
        )
    ...
    def update_queue(self: BuilderType,
                     update_queue: "Queue[object]") -> BuilderType:
    ...
    def concurrent_updates(
            self: BuilderType, concurrent_updates: Union[bool, int]) -> BuilderType:
