...
_CheckUpdateType = Tuple[object, ConversationKey,
                         BaseHandler[Update, CCT], object]
...
class PendingState:
    ...
    def resolve(self) -> object:
        ...
        if res is None and self.old_state is None:
            ...
            # returning None from a callback means that we want to stay in the
            # old state
            return self.old_state
...
class ConversationHandler(BaseHandler[Update, CCT]):
    ...
    def __init__(
        ...
        # these imports need to be here because of circular import error
        # otherwise
        ...
        if persistent and not self.name:
            raise ValueError(
                "Conversations can't be persistent when handler is unnamed.")
        ...
        for handler in all_handlers:
            ...
            if self.conversation_timeout and isinstance(
    ...
    def states(self, value: object) -> NoReturn:
        raise AttributeError(
            "You can not assign a new value to states after initialization.")
    ...
    def per_user(self, value: object) -> NoReturn:
        raise AttributeError(
            "You can not assign a new value to per_user after initialization.")
    ...
    def per_chat(self, value: object) -> NoReturn:
        raise AttributeError(
            "You can not assign a new value to per_chat after initialization.")
    ...
    def per_message(self, value: object) -> NoReturn:
        raise AttributeError(
            "You can not assign a new value to per_message after initialization.")
    ...
    def name(self, value: object) -> NoReturn:
        raise AttributeError(
            "You can not assign a new value to name after initialization.")
    ...
    def persistent(self, value: object) -> NoReturn:
        raise AttributeError(
            "You can not assign a new value to persistent after initialization.")
    ...
    def _get_key(self, update: Update) -> ConversationKey:
        ...
        if self.per_chat:
            if chat is None:
                raise RuntimeError(
                    "Can't build key for update without effective chat!")
            key.append(chat.id)
    ...
    def _schedule_job(
        ...
        try:
            # both job_queue & conversation_timeout are checked before calling
            # _schedule_job
    ...
    def check_update(self, update: object) -> Optional[_CheckUpdateType[CCT]]:
        ...
        if isinstance(state, PendingState):
            ...
            if state.done():
                res = state.resolve()
                # Special case if an error was raised in a non-blocking
                # entry-point
        ...
        _logger.debug(
            "Selecting conversation %s with state %s",
            str(key),
            str(state))
        ...
        # Get the handler list for current state, if we didn't find one yet and
        # we're still here
        ...
        async with self._timeout_jobs_lock:
            if self.conversation_timeout:
                if application.job_queue is None:
                    ...
                    # checking if the new state is self.END is done in
                    # _schedule_job
