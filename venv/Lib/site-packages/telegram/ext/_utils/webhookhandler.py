...
# this module. Doing it here would be tricky, as the classes below
# subclass tornado classes
import tornado.web
    ...
    async def shutdown(self) -> None:
        async with self._shutdown_lock:
            if not self.is_running:
                self._logger.debug(
                    "Webhook Server is already shut down. Returning")
                return
            self.is_running = False
...
class TelegramHandler(tornado.web.RequestHandler):
    ...
    def initialize(self, bot: "Bot", update_queue: asyncio.Queue,
    ...
    async def post(self) -> None:
        ...
        if update:
            self._logger.debug(
                ...
                # For some reason pylint thinks update is a general
                # TelegramObject
    ...
    def _validate_post(self) -> None:
        ...
        # verifying that the secret token is the one the user set when the user
        # set one
        if self.secret_token is not None:
            ...
            if token != self.secret_token:
                self._logger.debug(
                    "Request had the wrong secret token: %s", token)
                raise tornado.web.HTTPError(
