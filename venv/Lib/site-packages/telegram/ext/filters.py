...
class MessageFilter(BaseFilter):
    ...
    def check_update(
            self, update: Update) -> Optional[Union[bool, FilterDataDict]]:
    ...
    @abstractmethod
    def filter(
            self, message: Message) -> Optional[Union[bool, FilterDataDict]]:
...
class _MergedFilter(UpdateFilter):
    ...
    def __init__(
        ...
        if self.or_filter and not isinstance(
                self.and_filter, bool) and self.or_filter.data_filter:
            self.data_filter = True

    @staticmethod
    def _merge(base_output: Union[bool, Dict],
               comp_output: Union[bool, Dict]) -> FilterDataDict:
...
class Caption(MessageFilter):
    ...
    def __init__(self, strings: Union[List[str], Tuple[str, ...]] = None):
        ...
        super().__init__(
            name=f"filters.Caption({strings})" if strings else "filters.CAPTION")
...
class CaptionEntity(MessageFilter):
    ...
    def filter(self, message: Message) -> bool:
        return any(entity.type ==
                   self.entity_type for entity in message.caption_entities)
...
class CaptionRegex(MessageFilter):
    ...
    def __init__(self, pattern: Union[str, Pattern[str]]):
        ...
        super().__init__(
            name=f"filters.CaptionRegex({self.pattern})",
            data_filter=True)

    def filter(
            self, message: Message) -> Optional[Dict[str, List[Match[str]]]]:
        if message.caption:
...
class _ChatUserBaseFilter(MessageFilter, ABC):
    ...
    def _get_chat_or_user(
            self, message: Message) -> Union[TGChat, TGUser, None]:
        ...
    ...
    def _parse_username(username: Optional[SCT[str]]) -> Set[str]:
        ...
        return {chat[1:] if chat.startswith(
            "@") else chat for chat in username}
    ...
    def filter(self, message: Message) -> bool:
        ...
        if chat_or_user:
            ...
            if self.usernames:
                return bool(
                    chat_or_user.username and chat_or_user.username in self.usernames)
...
class Command(MessageFilter):
    ...
    def __init__(self, only_start: bool = True):
        ...
        super().__init__(
            f"filters.Command({only_start})" if not only_start else "filters.COMMAND")
    ...
    def filter(self, message: Message) -> bool:
        ...
        if self.only_start:
            return bool(
                first.type == MessageEntity.BOT_COMMAND and first.offset == 0)
        return bool(
            any(e.type == MessageEntity.BOT_COMMAND for e in message.entities))
...
class Entity(MessageFilter):
    ...
    def filter(self, message: Message) -> bool:
        return any(entity.type ==
                   self.entity_type for entity in message.entities)
...
class ForwardedFrom(_ChatUserBaseFilter):
    ...
    def _get_chat_or_user(
            self, message: Message) -> Union[TGUser, TGChat, None]:
...
class Regex(MessageFilter):
    ...
    def __init__(self, pattern: Union[str, Pattern[str]]):
        ...
        self.pattern: Pattern[str] = pattern
        super().__init__(
            name=f"filters.Regex({self.pattern})",
            data_filter=True)
...
class Sticker:
    ...
    class _Animated(MessageFilter):
        ...
        def filter(self, message: Message) -> bool:
            return bool(message.sticker) and bool(
                message.sticker.is_animated)  # type: ignore
    ...
    class _Static(MessageFilter):
        ...
        def filter(self, message: Message) -> bool:
            return bool(message.sticker) and (
                ...
                not bool(message.sticker.is_animated)
                # type: ignore[union-attr]
                and not bool(message.sticker.is_video)
            )
    ...
    class _Video(MessageFilter):
        ...
        def filter(self, message: Message) -> bool:
            return bool(message.sticker) and bool(
                message.sticker.is_video)  # type: ignore
