...
HandledTypes = TypeVar(
    "HandledTypes", bound=Union[Message, CallbackQuery, Chat])
...
class ExtBot(Bot, Generic[RLARGS]):
    ...
    # using object() would be a tiny bit safer, but a string plays better with
    # the typing setup
    __RL_KEY = uuid4().hex
    ...
    def rate_limiter(self) -> Optional["BaseRateLimiter[RLARGS]"]:
        ...
        .. versionadded:: 20.0
        """
        # This is a property because the rate limiter shouldn't be changed at
        # runtime
    ...
    def _replace_keyboard(
            self, reply_markup: Optional[ReplyMarkup]) -> Optional[ReplyMarkup]:
        # If the reply_markup is an inline keyboard and we allow arbitrary callback data, let the
        # CallbackDataCache build a new keyboard with the data replaced.
        # Otherwise return the input
        if isinstance(
                reply_markup, InlineKeyboardMarkup) and self.callback_data_cache is not None:
    ...
    def insert_callback_data(self, update: Update) -> None:
        ...
        # Finally there is effective_chat.pinned message, but that's only
    ...
    def _insert_callback_data(self, obj: HandledTypes) -> HandledTypes:
        ...
        if isinstance(obj, Message):
            if obj.reply_to_message:
                # reply_to_message can't contain further reply_to_messages, so
                # no need to check
                self.callback_data_cache.process_message(obj.reply_to_message)
                if obj.reply_to_message.pinned_message:
                    # pinned messages can't contain reply_to_message, no need
                    # to check
                    self.callback_data_cache.process_message(
                        obj.reply_to_message.pinned_message)
            if obj.pinned_message:
                # pinned messages can't contain reply_to_message, no need to
                # check
