...
def _custom_reduction(
        cls: TelegramObj) -> Tuple[Callable, Tuple[Type[TelegramObj], dict]]:
    """
    This method is used for pickling. The bot attribute is preserved so _BotPickler().persistent_id
    works as intended.
    """
    data = cls._get_attrs(
        include_private=True)  # pylint: disable=protected-access
    # MappingProxyType is not pickable, so we convert it to a dict
    # no need to convert back to MPT in _reconstruct_to, since it's done in
    # __setstate__
...
class _BotPickler(pickle.Pickler):
    ...
    def __init__(self, bot: Bot, *args: Any, **kwargs: Any):
        ...
        if py_ver < (3, 8):  # self.reducer_override is used above this version
            ...
            # is deleted in __getstate__, which is used during regular pickling
            # (via pickle.dumps)
...
class PicklePersistence(BasePersistence[UD, CD, BD]):
    ...
    def _load_singlefile(self) -> None:
        try:
            ...
            raise TypeError(
                f"File {filename} does not contain valid pickle data") from exc
        except Exception as exc:
            raise TypeError(
                f"Something went wrong unpickling {self.filepath.name}") from exc
    ...
    def _load_file(self, filepath: Path) -> Any:
        try:
            ...
            raise TypeError(
                f"File {filepath.name} does not contain valid pickle data") from exc
        except Exception as exc:
            raise TypeError(
                f"Something went wrong unpickling {filepath.name}") from exc
    ...
    def _dump_singlefile(self) -> None:
        ...
        with self.filepath.open("wb") as file:
            _BotPickler(
                self.bot,
                file,
                protocol=pickle.HIGHEST_PROTOCOL).dump(data)
    ...
    async def update_conversation(
        ...
        if not self.on_flush:
            if not self.single_file:
                self._dump_file(
                    Path(f"{self.filepath}_conversations"),
                    self.conversations)
            else:
    ...
    async def update_user_data(self, user_id: int, data: UD) -> None:
        ...
        if not self.on_flush:
            if not self.single_file:
                self._dump_file(
                    Path(f"{self.filepath}_user_data"),
                    self.user_data)
            else:
                self._dump_singlefile()
    ...
    async def update_chat_data(self, chat_id: int, data: CD) -> None:
        ...
        if not self.on_flush:
            if not self.single_file:
                self._dump_file(
                    Path(f"{self.filepath}_chat_data"),
                    self.chat_data)
            else:
                self._dump_singlefile()
    ...
    async def update_bot_data(self, data: BD) -> None:
        ...
        if not self.on_flush:
            if not self.single_file:
                self._dump_file(
                    Path(f"{self.filepath}_bot_data"),
                    self.bot_data)
            else:
                self._dump_singlefile()
    ...
    async def update_callback_data(self, data: CDCData) -> None:
        ...
        if not self.on_flush:
            if not self.single_file:
                self._dump_file(
                    Path(f"{self.filepath}_callback_data"),
                    self.callback_data)
            else:
                self._dump_singlefile()
