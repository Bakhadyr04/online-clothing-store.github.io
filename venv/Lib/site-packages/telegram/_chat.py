...
class Chat(TelegramObject):
    ...
    def de_json(cls, data: Optional[JSONDict], bot: "Bot") -> Optional["Chat"]:
        ...
        data["permissions"] = ChatPermissions.de_json(
    ...
    def mention_markdown(self, name: str = None) -> str:
        ...
        if self.type == self.PRIVATE:
            ...
            raise TypeError(
                "Can not create a mention to a private chat without first name")
        if self.username:
            if name:
            ...
            raise TypeError(
                "Can not create a mention to a public chat without title")
        ...
        if self.type == self.PRIVATE:
            ...
            if self.full_name:
                return helpers_mention_markdown(
                    self.id, self.full_name, version=2)
            raise TypeError(
                "Can not create a mention to a private chat without first name")
        if self.username:
            if name:
