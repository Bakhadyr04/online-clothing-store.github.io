...
class ChatMemberUpdated(TelegramObject):
    ...
    def de_json(cls, data: Optional[JSONDict],
        ...
        data["invite_link"] = ChatInviteLink.de_json(
            data.get("invite_link"), bot)

        return super().de_json(data=data, bot=bot)

    def _get_attribute_difference(
            self, attribute: str) -> Tuple[object, object]:
        try:
    ...
    def difference(
        ...
        Tuple[
            Union[str, bool, datetime.datetime,
                  User], Union[str, bool, datetime.datetime, User]
        ],
    ]:
        ...
        # user.to_dict() is unhashable, so that needs some special casing
        # further down
        ...
        # we can't directly use the values from old_dict ^ new_dict b/c that
        # set is unordered
        attributes = (
            entry[0] for entry in set(
                old_dict.items()) ^ set(
                new_dict.items()))

        result = {attribute: self._get_attribute_difference(
            attribute) for attribute in attributes}
        if old_user_dict != new_user_dict:
            result["user"] = (
                self.old_chat_member.user,
