    ...
    def _get_encoded_url(self) -> str:
        ...
        return urllib_parse.urlunsplit(
            urllib_parse.SplitResult(
                sres.scheme, sres.netloc, urllib_parse.quote(
                    sres.path), sres.query, sres.fragment
            )
        )

    def _prepare_decrypt(self, buf: bytes) -> bytes:
        return decrypt(b64decode(self._credentials.secret),
                       b64decode(self._credentials.hash), buf)
        ...
        *,
        ...
        if local_file and self._credentials:
            ...
            if custom_path is not None:
                ...
                path = Path(str(file_to_decrypt.parent) +
                            "/decrypted_" + file_to_decrypt.name)
