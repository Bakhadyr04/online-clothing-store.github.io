...
class Message(TelegramObject):
    ...
    def __init__(
        ...
        *,
        ...
        self.entities: Tuple["MessageEntity", ...] = parse_sequence_arg(
            entities)
        self.caption_entities: Tuple["MessageEntity", ...] = parse_sequence_arg(
            caption_entities)
    ...
    def de_json(cls, data: Optional[JSONDict],
        ...
        data["caption_entities"] = MessageEntity.de_list(
            data.get("caption_entities"), bot)
        data["forward_from"] = User.de_json(data.get("forward_from"), bot)
        data["forward_from_chat"] = Chat.de_json(
        ...
        data["new_chat_members"] = User.de_list(
            data.get("new_chat_members"), bot)
        data["left_chat_member"] = User.de_json(
            data.get("left_chat_member"), bot)
        data["new_chat_photo"] = PhotoSize.de_list(
        ...
        data["video_chat_started"] = VideoChatStarted.de_json(
            data.get("video_chat_started"), bot)
        data["video_chat_ended"] = VideoChatEnded.de_json(
        ...
        data["web_app_data"] = WebAppData.de_json(
            data.get("web_app_data"), bot)
        data["forum_topic_closed"] = ForumTopicClosed.de_json(
            data.get("forum_topic_closed"), bot)
    ...
    def _quote(self, quote: Optional[bool],
               reply_to_message_id: Optional[int]) -> Optional[int]:
        ...
        if quote is not None:
            ...
            # logic into ExtBot
            if hasattr(self.get_bot(), "defaults") and self.get_bot(
            ).defaults:  # type: ignore
                # type: ignore[attr-defined]
                default_quote = self.get_bot().defaults.quote
            else:
                default_quote = None
            if (default_quote is None and self.chat.type !=
                    Chat.PRIVATE) or default_quote:
                return self.message_id
    ...
    async def reply_media_group(
        self,
        media: Sequence[
            Union["InputMediaAudio", "InputMediaDocument",
    ...
    def parse_entity(self, entity: MessageEntity) -> str:
        ...
        entity_text = entity_text[entity.offset *
                                  2: (entity.offset + entity.length) * 2]
    ...
    def parse_entities(
            self, types: List[str] = None) -> Dict[MessageEntity, str]:
        """
    ...
    def _parse_html(
        ...
        if offset == 0:
            if sys.maxunicode == 0xFFFF:
                ...
                html_text += escape(
                    message_text[last_offset *
                                 2:].decode("utf-16-le")  # type: ignore
                )
        else:
            if sys.maxunicode == 0xFFFF:
                html_text += message_text[last_offset:]
    ...
    def caption_html(self) -> str:
        ...
        return self._parse_html(
            self.caption, self.parse_caption_entities(), urled=False)
    ...
    def caption_html_urled(self) -> str:
        ...
        """
        return self._parse_html(
            self.caption, self.parse_caption_entities(), urled=True)
            ...
            if entity not in parsed_entities:
                ...
                if entity.type == MessageEntity.TEXT_LINK:
                    if version == 1:
                        ...
                        # Links need special escaping. Also can't have entities
                        # nested within
                        url = escape_markdown(
                    ...
                    # Monospace needs special escaping. Also can't have
                ...
                if offset == 0:
                    if sys.maxunicode == 0xFFFF:
                        markdown_text += (
                            escape_markdown(
                                message_text[last_offset: entity.offset -
        ...
        if offset == 0:
            if sys.maxunicode == 0xFFFF:
                markdown_text += escape_markdown(
                    message_text[last_offset:], version=version)
            else:
                markdown_text += escape_markdown(
                    message_text[last_offset *
                                 2:].decode("utf-16-le"),  # type: ignore
    ...
    def text_markdown(self) -> str:
        ...
        return self._parse_markdown(
            self.text, self.parse_entities(), urled=False)
    ...
    def text_markdown_v2(self) -> str:
        ...
        """
        return self._parse_markdown(
            self.text, self.parse_entities(), urled=False, version=2)
    ...
    def text_markdown_urled(self) -> str:
        ...
        """
        return self._parse_markdown(
            self.text, self.parse_entities(), urled=True)
    ...
    def text_markdown_v2_urled(self) -> str:
        ...
        """
        return self._parse_markdown(
            self.text, self.parse_entities(), urled=True, version=2)
