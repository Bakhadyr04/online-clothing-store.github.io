...
try:
    ...
    MGF1, OAEP, Cipher, AES, CBC = (None, None, None, None, None)
    # type: ignore[misc,assignment]
    SHA1, SHA256, SHA512, Hash = (None, None, None, None)
...
def decrypt(secret, hash, data):  # skipcq: PYL-W0622
    ...
    if data_hash != hash:
        # Raise a error that is caught inside telegram.PassportData and
    ...
    def decrypted_data(self) -> "Credentials":
        ...
        if self._decrypted_data is None:
            self._decrypted_data = Credentials.de_json(
                decrypt_json(
                    self.decrypted_secret, b64decode(
                        self.hash), b64decode(
                        self.data)),
                self.get_bot(),
            )
...
class Credentials(TelegramObject):
    ...
    def de_json(cls, data: Optional[JSONDict],
        ...
        data["secure_data"] = SecureData.de_json(
            data.get("secure_data"), bot=bot)
...
class SecureData(TelegramObject):
    ...
    def de_json(cls, data: Optional[JSONDict],
        ...
        data["rental_agreement"] = SecureValue.de_json(
            data.get("rental_agreement"), bot=bot)
        data["bank_statement"] = SecureValue.de_json(
            data.get("bank_statement"), bot=bot)
        data["utility_bill"] = SecureValue.de_json(
            data.get("utility_bill"), bot=bot)
        data["address"] = SecureValue.de_json(data.get("address"), bot=bot)
        data["identity_card"] = SecureValue.de_json(
            data.get("identity_card"), bot=bot)
        data["driver_license"] = SecureValue.de_json(
            data.get("driver_license"), bot=bot)
        data["internal_passport"] = SecureValue.de_json(
            data.get("internal_passport"), bot=bot)
        data["passport"] = SecureValue.de_json(data.get("passport"), bot=bot)
        data["personal_details"] = SecureValue.de_json(
            data.get("personal_details"), bot=bot)
...
class SecureValue(TelegramObject):
    ...
    def __init__(
        ...
        *,
        ...
        self.translation: Tuple["FileCredentials", ...] = parse_sequence_arg(
            translation)
    ...
    def de_json(cls, data: Optional[JSONDict],
        ...
        data["front_side"] = FileCredentials.de_json(
            data.get("front_side"), bot=bot)
        data["reverse_side"] = FileCredentials.de_json(
            data.get("reverse_side"), bot=bot)
        data["selfie"] = FileCredentials.de_json(data.get("selfie"), bot=bot)
        data["files"] = FileCredentials.de_list(data.get("files"), bot=bot)
        data["translation"] = FileCredentials.de_list(
            data.get("translation"), bot=bot)
...
class DataCredentials(_CredentialsBase):
    ...
    def __init__(self, data_hash: str, secret: str, *,
                 api_kwargs: JSONDict = None):
...
class FileCredentials(_CredentialsBase):
    ...
    def __init__(self, file_hash: str, secret: str, *,
                 api_kwargs: JSONDict = None):
