...
class EncryptedPassportElement(TelegramObject):
    ...
    def __init__(
        ...
        *,
        ...
        self.translation: Tuple[PassportFile, ...] = parse_sequence_arg(
            translation)
        self.hash: str = hash
    ...
    def de_json(cls, data: Optional[JSONDict],
        ...
        data["reverse_side"] = PassportFile.de_json(
            data.get("reverse_side"), bot)
        data["selfie"] = PassportFile.de_json(data.get("selfie"), bot)
        data["translation"] = PassportFile.de_list(
            data.get("translation"), bot) or None
        ...
        if data["type"] not in ("phone_number", "email"):
            ...
            if secure_data.data is not None:
                ...
                if data["type"] == "personal_details":
                    data["data"] = PersonalDetails.de_json(
                        data["data"], bot=bot)
                elif data["type"] in (
                    "passport",
                    "internal_passport",
                    ...
                    data["data"] = IdDocumentData.de_json(
                        data["data"], bot=bot)
                elif data["type"] == "address":
                    data["data"] = ResidentialAddress.de_json(
                        data["data"], bot=bot)

            data["files"] = (
                PassportFile.de_list_decrypted(
                    data.get("files"), bot, secure_data.files) or None
            )
