...
class TelegramObject:
    ...
    def __init__(self, *, api_kwargs: JSONDict = None) -> None:
        ...
        # We don't do anything with api_kwargs here - see docstring of
        # _apply_api_kwargs
    ...
    def _apply_api_kwargs(self, api_kwargs: JSONDict) -> None:
        ...
        * There probably is no use case for it anyway. If you manually initialize a TO subclass,
        ...
        """
        # we convert to list to ensure that the list doesn't change length
        # while we loop
    ...
    def __repr__(self) -> str:
        ...
        # In our case "unambiguous" and "readable" largely coincide, so we can
        # use the same logic.
    ...
    def __setstate__(self, state: Dict[str, object]) -> None:
        ...
        # get api_kwargs first because we may need to add entries to it (see
        # try-except below)
        ...
        # is necessary, since __getstate__ converts it to a dict as MPT is not
        # pickable.
    ...
    def __deepcopy__(self: Tele_co, memodict: Dict[int, object]) -> Tele_co:
        ...
        # unfreeze the new object for setting the attributes
        setattr(result, "_frozen", False)

        # now we set the attributes in the deepcopied object
        for k in self._get_attrs_names(include_private=True):
            if k == "_frozen":
                # Setting the frozen status to True would prevent the
    ...
    def _get_attrs_names(self, include_private: bool) -> Iterator[str]:
        ...
        # and then get their attributes. The `[:-1]` slice excludes the
        # `object` class
        all_slots = (
            s for c in self.__class__.__mro__[
                :-1] for s in c.__slots__)  # type: ignore
        # chain the class's slots with the user defined subclass __dict__
        # (class has no slots)
        all_attrs = (
            chain(
                all_slots,
                self.__dict__.keys()) if hasattr(
                self,
                "__dict__") else all_slots
        )
    ...
    @classmethod
    def de_json(cls: Type[Tele_co], data: Optional[JSONDict],
                bot: "Bot") -> Optional[Tele_co]:
    ...
    def _de_json(
        ...
        # try-except is significantly faster in case we already have a correct
        # argument set
        try:
            ...
            for key, value in data.items():
                (existing_kwargs if key in cls.__INIT_PARAMS else api_kwargs)[
                    key] = value
    ...
    def de_list(
        ...
        return tuple(obj for obj in (cls.de_json(d, bot)
                     for d in data) if obj is not None)
    ...
    def to_dict(self, recursive: bool = True) -> JSONDict:
        ...
        for key, value in out.items():
            if isinstance(value, (tuple, list)):
                if not value:
                    # not popping directly to avoid changing the dict size
                    # during iteration
                    pop_keys.add(key)
                    continue
                ...
                for item in value:
                    ...
                    # This branch is useful for e.g.
                    # Tuple[Tuple[PhotoSize|KeyboardButton]]
                    elif isinstance(item, (tuple, list)):
                        val.append(
                            [
                                i.to_dict(
                                    recursive=recursive) if hasattr(
                                    i, "to_dict") else i
                                for i in item
                            ]
                        )
                    # if it's not a TGObject, just append it. E.g. [TGObject,
                    # 2]
                    else:
                        val.append(item)
                out[key] = val
