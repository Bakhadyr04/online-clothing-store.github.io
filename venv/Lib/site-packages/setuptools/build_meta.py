...
SETUPTOOLS_ENABLE_FEATURES = os.getenv(
    "SETUPTOOLS_ENABLE_FEATURES", "").lower()
...
class Distribution(setuptools.dist.Distribution):
    ...
    def patch(cls):
        ...
        # type: ignore[misc] # monkeypatching
...
_ConfigSettings: TypeAlias = Union[Mapping[str,
...
class _ConfigSettingsTranslator:
    ...
    def _get_config(self, key: str,
                    config_settings: _ConfigSettings) -> list[str]:
        """
    ...
    def __dist_info_args(
            self, config_settings: _ConfigSettings) -> Iterator[str]:
        """
...
class _BuildMetaBackend(_ConfigSettingsTranslator):
    ...
    def _find_info_directory(
            self, metadata_directory: StrPath, suffix: str) -> Path:
        for parent, dirs, _ in os.walk(metadata_directory):
            candidates = [f for f in dirs if f.endswith(suffix)]

            if len(candidates) != 0 or len(dirs) != 1:
                assert len(
                    candidates) == 1, f"Multiple {suffix} directories found"
    ...
    def _get_dist_info_dir(
            self, metadata_directory: StrPath | None) -> str | None:
...
class _IncompatibleBdistWheel(SetuptoolsDeprecationWarning):
    ...
    # Initially introduced in 2024/10/15, but maybe too disruptive to be
    # enforced?
