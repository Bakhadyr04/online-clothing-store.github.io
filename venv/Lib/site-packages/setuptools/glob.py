...
def glob1(dirname: StrOrBytesPath, pattern: str |
          bytes) -> list[str] | list[bytes]:
    if not dirname:
    ...
    try:
        ...
        return []
    # mypy false-positives: str or bytes type possibility is always kept in
    # sync
...
def glob2(dirname: StrOrBytesPath, pattern: str |
          bytes) -> Iterator[str | bytes]:
