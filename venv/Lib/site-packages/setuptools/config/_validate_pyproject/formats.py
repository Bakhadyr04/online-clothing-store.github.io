...
def _download_classifiers() -> str:
    ...
    with urlopen(url, context=context) as response:  # noqa: S310 (audit URLs)
        headers = Message()
        headers["content_type"] = response.getheader(
            "content-type", "text/plain")
        return response.read().decode(headers.get_param(
            "charset", "utf-8"))  # type: ignore[no-any-return]
...
class _TroveClassifier:
    ...
    def __call__(self, value: str) -> bool:
        ...
        if os.getenv("NO_NETWORK") or os.getenv(
...
RECOMMEDED_ENTRYPOINT_REGEX = re.compile(
    f"^{RECOMMEDED_ENTRYPOINT_PATTERN}$", re.I)
...
def python_entrypoint_reference(value: str) -> bool:
    ...
    if "[" in rest:
        ...
        if extras_.strip()[-1] != "]":
            return False
        extras = (
            x.strip() for x in extras_.strip(
                string.whitespace +
                "[]").split(","))
    ...
    identifiers = _chain(module_parts, obj.split(".")
                         ) if rest else module_parts
