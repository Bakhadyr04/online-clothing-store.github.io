...
def _people(dist: Distribution,
            val: list[dict], _root_dir: StrPath | None, kind: str):
    field = []
    email_field = []
    for person in val:
...
def _optional_dependencies(dist: Distribution, val: dict,
                           _root_dir: StrPath | None):
...
def _unify_entry_points(project_table: dict):
    ...
    for key, value in list(project.items()):  # eager to allow modifications
        ...
        if norm_key in renaming:
            # Don't skip even if value is empty (reason: reset missing
            # `dynamic`)
    ...
    if entry_points:
        ...
        # intentional (for resetting configurations that are missing
        # `dynamic`).
...
def _load_ep(ep: metadata.EntryPoint) -> tuple[str, type] | None:
    if ep.value.startswith("wheel.bdist_wheel"):
        ...
        # TODO: remove check when `bdist_wheel` has been fully removed from
        # pypa/wheel
        return None
...
def _normalise_cmd_options(desc: _OptionsList) -> set[str]:
    return {_normalise_cmd_option_key(
        fancy_option[0]) for fancy_option in desc}
...
def _attrgetter(attr):
    ...
    True
    """
    return partial(reduce, lambda acc, x: getattr(
        acc, x, None), attr.split("."))
