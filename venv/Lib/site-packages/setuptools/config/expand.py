...
def read_files(
    ...
    _filepaths = (os.path.join(root_dir, path)
                  for path in always_iterable(filepaths))
    return '\n'.join(
        _read_file(path)
...
def _assert_local(filepath: StrPath, root_dir: str):
    if Path(os.path.abspath(root_dir)) not in Path(
            os.path.abspath(filepath)).parents:
...
def cmdclass(
    ...
    """
    return {k: resolve_class(v, package_dir, root_dir)
...
def find_packages(
    ...
    **kwargs,
    ...
    if len(search) == 1 and all(not _same_path(
            search[0], x) for x in (".", root_dir)):
    ...
    for path in search:
        ...
        if pkgs and not (
            fill_package_dir.get("") == path or os.path.samefile(
...
def entry_points(
    ...
    parser = ConfigParser(
        default_section=None, delimiters=(
...
class EnsurePackagesDiscovered:
    ...
    def __call__(self):
        ...
        if not self._called:
            self._called = True
            # Skip name, we can still be parsing
...
class LazyMappingProxy(Mapping[_K, _V_co]):
    ...
    def __init__(
            self, obtain_mapping_value: Callable[[], Mapping[_K, _V_co]]) -> None:
