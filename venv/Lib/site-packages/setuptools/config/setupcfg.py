...
def _apply(
    ...
    try:
        # TODO: Temporary cast until mypy 1.12 is released with upstream fixes
        # from typeshed
        _Distribution.parse_config_files(
            dist, filenames=cast(list[str], filenames))
...
def _warn_accidental_env_marker_misconfig(
        label: str, orig_value: str, parsed: list):
...
class ConfigHandler(Generic[Target]):
    ...
    def _parse_dict(cls, value):
        ...
        for line in cls._parse_list(value):
            ...
            if sep != separator:
                raise OptionError(
                    f"Unable to parse option value to dict: {value}")
    ...
    def _parse_attr(self, value, package_dir, root_dir: StrPath):
        ...
        # Make sure package_dir is populated correctly, so `attr:` directives
        # can work
    ...
    def _parse_section_to_dict(cls, section_options, values_parser=None):
        ...
        parser = (
            lambda _,
            v: values_parser(v)) if values_parser else (
            lambda _,
            v: v)
    ...
    def _deprecated_config_handler(self, func, msg, **kw):
        ...
        def config_handler(*args, **kwargs):
            ...
            _DeprecatedConfig.emit(
                "Deprecated config in `setup.cfg`", msg, **kw)
...
class ConfigMetadataHandler(ConfigHandler["DistributionMetadata"]):
    ...
    def _parse_version(self, value):
        ...
        return expand.version(self._parse_attr(
            value, self.package_dir, self.root_dir))
...
class ConfigOptionsHandler(ConfigHandler["Distribution"]):
    ...
    def _parse_cmdclass(self, value):
        ...
        return expand.cmdclass(self._parse_dict(
            value), package_dir, self.root_dir)
    ...
    def parse_section_packages__find(self, section_options):
        ...
        find_kwargs = {
            k: v for k,
            v in section_data.items() if k in valid_keys and v}
    ...
    def _parse_package_data(self, section_options):
        package_data = self._parse_section_to_dict(
            section_options, self._parse_list)
    ...
    def parse_section_extras_require(self, section_options) -> None:
        ...
        parsed = self._parse_section_to_dict_with_key(
            section_options,
            lambda k, v: self._parse_requirements_list(
                f"extras_require[{k}]", v),
        )
