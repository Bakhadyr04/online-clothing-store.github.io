...
class RangeMap(Dict[_RangeMapKT, _VT]):
    ...
    def __init__(
        self,
        source: (
            SupportsKeysAndGetItem[_RangeMapKT,
                                   _VT] | Iterable[tuple[_RangeMapKT, _VT]]
        ),
        sort_params: Mapping[str, Any] = {},
        key_match_comparator: Callable[[
            _RangeMapKT, _RangeMapKT], bool] = operator.le,
    ):
    ...
    # type: ignore[override] # Signature simplified over dict and Mapping
    @overload
    def get(self, key: _RangeMapKT, default: _T) -> _VT | _T: ...
    @overload
    def get(self, key: _RangeMapKT, default: None = None) -> _VT | None: ...

    def get(self, key: _RangeMapKT, default: _T |
            None = None) -> _VT | _T | None:
        """
...
class DictStack(list, collections.abc.MutableMapping):
    ...
    def __iter__(self):
        ...
        return iter(set(itertools.chain.from_iterable(c.keys()
                    for c in dicts)))
