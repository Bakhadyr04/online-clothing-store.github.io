...
if sys.version_info >= (3, 13):
    ...
    def evaluate_forwardref(forwardref: ForwardRef,
                            memo: TypeCheckMemo) -> Any:
    ...
    def evaluate_forwardref(forwardref: ForwardRef,
        ...
        if not forwardref.__forward_evaluated__:
            forwardref.__forward_code__ = compile_type_hint(
                forwardref.__forward_arg__)
        ...
        try:
            ...
            if sys.version_info < (3, 10):
                # Try again, with the type substitutions (list -> List etc.) in
                # place
...
_functions_map: WeakValueDictionary[CodeType,
                                    FunctionType] = WeakValueDictionary()
...
def get_type_name(type_: Any) -> str:
    ...
    if module and module not in (
            None, "typing", "typing_extensions", "builtins"):
        name = module + "." + name
...
def qualified_name(obj: Any, *, add_class_prefix: bool = False) -> str:
    ...
    name = qualname if module in (
        "typing", "builtins") else f"{module}.{qualname}"
    return prefix + name
...
def get_stacklevel() -> int:
    ...
    while frame and frame.f_globals.get(
            "__name__", "").startswith("typeguard."):
        level += 1
        frame = frame.f_back
