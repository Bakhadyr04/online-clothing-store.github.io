...
class TransformMemo:
    ...
    variable_annotations: dict[str, expr] = field(
        init=False, default_factory=dict)
    configuration_overrides: dict[str, Any] = field(
        init=False, default_factory=dict)
    ...
    def __post_init__(self) -> None:
        ...
        if self.node:
            for index, child in enumerate(self.node.body):
                if isinstance(
                        child, ImportFrom) and child.module == "__future__":
    ...
    def insert_imports(self, node: Module | FunctionDef |
                       AsyncFunctionDef) -> None:
        """Insert imports needed by injected code."""
        if not self.load_names:
            return

        # Insert imports after any "from __future__ ..." imports and any
        # docstring
        for modulename, names in self.load_names.items():
            aliases = [
                alias(orig_name, new_name.id if orig_name !=
                      new_name.id else None)
                for orig_name, new_name in sorted(names.items())
            ]
            node.body.insert(
                self.code_inject_index, ImportFrom(
                    modulename, aliases, 0))

    def name_matches(self, expression: expr | Expr |
                     None, *names: str) -> bool:
...
class AnnotationTransformer(NodeTransformer):
    ...
    def visit(self, node: AST) -> Any:
        ...
        if isinstance(node, expr) and self._memo.name_matches(
                node, *literal_names):
            return node
    ...
    def visit_Subscript(self, node: Subscript) -> Any:
        ...
        if node.slice:
            ...
            if isinstance(slice_value, Tuple):
                ...
                # If all items in the subscript were Any, erase the subscript
                # entirely
                ...
                for index, item in enumerate(items):
                    if item is None:
                        items[index] = self.transformer._get_import(
                            "typing", "Any")
...
class TypeguardTransformer(NodeTransformer):
    ...
    def _use_memo(
        ...
        new_memo = TransformMemo(
            node, self._memo, self._memo.path + (node.name,))
        old_memo = self._memo
        self._memo = new_memo
    ...
    def _convert_annotation(self, annotation: expr | None) -> expr | None:
        ...
        new_annotation = cast(
            expr, AnnotationTransformer(self).visit(annotation))
        if isinstance(new_annotation, expr):
            ...
            names = {node.id for node in walk(
                new_annotation) if isinstance(node, Name)}
    ...
    def visit_FunctionDef(
        ...
        with self._use_memo(node):
            ...
            if self._target_path is None or self._memo.path == self._target_path:
                ...
                for decorator in node.decorator_list.copy():
                    if self._memo.name_matches(decorator, "typing.overload"):
                        ...
                        # Remove the decorator to prevent duplicate
                        # instrumentation
                        node.decorator_list.remove(decorator)
                ...
                for arg in all_args:
                    annotation = self._convert_annotation(
                        deepcopy(arg.annotation))
                    if annotation:
                        arg_annotations[arg.arg] = annotation

                if node.args.vararg:
                    annotation_ = self._convert_annotation(
                    ...
                    if annotation_:
                        ...
                        if sys.version_info < (3, 9):
                            subscript_slice = Index(
                                subscript_slice, ctx=Load())
            ...
            if arg_annotations:
                ...
                node.body.insert(
                    self._memo.code_inject_index, Expr(
                        Call(func_name, args, []))
                )
            ...
            if self._memo.memo_var_name:
                ...
                while memo:
                    if isinstance(memo.node, (FunctionDef, AsyncFunctionDef)):
                        # This is a nested function. Use the function name
                        # as-is.
                        del names[:-1]
                        break
                ...
                self._memo.memo_var_name.id = self._memo.get_unused_name(
                    "memo")
                memo_store_name = Name(
                    id=self._memo.memo_var_name.id, ctx=Store())
                ...
                memo_expr = Call(
                    ...
                    [keyword(key, value)
                     for key, value in memo_kwargs.items()],
                )
                node.body.insert(
    ...
    def visit_Yield(self, node: Yield) -> Yield | Call:
        ...
        if (
            ...
            func_name = self._get_import(
                "typeguard._functions", "check_yield_type")
        ...
        if (
            ...
            func_name = self._get_import(
                "typeguard._functions", "check_send_type")
            old_node = node
            call_node = Call(
                func_name,
                [
            ...
            for target in node.targets:
                ...
                for exp in elts:
                    ...
                    if isinstance(exp, Name):
                        ...
                        name = prefix + exp.id
                        annotation = self._memo.variable_annotations.get(
                            exp.id)
                        if annotation:
    ...
    def visit_AugAssign(self, node: AugAssign) -> Any:
        ...
        if isinstance(self._memo.node, (FunctionDef, AsyncFunctionDef)) and isinstance(
            ...
            operator_call = Call(
                operator_func, [
                    Name(
                        node.target.id, ctx=Load()), node.value], []
            )
            check_call = Call(
                self._get_import(
