...
def check_callable(
    ...
    if args:
        ...
        if isinstance(argument_types, list) and not any(
            ...
            # The callable must not have keyword-only arguments without
            # defaults
...
def check_class(
    ...
    if expected_class is Any:
        ...
        for arg in get_args(expected_class):
            ...
            formatted_errors = indent(
                "\n".join(f"{key}: {error}" for key,
                          error in errors.items()), "  "
            )
            raise TypeCheckError(
                f"did not match any element in the union:\n{formatted_errors}"
            )
...
def check_type_internal(
    ...
    if annotation is Any or annotation is SubclassableAny or isinstance(
            value, Mock):
        return

    # Skip type checks if value is an instance of a class that inherits from
    # Any
    ...
    if origin_type is not None:
        ...
        if origin_type in (
                tuple, Tuple) and annotation is not Tuple and not args:
            args = ((),)
    else:
