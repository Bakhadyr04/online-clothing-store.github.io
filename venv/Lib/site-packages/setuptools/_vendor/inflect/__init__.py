...
def joinstem(cutpoint: Optional[int] = 0,
             words: Optional[Iterable[str]] = None) -> str:
    """
...
si_sb_irregular_compound = {
    v: k for (k, v) in pl_sb_irregular_compound.items()}
...
pl_adj_poss_keys = re.compile(
    rf"^({enclose('|'.join(pl_adj_poss))})$",
    re.IGNORECASE)
...
A_y_cons = re.compile(
    r"^(y(b[lor]|cl[ea]|fere|gg|p[ios]|rou|tt))",
    re.IGNORECASE)
...
INDEFINITE_ARTICLE_TEST = re.compile(
    r"\A(\s*)(?:an?\s+)?(.+?)(\s*)\Z", re.IGNORECASE)
...
class engine:
    ...
    @typechecked
    def ud_match(self, word: Word,
                 wordlist: Sequence[Optional[Word]]) -> Optional[str]:
        for i in range(len(wordlist) - 2, -
                       2, -2):  # backwards through even elements
    ...
    def _get_value_from_ast(self, obj):
        ...
        # ex: p.inflect("I plural(see)") instead of p.inflect("I
        # plural('see')")
    ...
    def postprocess(self, orig: str, inflected) -> str:
        ...
        if "|" in inflected:
            ...
            if len(word_options[0].split(" ")) == len(
                    word_options[1].split(" ")):
                result = inflected.split("|")[
                    self.classical_dict["all"]].split(" ")
    ...
    @typechecked
    def plural(self, text: Word,
               count: Optional[Union[str, int, Any]] = None) -> str:
        """
    ...
    def plural_verb(
        ...
        plural = self.postprocess(
            word,
            self._pl_special_verb(
                word, count) or self._pl_general_verb(
                word, count),
        )
        return f"{pre}{plural}{post}"
    ...
    def plural_adj(
        ...
        if not word:
            return text
        plural = self.postprocess(
            word, self._pl_special_adjective(
                word, count) or word)
        return f"{pre}{plural}{post}"
    ...
    def _pl_reg_plurals(self, pair: str, stems: str,
                        end1: str, end2: str) -> bool:
    ...
    def _pl_check_plurals_adj(self, word1: str, word2: str) -> bool:
        word1a = word1[: word1.rfind("'")] if word1.endswith(
            ("'s", "'")) else ""
        word2a = word2[: word2.rfind("'")] if word2.endswith(
            ("'s", "'")) else ""
    ...
    def get_count(
            self, count: Optional[Union[str, int]] = None) -> Union[str, int]:
    ...
    def _plnoun(  # noqa: C901
        ...
        if self.classical_dict["herd"] and word.last.lower(
        ) in pl_sb_uninflected_herd:
            return word

        # HANDLE COMPOUNDS ("Governor General", "mother-in-law",
        # "aide-de-camp", ETC.)
                ...
                for pk, pv in pl_prep_bysize.items():
                    if word.lowered[:pk] in pv:  # starts with a prep
                        if word.lowered.split() == [
                            ...
                            return word.lowered[:-k] + \
                                pl_pron_acc[word.lowered[-k:]]
        ...
        if word.lowered[-2:] in ("ch", "sh", "zz",
                                 "ss") or word.lowered[-1] == "x":
            return f"{word}es"
    ...
    def _handle_prepositional_phrase(cls, phrase, transform, sep):
        ...
        if len(parts) < 3:
            raise ValueError(
                "Cannot handle words with fewer than two separators")
    ...
    def _handle_long_compounds(
            self, word: Words, count: int) -> Union[str, None]:
        """
    ...
    def _pl_special_verb(  # noqa: C901
        ...
        if words.first.endswith(
                "n't") and words.first[:-3] in plverb_irregular_pres:
            return (
    ...
    def _sinoun(  # noqa: C901
        ...
        if self.classical_dict["herd"] and words.last.lower(
        ) in pl_sb_uninflected_herd:
            return word
        ...
        if self.classical_dict["ancient"]:
            ...
            if words.lowered[-4:] in ("eaux", "ieux"):
                return word[:-1]
            if words.lowered[-5:] in ("ynges", "inges",
                                      "anges") and len(word) > 6:
                return word[:-3] + "x"
    ...
    def hundsub(self, mo: Match) -> str:
        ret = self.hundfn(
            int(mo.group(1)), int(mo.group(2)), int(
                mo.group(3)), self.mill_count
        )
        self.mill_count += 1
        return ret
