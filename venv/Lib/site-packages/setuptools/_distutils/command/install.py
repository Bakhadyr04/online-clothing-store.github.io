...
def _remove_set(ob, attrs):
    ...
    """
    return {key: value for key, value in attrs.items() if getattr(ob,
                                                                  key) is None}
...
class install(Command):
    ...
    def finalize_unix(self):
        ...
        if self.user:
            ...
            if self.prefix is None:
                ...
                self.exec_prefix = os.path.normpath(
                    sys.exec_prefix) + _prefix_addition
    ...
    def create_path_file(self):
        ...
        if self.install_path_file:
            self.execute(
                write_file, (filename, [self.extra_dirs]
                             ), f"creating {filename}"
            )
        else:
