...
class bdist_rpm(Command):
    ...
    def run(self):  # noqa: C901
        ...
        spec_path = os.path.join(
            spec_dir, f"{self.distribution.get_name()}.spec")
        self.execute(
            write_file, (spec_path, self._make_spec_file()
                         ), f"writing '{spec_path}'"
        )
        ...
        if self.icon:
            if os.path.exists(self.icon):
                ...
                raise DistutilsFileError(
                    f"icon file '{self.icon}' does not exist")
        ...
        if self.rpm3_mode:
            rpm_cmd.extend(
                ['--define', f'_topdir {os.path.abspath(self.rpm_base)}'])
    ...
    def _make_spec_file(self):  # noqa: C901
        ...
        if fixed_hook != vendor_hook:
            spec_file.append(
                '# Workaround for https://bugs.python.org/issue14443')
