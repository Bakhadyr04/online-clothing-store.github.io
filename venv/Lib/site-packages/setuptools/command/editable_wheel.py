    ...
    def convert(cls, mode: str | None) -> _EditableMode:
        ...
        if _mode not in _EditableMode.__members__:
            raise errors.OptionError(
                f"Invalid editable mode: {mode!r}. Try: 'strict'.")
        ...
        if _mode == "COMPAT":
            SetuptoolsDeprecationWarning.emit(
                ...
                # needs work.
                # Moreover, `pip` is still hiding this warning, so users are
                # not aware.
            )
...
class editable_wheel(Command):
    ...
    def _install_namespaces(self, installation_dir, pth_prefix):
        ...
        src_root = Path(
            self.project_dir,
            self.package_dir.get(
                "",
                ".")).resolve()
        installer = _NamespaceInstaller(
            dist, installation_dir, pth_prefix, src_root)
        installer.install_namespaces()

    def _find_egg_info_dir(self) -> str | None:
        parent_dir = Path(
            self.dist_info_dir).parent if self.dist_info_dir else Path()
    ...
    def _configure_build(
        ...
        # package data)
        egg_info = cast(
            egg_info_cls, dist.reinitialize_command(
                "egg_info", reinit_subcommands=True)
        )
        egg_info.egg_base = str(tmp_dir)
        egg_info.ignore_egg_info_in_manifest = True

        build = cast(
            build_cls, dist.reinitialize_command(
                "build", reinit_subcommands=True)
        )
        install = cast(
            install_cls, dist.reinitialize_command(
                "install", reinit_subcommands=True)
        )
    ...
    def _create_wheel_file(self, bdist_wheel):
        ...
        with unpacked_wheel as unpacked, build_lib as lib, build_tmp as tmp:
            ...
            files, mapping = self._run_build_commands(
                dist_name, unpacked, lib, tmp)
    ...
    def _select_strategy(
        ...
        if mode is _EditableMode.STRICT:
            auxiliary_dir = _empty_dir(
                Path(self.project_dir, "build", build_name))
            return _LinkTree(self.distribution, name, auxiliary_dir, build_lib)

        packages = _find_packages(self.distribution)
        has_simple_layout = _simple_layout(
            packages, self.package_dir, project_dir)
        is_compat_mode = mode is _EditableMode.COMPAT
        if set(self.package_dir) == {
                ""} and has_simple_layout or is_compat_mode:
            # src-layout(ish) is relatively safe for a simple pth file
            src_dir = self.package_dir.get("", ".")
            return _StaticPth(self.distribution, name, [
                              Path(project_dir, src_dir)])
...
class _StaticPth:
    def __init__(self, dist: Distribution, name: str,
                 path_entries: list[Path]) -> None:
        self.dist = dist
        self.name = name
        self.path_entries = path_entries

    def __call__(self, wheel: WheelFile,
                 files: list[str], mapping: Mapping[str, str]):
...
class _LinkTree(_StaticPth):
    ...
    def _create_links(self, outputs, output_mapping: Mapping[str, str]):
        ...
        normalised = ((self._normalize_output(k), v)
...
class _TopLevelFinder:
    ...
    def template_vars(self) -> tuple[str, str,
                                     dict[str, str], dict[str, list[str]]]:
        src_root = self.dist.src_root or os.curdir
        top_level = chain(
            _find_packages(
                self.dist), _find_top_level_modules(
                self.dist))
...
def _encode_pth(content: str) -> bytes:
    ...
    with io.BytesIO() as buffer:
        ...
        # TODO: Python 3.13 replace the whole function with `bytes(content,
        # "utf-8")`
...
def _simple_layout(
    ...
    """
    layout = {
        pkg: find_package_path(
            pkg,
            package_dir,
            project_dir) for pkg in packages}
    if not layout:
    ...
    for pkg in pkg_roots:
        ...
        for i in range(len(parts) - 1, 0, -1):
            ...
            if not path.exists() or partial_name not in pkg_roots:
                # partial_name not in pkg_roots ==> purposefully/accidentally
                # skipped
                yield partial_name
...
class _NamespaceInstaller(namespaces.Installer):
    def __init__(self, distribution, installation_dir,
                 editable_name, src_root) -> None:
    ...
    def _get_nspkg_file(self):
        """Installation target."""
        return os.path.join(self.installation_dir,
                            self.editable_name + self.nspkg_ext)
...
def _finder_template(
    ...
    return _FINDER_TEMPLATE.format(
        name=name, mapping=mapping, namespaces=namespaces)
