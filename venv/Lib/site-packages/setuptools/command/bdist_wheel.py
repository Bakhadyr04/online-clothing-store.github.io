...
def get_platform(archive_root: str | None) -> str:
    ...
    if result.startswith(
            "macosx") and archive_root is not None:  # pragma: no cover
...
class bdist_wheel(Command):
    ...
    def finalize_options(self) -> None:
        ...
        if self.dist_info_dir is None:
            egg_info = cast(
                egg_info_cls,
                self.distribution.get_command_obj("egg_info"))
    ...
    def get_tag(self) -> tuple[str, str, str]:
        ...
        if self.plat_name_supplied and self.plat_name:
            ...
            # macosx contains system version in platform name so need special
            # handle
        ...
        if self.root_is_pure:
            ...
            if self.py_limited_api and (
                    impl_name + impl_ver).startswith("cp3"):
    ...
    def license_paths(self) -> Iterable[str]:
        ...
        if setuptools_major_version >= 42:
            # Setuptools recognizes the license_files option but does not do
            # globbing
            patterns = cast(Sequence[str],
                            self.distribution.metadata.license_files)
        else:
            # Prior to those, wheel is entirely responsible for handling
            # license files
    ...
    def egg2dist(self, egginfo_path: str, distinfo_path: str) -> None:
        ...
        def adios(p: str) -> None:
            ...
            if os.path.exists(p) and not os.path.islink(
