...
class TestFindParentPackage:
    def test_single_package(self, tmp_path):
        ...
        assert find_parent_package(
            packages, {"": "src"}, tmp_path) == "namespace.pkg"
...
class TestDiscoverPackagesAndPyModules:
    ...
    def test_purposefully_empty(
        ...
        files = self.FILES[circumstance] + \
            ["mod.py", "other.py", "src/pkg/__init__.py"]
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                # Just the top-level package can have `-stubs`, ignore nested
                # ones
...
class TestNoConfig:
    ...
    def test_build_with_discovered_name(self, tmp_path):
        ...
        dist_file = tmp_path / \
            f"dist/ns_nested_pkg-{self.DEFAULT_VERSION}.tar.gz"
...
class TestWithPackageData:
    ...
    def test_include_package_data(self, tmp_path, src_root, files):
        ...
        expected = {
            os.path.normpath(
                f"{src_root}/proj/file1.txt").replace(os.sep, "/"),
            os.path.normpath(
                f"{src_root}/proj/nested/file2.txt").replace(os.sep, "/"),
        }
