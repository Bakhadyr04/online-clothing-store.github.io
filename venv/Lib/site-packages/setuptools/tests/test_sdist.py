...
class TestSdistTest:
    ...
    def test_exclude_dev_only_cache_folders(self, source_dir):
        included = {
            ...
            # we just exclude dev-only cache folders at the root of the
            # repository:
    ...
    def make_strings(cls, item):
        if isinstance(item, dict):
            return {key: cls.make_strings(value)
    ...
    def test_add_files_referenced_by_config_directives(
    ...
    def test_build_subcommand_source_files(self, source_dir):
        ...
        # Sanity check: without custom commands file list should not be
        # affected
...
class TestRegressions:
    ...
    def test_symlink_in_extension_depends(
            self, monkeypatch, tmp_path, dep_path):
        # Given a project with a symlinked dir and a "depends" targeting that
        # dir
        files = self.files_for_symlink_in_extension_depends(tmp_path, dep_path)
        jaraco.path.build(files, prefix=str(tmp_path))
        symlink_or_skip_test(
            tmp_path / "external",
            tmp_path / "project/myheaders")
    ...
    @pytest.mark.parametrize(
        "dep_path", ("$tmp_path$/external/dir/file.h",
                     "../external/dir/file.h")
    )
    def test_external_path_in_extension_depends(
            self, monkeypatch, tmp_path, dep_path):
        # Given a project with a "depends" targeting an external dir
        files = self.files_for_external_path_in_extension_depends(
            tmp_path, dep_path)
        jaraco.path.build(files, prefix=str(tmp_path))
        # When `sdist` runs, there should be no error
        members = run_sdist(monkeypatch, tmp_path / "project")
