    ...
    with pytest.warns(SetuptoolsDeprecationWarning, match=msg):
        ...
        if os.getenv("SETUPTOOLS_USE_DISTUTILS") == "stdlib":
            ...
            warnings.filterwarnings(
                ...
                # This warning is already fixed in pypa/distutils but not in
                # stdlib
            )
...
def test_existing_egg_info(tmpdir_cwd, monkeypatch):
    ...
    # We need to ensure it is not cached yet, otherwise it may impact on the
    # tests
    build_py.__dict__.pop('data_files', None)
    dist.reinitialize_command(egg_info)

    # == Sanity check ==
    # Ensure that if existing_egg_info is not given, build_py attempts to run
    # egg_info
    ...
    assert outputs
    example = str(Path(build_py.build_lib, "mypkg/__init__.py")
                  ).replace(os.sep, "/")
...
class TestTypeInfoFiles:
    ...
    def test_type_files_included_by_default(
            self, tmpdir_cwd, pyproject, example):
        structure = {
