...
def wheel_paths(tmp_path_factory):
    ...
    for name in EXAMPLES:
        ...
        with jaraco.path.DirectoryStack().context(example_dir):
            bdist_wheel_cmd(
                bdist_dir=str(build_dir),
                dist_dir=str(dist_dir)).run()
...
def test_licenses_override(dummy_dist, monkeypatch,
                           tmp_path, config_file, config):
    ...
    with ZipFile("dist/dummy_dist-1.0-2a-py3-none-any.whl") as wf:
        for zinfo in wf.filelist:
            assert zinfo.date_time >= (
                1980, 1, 1, 0, 0, 0)  # min epoch is used
