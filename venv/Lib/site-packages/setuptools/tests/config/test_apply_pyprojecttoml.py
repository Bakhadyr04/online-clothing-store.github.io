...
def test_apply_pyproject_equivalent_to_setupcfg(url, monkeypatch, tmp_path):
    ...
    if any(getattr(d, "package_data", None) for d in (dist_toml, dist_cfg)):
        pkg_data_toml = {(k, *sorted(v))
                         for k, v in dist_toml.package_data.items()}
        pkg_data_cfg = {(k, *sorted(v))
                        for k, v in dist_cfg.package_data.items()}
    ...
    if any(getattr(d, "extras_require", None) for d in (dist_toml, dist_cfg)):
        extra_req_toml = {(k, *sorted(v))
                          for k, v in dist_toml.extras_require.items()}
        extra_req_cfg = {(k, *sorted(v))
                         for k, v in dist_cfg.extras_require.items()}
...
class TestLicenseFiles:
    ...
    def test_both_license_and_license_files_defined(self, tmp_path):
        ...
        # by being explicit. On the other hand, contents should be added to
        # `license`
...
class TestInteropCommandLineParsing:
    def test_version(self, tmp_path, monkeypatch, capsys):
        ...
        # This test can be removed once the CLI interface of setup.py is
        # removed
        monkeypatch.chdir(tmp_path)
        toml_config = """
        [project]
...
def core_metadata(dist) -> str:
    ...
    skip_lines = set()
    # ---- DIFF NORMALISATION ----
    # PEP 621 is very particular about author/maintainer metadata conversion,
    # so skip
    skip_prefixes += ("Author:", "Author-email:",
                      "Maintainer:", "Maintainer-email:")
    # May be redundant with Home-page
    skip_prefixes += ("Project-URL: Homepage,", "Home-page:")
    # May be missing in original (relying on default) but backfilled in the
    # TOML
