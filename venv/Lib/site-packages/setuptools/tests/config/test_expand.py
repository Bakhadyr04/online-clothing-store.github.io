...
class TestReadAttr:
    ...
    def test_read_attr(self, tmp_path, monkeypatch):
        ...
        with monkeypatch.context() as m:
            m.chdir(tmp_path)
            # Make sure it can read the attr statically without evaluating the
            # module
        ...
        # Make sure the same APIs work outside cwd
        assert expand.read_attr(
            'pkg.sub.VERSION',
            root_dir=tmp_path) == '0.1.1'
        values = expand.read_attr(
            'lib.mod.VALUES', {
                'lib': 'pkg/sub'}, tmp_path)
...
def test_resolve_class(monkeypatch, tmp_path, package_dir,
                       file, module, return_value):
...
def test_find_packages(tmp_path, args, pkgs):
    ...
    where = [
        ...
        str((tmp_path / p).resolve()).replace(os.sep, "/")
