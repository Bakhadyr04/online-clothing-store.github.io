...
class TestInstallRequires:
    def test_setup_install_includes_dependencies(self, tmp_path, mock_index):
        ...
        """
        # TODO: Remove these tests once `setup.py install` is completely
        # removed
...
data-requires-python="{dep_2_0_python_requires}">{dep_2_0_sdist}</a><br/>
        ...
        with contexts.save_pkg_resources_state():
            ...
            run_setup(test_setup_py, ['--version'])
        eggs = list(
            map(str, pkg_resources.find_distributions(
                os.path.join(test_pkg, '.eggs')))
        )
        assert eggs == ['dep 1.0']

    @pytest.mark.parametrize('with_dependency_links_in_setup_py',
                             (False, True))
    def test_setup_requires_with_find_links_in_setup_cfg(
        ...
        with contexts.save_pkg_resources_state():
            with contexts.tempdir() as temp_dir:
                make_trivial_sdist(
                    os.path.join(
                        temp_dir, 'python-xlib-42.tar.gz'), 'python-xlib', '42'
                )
    ...
    def test_setup_requires_with_transitive_extra_dependency(
        ...
        with contexts.save_pkg_resources_state():
            with contexts.tempdir() as temp_dir:
                ...
                make_trivial_sdist(
                    os.path.join(
                        temp_dir, 'extra_dep-1.0.tar.gz'), 'extra_dep', '1.0'
                )
...
class TestCommandSpec:
    ...
    def test_from_param_raises_expected_error(self) -> None:
        ...
        with pytest.raises(TypeError) as exc_info:
            # type: ignore[arg-type] # We want a type error here
            ei.CommandSpec.from_param(object())
        assert (
...
def test_use_correct_python_version_string(tmpdir, tmpdir_cwd, monkeypatch):
    ...
    # or at least public enough so we can have a test to make sure they are
    # correct
