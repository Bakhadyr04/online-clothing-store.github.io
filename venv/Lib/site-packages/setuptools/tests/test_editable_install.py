...
class TestLegacyNamespaces:
    # legacy => pkg_resources.declare_namespace(...) +
...
class TestPep420Namespaces:
    ...
    def test_namespace_accidental_config_in_lenient_mode(self, venv, tmp_path):
        ...
        venv.run(["python", "-m", "pip", "-v",
                 "install", "-e", str(pkg_A), *opts])
        out = venv.run(
...
class TestFinderTemplate:
    ...
    def test_combine_namespaces_nested(self, tmp_path):
        ...
        with stack.context(tmp_path):
            ...
            code = next(v for k, v in finder.get_implementation()
                        if k.endswith(".py"))
...
def test_encode_pth():
    ...
    content = _encode_pth(
        "tkmilan_ç_utf8")  # no warnings (would be turned into errors)
