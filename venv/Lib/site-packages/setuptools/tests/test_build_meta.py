        ...
        def method(*args, **kw):
            ...
            try:
                ...
                return self.pool.submit(
                    caller, name, *args, **kw).result(TIMEOUT)
...
class TestBuildMetaBackend:
    ...
    def test_build_with_pyproject_config(self, tmpdir, setup_script):
        ...
        with ZipFile(os.path.join(tmpdir, "temp", wheel_file)) as zipfile:
            ...
            license = str(
                zipfile.read("foo-0.1.dist-info/LICENSE.txt"),
                "utf-8")
            epoints = str(
                zipfile.read("foo-0.1.dist-info/entry_points.txt"),
                "utf-8")
    ...
    def test_setup_requires(self, setup_literal,
                            requirements, use_wheel, tmpdir_cwd):
        files = {
            'setup.py': DALS(
                """
