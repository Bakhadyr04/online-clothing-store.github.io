...
class AsyncToSync(Generic[_P, _R]):
    ...
    def __init__(
        ...
        if not callable(awaitable) or (
                not iscoroutinefunction(awaitable) and not iscoroutinefunction(
    ...
    def __call__(self, *args: _P.args, **kwargs: _P.kwargs) -> _R:
        ...
        if not self.force_new_loop and not self.main_event_loop:
            ...
            main_event_loop_pid = getattr(SyncToAsync.threadlocal,
                                          "main_event_loop_pid", None)
...
class SyncToAsync(Generic[_P, _R]):
    ...
    async def __call__(self, *args: _P.args, **kwargs: _P.kwargs) -> _R:
        ...
        if self._thread_sensitive:
            current_thread_executor = getattr(AsyncToSync.executors, "current",
                                              None)
    ...
    def __get__(self, parent: Any,
                objtype: Any) -> Callable[_P, Coroutine[Any, Any, _R]]:
        """
        Include self for methods
        """
        func = functools.partial(self.__call__, parent)
...
def async_to_sync(
    ...
    *,
    force_new_loop: bool = False,
        ...
        Callable[
            [
                Union[Callable[_P, Coroutine[Any, Any, _R]], Callable[
                    _P, Awaitable[_R]]]
            ],
            Callable[_P, _R],
        ],
