...
class diff_match_patch:
        ...
        while pointer < len(diffs):
            if diffs[pointer][0] == self.DIFF_INSERT:
                ...
                if count_delete >= 1 and count_insert >= 1:
                    ...
                    subDiff = self.diff_main(text_delete, text_insert, False,
                                             deadline)
    ...
    def diff_bisect(self, text1, text2, deadline):
        ...
        for d in range(max_d):
            ...
            for k1 in range(-d + k1start, d + 1 - k1end, 2):
                ...
                if k1 == - \
                        d or (k1 != d and v1[k1_offset - 1] < v1[k1_offset + 1]):
                    x1 = v1[k1_offset + 1]
                else:
                    x1 = v1[k1_offset - 1] + 1
                y1 = x1 - k1
                ...
                if x1 > text1_length:
                    ...
                    if k2_offset >= 0 and k2_offset < v_length and v2[
                            k2_offset] != -1:
                        # Mirror x2 onto top-left coordinate system.
                        x2 = text1_length - v2[k2_offset]
                        if x1 >= x2:
                            # Overlap detected.
                            return self.diff_bisectSplit(
                                text1, text2, x1, y1, deadline)

            # Walk the reverse path one step.
            for k2 in range(-d + k2start, d + 1 - k2end, 2):
                k2_offset = v_offset + k2
                if k2 == - \
                        d or (k2 != d and v2[k2_offset - 1] < v2[k2_offset + 1]):
                    x2 = v2[k2_offset + 1]
                else:
                    x2 = v2[k2_offset - 1] + 1
                y2 = x2 - k2
                ...
                if x2 > text1_length:
                    ...
                    if k1_offset >= 0 and k1_offset < v_length and v1[
                            k1_offset] != -1:
                        x1 = v1[k1_offset]
    ...
    def diff_commonPrefix(self, text1, text2):
        ...
        while pointermin < pointermid:
            if text1[pointerstart:pointermid] == text2[
    ...
    def diff_commonSuffix(self, text1, text2):
        ...
        pointerend = 0
        while pointermin < pointermid:
            if (text1[-pointermid:len(text1) -
                      pointerend] == text2[-pointermid:len(text2) -
                                           pointerend]):
                pointermin = pointermid
    ...
    def diff_halfMatch(self, text1, text2):
        ...
        def diff_halfMatchI(longtext, shorttext, i):
            ...
            while j != -1:
                prefixLength = self.diff_commonPrefix(longtext[i:],
                                                      shorttext[j:])
                suffixLength = self.diff_commonSuffix(longtext[:i],
                                                      shorttext[:j])
    ...
    def diff_cleanupSemantic(self, diffs):
        ...
        while pointer < len(diffs):
            if diffs[pointer][0] == self.DIFF_EQUAL:  # Equality found.
                ...
                # sides of it.
                if (lastEquality and (len(lastEquality) <= max(
                        length_insertions1, length_deletions1))
                        and (len(lastEquality) <= max(length_insertions2,
                                                      length_deletions2))):
                    # Duplicate record.
            ...
            if (diffs[pointer - 1][0] == self.DIFF_DELETE
                ...
                if overlap_length1 >= overlap_length2:
                    if (overlap_length1 >= len(deletion) / 2.0
                        ...
                        diffs.insert(
                            pointer,
                            (self.DIFF_EQUAL, insertion[:overlap_length1]))
                        diffs[pointer - 1] = (
                            self.DIFF_DELETE,
                    ...
                    if (overlap_length2 >= len(deletion) / 2.0
                        ...
                        diffs.insert(
                            pointer,
                            (self.DIFF_EQUAL, deletion[:overlap_length2]))
                        diffs[pointer - 1] = (
                            self.DIFF_INSERT,
        ...
        while pointer < len(diffs) - 1:
            if (diffs[pointer - 1][0] == self.DIFF_EQUAL
                ...
                bestScore = diff_cleanupSemanticScore(
                    equality1, edit) + diff_cleanupSemanticScore(
                        edit, equality2)
    ...
    def diff_cleanupEfficiency(self, diffs):
        ...
        while pointer < len(diffs):
            if diffs[pointer][0] == self.DIFF_EQUAL:  # Equality found.
                if len(diffs[pointer][1]) < self.Diff_EditCost and (post_ins or
                                                                    post_del):
                    # Candidate found.
                    equalities.append(pointer)
                    pre_ins = post_ins
                    ...
                    if count_delete != 0 and count_insert != 0:
                        ...
                        if commonlength != 0:
                            ...
                            if x >= 0 and diffs[x][0] == self.DIFF_EQUAL:
                                ...
                                diffs.insert(0, (self.DIFF_EQUAL,
        ...
        for token in tokens:
            ...
            if token[0] == "+":
                ...
                try:
                    ...
                    raise ValueError("Invalid number in diff_fromDelta: " +
                                     param)
                if n < 0:
                    raise ValueError("Negative number in diff_fromDelta: " +
                                     param)
    ...
    def match_bitap(self, text, pattern, loc):
        ...
        if best_loc != -1:
            score_threshold = min(match_bitapScore(0, best_loc),
                                  score_threshold)
    ...
    def patch_addContext(self, patch, text):
        ...
        while text.find(pattern) != text.rfind(pattern) and (
                self.Match_MaxBits == 0 or len(pattern)
                < self.Match_MaxBits - self.Patch_Margin - self.Patch_Margin):
            padding += self.Patch_Margin
            pattern = text[max(0, patch.start2 - padding):patch.start2 +
        ...
        # Add the suffix.
        suffix = text[patch.start2 + patch.length1:patch.start2 +
                      patch.length1 + padding]
        if suffix:
    ...
    def patch_make(self, a, b=None, c=None):
        ...
        for x in range(len(diffs)):
            ...
            elif (diff_type == self.DIFF_EQUAL
                  and len(diff_text) <= 2 * self.Patch_Margin
                  and len(patch.diffs) != 0 and len(diffs) != x + 1):
                # Small equality inside a patch.
                patch.diffs.append(diffs[x])
                patch.length1 += len(diff_text)
                patch.length2 += len(diff_text)

            if diff_type == self.DIFF_EQUAL and len(
                    diff_text) >= 2 * self.Patch_Margin:
        ...
        for patch in patches:
            ...
            if start_loc == -1:
                ...
                if text1 == text2:
                    ...
                    if (len(text1) > self.Match_MaxBits
                            and self.diff_levenshtein(diffs) /
                            float(len(text1)) > self.Patch_DeleteThreshold):
                        ...
                        for op, data in patch.diffs:
                            ...
                            if op == self.DIFF_INSERT:  # Insertion
                                text = (text[:start_loc + index2] + data +
                                        text[start_loc + index2:])
                            elif op == self.DIFF_DELETE:  # Deletion
                                text = (text[:start_loc + index2] +
                                        text[start_loc + self.diff_xIndex(
                                            diffs, index1 + len(data)):])
                            if op != self.DIFF_DELETE:
                                index1 += len(data)
    ...
    def patch_splitMax(self, patches):
        ...
        for x in range(len(patches)):
            ...
            while len(bigpatch.diffs) != 0:
                ...
                postcontext = self.diff_text1(
                    bigpatch.diffs)[:self.Patch_Margin]
                if postcontext:
                    patch.length1 += len(postcontext)
                    patch.length2 += len(postcontext)
                    if len(patch.diffs
                           ) != 0 and patch.diffs[-1][0] == self.DIFF_EQUAL:
                        patch.diffs[-1] = (
                            self.DIFF_EQUAL,
