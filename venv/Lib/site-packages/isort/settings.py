...
FALLBACK_CONFIG_SECTIONS: Tuple[str, ...] = (
...
class _Config:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.known_standard_library:
            object.__setattr__(
                self, "known_standard_library", frozenset(
                    getattr(stdlibs, self.py_version).stdlib)
            )
...
class Config(_Config):
    def __init__(
        ...
        **config_overrides: Any,
        ...
        if config:
            ...
            config_vars["py_version"] = config_vars["py_version"].replace(
                "py", "")
        ...
        # Therefore we extract quiet early here in a variable and use that in
        # warning conditions.
        ...
        for key, value in tuple(combined_config.items()):
            ...
            if key.startswith(KNOWN_PREFIX) and key not in (
                ...
                if maps_to_section in KNOWN_SECTION_MAPPING:
                    ...
                    if maps_to_section not in combined_config.get(
                            "sections", ()) and not quiet:
                        warn(
        ...
        if "src_paths" not in combined_config:
            ...
            for src_path in combined_config.get("src_paths", ()):
                full_paths = (
                    path_root.glob(
                        src_path) if "*" in str(src_path) else [path_root / src_path]
                )
    ...
    def is_skipped(self, file_path: Path) -> bool:
        ...
        if self.directory and Path(
                self.directory) in file_path.resolve().parents:
        ...
        if not (os.path.isfile(os_path) or os.path.isdir(
                os_path) or os.path.islink(os_path)):
            return True
        ...
        if self.skip_gitignore:
            ...
            for folder in self.git_ls_files:
                ...
                git_folder = self._check_folder_git_ls_files(
                    str(file_path.parent))

            # git_ls_files are good files you should parse. If you're not in
            # the allow list, skip.
    ...
    def known_patterns(self) -> List[Tuple[Pattern[str], str]]:
        ...
        pattern_sections = [
            STDLIB] + [section for section in self.sections if section != STDLIB]
        for placement in reversed(pattern_sections):
            known_placement = KNOWN_SECTION_MAPPING.get(
                placement, placement).lower()
            config_key = f"{KNOWN_PREFIX}{known_placement}"
            known_modules = getattr(
                self, config_key, self.known_other.get(
                    known_placement, ()))
    ...
    def sorting_function(self) -> Callable[..., List[str]]:
        ...
        if self.sort_order == "natural":
            ...
            for sort_plugin in pkg_resources.iter_entry_points(
                    "isort.sort_function"):
...
def _get_str_to_type_converter(
        setting_name: str) -> Union[Callable[[str], Any], Type[Any]]:
    ...
    while current_directory and tries < MAX_CONFIG_SEARCH_DEPTH:
        for config_file_name in CONFIG_SOURCES:
            potential_config_file = os.path.join(
                current_directory, config_file_name)
...
def _get_config_data(
        file_path: str, sections: Tuple[str, ...]) -> Dict[str, Any]:
    ...
    if settings:
        ...
        if file_path.endswith(".editorconfig"):
            ...
            if indent_style == "space":
                settings["indent"] = " " * \
                    (indent_size and int(indent_size) or 4)

            elif indent_style == "tab":
                settings["indent"] = "\t" * \
                    (indent_size and int(indent_size) or 1)
            ...
            if max_line_length and (
                ...
                settings["line_length"] = (
                    float("inf") if max_line_length == "off" else int(
                        max_line_length)
                )
            settings = {
                key: value
        ...
        for key, value in settings.items():
            ...
            if existing_value_type == tuple:
                ...
                settings[key] = frozenset(
                    _as_list(settings.get(key)))  # type: ignore
            elif existing_value_type == bool:
                # Only some configuration formats support native boolean
                # values.
                if not isinstance(value, bool):
                    value = _as_bool(value)
                settings[key] = value
            elif key.startswith(KNOWN_PREFIX):
                settings[key] = _abspaths(
                    os.path.dirname(file_path), _as_list(value))
