...
def format_natural(import_line: str) -> str:
    ...
    if not import_line.startswith(
            "from ") and not import_line.startswith("import "):
...
def show_unified_diff(
    *,
    ...
    file_mtime = str(
        datetime.now() if file_path is None else datetime.fromtimestamp(
...
class BasicPrinter:
    ...
    def __init__(self, error: str, success: str,
                 output: Optional[TextIO] = None):
...
def create_terminal_printer(
    ...
    return (
        ColoramaPrinter(
            error,
            success,
            output) if color else BasicPrinter(
            error,
            success,
            output)
    )
