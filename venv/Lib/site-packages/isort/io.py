...
class File:
    ...
    @staticmethod
    def detect_encoding(
            filename: Union[str, Path], readline: Callable[[], bytes]) -> str:
        try:
    ...
    def from_contents(contents: str, filename: str) -> "File":
        encoding = File.detect_encoding(
            filename, BytesIO(
                contents.encode("utf-8")).readline)
        return File(stream=StringIO(contents), path=Path(
            filename).resolve(), encoding=encoding)
