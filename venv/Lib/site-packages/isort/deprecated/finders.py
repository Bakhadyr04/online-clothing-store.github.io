...
class PathFinder(BaseFinder):
    def __init__(self, config: Config, path: str = ".") -> None:
        ...
        # virtual env
        self.virtual_env = self.config.virtual_env or os.environ.get(
            "VIRTUAL_ENV")
        if self.virtual_env:
    ...
    def find(self, module_name: str) -> Optional[str]:
        for prefix in self.paths:
            ...
            is_package = exists_case_sensitive(
                package_path) and os.path.isdir(package_path)
            if is_module or is_package:
                ...
                for src_path in self.config.src_paths:
                    if src_path in path_obj.parents and not self.config.is_skipped(
                            path_obj):
    ...
    def __init__(
        ...
        for finder_cls in finder_classes:
            try:
                ...
                # if one finder fails to instantiate isort can continue using
                # the rest
                if self.verbose:
                    print(
                        (
