...
def process(
    ...
    add_imports: List[str] = [format_natural(
        addition) for addition in config.add_imports]
    import_section: str = ""
                ...
                if current:
                    if add_imports:
                        ...
                        current += add_line_separator + \
                            add_line_separator.join(add_imports)
                        add_imports = []
    ...
    for index, line in enumerate(chain(input_stream, (None,))):
        if line is None:
            ...
            if not (in_quote or was_in_quote or in_top_comment):
                if isort_off:
                    ...
                    code_sorting = LITERAL_TYPE_MAPPING.get(
                        rhs.lstrip()[0], "tuple")
                    code_sorting_indent = line[: -len(line.lstrip())]
                    not_imports = True
                    ...
                    if not stripped_line:
                        ...
                        if is_reexport:
                            output_stream.seek(sort_section_pointer, 0)
                        sort_section_pointer += output_stream.write(
                            sorted_code)
                        not_imports = True
                        code_sorting = False
                        code_sorting_section = ""
        ...
        if not_imports:
            ...
            if import_section:
                if add_imports and (
                        contains_imports or not config.append_only) and not indent:
                    import_section = (
                ...
                if not contains_imports:
                    ...
                    leading_whitespace = import_section[: -
                                                        len(import_section.lstrip())]
                    trailing_whitespace = import_section[len(
                    ...
                    parsed_content = parse.file_contents(
