...
def module_with_reason(
        name: str, config: Config = DEFAULT_CONFIG) -> Tuple[str, str]:
...
def _known_pattern(name: str, config: Config) -> Optional[Tuple[str, str]]:
    ...
    for module_name_to_check in module_names_to_check:
        for pattern, placement in config.known_patterns:
            if placement in config.sections and pattern.match(
                    module_name_to_check):
                return (
                    placement, f"Matched configured known pattern {pattern}")
...
def _src_path(
    ...
    for src_path in src_paths:
        ...
        if nested_module and (
            ...
            return _src_path(nested_module[0],
                             config, (module_path,), new_prefix)
        if (
...
def _src_path_is_module(src_path: Path, module_name: str) -> bool:
    return (
        module_name == src_path.name and src_path.is_dir(
