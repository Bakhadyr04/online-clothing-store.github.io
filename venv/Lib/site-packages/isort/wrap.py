...
def import_statement(
    ...
    if config.balanced_wrapping:
        ...
        while len(
                lines[-1]) < minimum_length and len(lines) == line_count and line_length > 10:
...
def line(content: str, line_separator: str,
                ...
                if config.use_parentheses:
                    if splitter == "as ":
                        ...
                        if config.comment_prefix in lines[-1] and lines[-1].endswith(
                                ")"):
                            content, comment = lines[-1].split(
                                config.comment_prefix, 1)
                            lines[-1] = content + ")" + \
                                config.comment_prefix + comment[:-1]
