    ...
    if tag is ET.Comment:
        ...
        if tag is None:
            ...
            if new_namespace_prefixes:
                ...
                if is_html:
                    write(
                        "".join([f' {k}="{ET._escape_attrib_html(v)}"' for k, v in ns_attrs]))
                else:
                    write(
            ...
            if item_parts:
                if is_html:
                    write(
                        "".join([f' {k}="{ET._escape_attrib_html(v)}"' for k, v in item_parts]))
                else:
                    write(
                        "".join([f' {k}="{ET._escape_attrib(v)}"' for k, v in item_parts]))
            if is_html:
                write(">")
                ltag = tag.lower()
                if text:
