...
class HistoricalRecordContextHelperTestCase(TestCase):
    ...
    def test__context_for_delta_changes__basic_usage_works_as_expected(self):
        ...
        poll = Poll.objects.create(
            question="old?",
            pub_date=parse_datetime(old_date))
    ...
    def test__context_for_delta_changes__with_string_len_around_character_limit(
            self):
        now = datetime.now()
    ...
    def test__context_for_delta_changes__preserves_html_safe_strings(self):
        def get_context_dict_old_and_new(
                old_value, new_value) -> Tuple[str, str]:
        ...
        old, new = get_context_dict_old_and_new(
            old_safe_string, new_safe_string)
        ...
        old, new = get_context_dict_old_and_new(
            old_half_safe_list, new_half_safe_list)
        self.assertEqual(old, "[Hey, &lt;i&gt;Hey&lt;/i&gt;]")
        self.assertEqual(new, "[&lt;b&gt;Hello&lt;/b&gt;, Hello]")
        self.assertTrue(is_safe_str(old) and is_safe_str(new))
        ...
        old, new = get_context_dict_old_and_new(old_safe_list, new_safe_list)
        self.assertEqual(
            old, f"[Hey, &lt;i&gt;{'A' * 53}[947 chars]&lt;/i&gt;]")
        self.assertEqual(
            new, f"[&lt;b&gt;Hello&lt;/b&gt;, {'B' * 47}[949 chars]BBBB]")
        self.assertTrue(is_safe_str(old) and is_safe_str(new))
