...
class HistoricalRecordsTest(TestCase):
    ...
    def test_cascade_delete_history(self):
        thames = WaterLevel.objects.create(
            waters="Thames", level=2.5, date=today)
    ...
    def test_foreignkey_field(self):
        ...
        self.assertRecordValues(
            create_record,
            Choice,
            {"poll_id": why_poll.id, "votes": 0,
                "id": choice.id, "history_type": "+"},
        )
        self.assertRecordValues(
            update_record,
            Choice,
            {"poll_id": how_poll.id, "votes": 0,
                "id": choice.id, "history_type": "~"},
        )
    ...
    def test_specify_history_user(self):
        ...
        document.save()
        self.assertEqual(
            [d.history_user for d in document.history.all()], [
                None, user2, user1]
        )
    ...
    def test_specify_history_date_1(self):
        ...
        temperature.save()
        self.assertEqual(
            [t.history_date for t in temperature.history.all()], [
                today, yesterday]
        )

    def test_specify_history_date_2(self):
        river = WaterLevel.objects.create(
            waters="Thames", level=2.5, date=today)
        river.level = 2.6
    ...
    def test_non_default_primary_key_save(self):
        ...
        library.save()
        self.assertEqual(
            [lib.book_id for lib in library.history.all()], [
    ...
    def test_unicode_verbose_name_plural(self):
        ...
        instance.save()
        self.assertEqual(
            "historical \u570b", instance.history.all()[
                0]._meta.verbose_name_plural
        )
    ...
    def test_historical_verbose_name_plural_follows_model_verbose_name_plural(
            self):
        library = Library()
        library.save()
        self.assertEqual(
    ...
    def test_history_diff_includes_changed_fields_of_base_model(self):
        r = InheritedRestaurant.objects.create(
            name="McDonna", serves_hot_dogs=False)
    ...
    def test_history_diff_arg__foreign_keys_are_objs__returns_expected_fk_values(
            self):
        ...
        expected_pk_delta = ModelDelta(
            expected_pk_changes, [
                "choice", "poll", "votes"], old_record, new_record
        )
        ...
        with self.assertNumQueries(2):  # Once for each poll in the new record
            delta = new_record.diff_against(
                old_record, foreign_keys_are_objs=True)
    ...
    def test_history_diff_arg__foreign_keys_are_objs__returns_expected_m2m_values(
            self):
        poll = PollWithManyToMany.objects.create(
            question="why?", pub_date=today)
        ...
        expected_obj_change = dataclasses.replace(
            expected_obj_change,
            new=[
                {"pollwithmanytomany": poll,
                    "place": DeletedObject(Place, place1_pk)},
                {"pollwithmanytomany": poll,
                    "place": DeletedObject(Place, place2_pk)},
            ],
        )
        expected_obj_delta = dataclasses.replace(
    ...
    def test_history_diff_with_excluded_fields(self):
        ...
        with self.assertNumQueries(0):
            delta = new_record.diff_against(
                old_record, excluded_fields=("question",))
    ...
    def test_history_diff_with_included_fields(self):
        ...
        with self.assertNumQueries(0):
            delta = new_record.diff_against(
                old_record, included_fields=["question"])
    ...
    def test_history_with_unknown_field(self):
        ...
        with self.assertRaises(KeyError):
            with self.assertNumQueries(0):
                new_record.diff_against(
                    old_record, included_fields=["unknown_field"])

        with self.assertNumQueries(0):
            new_record.diff_against(
                old_record, excluded_fields=["unknown_field"])
...
class GetPrevRecordAndNextRecordTestCase(TestCase):
    ...
    def test_get_prev_record(self):
        ...
        second_record = self.poll.history.filter(
            question="ask questions?").get()
    ...
    def test_get_prev_record_with_excluded_field(self):
        ...
        second_record = instance.history.filter(
            question="ask questions?").get()
    ...
    def test_get_next_record_none_if_most_recent(self):
        self.poll.question = "ask questions?"
        self.poll.save()
        recent_record = self.poll.history.filter(
            question="ask questions?").get()
...
class CustomModelNameTests(unittest.TestCase):
    ...
    def test_register_history_model_with_custom_model_name_from_abstract_model(
            self):
    ...
    def test_register_history_model_with_custom_model_name_from_external_model(
            self):
...
class AppLabelTest(TestCase):
    ...
    def test_default_app_label(self):
        self.assertEqual(
            self.get_table_name(
                ExternalModel.objects), "external_externalmodel"
        )
        self.assertEqual(
    ...
    def test_register_app_label(self):
        ...
        self.assertEqual(
            self.get_table_name(
                ConcreteExternal.objects), "tests_concreteexternal"
        )
        self.assertEqual(
    ...
    def test_get_model(self):
        ...
        self.assertEqual(
            get_model(
                "external",
                "HistoricalExternalModelSpecifiedWithAppParam"),
            ExternalModelSpecifiedWithAppParam.histories.model,
        )

        self.assertEqual(
            get_model(
                "external", "ExternalModelRegistered"), ExternalModelRegistered
        )
        self.assertEqual(
...
class TestOrderWrtField(TestCase):
    ...
    def test_restore_object_with_changed_order(self):
        ...
        self.assertSequenceEqual(
            self.series.get_serieswork_order(),
            chronological)
...
class TestMissingOneToOne(TestCase):
    ...
    def test_history_is_complete(self):
        historical_manager_ids = list(
            self.employee.history.order_by(
                "pk").values_list("manager_id", flat=True)
        )
        self.assertEqual(
            historical_manager_ids, [
                self.manager1_id, self.manager2.id])
...
class CustomTableNameTest1(TestCase):
    ...
    def test_custom_table_name_from_register(self):
        self.assertEqual(
            self.get_table_name(
                ContactRegister.history), "contacts_register_history"
        )
...
class ManyToManyTest(TestCase):
    ...
    def test_delete_child(self):
        ...
        self.assertEqual(
            historical_place_values["history_id"],
            m2m_record.history_id)
        self.assertEqual(
            historical_place_values["place_id"],
            original_place_id)
        self.assertEqual(
    ...
    def test_m2m_relation(self):
        # Ensure only the correct M2Ms are saved and returned for history
        # objects
        poll_2 = PollWithManyToMany.objects.create(
            question="Why", pub_date=today)
        place_2 = Place.objects.create(name="Place 2")
    ...
    def test_diff_against(self):
        ...
        expected_change = ModelChange(
            "places", [], [
                {"pollwithmanytomany": self.poll.pk, "place": self.place.pk}]
        )
        expected_delta = ModelDelta(
        ...
        with self.assertNumQueries(2):  # Once for each record
            delta = add_record.diff_against(
                create_record, included_fields=["places"])
        self.assertEqual(delta, expected_delta)

        with self.assertNumQueries(0):
            delta = add_record.diff_against(
                create_record, excluded_fields=["places"])
        expected_delta = dataclasses.replace(
        ...
        expected_change = ModelChange(
            "places", [{"place": self.place.pk,
                        "pollwithmanytomany": self.poll.pk}], []
        )
        expected_delta = ModelDelta(
...
class MultiDBExplicitHistoryUserIDTest(TestCase):
    ...
    def test_history_user_with_integer_field(self):
        ...
        self.assertEqual(
            self.user.id,
            instance.history.first().history_user_id)
...
class RelatedNameTest(TestCase):
    ...
    def test_revert(self):
        ...
        self.assertEqual(
            Street.objects.filter(
                history__history_user=self.user_one.pk).count(), 0
        )
...
class SaveHistoryInSeparateDatabaseTestCase(TestCase):
    ...
    def test_history_model_saved_in_separate_db_on_delete(self):
        ...
        self.assertEqual(
            2, ModelWithHistoryInDifferentDb.history.filter(
                id=id).count())
        self.assertEqual(
            2,
            ModelWithHistoryInDifferentDb.history.using(
                "other").filter(id=id).count(),
        )
        self.assertEqual(
...
class ModelWithMultipleNoDBIndexTest(TestCase):
    ...
    def test_field_indices(self):
        for field in ["name", "fk"]:
            ...
            self.assertFalse(
                self.history_model._meta.get_field(field).db_index)

            # keeps index
            keeps_index = "%s_keeps_index" % field
            self.assertTrue(self.model._meta.get_field(keeps_index).db_index)
            self.assertTrue(
                self.history_model._meta.get_field(keeps_index).db_index)
...
class ModelWithSingleNoDBIndexUniqueTest(TestCase):
    ...
    def test_unique_field_index(self):
        ...
        # keeps index
        self.assertTrue(self.model._meta.get_field(
            "name_keeps_index").db_index)
        self.assertTrue(self.history_model._meta.get_field(
            "name_keeps_index").db_index)
...
class HistoricForeignKeyTest(TestCase):
    ...
    def test_historic_to_non_historic(self):
        ...
        histpart = TestHistoricParticipantToOrganization.objects.get(
            name="original")
    ...
    def test_historic_to_historic(self):
        ...
        # forward relationships - see how natural chasing timepoint relations
        # is
        ...
        # at t1
        ot1 = TestOrganizationWithHistory.history.as_of(
            t1_one_participant).all()[0]
        self.assertEqual(ot1.historic_participants.count(), 1)
        self.assertEqual(ot1.historic_participants.all()[0].name, p1.name)
        # at t2
        ot2 = TestOrganizationWithHistory.history.as_of(
            t2_two_participants).all()[0]
        self.assertEqual(ot2.historic_participants.count(), 2)
        self.assertIn(
            p1.name, [
                item.name for item in ot2.historic_participants.all()])
        self.assertIn(
            p2.name, [
                item.name for item in ot2.historic_participants.all()])
        # at t3
        ot3 = TestOrganizationWithHistory.history.as_of(
            t3_one_participant).all()[0]
