...
class AdminSiteTest(TestCase):
    ...
    def test_history_list_doesnt_contain_too_long_diff_changes(self):
        ...
        def create_and_change_poll(
                *, initial_question, changed_question) -> Poll:
    ...
    def test_overriding__historical_record_context_helper__with_custom_m2m_string(
            self):
        self.login()
    ...
    def assert_history_view_response_contains(
        ...
        self.assertContains(
            response,
            "Choose a date",
            count=None if choose_date else 0)
    ...
    def _test_history_view_response_text_with_revert_disabled(
            self, *, disabled):
    ...
    def test_history_view_response_text__revert_disabled(self):
        self._test_history_view_response_text_with_revert_disabled(
            disabled=True)
    ...
    def test_history_view__enforce_history_permissions_and_revert_enabled(
            self):
        user = User.objects.create(
            username="astronomer",
            is_staff=True,
    ...
    def _test_permission_combos_with_enforce_history_permissions(
            self, *, enforced):
        user = User.objects.create(
            username="astronomer",
            is_staff=True,
        ...
        self.assertEqual(
            admin.has_view_history_permission(request),
            not enforced)
        self.assertEqual(
            admin.has_change_history_permission(request),
            not enforced)

        # has concrete view/change and historical change -> view_history is
        # false
        user.user_permissions.clear()
        user.user_permissions.add(
            Permission.objects.get(codename="view_planet"),
        ...
        # has concrete view/change and historical view/change -> view_history
        # is true
        user.user_permissions.clear()
        user.user_permissions.add(
            Permission.objects.get(codename="view_planet"),
    ...
    def test_permission_combos__enforce_history_permissions(self):
        self._test_permission_combos_with_enforce_history_permissions(
            enforced=True)
