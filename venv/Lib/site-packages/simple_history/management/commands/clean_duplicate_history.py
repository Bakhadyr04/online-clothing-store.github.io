...
class Command(populate_history.Command):
        ...
        for model, history_model in to_process:
            ...
            for o in model_query.iterator():
                self._process_instance(
                    o, model, stop_date=stop_date, dry_run=dry_run)
    ...
    def _process_instance(self, instance, model, stop_date=None, dry_run=True):
        ...
        with transaction.atomic():
            ...
            for f2 in o_qs[1:]:
                ...
                f1 = f2
            if extra_one:
                entries_deleted += self._check_and_delete(
                    f1, extra_one, dry_run)

        self.log(
            self.DONE_CLEANING_FOR_MODEL.format(
    ...
    def _check_and_delete(self, entry1, entry2, dry_run=True):
        delta = entry1.diff_against(
            entry2, excluded_fields=self.excluded_fields)
