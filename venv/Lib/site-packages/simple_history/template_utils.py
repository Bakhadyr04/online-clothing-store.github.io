...
class HistoricalRecordContextHelper:
    ...
    def context_for_delta_changes(
            self, delta: ModelDelta) -> List[Dict[str, Any]]:
        """
    ...
    def prepare_delta_change_value(
        ...
        if isinstance(field_meta, ManyToManyField):
            ...
            # Display a list of only the instances of the M2M field's related
            # model
            display_value = [
    ...
    def stringify_delta_change_values(
        ...
        old_short, new_short = diff_display.common_shorten_repr(
            old_str, new_str)
...
class ObjDiffDisplay:
    ...
    def common_shorten_repr(self, *args: Any) -> Tuple[str, ...]:
        ...
        return tuple(
            prefix + self.shorten(s[prefix_len:],
                                  self.min_diff_len, self.min_end_len)
            for s in args
        )
    ...
    def shortened_str(self, prefix: str, num_skipped_chars: int,
                      suffix: str) -> str:
        """
