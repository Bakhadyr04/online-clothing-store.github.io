...
class SimpleHistoryAdmin(admin.ModelAdmin):
    ...
    def history_view(self, request, object_id, extra_context=None):
        ...
        if not self.has_view_history_or_change_history_permission(
                request, obj):
    ...
    def get_history_queryset(
        ...
        # Prefetch related objects to reduce the number of DB queries when
        # diffing
    ...
    def set_history_delta_changes(
        ...
        for current in historical_records:
            if previous is None:
                previous = current
                continue
            # Related objects should have been prefetched in
            # `get_history_queryset()`
            delta = previous.diff_against(
                current, foreign_keys_are_objs=foreign_keys_are_objs
            )
            helper = self.get_historical_record_context_helper(
                request, previous)
            previous.history_delta_changes = helper.context_for_delta_changes(
                delta)
    ...
    def history_form_view(self, request, object_id,
                          version_id, extra_context=None):
        request.current_app = self.admin_site.name
    ...
    def has_view_history_permission(self, request, obj=None):
        if self.enforce_history_permissions:
            ...
            codename_view_history = get_permission_codename(
                "view", opts_history)
    ...
    def has_change_history_permission(self, request, obj=None):
        if self.enforce_history_permissions:
            opts_history = get_history_model_for_model(self.model)._meta
            codename_change_history = get_permission_codename(
                "change", opts_history)
            return request.user.has_perm(
