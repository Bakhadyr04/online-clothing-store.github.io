...
class HistoricalRecords:
    ...
    def __init__(
        ...
        try:
            ...
            raise TypeError(
                "The `m2m_bases` option must be a list or a tuple.")
    ...
    def create_history_m2m_model(self, model, through_model):
        ...
        attrs.update(
            Meta=type(
                "Meta",
                (),
                self.get_meta_options_m2m(through_model)))
    ...
    def copy_fields(self, model):
        ...
        for field in self.fields_included(model):
            ...
            if isinstance(field, models.ForeignKey):
                ...
                # record rather than the base record. We can use
                # old_field.model here.
    ...
    def get_extra_fields(self, model, fields):
        ...
        def get_instance(self):
            ...
            if self._history_excluded_fields:
                ...
                try:
                    values = (
                        model.objects.filter(pk=getattr(
                    ...
                    attrs.update(values)
            result = model(**attrs)
            # this is the only way external code could know an instance is
            # historical
    ...
    def post_save(self, instance, created, using=None, **kwargs):
        ...
        if not kwargs.get("raw", False):
            self.create_historical_record(
                instance, created and "+" or "~", using=using)
    ...
    def m2m_changed(self, instance, action, attr, pk_set, reverse, **_):
        ...
        if action in ("post_add", "post_remove", "post_clear"):
            # It should be safe to ~ this since the row must exist to modify
            # m2m on it
    ...
    def create_historical_record_m2ms(self, history_instance, instance):
        for field in history_instance._history_m2m_fields:
            ...
            through_model_field_names = [
                f.name for f in through_model._meta.fields]
...
class HistoricReverseManyToOneDescriptor(ReverseManyToOneDescriptor):
    ...
    def related_manager_cls(self):
        ...
        class HistoricRelationModelManager(
...
class HistoricalChanges(ModelTypeHint):
    def diff_against(
        ...
        *,
        ...
        included_m2m_fields = {
            field.name for field in old_history._history_m2m_fields}
        if included_fields is None:
            included_fields = {
                f.name for f in old_history.tracked_fields if f.editable}
        else:
            included_m2m_fields = included_m2m_fields.intersection(
                included_fields)
            ...
            if old_value != new_value:
                ...
                if foreign_keys_are_objs and isinstance(
                    ...
                    def get_value(record, foreign_key):
                        ...
                        # if the object has not been refreshed from the
                        # database
                        except ObjectDoesNotExist:
                            value = None
                        ...
                        if value is None:
                            value = DeletedObject(
                                field_meta.related_model, foreign_key)
                        return value
    ...
    def _get_m2m_field_changes_for_diff(
        ...
        for field in m2m_fields:
            ...
            reverse_field_name = utils.get_m2m_reverse_field_name(
                original_field_meta)
            # Sort the M2M rows by the related object, to ensure a consistent
            # order
            old_m2m_qs = getattr(
                old_history, field).order_by(reverse_field_name)
                ...
                if foreign_keys_are_objs:
                    ...
                    def rows_with_foreign_key_objs(m2m_qs):
                        def get_value(obj, through_field):
                            ...
                            # usually already be None instead of raising this
                            # exception
                            except ObjectDoesNotExist:
                                value = None

                            if value is None:
                                meta = m2m_through_model_opts.get_field(
                                    through_field)
                                foreign_key = getattr(obj, meta.attname)
                                value = DeletedObject(
                                    meta.related_model, foreign_key)
                            return value

                        # Replicate the format of the return value of
                        # QuerySet.values()
                        return [
                            {
                                through_field: get_value(
                                    through_obj, through_field)
...
ModelChangeValue = Union[Any, DeletedObject,
                         List[Dict[str, Union[Any, DeletedObject]]]]
