...
sys.path.extend(
    ((vendor_path := os.path.join(
        ...
        'setuptools',
        '_vendor')) not in sys.path) *
    [vendor_path])  # fmt: skip
# workaround for #4476
...
# Type aliases
_NestedStr: TypeAlias = Union[str,
                              Iterable[Union[str, Iterable["_NestedStr"]]]]
...
# Any object works, but let's indicate we expect something like a module
# (optionally has __loader__ or __file__)
_ModuleLike: TypeAlias = Union[object, types.ModuleType]
# Any: Should be _ModuleLike but we end up with issues where _ModuleLike
# doesn't have _ZipLoaderModule's __loader__
_ProviderFactoryType: TypeAlias = Callable[[Any], "IResourceProvider"]
_DistFinderType: TypeAlias = Callable[[
    _T, str, bool], Iterable["Distribution"]]
_NSHandlerType: TypeAlias = Callable[[
    _T, str, str, types.ModuleType], Union[str, None]]
_AdapterT = TypeVar(
...
def get_provider(moduleOrReq: str |
                 Requirement) -> IResourceProvider | Distribution:
...
def compatible_platforms(provided: str | None, required: str | None) -> bool:
    ...
    if reqMac:
        ...
        if provMac.group(1) != reqMac.group(
                1) or provMac.group(3) != reqMac.group(3):
            return False
...
def load_entry_point(dist: _EPDistType, group: str,
                     name: str) -> _ResolvedEntryPoint:
...
class _ReqExtras(dict["Requirement", tuple[str, ...]]):
    ...
    def markers_pass(self, req: Requirement,
                     extras: tuple[str, ...] | None = None):
        """
...
class ResourceManager:
    ...
    def resource_isdir(
        ...
        return get_provider(
            package_or_requirement).resource_isdir(resource_name)
    ...
    def resource_listdir(
        ...
        return get_provider(
            package_or_requirement).resource_listdir(resource_name)
    ...
    def postprocess(self, tempname: StrOrBytesPath,
...
class NullProvider:
    ...
    def metadata_isdir(self, name: str) -> bool:
        return bool(self.egg_info and self._isdir(
...
is not allowed.
        ...
        if (path.startswith("\\") or ntpath.isabs(
                path)) and not posixpath.isabs(path):
...
class EmptyProvider(NullProvider):
    ...
    # A special case, we don't want all Providers inheriting from NullProvider
    # to have a potentially None module_path
...
def find_distributions(path_item: str,
                       only: bool = False) -> Iterable[Distribution]:
    ...
    if _is_unpacked_egg(path_item):
        yield Distribution.from_filename(
            ...
            metadata=PathMetadata(
                path_item, os.path.join(
                    path_item, 'EGG-INFO')),
        )
        return

    entries = (os.path.join(path_item, child)
...
def _rebuild_mod_path(orig_path, package_name,
...
def fixup_namespace_packages(
        path_item: str, parent: str | None = None) -> None:
...
class Distribution:
    ...
    def check_version_conflict(self):
        ...
        for modname in self._get_metadata('top_level.txt'):
            ...
            if fn and (
                normalize_path(fn).startswith(
                    loc) or fn.startswith(self.location)
            ):
                continue
            issue_warning(
    ...
    def clone(self, **kw: str | int | IResourceProvider | None) -> Self:
        ...
        # Unsafely unpacking. But keeping **kw for backwards and subclassing
        # compatibility
...
def parse_requirements(strs: _NestedStr) -> map[Requirement]:
    ...
    """
    return map(Requirement, join_continuation(
        map(drop_comment, yield_lines(strs))))
...
def _find_adapter(registry: Mapping[type, _AdapterT], ob: object) -> _AdapterT:
    ...
    # So we're raising a TypeError to keep backward compatibility if anyone
...
# This must go before calls to `_call_aside`. See
# https://github.com/pypa/setuptools/pull/4422
def _read_utf8_with_fallback(
        file: str, fallback_encoding=_LOCALE_ENCODING) -> str:
