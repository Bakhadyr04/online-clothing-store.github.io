...
def __getattr__(name):
    for enum, prefix in {Disposal: "APNG_DISPOSE_",
                         Blend: "APNG_BLEND_"}.items():
...
class PngInfo:
    ...
    def add_itxt(self, key, value, lang="", tkey="", zip=False):
        ...
        if zip:
            self.add(
                b"iTXt",
                key + b"\0\x01\0" + lang + b"\0" +
                tkey + b"\0" + zlib.compress(value),
            )
        else:
            self.add(
                b"iTXt",
...
class PngStream(ChunkStream):
    ...
    def chunk_iCCP(self, pos, length):
        ...
        if comp_method != 0:
            raise SyntaxError(
                f"Unknown compression method {comp_method} in iCCP chunk")
        try:
    ...
    def chunk_cHRM(self, pos, length):
        ...
        self.im_info["chromaticity"] = tuple(
            elt / 100000.0 for elt in raw_vals)
        return s
    ...
    def chunk_zTXt(self, pos, length):
        ...
        if comp_method != 0:
            raise SyntaxError(
                f"Unknown compression method {comp_method} in zTXt chunk")
        try:
    ...
    def _open(self):
        ...
        if self.png.im_n_frames is not None:
            ...
            if self.default_image:
                # IDAT chunk contains default image and not first animation
                # frame
                self.n_frames += 1
            self._seek(0)
    ...
    def _seek(self, frame, rewind=False):
        if frame == 0:
            ...
            while True:
                ...
                if cid == b"fcTL":
                    if frame_start:
                        # there must be at least one fdAT chunk between fcTL
                        # chunks
        ...
        # setup frame disposal (actual disposal done when needed in the next
        # _seek())
...
def _write_multiple_frames(im, fp, chunk, rawmode):
    ...
    disposal = im.encoderinfo.get(
        "disposal", im.info.get(
            "disposal", Disposal.OP_NONE))
    ...
    for im_seq in chain:
        for im_frame in ImageSequence.Iterator(im_seq):
            ...
            if im_frames:
                ...
                if prev_disposal == Disposal.OP_PREVIOUS and len(
                        im_frames) < 2:
...
def _save(im, fp, filename, chunk=putchunk, save_all=False):
    ...
    transparency = im.encoderinfo.get(
        "transparency", im.info.get(
            "transparency", None))
