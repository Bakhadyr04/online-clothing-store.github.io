...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def __getstate__(self):
        return [self.info, self.mode, self.size,
                self.getpalette(), self.tobytes()]
                    ...
                    if self.mode == "P":
                        ...
                        if isinstance(t, tuple):
                            ...
                            try:
                                ...
                                if str(
                                        e) == "cannot allocate more than 256 colors":
    ...
    def remap_palette(self, dest_map, source_palette=None):
        ...
        if "transparency" in self.info:
            try:
                m_im.info["transparency"] = dest_map.index(
    ...
    def rotate(
        ...
        if expand:
            ...
            matrix[2], matrix[5] = transform(-(nw - w) /
                                             2.0, -(nh - h) / 2.0, matrix)
            w, h = nw, nh
    ...
    def getchannel(self, channel):
        ...
        if isinstance(channel, str):
            try:
                ...
                raise ValueError(
                    f'The image has no channel "{channel}"') from e
    ...
    def __transformer(
        ...
        if resample not in (
            ...
            if resample in (Resampling.BOX, Resampling.HAMMING,
                            Resampling.LANCZOS):
                message = {
                    Resampling.BOX: "Image.Resampling.BOX",
                    Resampling.HAMMING: "Image.Resampling.HAMMING",
...
def new(mode, size, color=0):
    ...
    im = Image()
    if mode == "P" and isinstance(
            color, (list, tuple)) and len(color) in [3, 4]:
        # RGB or RGBA value for a P image
        from . import ImagePalette
...
class Exif(MutableMapping):
            ...
            if tag in [0x8769, 0x8825]:
                ...
                if tag == 0x927C:
                    ...
                    if tag_data[:8] == b"FUJIFILM":
                        ...
                        makernote = {}
                        for i in range(0, struct.unpack(
                                "<H", ifd_data[:2])[0]):
                        ...
                        for i in range(0, struct.unpack(
                            ...
                            if ifd_tag == 0x1101:
                                ...
                                self.fp.read(4)
                                # Seconds since 2000
                                camerainfo["TimeStamp"] = i32le(
                                    self.fp.read(12))

                                self.fp.read(4)
                                camerainfo["InternalSerialNumber"] = self.fp.read(
                                    4)
    ...
    def __contains__(self, tag):
        return tag in self._data or (
            self._info is not None and tag in self._info)
