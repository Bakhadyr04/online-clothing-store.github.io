...
class ImageDraw:
            ...
            if joint == "curve" and width > 4:
                ...
                for i in range(1, len(xy) - 1):
                    ...
                    def coord_at_angle(coord, angle):
                        ...
                        distance = width / 2 - 1
                        return tuple(
                            p + (math.floor(p_d) if p_d >
                                 0 else math.ceil(p_d))
                            for p, p_d in (
        ...
        if fill is not None:
            ...
            if full_x:
                self.draw.draw_rectangle(
                    (x0, y0 + r + 1, x1, y1 - r - 1), fill, 1)
            else:
                self.draw.draw_rectangle(
                    (x0 + r + 1, y0, x1 - r - 1, y1), fill, 1)
            if not full_x and not full_y:
                self.draw.draw_rectangle(
                    (x0, y0 + r + 1, x0 + r, y1 - r - 1), fill, 1)
                self.draw.draw_rectangle(
                    (x1 - r, y0 + r + 1, x1, y1 - r - 1), fill, 1)
        if ink is not None and ink != fill and width != 0:
            draw_corners(False)
    ...
    def _multiline_spacing(self, font, spacing, stroke_width):
        # this can be replaced with self.textbbox(...)[3] when textsize is
        # removed
    ...
    def text(
        ...
        **kwargs,
        ...
        def draw_text(ink, stroke_width=0, stroke_offset=None):
            ...
            if stroke_offset:
                coord = coord[0] + \
                    stroke_offset[0], coord[1] + stroke_offset[1]
            if mode == "RGBA":
        ...
        if ink is not None:
            ...
            if stroke_width:
                stroke_ink = getink(
                    stroke_fill) if stroke_fill is not None else ink
    ...
    def textbbox(
        ...
        return bbox[0] + xy[0], bbox[1] + \
            xy[1], bbox[2] + xy[0], bbox[3] + xy[1]
...
def _compute_regular_polygon_vertices(bounding_circle, n_sides, rotation):
    ...
    if not all(isinstance(i, (int, float))
               for i in (*centroid, polygon_radius)):
