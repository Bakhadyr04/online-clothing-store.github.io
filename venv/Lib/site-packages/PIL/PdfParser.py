...
def pdf_repr(x):
    if x is True:
        ...
        return b"(D:" + time.strftime("%Y%m%d%H%M%SZ",
                                      x).encode("us-ascii") + b")"
...
class PdfParser:
    ...
    def interpret_trailer(cls, trailer_data):
        ...
        check_format_condition(
            b"Root" in trailer and isinstance(
                trailer[b"Root"], IndirectReference),
    ...
    def get_value(cls, data, offset, expect_indirect=None, max_nesting=-1):
        ...
        raise PdfFormatError("unrecognized object: " +
                             repr(data[offset: offset + 32]))
    ...
    re_xref_section_start = re.compile(
        whitespace_optional + rb"xref" + newline)
