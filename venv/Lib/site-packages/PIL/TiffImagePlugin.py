...
class ImageFileDirectory_v2(MutableMapping):
    ...
    def __init__(self, ifh=b"II\052\0\0\0\0\0", prefix=None, group=None):
        ...
        self.reset()
        (self.next,) = (
            self._unpack("Q", ifh[8:]) if self._bigtiff else self._unpack(
                "L", ifh[4:])
        )
    ...
    def _setitem(self, tag, value, legacy_api):
        ...
        is_ifd = self.tagtype[tag] == TiffTags.LONG and isinstance(
            values, dict)
        if not is_ifd:
    ...
    def tobytes(self, offset=0):
        ...
        for tag, value in sorted(self._tags_v2.items()):
            ...
            msg += " - value: " + (
                "<table: %d bytes>" % len(data) if len(
                    data) >= 16 else str(values)
            )
            logger.debug(msg)
...
class TiffImageFile(ImageFile.ImageFile):
    ...
    def _setup(self):
        ...
        if len(sample_format) > 1 and max(
                sample_format) == min(sample_format) == 1:
...
def _save(im, fp, filename):
    ...
    for key in info:
        if isinstance(
                info, Image.Exif) and key in TiffTags.TAGS_V2_GROUPS.keys():
    ...
    if libtiff:
        rows_per_strip = 1 if stride == 0 else min(
            STRIP_SIZE // stride, im.size[1])
...
class AppendingTiffWriter:
    ...
    def rewriteLastShortToLong(self, value):
        ...
        if bytes_written is not None and bytes_written != 4:
            raise RuntimeError(
                f"wrote only {bytes_written} bytes but wanted 4")
    ...
    def rewriteLastShort(self, value):
        ...
        if bytes_written is not None and bytes_written != 2:
            raise RuntimeError(
                f"wrote only {bytes_written} bytes but wanted 2")
