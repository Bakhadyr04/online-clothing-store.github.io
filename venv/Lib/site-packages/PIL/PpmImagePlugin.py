...
class PpmImageFile(ImageFile.ImageFile):
    ...
    def _read_token(self):
        token = b""
        while len(
                token) <= 10:  # read until next whitespace or limit of 10 characters
            c = self.fp.read(1)
            if not c:
                break
        ...
        if not token:
            ...
            raise ValueError(
                f"Token too long in file header: {token.decode()}")
        return token
    ...
    def _open(self):
        ...
        for ix in range(3):
            ...
            if ix == 0:  # token is the x size
                ...
                if decoder_name != "ppm_plain":
                    # If maxval matches a bit depth, use the raw decoder
                    # directly
        ...
        self.tile = [
            (decoder_name, (0, 0, xsize, ysize), self.fp.tell(), args)]
        ...
        while len(data) != total_bytes:
            ...
            for token in tokens:
                if token not in (48, 49):
                    raise ValueError(
                        f"Invalid token for this mode: {bytes([token])}")
    ...
    def _decode_blocks(self, maxval):
        ...
        while len(data) != total_bytes:
            ...
            for token in tokens:
                ...
                if value > maxval:
                    raise ValueError(
                        f"Channel value too large for this mode: {value}")
...
class PpmDecoder(ImageFile.PyDecoder):
    ...
    def decode(self, buffer):
        ...
        out_max = 65535 if self.mode == "I" else 255
        bands = Image.getmodebands(self.mode)
        while len(data) < self.state.xsize * \
                self.state.ysize * bands * out_byte_count:
            ...
            for b in range(bands):
                value = (
                    pixels[b] if in_byte_count == 1 else i16(
                        pixels, b * in_byte_count)
                )
