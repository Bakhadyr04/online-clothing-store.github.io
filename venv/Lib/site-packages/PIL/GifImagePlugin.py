            ...
            if self.mode == "P":
                if (
                    ...
                    if "transparency" in self.info:
                        ...
                        self.im = self.im.convert(
                            "RGBA", Image.Dither.FLOYDSTEINBERG)
                        self.mode = "RGBA"
                        del self.info["transparency"]
                    else:
                        self.mode = "RGB"
                        self.im = self.im.convert(
                            "RGB", Image.Dither.FLOYDSTEINBERG)

        def _rgb(color):
            if self._frame_palette:
                color = tuple(
                    self._frame_palette.palette[color * 3: color * 3 + 3])
            else:
...
def _write_multiple_frames(im, fp, palette):
    ...
    for imSequence in itertools.chain(
        ...
        for im_frame in ImageSequence.Iterator(imSequence):
            ...
            if "transparency" in im_frame.info:
                encoderinfo.setdefault(
                    "transparency", im_frame.info["transparency"])
    ...
    if len(im_frames) > 1:
        ...
        # Since multiple frames will not be written, add together the frame
        # durations
...
def _get_optimize(im, info):
    ...
    if im.mode in ("P", "L") and info and info.get("optimize", 0):
        ...
        if optimise or im.width * im.height < 512 * 512:
            ...
            if optimise or max(used_palette_colors) >= len(
                    used_palette_colors):
            ...
            if (
                ...
                # check that the palette is not already the smallest possible
                # size
