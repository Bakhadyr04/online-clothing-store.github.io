...
def check_bidi(label: str, check_ltr: bool = False) -> bool:
    ...
    for (idx, cp) in enumerate(label, 1):
        ...
        if rtl:
            # Bidi rule 2
            if not direction in ['R', 'AL', 'AN', 'EN',
                                 'ES', 'CS', 'ET', 'ON', 'BN', 'NSM']:
                raise IDNABidiError(
...
def valid_contexto(label: str, pos: int, exception: bool = False) -> bool:
    ...
    if cp_value == 0x00b7:
        ...
        for cp in label:
            if cp == '\u30fb':
                continue
            if _is_script(cp, 'Hiragana') or _is_script(
                    cp, 'Katakana') or _is_script(cp, 'Han'):
                return True
        return False
...
def uts46_remap(domain: str, std3_rules: bool = True,
...
def encode(s: Union[str, bytes, bytearray], strict: bool = False, uts46: bool = False,
