...
class Wheel(object):
    ...
    def filename(self):
        ...
        return '%s-%s%s-%s-%s-%s.whl' % (self.name,
                                         version, buildver, pyver, abi, arch)
    ...
    def _get_dylib_cache(self):
        ...
        if cache is None:
            ...
            base = os.path.join(get_cache_base(), str(
                'dylib-cache'), '%s.%s' % sys.version_info[:2])
            cache = Cache(base)
        return cache
                ...
                if abi != 'none' and sys.platform.startswith('linux'):
                    ...
                    if len(parts) == 2:
                        if parts >= (2, 5):
                            result.append(
                                (''.join(
                                    (IMP_PREFIX, version)), abi, 'manylinux1_%s' %
                                    arch))
                        if parts >= (2, 12):
                            result.append(
                                (''.join(
                                    (IMP_PREFIX, version)), abi, 'manylinux2010_%s' %
                                    arch))
                        if parts >= (2, 17):
                            result.append(
                                (''.join(
                                    (IMP_PREFIX, version)), abi, 'manylinux2014_%s' %
                                    arch))
                        result.append((''.join(
