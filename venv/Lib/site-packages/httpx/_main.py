...
def get_lexer_for_response(response: Response) -> str:
    ...
    if content_type is not None:
        ...
        try:
            return typing.cast(
                str, pygments.lexers.get_lexer_for_mimetype(
                    mime_type.strip()).name
            )
...
def format_request_headers(request: httpcore.Request,
                           http2: bool = False) -> str:
...
def print_request_headers(request: httpcore.Request,
...
def print_response_headers(
    ...
    console = rich.console.Console()
    http_text = format_response_headers(
        http_version, status, reason_phrase, headers)
    syntax = rich.syntax.Syntax(
        http_text,
        "http",
        theme="ansi_dark",
