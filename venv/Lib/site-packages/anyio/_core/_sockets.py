...
async def connect_tcp(
    ...
    *,
    ...
    if addr_obj is not None:
        ...
        # and the second one is an IPv4 addresses. The rest can be in whatever
        # order.
    ...
    try:
        ...
        for fam, kind, *_, sockaddr in sorted(set(gai_res)):
            ...
            if sys.platform == "win32":
                raw_socket.setsockopt(socket.SOL_SOCKET,
                                      socket.SO_EXCLUSIVEADDRUSE, 1)
            else:
                raw_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR,
                                      1)

            if reuse_port:
                raw_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT,
                                      1)
...
async def create_udp_socket(
    ...
    *,
    ...
    sock = await get_async_backend().create_udp_socket(family, local_address,
                                                       None, reuse_port)
...
async def create_connected_udp_socket(
    ...
    *,
    ...
    gai_res = await getaddrinfo(str(remote_host),
                                remote_port,
                                family=family,
                                type=socket.SOCK_DGRAM)
    family = cast(AnyIPAddressFamily, gai_res[0][0])
...
async def create_unix_datagram_socket(
    *,
    ...
    """
    raw_socket = await setup_unix_local_socket(local_path, local_mode,
                                               socket.SOCK_DGRAM)
...
async def getaddrinfo(
    ...
    *,
    ...
    gai_res = await get_async_backend().getaddrinfo(encoded_host,
                                                    port,
                                                    family=family,
                                                    type=type,
                                                    proto=proto,
...
def getnameinfo(sockaddr: IPSockAddrType,
                flags: int = 0) -> Awaitable[tuple[str, str]]:
    """
...
async def setup_unix_local_socket(
    ...
    if path is not None:
        ...
        # Linux abstract namespace sockets aren't backed by a concrete file so
        # skip stat call
    ...
    if path_str is not None:
        try:
            await to_thread.run_sync(raw_socket.bind,
                                     path_str,
                                     abandon_on_cancel=True)
            if mode is not None:
                await to_thread.run_sync(chmod,
                                         path_str,
                                         mode,
                                         abandon_on_cancel=True)
        except BaseException:
