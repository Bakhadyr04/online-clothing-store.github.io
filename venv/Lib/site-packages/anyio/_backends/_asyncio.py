    ...
    class Runner:
        ...
        def run(self, coro: Coroutine[T_Retval], *, context=None) -> T_Retval:
            ...
            if (threading.current_thread() is threading.main_thread()
                    and signal.getsignal(
                        signal.SIGINT) is signal.default_int_handler):
        ...
        def _do_shutdown(future: asyncio.futures.Future) -> None:
            try:
                loop._default_executor.shutdown(
                    wait=True)  # type: ignore[attr-defined]
...
_run_vars: WeakKeyDictionary[asyncio.AbstractEventLoop,
...
def _task_started(task: asyncio.Task) -> bool:
    ...
    try:
        ...
        # task coro is async_genenerator_asend
        # https://bugs.python.org/issue37771
        raise Exception(
            f"Cannot determine if task {task} has started or not") from None
...
class CancelScope(BaseCancelScope):
    ...
    def __enter__(self) -> CancelScope:
        ...
        # Start cancelling the host task if the scope was cancelled before
        # entering
    ...
    def __exit__(
        ...
        try:
            ...
            if self._cancel_called and not self._parent_cancellation_is_visible_to_us:
                # For each level-cancel() call made on the host task, call
                # uncancel()
                ...
                if exc_val is not None:
                    for exc in iterate_exceptions(exc_val):
                        if isinstance(
                                exc,
                                CancelledError) and is_anyio_cancellation(exc):
    ...
    def _parent_cancellation_is_visible_to_us(self) -> bool:
        return (self._parent_scope is not None and not self.shield
    ...
    def _timeout(self) -> None:
        if self._deadline != math.inf:
            ...
            if loop.time() >= self._deadline:
                ...
                self._timeout_handle = loop.call_at(self._deadline,
                                                    self._timeout)
    ...
    def _deliver_cancellation(self, origin: CancelScope) -> bool:
        ...
        for task in self._tasks:
            ...
            if task is not current and (task is self._host_task
                                        or _task_started(task)):
        ...
        for scope in self._child_scopes:
            if not scope._shield and not scope.cancel_called:
                should_retry = scope._deliver_cancellation(
                    origin) or should_retry
...
class TaskStateStore(MutableMapping["Awaitable[Any] | asyncio.Task",
                                    TaskState]):
    ...
    def __setitem__(self, key: asyncio.Task | Awaitable[Any], value: TaskState,
                    /) -> None:
    ...
    def started(self, value: T_contra | None = None) -> None:
        try:
            ...
            if not self._future.cancelled():
                raise RuntimeError(
                    "called 'started' twice on the same task status") from None
...
class TaskGroup(abc.TaskGroup):
    ...
    async def __aexit__(
        ...
        try:
            ...
            try:
                    ...
                    with CancelScope() as wait_scope:
                        while self._tasks:
                            try:
                                ...
                                # cancellation exception with one raise by a
                                # cancel scope.
                ...
                if self.cancel_scope.__exit__(type(exc), exc,
                                              exc.__traceback__):
                    return True
    ...
    def _spawn(
        ...
        if not iscoroutine(coro):
            prefix = f"{func.__module__}." if hasattr(func,
                                                      "__module__") else ""
            raise TypeError(
...
class WorkerThread(Thread):
    ...
    def __init__(
        ...
        self.queue: Queue[tuple[Context, Callable, tuple, asyncio.Future,
                                CancelScope] | None] = Queue(2)
            ...
            while True:
                ...
                if not future.cancelled():
                    ...
                    if not self.loop.is_closed():
                        self.loop.call_soon_threadsafe(self._report_result,
                                                       future, result,
                                                       exception)
...
def _forcibly_shutdown_process_pool_on_exit(workers: set[Process],
                                            _task: object) -> None:
    """
...
class SocketStream(abc.SocketStream):
    ...
    async def receive(self, max_bytes: int = 65536) -> bytes:
        with self._receive_guard:
            if (not self._protocol.read_event.is_set()
...
class _RawSocketMixin:
    ...
    def _wait_until_readable(
            self, loop: asyncio.AbstractEventLoop) -> asyncio.Future:
...
class UNIXSocketStream(_RawSocketMixin, abc.UNIXSocketStream):
    ...
    async def send_fds(self, message: bytes,
                       fds: Collection[int | IOBase]) -> None:
        if not message:
    ...
    async def accept(self) -> abc.SocketStream:
        ...
        with self._accept_guard:
            ...
            with CancelScope() as self._accept_scope:
                try:
                    client_sock, _addr = await self._loop.sock_accept(
                        self._raw_socket)
                except asyncio.CancelledError:
        ...
        with self._accept_guard:
            while True:
                try:
                    ...
                    self._loop.add_reader(self.__raw_socket, f.set_result,
                                          None)
                    f.add_done_callback(
...
class Lock(BaseLock):
    ...
    def statistics(self) -> LockStatistics:
        task_info = AsyncIOTaskInfo(
            self._owner_task) if self._owner_task else None
...
class AsyncIOTaskInfo(TaskInfo):
    ...
    def has_pending_cancellation(self) -> bool:
        if not (task := self._task()):
            # If the task isn't around anymore, it won't have a pending
            # cancellation
            return False
...
class TestRunner(abc.TestRunner):
    _send_stream: MemoryObjectSendStream[tuple[Awaitable[Any],
                                               asyncio.Future[Any]]]
        ...
        if self._exceptions:
            ...
            if len(exceptions) == 1:
                ...
                raise BaseExceptionGroup(
                    "Multiple exceptions occurred in asynchronous callbacks",
                    exceptions)

    async def _run_tests_and_fixtures(
        self,
        receive_stream: MemoryObjectReceiveStream[tuple[
            Awaitable[T_Retval], asyncio.Future[T_Retval]]],
    ) -> None:
        ...
        **kwargs: P.kwargs,
        ...
        if not self._runner_task:
            self._send_stream, receive_stream = create_memory_object_stream[
                tuple[Awaitable[Any], asyncio.Future]](1)
...
class AsyncIOBackend(AsyncBackend):
    ...
    @classmethod
    def create_capacity_limiter(cls,
                                total_tokens: float) -> abc.CapacityLimiter:
        ...
        async with limiter or cls.current_default_thread_limiter():
            with CancelScope(shield=not abandon_on_cancel) as scope:
                ...
                if not idle_workers:
                    ...
                    while idle_workers:
                        if (now - idle_workers[0].idle_since
                ...
                worker.queue.put_nowait(
                    (context, func, args, future, worker_scope))
                return await future
    ...
    def check_cancelled(cls) -> None:
        ...
        while scope is not None:
            if scope.cancel_called:
                raise CancelledError(
                    f"Cancelled by cancel scope {id(scope):x}")
    ...
    async def open_process(
        ...
        **kwargs: Any,
        ...
        stdin_stream = StreamWriterWrapper(
            process.stdin) if process.stdin else None
        stdout_stream = StreamReaderWrapper(
            process.stdout) if process.stdout else None
        stderr_stream = StreamReaderWrapper(
            process.stderr) if process.stderr else None
        return Process(process, stdin_stream, stdout_stream, stderr_stream)

    @classmethod
    def setup_process_pool_exit_at_shutdown(cls,
                                            workers: set[abc.Process]) -> None:
        create_task(
            _shutdown_process_pool_on_exit(workers),
            name="AnyIO process pool shutdown task",
        )
        find_root_task().add_done_callback(
            partial(_forcibly_shutdown_process_pool_on_exit,
    ...
    async def connect_tcp(
            ...
            host: str,
            port: int,
            local_address: IPSockAddrType | None = None) -> abc.SocketStream:
        transport, protocol = cast(
            tuple[asyncio.Transport, StreamProtocol],
            await
            get_running_loop().create_connection(StreamProtocol,
                                                 host,
                                                 port,
    ...
    async def create_udp_socket(
        ...
        transport, protocol = await get_running_loop(
        ...
        if remote_path:
            while True:
                try:
                    ...
                    loop.add_writer(raw_socket, f.set_result, None)
                    f.add_done_callback(
                        lambda _: loop.remove_writer(raw_socket))
                    await f
                except BaseException:
                    raw_socket.close()
                    raise
    ...
    async def getaddrinfo(
        ...
        *,
        ...
        return await get_running_loop().getaddrinfo(host,
                                                    port,
                                                    family=family,
                                                    type=type,
                                                    proto=proto,
    ...
    @classmethod
    async def getnameinfo(cls,
                          sockaddr: IPSockAddrType,
    ...
    def open_signal_receiver(
        cls, *signals:
            Signals) -> AbstractContextManager[AsyncIterator[Signals]]:
    ...
    def get_running_tasks(cls) -> Sequence[TaskInfo]:
        return [
            AsyncIOTaskInfo(task) for task in all_tasks() if not task.done()
        ]
