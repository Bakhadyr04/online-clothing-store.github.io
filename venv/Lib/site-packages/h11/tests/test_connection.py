...
def test__keep_alive() -> None:
    ...
    assert not _keep_alive(
        Request(method="GET", target="/", headers=[],
                http_version="1.0")  # type: ignore[arg-type]
    )
...
def test__body_framing() -> None:
    ...
    for kwargs in [{"te": True}, {"cl": 100, "te": True}]:
        kwargs = cast(Dict[str, Any], kwargs)
        for meth, r in [(None, req(**kwargs)),
                        (b"GET", resp(**kwargs))]:  # type: ignore
            assert _body_framing(meth, r) == ("chunked", ())

    # Content-Length
    for meth, r in [(None, req(cl=100)),
                    (b"GET", resp(cl=100))]:  # type: ignore
        assert _body_framing(meth, r) == ("content-length", (100,))

    # No headers
    assert _body_framing(
        None, req()) == (
        "content-length", (0,))  # type: ignore
...
def test_Connection_basics_and_content_length() -> None:
    ...
    data = p.send(SERVER, Data(data=b"1"), expect=[
...
def test_client_talking_to_http10_server() -> None:
    ...
    assert receive_and_get(c, b"HTTP/1.0 200 OK\r\n\r\n") == [
        Response(
            ...
            headers=[],
            http_version="1.0",
            reason=b"OK")  # type: ignore[arg-type]
    ]
...
def test_sendfile() -> None:
    ...
    def setup(
        ...
        receive_and_get(
            c, "GET / HTTP/{}\r\nHost: a\r\n\r\n".format(
                http_version).encode("ascii")
        )
        headers = []
        if header:
            headers.append(header)
        c.send(Response(status_code=200, headers=headers))
        return c, c.send_with_data_passthrough(
            Data(data=placeholder))  # type: ignore
