...
class Connection:
    ...
    def _process_event(self, role: Type[Sentinel], event: Event) -> None:
        ...
        if role is self.their_role and type(event) in (
            ...
            event = cast(
                Union[Request, Response, InformationalResponse], event)
    ...
    def _get_io_object(
        ...
        if state is SEND_BODY:
            ...
            framing_type, args = _body_framing(
                cast(bytes, self._request_method), cast(
                    Union[Request, Response], event)
            )
            return io_dict[SEND_BODY][framing_type](
                *args)  # type: ignore[index]
        else:
    ...
    def send_with_data_passthrough(
            self, event: Event) -> Optional[List[bytes]]:
    ...
    # side channels.
    def _clean_up_response_headers_for_sending(
        ...
        if not self._cstate.keep_alive or need_close:
            ...
            headers = set_comma_header(
                headers, b"connection", sorted(connection))
