...
class Dataset:
    ...
    def __str__(self):
        ...
        format_string = '|'.join('{%s:%s}' %
                                 item for item in enumerate(field_lens))
    ...
    def _set_in_format(self, fmt_key, in_stream, **kwargs):
        ...
        return registry.get_format(fmt_key).import_set(
            self, in_stream, **kwargs)
    ...
    def _package(self, dicts=True):
        ...
        if self.headers:
            if dicts:
                data = [dict(list(zip(self.headers, format_row(row))))
                        for row in _data]
            else:
                data = [list(self.headers)] + [format_row(row)
                                               for row in _data]
        else:
