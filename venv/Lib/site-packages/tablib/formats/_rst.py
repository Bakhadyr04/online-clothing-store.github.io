...
class ReSTFormat:
    ...
    @classmethod
    def _row_to_lines(cls, values, widths, wrapper,
                      sep='|', justify=JUSTIFY_LEFT):
        """
    ...
    def _get_column_widths(
            cls, dataset, max_table_width=MAX_TABLE_WIDTH, pad_len=3):
        """
        ...
        if total > max_table_width - (pad_len * len(median_lens)):
            column_widths = (
                max_table_width *
                lens //
                total for lens in median_lens)
        else:
            column_widths = (lens for lens in median_lens)
        # Allow for separator and padding:
        column_widths = (
            w - pad_len if w > pad_len else w for w in column_widths)
