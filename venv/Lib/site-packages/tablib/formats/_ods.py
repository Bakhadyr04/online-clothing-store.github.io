...
class ODSFormat:
    ...
    def export_set(cls, dataset):
        ...
        ws = table.Table(
            name=dataset.title if dataset.title else 'Tablib Dataset')
    ...
    def import_sheet(cls, dset, sheet, headers=True, skip_lines=0):
        ...
        for i, row in enumerate(sheet.childNodes):
            ...
            if i < skip_lines:
                continue
            row_vals = [cls.read_cell(cell)
                        for cell in row.childNodes if is_real_cell(cell)]
            if not row_vals:
                continue
    ...
    def read_cell(cls, cell, value_type=None):
        ...
        if value_type == 'time':
            ...
            try:
                ...
                # backwards compatibility for times exported with older tablib
                # versions
        ...
        for row_number, row in enumerate(_package, start=1):
            ...
            for j, col in enumerate(row):
                if isinstance(col, numbers.Number):
                    ...
                    cell = table.TableCell(
                        valuetype="date", datevalue=date_value, stylename=ds)
