...
def augment_visit(linter: PyLinter, checker_method, augmentation):
    ...
    try:
        ...
        checker = get_checker(
            linter,
            get_class(
                checker_method.__module__,
                checker_method.__qualname__))

    old_method = getattr(checker, checker_method.__name__)
    setattr(
        checker,
        checker_method.__name__,
        AugmentFunc(
            old_method,
            augmentation))
...
def suppress_message(linter: PyLinter, checker_method,
                     message_id_or_symbol, test_func):
    """
...
class DoSuppress:
    ...
    def symbols(self) -> List:
        ...
        # This also gives us compatability with a broader range of pylint
        # versions.
        ...
        try:
            pylint_messages = self.get_message_definitions(
                self.message_id_or_symbol)
            the_symbols = [
                symbol
