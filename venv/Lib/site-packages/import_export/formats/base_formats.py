...
class TablibFormat(Format):
    ...
    def export_data(self, dataset, **kwargs):
        if getattr(settings, "IMPORT_EXPORT_ESCAPE_FORMULAE_ON_EXPORT",
                   False) is True:
...
class XLSX(TablibFormat):
    ...
    def export_data(self, dataset, **kwargs):
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                raise ValueError(
                    _("export failed due to IllegalCharacterError"))
    ...
    def _escape_illegal_chars(self, dataset):
        ...
        def _do_escape(cell):
            if type(cell) is str:
                cell = ILLEGAL_CHARACTERS_RE.sub(
                    "\N{REPLACEMENT CHARACTER}", cell)
            return cell
