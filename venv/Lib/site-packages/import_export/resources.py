...
class Resource(metaclass=DeclarativeMetaclass):
    ...
    def __init__(self, **kwargs):
        ...
        # lists to hold model instances in memory when bulk operations are
        # enabled
    ...
    def get_instance(self, instance_loader, row):
        ...
        """
        import_id_fields = [self.fields[f]
                            for f in self.get_import_id_fields()]
    ...
    def bulk_delete(self, using_transactions, dry_run,
                    raise_errors, result=None):
        """
    ...
    def save_instance(self, instance, is_create, row, **kwargs):
        ...
        if self._meta.use_bulk:
            ...
            if not self._is_using_transactions(
                    kwargs) and self._is_dry_run(kwargs):
    ...
    def skip_row(self, instance, original, row, import_validation_errors=None):
        ...
        for field in self.get_import_fields():
            ...
            if isinstance(field.widget, widgets.ManyToManyWidget):
                ...
                original_values = (
                    [] if original.pk is None else list(
        ...
        try:
            ...
            if self.for_delete(row, instance):
                if new:
                    ...
                    if self._meta.store_instance:
                        # create a copy before deletion so id fields are
                        # retained
            ...
            row_result.errors.append(
                self.get_error_result_class()(
                    e, row=row, number=kwargs["row_number"])
            )
    ...
    def import_data(
        ...
        **kwargs,
        ...
        using_transactions = (
            use_transactions or dry_run) and supports_transactions

        if self._meta.batch_size is not None and (
            not isinstance(
                self._meta.batch_size,
                int) or self._meta.batch_size < 0
        ):
    ...
    def export_resource(self, instance, selected_fields=None, **kwargs):
        ...
        return [self.export_field(field, instance, **kwargs)
                for field in export_fields]

    def get_export_headers(self, selected_fields=None):
        export_fields = self.get_export_fields(selected_fields)
        return [force_str(field.column_name)
                for field in export_fields if field]
    ...
    def export(self, queryset=None, **kwargs):
        ...
        for obj in self.iter_queryset(queryset):
            r = self.export_resource(
                obj, selected_fields=export_fields, **kwargs)
            dataset.append(r)
    ...
    def _select_field(self, target_field_name):
        ...
        # but warn if not
        warn(
            f"cannot identify field for export with name '{target_field_name}'")
    ...
    def _get_ordered_field_names(self, order_field):
        ...
        defined_fields = order_fields + \
            tuple(getattr(self._meta, "fields") or ())
...
class ModelResource(Resource, metaclass=ModelDeclarativeMetaclass):
    ...
    def get_fk_widget(cls, field):
        ...
        use_natural_foreign_keys = (
            has_natural_foreign_key(
                model) and cls._meta.use_natural_foreign_keys
        )
    ...
    def widget_from_django_field(cls, f, default=widgets.Widget):
        ...
        if internal_type in cls.WIDGETS_MAP:
            ...
            # iterate base classes to determine the correct widget class to
            # use.
    ...
    def widget_kwargs_for_field(cls, field_name, django_field):
        ...
        if (
            ...
            widget_kwargs.update(
                {"coerce_to_string": True, "allow_blank": True})
