...
class SearchFilter(BaseFilterBackend):
    ...
    def must_call_distinct(self, queryset, search_fields):
        ...
        for search_field in search_fields:
            ...
            if isinstance(
                    queryset, models.QuerySet) and search_field in queryset.query.annotations:
                continue
            parts = search_field.split(LOOKUP_SEP)
            for part in parts:
                field = opts.get_field(part)
                if hasattr(field, 'get_path_info'):
                    # This field is a relation, update opts to follow the
                    # relation
                    path_info = field.get_path_info()
                    opts = path_info[-1].to_opts
                    if any(path.m2m for path in path_info):
                        # This field is a m2m relation so we know we need to
                        # call distinct
                        return True
                else:
                    # This field has a custom __ query transform but is not a
                    # relational field.
                    break
        return False
...
class OrderingFilter(BaseFilterBackend):
    ...
    def remove_invalid_fields(self, queryset, fields, view, request):
        valid_fields = [
            item[0] for item in self.get_valid_fields(
                queryset, view, {
                    'request': request})]
