...
def apply_suffix_patterns(urlpatterns, suffix_pattern,
                          suffix_required, suffix_route=None):
    ret = []
    for urlpattern in urlpatterns:
        if isinstance(urlpattern, URLResolver):
            ...
            if isinstance(urlpattern.pattern, RoutePattern):
                ...
                new_pattern = path(
                    route, include(
                        (patterns, app_name), namespace), kwargs)
            else:
                new_pattern = re_path(
                    regex, include(
                        (patterns, app_name), namespace), kwargs)
            ...
            # Regular URL pattern
            regex = urlpattern.pattern.regex.pattern.rstrip(
                '$').rstrip('/') + suffix_pattern
            ...
            if isinstance(urlpattern.pattern, RoutePattern):
                ...
                route = str(urlpattern.pattern).rstrip(
                    '$').rstrip('/') + suffix_route
