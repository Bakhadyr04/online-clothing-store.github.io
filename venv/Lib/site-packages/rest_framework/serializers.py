...
class Serializer(BaseSerializer, metaclass=SerializerMetaclass):
    ...
    def errors(self):
        ret = super().errors
        if isinstance(ret, list) and len(ret) == 1 and getattr(
                ret[0], 'code', None) == 'null':
...
class ModelSerializer(Serializer):
    ...
    * Default `.create()` and `.update()` implementations are provided.
    ...
    def build_standard_field(self, field_name, model_field):
        ...
        is_django_jsonfield = hasattr(
            models, 'JSONField') and isinstance(
            model_field, models.JSONField)
        if (postgres_fields and isinstance(model_field,
                                           postgres_fields.JSONField)) or is_django_jsonfield:
        ...
        if postgres_fields and isinstance(
    ...
    def get_unique_together_constraints(self, model):
        ...
        for parent_class in [model] + list(model._meta.parents):
            ...
            for constraint in parent_class._meta.constraints:
                if isinstance(constraint, models.UniqueConstraint) and len(
                        constraint.fields) > 1:
                    yield (
                        constraint.fields,
