...
class TestSubarray:
    ...
    def test_subarray_cast_copies(self):
        ...
        for fields in cast:
            assert isinstance(fields, tuple) and len(fields) == 1
            subarr = fields[0]
...
class TestPromotion:
    ...
    @pytest.mark.parametrize(["other", "expected"],
                             [(2**16 - 1, np.complex64),
                              ...
                              # Base of the double value to sidestep any
                              # rounding issues:
                              (np.longdouble(np.nextafter(1.7e308, 0.)), np.clongdouble),
                              # Additionally use "nextafter" so the cast can't
                              # round down:
                              ...
                              # Base of the double value to sidestep any
                              # rounding issues:
                              (np.clongdouble(np.nextafter(
                                  1.7e308, 0.) * 1j), np.clongdouble),
                              # Additionally use "nextafter" so the cast can't
                              # round down:
                              (np.clongdouble(np.nextafter(
                                  1.7e308, np.inf)), np.clongdouble),
                              ])
