                ...
                if _intentcallbackpattern.match(ap):
                    if k not in groupcache[groupcounter]['args']:
                        ...
                        errmess('analyzeline: intent(callback) %s is already'
                                ' in argument list\n' % (k))
            if case in ['optional', 'required', 'public',
                        'external', 'private', 'intrinsic']:
                ap = case
...
def updatevars(typespec, selector, attrspec, entitydecl):
    ...
    el1 = []
    for e in el:
        for e1 in [x.strip() for x in markoutercomma(
                removespaces(markinnerspaces(e)), comma=' ').split('@ @')]:
            if e1:
...
def _selected_real_kind_func(p, r=0, radix=0):
    ...
    if machine.startswith(('aarch64', 'alpha', 'arm64', 'loongarch',
                          'mips', 'power', 'ppc', 'riscv', 's390x', 'sparc')):
        if p <= 33:
            return 16
    else:
        if p < 19:
...
def analyzevars(block):
    ...
    if block['block'] not in [
            'module', 'pythonmodule', 'python module', 'block data']:
...
def determineexprtype(expr, vars, rules={}):
    ...
    if m:
        ...
        return {'typespec': 'real'}
    for op in ['+', '-', '*', '/']:
        for e in [x.strip() for x in markoutercomma(
                expr, comma=op).split('@' + op + '@')]:
            if e in vars:
