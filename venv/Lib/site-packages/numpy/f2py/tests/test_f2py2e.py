...
# Tests for CLI commands which call meson will fail if no compilers are
# present, these are to be skipped
...
def test_no_freethreading_compatible(hello_world_f90, monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        f'f2py -m blah {ipath} -c --no-freethreading-compatible'.split())
...
def test_freethreading_compatible(hello_world_f90, monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        f'f2py -m blah {ipath} -c --freethreading-compatible'.split())
