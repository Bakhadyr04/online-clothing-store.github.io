...
class Drawing(xmlwriter.XMLwriter):
    ...
    def _write_two_cell_anchor(self, index, drawing_properties):
        ...
        if anchor_type == 1:
            ...
            self._write_graphic_frame(
                index, rel_index, name, description, decorative)
        elif anchor_type == 2:
    ...
    def _write_c_nv_pr(self, index, name, description,
    ...
    def _write_sp(
        ...
        if shape and shape.connect:
            ...
            if shape.text is not None:
                self._write_tx_body(
                    col_absolute, row_absolute, width, height, shape)
    ...
    def _write_sp_pr(self, col_absolute, row_absolute,
                     width, height, shape=None):
    ...
    def _write_xdr_sp_pr(self, index, col_absolute,
                         row_absolute, width, height, shape):
    ...
    def _write_a_xfrm(self, col_absolute, row_absolute,
                      width, height, shape=None):
        # Write the <a:xfrm> element.
        attributes = []
    ...
    def _write_tx_body(self, col_absolute, row_absolute, width, height, shape):
        ...
        if shape.textlink != "":
            ...
            self._write_font_run(
                font, style_attrs, latin_attrs, "a:endParaRPr")
