...
class Chart(xmlwriter.XMLwriter):
    ...
    def set_title(self, options=None):
        ...
        self.title_layout = self._get_layout_properties(
            options.get("layout"), True)
    ...
    def set_up_down_bars(self, options=None):
        ...
        if options.get("down"):
            if "border" in options["down"]:
                # Map border to line.
                down_line = Shape._get_line_properties(
                    options["down"]["border"])
    ...
    def _convert_axis_args(self, axis, user_options):
        ...
        axis["display_units_visible"] = options.get(
            "display_units_visible", True)

        # Map major_gridlines properties.
        if options.get(
                "major_gridlines") and options["major_gridlines"]["visible"]:
        ...
        if options.get(
                "minor_gridlines") and options["minor_gridlines"]["visible"]:
        ...
        # Set the gradient fill properties for the series.
        axis["gradient"] = Shape._get_gradient_properties(
            options.get("gradient"))
        ...
        axis["minor_tick_mark"] = self._get_tick_type(
            options.get("minor_tick_mark"))
        axis["major_tick_mark"] = self._get_tick_type(
            options.get("major_tick_mark"))
    ...
    def _get_labels_properties(self, labels):
        ...
        if position:
            if position in self.label_positions:
                ...
                warn(
                    "Unsupported label position '%s' for this chart type" %
                    position)
                return
    ...
    def _get_legend_properties(self, options=None):
        ...
        legend["layout"] = self._get_layout_properties(
            options.get("layout"), False)
    ...
    def _get_layout_properties(self, args, is_text):
        ...
        for prop in properties:
            if prop not in args.keys():
                warn(
                    "Property '%s' must be specified in layout options" %
                    prop)
                return
            ...
            try:
                ...
                warn(
                    "Property '%s' value '%s' must be numeric in layout" % (
                        prop, value)
                )
                return
    ...
    def _get_points_properties(self, user_points):
        ...
        for user_point in user_points:
            ...
            if user_point is not None:
                ...
                # Set the pattern fill properties for the series.
                pattern = Shape._get_pattern_properties(
                    user_point.get("pattern"))

                # Set the gradient fill properties for the series.
                gradient = Shape._get_gradient_properties(
                    user_point.get("gradient"))
    ...
    def _write_legend(self):
        ...
        if legend.get("delete_series") and isinstance(
                legend["delete_series"], list):
