...
class Packager(object):
    ...
    def _write_vml_files(self):
        ...
        for worksheet in self.workbook.worksheets():
            ...
            if worksheet.has_vml:
                ...
                vml._set_xml_writer(
                    self._filename(
                        "xl/drawings/vmlDrawing" + str(index) + ".vml")
                )
    ...
    def _write_metadata_file(self):
        ...
        metadata.num_embedded_images = len(
            self.workbook.embedded_images.images)
    ...
    def _write_rich_value_rel(self):
        ...
        xml_file.num_embedded_images = len(
            self.workbook.embedded_images.images)
        xml_file._set_xml_writer(filename)
        xml_file._assemble_xml_file()
    ...
    def _write_root_rels_file(self):
        ...
        rels._add_package_relationship(
    ...
    def _write_workbook_rels_file(self):
        ...
        for worksheet in self.workbook.worksheets():
            if worksheet.is_chartsheet:
                rels._add_document_relationship(
                    "/chartsheet", "chartsheets/sheet" +
                    str(chartsheet_index) + ".xml"
                )
                chartsheet_index += 1
            else:
                rels._add_document_relationship(
                    "/worksheet", "worksheets/sheet" +
                    str(worksheet_index) + ".xml"
                )
                worksheet_index += 1
    ...
    def _write_rich_value_rels_files(self):
        ...
        # Create .rels file such as /xl/worksheets/_rels/sheet1.xml.rels.
        rels._set_xml_writer(self._filename(
            ...
            if not self.in_memory:
                ...
                if image_data:
                    ...
                    try:
                        os.chmod(
                            os_filename, os.stat(
                                os_filename).st_mode | stat.S_IWRITE
                        )
                    except OSError:
                        pass
            else:
    ...
    def _add_vba_project_signature(self):
        ...
        if not self.in_memory:
            # In file mode we just write or copy the VBA project signature
            # file.
