...
class Worksheet(xmlwriter.XMLwriter):
            ...
            if self.nan_inf_to_errors:
                if isnan(number):
                    return self._write_formula(
                        row, col, "#NUM!", cell_format, "#NUM!")
                elif number == math.inf:
                    return self._write_formula(
                        row, col, "1/0", cell_format, "#DIV/0!")
                elif number == -math.inf:
                    return self._write_formula(
                        row, col, "-1/0", cell_format, "#DIV/0!")
            else:
    ...
    def embed_image(self, row, col, filename, options=None):
        ...
        self.table[row][col] = cell_error_tuple(
            "#VALUE!", cell_format, image_index)
    ...
    def set_column_pixels(
        ...
        return self.set_column(first_col, last_col,
                               width, cell_format, options)
        ...
        for row_num in range(self.dim_rowmin, self.dim_rowmax + 1):
            ...
            for col_num in range(self.dim_colmin, self.dim_colmax + 1):
                if col_num in self.table[row_num]:
                    ...
                    if self.filter_cells.get(
                            (row_num, col_num)) and length > 0:
                        length += 16
    ...
    @convert_range_args
    def data_validation(self, first_row, first_col,
                        last_row, last_col, options=None):
        """
        ...
        if options.get("input_message") and len(
                options["input_message"]) > 255:
            warn(
        ...
        if options.get("error_message") and len(
                options["error_message"]) > 255:
            warn(
        ...
        if options["validate"] == "list" and isinstance(
                options["value"], list):
    ...
    def conditional_format(
        ...
        for param_key in options.keys():
            if param_key not in valid_parameter:
                warn(
                    "Unknown parameter '%s' in conditional_format()" %
                    param_key)
                return -2
        ...
        if options["type"] == "dataBar":
            ...
            options["bar_negative_color"] = xl_color(
    ...
    @convert_range_args
    def add_table(self, first_row, first_col,
                  last_row, last_col, options=None):
        """
        ...
        if "name" in options:
            ...
            if re.match(r"^[rcRC]$", name) or re.match(
                    r"^[rcRC]\d+[rcRC]\d+$", name):
                warn(
                    "Invalid name '%s' like a RC cell ref in add_table()" %
                    name)
                return -2
        ...
        table["a_range"] = xl_range(
            first_row, first_col, last_data_row, last_col)
            ...
            if "columns" in options:
                ...
                if user_data:
                    ...
                    if name in seen_names:
                        warn(
                            "Duplicate header name in add_table(): '%s'" %
                            name)
                        return -2
                    else:
                    ...
                    if user_data.get("total_function"):
                        ...
                        self._write_formula(
                            last_row, col_num, formula, xformat, value)
                        ...
                        self._write_string(
                            last_row, col_num, total_string, user_data.get(
                                "format")
                        )
    ...
    def set_header(self, header="", options=None, margin=None):
        ...
        if options.get("image_left"):
            self.header_images.append(
                [options.get("image_left"), options.get(
                    "image_data_left"), "LH"]
            )

        if options.get("image_center"):
            self.header_images.append(
                [options.get("image_center"), options.get(
                    "image_data_center"), "CH"]
            )

        if options.get("image_right"):
            self.header_images.append(
                [options.get("image_right"), options.get(
                    "image_data_right"), "RH"]
            )
    ...
    def _convert_date_time(self, dt_obj):
        ...
        return datetime_to_excel_datetime(
            dt_obj, self.date_1904, self.remove_timezone)
        ...
        if url:
            ...
            if target is not None:
                if len(target) > self.max_url_length:
                    ...
                    drawing_object["url_rel_index"] = self._get_drawing_rel_index(
                        url)
    ...
    def _position_object_emus(
        ...
        return (col_start, row_start, x1, y1, col_end,
    ...
    def _position_object_pixels(
        ...
        return [col_start, row_start, x1, y1,
    ...
    def _prepare_vml_objects(
        ...
        self.external_vml_links.append(
            ["/vmlDrawing", "../drawings/vmlDrawing" +
    ...
    def _csv_join(self, *items):
        ...
        items = [
            str(item) if not isinstance(
                item,
                str) else item for item in items]
    ...
    def _write_font(self, xf_format):
        ...
        if xf_format.font_name == "Calibri" and not xf_format.hyperlink:
            xml_writer._xml_empty_tag(
                "scheme", [("val", xf_format.font_scheme)])
        ...
        if user_props:
            ...
            for i in range(max_data):
                ...
                if user_props[i].get("type"):
                    valid_types = (
                        "percent", "percentile", "number", "formula")
            ...
            if (
                ...
                if self.table[row_num]:
                    ...
                    self._write_empty_row(
                        row_num, span, self.set_rows[row_num])
                else:
    ...
    def _write_filters(self, filters):
        ...
        non_blanks = [filter for filter in filters if str(
            filter).lower() != "blanks"]
        attributes = []
    ...
    def _write_data_bar_ext(self, param):
        ...
        guid = "{DA7ABA51-AAAA-BBBB-%04X-%012X}" % (
