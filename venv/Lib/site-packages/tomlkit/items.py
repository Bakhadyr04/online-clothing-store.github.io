...
@overload
def item(value: bool, _parent: Item | None = ...,
         _sort_keys: bool = ...) -> Bool: ...


@overload
def item(value: int, _parent: Item | None = ...,
         _sort_keys: bool = ...) -> Integer: ...


@overload
def item(value: float, _parent: Item | None = ...,
         _sort_keys: bool = ...) -> Float: ...


@overload
def item(value: str, _parent: Item | None = ...,
         _sort_keys: bool = ...) -> String: ...
...
@overload
def item(value: date, _parent: Item | None = ...,
         _sort_keys: bool = ...) -> Date: ...


@overload
def item(value: time, _parent: Item | None = ...,
         _sort_keys: bool = ...) -> Time: ...
...
@overload
def item(value: dict, _parent: Array = ...,
         _sort_keys: bool = ...) -> InlineTable: ...


@overload
def item(value: dict, _parent: Item | None = ...,
         _sort_keys: bool = ...) -> Table: ...


@overload
def item(value: ItemT, _parent: Item | None = ...,
         _sort_keys: bool = ...) -> ItemT: ...


def item(value: Any, _parent: Item | None = None,
         _sort_keys: bool = False) -> Item:
    ...
    raise ConvertError(
        f"Unable to convert an object of {type(value)} to a TOML item")
...
class _ArrayItemGroup:
    ...
    def __iter__(self) -> Iterator[Item]:
        return filter(
            lambda x: x is not None, (self.indent,
                                      self.value, self.comma, self.comment)
        )
...
class Array(Item, _CustomList):
    ...
    def __init__(
        ...
        list.__init__(
            self,
            [v for v in value if not isinstance(
                v, (Whitespace, Comment, Null))],
        )
            ...
            if isinstance(item, Whitespace):
                if "," not in item.s:
                    ...
                    if this_group.value is None:
                        # when comma is met and no value is provided, add a
                        # dummy Null
    ...
    def add_line(
        ...
        *items: Any,
        ...
        for i, el in enumerate(items):
            ...
            if isinstance(it, Comment) or add_comma and isinstance(
                    el, Whitespace):
                raise ValueError(
                    f"item type {type(it)} is not allowed in add_line")
    ...
    def insert(self, pos: int, value: Any) -> None:
        ...
        if indent is None and (idx > 0 or "\n" in default_indent):
            # apply default indent if it isn't the first item or the array is
            # multiline.
...
class Table(AbstractTable):
    ...
    def is_super_table(self) -> bool:
        ...
        if not self:
            return False
        # If the table has children and all children are tables, then it is a
        # super table.
...
class InlineTable(AbstractTable):
    ...
    def append(self, key: Key | str | None, _item: Any) -> InlineTable:
        ...
        if not isinstance(_item, (Whitespace, Comment)):
            if not _item.trivia.indent and len(
                    self._value) > 0 and not self._new:
                _item.trivia.indent = " "
...
class String(str, Item):
    ...
    def __init__(self, t: StringType, _: str, original: str,
                 trivia: Trivia) -> None:
        super().__init__(trivia)
    ...
    def from_raw(cls, value: str, type_=StringType.SLB, escape=True) -> String:
        ...
        string_value = escape_string(
            value, escaped) if escape and escaped else value
