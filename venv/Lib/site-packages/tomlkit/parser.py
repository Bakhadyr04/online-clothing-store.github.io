...
class Parser:
    ...
    def parse(self) -> TOMLDocument:
        ...
        while not self.end():
            ...
            if (key is not None and key.is_multi()
                ) or not self._merge_ws(value, body):
    ...
    def _merge_ws(self, item: Item, container: Container) -> bool:
        ...
        if not isinstance(item, Whitespace) or not isinstance(
                last, Whitespace):
            return False
    ...
    def _parse_item(self) -> tuple[Key | None, Item] | None:
        ...
        with self._state as state:
            while True:
                c = self._current
                if c == "\n":
                    # Found a newline; Return all whitespace found up to this
                    # point.
                    self.inc()
    ...
    def _parse_comment_trail(
            self, parse_trail: bool = True) -> tuple[str, str, str]:
        """
                ...
                while not self.end() and not self._current.is_nl():
                    ...
                    if code == CHR_DEL or code <= CTRL_CHAR_LIMIT and code != CTRL_I:
                        raise self.parse_error(
                            InvalidControlChar, code, "comments")
    ...
    def _parse_key_value(
            self, parse_comment: bool = False) -> tuple[Key, Item]:
        # Leading indent
        self.mark()
    ...
    def _parse_inline_table(self) -> InlineTable:
        ...
        while True:
            ...
            if trailing_comma:
                # consume closing bracket, EOF here is an issue (middle of
                # inline table)
                self.inc(exception=UnexpectedEofError)
    ...
    def _parse_string(self, delim: StringType) -> String:
        ...
        while True:
            ...
            if (
                ...
                close = ""
                if delim.is_multiline():
                    # Consume the delimiters to see if we are at the end of the
                    # string
                    close = ""
    ...
    def _parse_table(
        ...
        if any(" " in part.key.strip() and part.is_bare()
        ...
        if len(name_parts) > 1:
            ...
            for i, _name in enumerate(name_parts[1:]):
                ...
                if is_aot and i == len(name_parts) - 2:
                    table.raw_append(
                        _name, AoT(
                            [child], name=table.name, parsed=True))
                else:
        ...
        if isinstance(result, Null):
            ...
            if is_aot and (not self._aot_stack or full_key !=
                           self._aot_stack[-1]):
    ...
    def _peek_unicode(self, is_long: bool) -> tuple[str | None, str | None]:
        ...
        with self._state(save_marker=True, restore=True):
            ...
            if not self.inc_n(chars):
                ...
                if extracted[0].lower() == "d" and extracted[1].strip(
                        "01234567"):
                    return None, None
