...
class Container(_CustomDict):
    ...
    def add(self, key: Key | Item | str,
            item: Item | None = None) -> Container:
        """
    ...
    def _validate_out_of_order_table(
            self, key: SingleKey | None = None) -> None:
        if key is None:
            for k in self._map:
            ...
            if isinstance(item, Table):
                ...
                if item.is_aot_element():
                    ...
                    if not item.is_aot_element():
                        # Tried to define a table after an AoT with the same
                        # name.
                        raise KeyAlreadyPresent(key)
    ...
    def _render_table(self, key: Key, table: Table,
        ...
        if not table.is_super_table() or (
            ...
            newline_in_table_trivia = (
                "\n" if "\n" not in table.trivia.trail and len(
                    table.value) > 0 else ""
            )
            cur += (
    ...
    def _render_aot_table(self, table: Table,
                          prefix: str | None = None) -> str:
        cur = ""
        _key = prefix or ""
        open_, close = "[[", "]]"
    ...
    def _replace(self, key: Key | str, new_key: Key |
                 str, value: Item) -> None:
        if not isinstance(key, Key):
            key = SingleKey(key)
    ...
    def _replace_at(
        ...
        if isinstance(value, Table):
            ...
            next_ws = idx < last and isinstance(
                self._body[idx + 1][1], Whitespace)
    ...
    def _previous_item(self, idx: int | None = None,
                       ignore=(Null,)) -> Item | None:
        """Find the immediate previous item before index ``idx``.
...
def ends_with_whitespace(it: Any) -> bool:
    ...
    return (
        isinstance(
            it,
            Table) and isinstance(
            it.value._previous_item(),
            Whitespace)
