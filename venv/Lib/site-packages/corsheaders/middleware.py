...
class CorsMiddleware:
    ...
    def add_response_headers(self, request: HttpRequest,
        ...
        if (not conf.CORS_ALLOW_ALL_ORIGINS
        ...
        if (conf.CORS_ALLOW_PRIVATE_NETWORK
                and request.headers.get(ACCESS_CONTROL_REQUEST_PRIVATE_NETWORK)
                == "true"):
            response[ACCESS_CONTROL_ALLOW_PRIVATE_NETWORK] = "true"

        return response

    def origin_found_in_white_lists(self, origin: str,
                                    url: SplitResult) -> bool:
        return ((origin == "null" and origin in conf.CORS_ALLOWED_ORIGINS)
    ...
    def is_enabled(self, request: HttpRequest) -> bool:
        return bool(re.match(conf.CORS_URLS_REGEX,
                             request.path_info)) or self.check_signal(request)

    def check_signal(self, request: HttpRequest) -> bool:
        signal_responses = check_request_enabled.send(sender=None,
                                                      request=request)
    ...
    def _url_in_whitelist(self, url: SplitResult) -> bool:
        ...
        return any(origin.scheme == url.scheme and origin.netloc == url.netloc
