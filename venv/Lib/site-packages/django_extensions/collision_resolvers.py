...
class AppsOrderCR(LegacyCR, metaclass=ABCMeta):
    ...
    def resolve_collisions(self, namespace):
        ...
        for name, models in namespace.items():
            if len(models) > 0:
                sorted_models = self._sort_models_depending_on_priorities(
                    models)
    ...
    def resolve_collisions(self, namespace):
        ...
        for name, models in namespace.items():
            ...
            for model in models:
                ...
                assert isinstance(
                    new_name, str), "result of transform_import must be str!"
...
class CollisionResolvingRunner:
    ...
    def run_collision_resolver(self, models_to_import):
        ...
        dictionary_of_names = self._get_dictionary_of_names(
            models_to_import)  # type: Dict[str, str]
    ...
    def _get_dictionary_of_names(cls, models_to_import):
        ...
        cls._assert_is_collision_resolver_class_correct(
            collision_resolver_class)
    ...
    def _assert_is_collision_resolver_result_correct(cls, result):
        assert isinstance(
            result, dict), "Result of resolve_collisions function must be a dict!"
        for key, value in result.items():
            assert isinstance(
                key, str), "key in collision resolver result should be str not %s" % key
            assert isinstance(
                value, str), "value in collision resolver result should be str not %s" % value

    @classmethod
    def _assert_is_collision_resolver_class_correct(
            cls, collision_resolver_class):
