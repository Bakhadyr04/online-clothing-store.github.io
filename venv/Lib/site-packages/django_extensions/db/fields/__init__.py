...
class UniqueFieldMixin:
    ...
    def check_is_bool(self, attrname):
        if not isinstance(getattr(self, attrname), bool):
            raise ValueError(
                "'{}' argument must be True or False".format(attrname))
...
class AutoSlugField(UniqueFieldMixin, SlugField):
    ...
    def create_slug(self, model_instance, add):
        ...
        slugify_function = getattr(
            model_instance,
            'slugify_function',
            self.slugify_function)

        # slugify the original field content and set next step to 2
        def slug_for_field(lookup_value): return self.slugify_func(
...
class ShortUUIDField(UUIDFieldMixin, CharField):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not HAS_SHORT_UUID:
            raise ImproperlyConfigured(
                "'shortuuid' module is required for ShortUUIDField. (Do you have Python 2.5 or higher installed ?)")
        kwargs.setdefault('max_length', self.DEFAULT_MAX_LENGTH)
