        ...
        if i.getAttribute("type") == "UML - Class":
                    ...
                    if j.getAttribute("name") == "attributes":
                        for ll in j.getElementsByTagName("dia:composite"):
                            if ll.getAttribute("type") == "umlattribute":
                                ...
                                for k in ll.getElementsByTagName(
                                        "dia:attribute"):
                                    if k.getAttribute("name") == "name":
                                        nc = k.getElementsByTagName("dia:string")[
                                            0].childNodes[0].data[1:-1]
                                    elif k.getAttribute("name") == "type":
                                        tc = k.getElementsByTagName("dia:string")[
                                            0].childNodes[0].data[1:-1]
                                    elif k.getAttribute("name") == "value":
                                        val = k.getElementsByTagName("dia:string")[
                                            0].childNodes[0].data[1:-1]
                                        if val == '##':
                                            val = ''
                                    elif k.getAttribute("name") == "visibility" and k.getElementsByTagName("dia:enum")[0].getAttribute("val") == "2":
                                        if tc.replace(" ", "").lower().startswith(
                                                "manytomanyfield("):
                                            # If we find a class not in our
                                            # model that is marked as being to
                                            # another model
                                ...
                                if tc.replace(" ", "").startswith(
                                    ...
                                    tc = "models.CharField(max_length=" + \
                                        varch.group(1) + ")"
                                    if len(val) > 0:
                                        tc = tc.replace(")", ", " + val + " )")
            ...
            for j in a:
                if len(j.getAttribute("to")):
                    mycons[int(j.getAttribute("handle"))
                           ] = j.getAttribute("to")
            print(mycons)
            if 'A' not in mycons:
                herit.append(mycons)
            ...
            for j in a:
                if len(j.childNodes[0].data[1:-1]):
                    imports += str("from %s.models import *" %
                                   j.childNodes[0].data[1:-1])
