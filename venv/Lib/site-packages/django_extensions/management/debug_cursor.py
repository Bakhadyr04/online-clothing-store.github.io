...
def monkey_patch_cursordebugwrapper(
        print_sql=None, print_sql_location=False, truncate=None, logger=print, confprefix="DJANGO_EXTENSIONS"):
    if not print_sql:
        yield
    else:
        if truncate is None:
            truncate = getattr(
                settings, '%s_PRINT_SQL_TRUNCATE' %
                confprefix, DEFAULT_PRINT_SQL_TRUNCATE_CHARS)
        ...
        if getattr(settings, '%s_SQLPARSE_ENABLED' % confprefix, True):
            try:
                ...
                sqlparse_format_kwargs = getattr(
                    settings, '%s_SQLPARSE_FORMAT_KWARGS' %
                    confprefix, sqlparse_format_kwargs_defaults)
            except ImportError:
                sqlparse = None
                ...
                try:
                    ...
                    if sqlparse:
                        raw_sql = sqlparse.format(
                            raw_sql, **sqlparse_format_kwargs)
        ...
        class PrintCursorQueryWrapper(
                PrintQueryWrapperMixin, _CursorDebugWrapper):
            pass
