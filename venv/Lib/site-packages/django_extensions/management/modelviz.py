...
class ModelGraph:
    def __init__(self, app_labels, **kwargs):
        ...
        self.disable_abstract_fields = kwargs.get(
    ...
    def get_graph_data(self, as_json=False):
        ...
        if as_json:
            # We need to remove the model and field class because it is not
            # JSON serializable
        ...
        for app_label in self.app_labels:
            ...
            for appmodel in app_models:
                ...
                for field in attributes:
                    model = self.process_attributes(
                        field, model, pk, abstract_fields)

                if self.sort_fields:
                    model = self.sort_model_fields(model)

                for field in appmodel._meta.local_fields:
                    model = self.process_local_fields(
                        field, model, abstract_fields)
    ...
    def process_local_fields(self, field, model, abstract_fields):
        ...
        if field.attname.endswith(
                '_ptr_id') or field in abstract_fields or self.skip_field(field):
    ...
    def process_local_many_to_many(self, field, model):
        ...
        if isinstance(field, ManyToManyField):
            if hasattr(field.remote_field.through,
                       '_meta') and field.remote_field.through._meta.auto_created:
                relation = self.add_relation(
                    field,
                    newmodel,
            ...
            relation = self.add_relation(field, newmodel, mark_safe(
    ...
    def process_parent(self, parent, appmodel, model):
        ...
        if hasattr(parent, "_meta"):  # parent is a model
            ...
            # models.getModels, which is why they are printed by this without
            # any attributes.
            if _rel not in newmodel['relations'] and self.use_model(
                    _rel['target']):
        ...
        if self.relation_fields_only:
            if not isinstance(
                ...
                (ForeignKey, ManyToManyField, OneToOneField,
                 RelatedField, OneToOneRel, ManyToOneRel)
            ):
                return True
        return False
...
def generate_dot(
        graph_data, template='django_extensions/graph_models/digraph.dot'):
