...
class Command(EmailNotificationCommand):
    ...
    def handle(self, *args, **options):
        ...
        def get_directory_from_chdir():
            directory = options['chdir'] or getattr(
                settings, 'RUNSCRIPT_CHDIR', None)
            try:
        ...
        def run_script(mod, *script_args):
            ...
            try:
                ...
                if isinstance(exit_code, bool):
                    # convert boolean True to exit-code 0 and False to
                    # exit-code 1
                    ...
                    if exit_code != 0:
                        try:
                            raise CommandError(
                                ...
                                (mod.__name__, exit_code), returncode=exit_code)
                        except TypeError:
                            raise CommandError(
                                "'%s' failed with exit code %s" %
                ...
                self.last_exit_code = exit_code if isinstance(
                    exit_code, int) else 1
                if silent:
                    return
                if verbosity > 0:
                    print(
                        ERROR(
                            "Exception while running run() in '%s'" %
                            mod.__name__))
            ...
            try:
                ...
                try:
                    ...
                    module_file = os.path.join(
                        settings.BASE_DIR, *full_module_path.split('.')) + '.py'
        ...
        for script in scripts:
            ...
            if not script_modules:
                ...
                if verbosity > 0 and not silent:
                    print(
                        ERROR(
                            "No (valid) module for script '%s' found" %
                            script))
                continue
            ...
            try:
                ...
                if self.last_exit_code == 1:
                    ...
                    raise CommandError(
                        "An error has occurred running scripts. See errors above.")
                print(
                    ERROR("An error has occurred running scripts. See errors above."))
                if hasattr(self, 'running_tests'):
                    return
                sys.exit(self.last_exit_code)
