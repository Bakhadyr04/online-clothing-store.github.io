...
class SQLDiff:
    ...
    def SQL_FIELD_MISSING_IN_DB(self, style, qn, args): return "%s %s\n\t%s %s %s;" % (
        style.SQL_KEYWORD('ALTER TABLE'),
        style.SQL_TABLE(qn(args[0])),
        style.SQL_KEYWORD('ADD COLUMN'),
        style.SQL_FIELD(qn(args[1])),
        ' '.join(style.SQL_COLTYPE(a) if i == 0 else style.SQL_KEYWORD(a)
                 for i, a in enumerate(args[2:]))
    )

    def SQL_FIELD_MISSING_IN_MODEL(self, style, qn, args): return "%s %s\n\t%s %s;" % (
        style.SQL_KEYWORD('ALTER TABLE'),
        style.SQL_TABLE(qn(args[0])),
    ...
    def SQL_FKEY_MISSING_IN_DB(self, style, qn, args): return "%s %s\n\t%s %s %s %s %s (%s)%s;" % (
        style.SQL_KEYWORD('ALTER TABLE'),
        style.SQL_TABLE(qn(args[0])),
        style.SQL_KEYWORD('ADD COLUMN'),
        style.SQL_FIELD(qn(args[1])),
    ...
    def SQL_INDEX_MISSING_IN_DB(self, style, qn, args): return "%s %s\n\t%s %s (%s%s);" % (
    ...
    def SQL_INDEX_MISSING_IN_MODEL(self, style, qn, args): return "%s %s;" % (
        style.SQL_KEYWORD('DROP INDEX'),
        style.SQL_TABLE(qn(args[1]))
    )

    def SQL_UNIQUE_MISSING_IN_DB(self, style, qn, args): return "%s %s\n\t%s %s %s (%s);" % (
        style.SQL_KEYWORD('ALTER TABLE'),
        style.SQL_TABLE(qn(args[0])),
    ...
    def SQL_UNIQUE_MISSING_IN_MODEL(self, style, qn, args): return "%s %s\n\t%s %s %s;" % (
        style.SQL_KEYWORD('ALTER TABLE'),
        style.SQL_TABLE(qn(args[0])),
    ...
    def SQL_FIELD_TYPE_DIFFER(self, style, qn, args): return "%s %s\n\t%s %s %s;" % (
        style.SQL_KEYWORD('ALTER TABLE'),
        style.SQL_TABLE(qn(args[0])),
    ...
    def SQL_FIELD_PARAMETER_DIFFER(self, style, qn, args): return "%s %s\n\t%s %s %s;" % (
        style.SQL_KEYWORD('ALTER TABLE'),
        style.SQL_TABLE(qn(args[0])),
        style.SQL_KEYWORD("MODIFY"),
        style.SQL_FIELD(qn(args[1])),
    ...
    def SQL_NOTNULL_DIFFER(self, style, qn, args): return "%s %s\n\t%s %s %s %s;" % (
        style.SQL_KEYWORD('ALTER TABLE'),
        style.SQL_TABLE(qn(args[0])),
        style.SQL_KEYWORD('MODIFY'),
        style.SQL_FIELD(qn(args[1])),
    ...
    def SQL_TABLE_MISSING_IN_MODEL(
        self,
        style,
        qn,
        args): return style.NOTICE(
        '-- Model missing for table: %s' %
        args[0])
    ...
    def load(self):
        ...
        self.db_tables = [
            table_info.name for table_info in self.introspection.get_table_list(
                self.cursor)]
    ...
    def get_field_db_type_kwargs(
            self, current_kwargs, description, field=None, table_name=None, reverse_type=None):
        return {}
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        ...
        if type_code in DATA_TYPES_REVERSE_OVERRIDE:
            ...
            try:
                reverse_type = self.introspection.get_field_type(
                ...
                if not reverse_type:
                    ...
                    if key not in self.unknown_db_fields:
                        ...
                        self.add_difference(
                            'comment', "Unknown database type for field '%s' (%s)" %
        ...
        if type_code == 16946 and field and getattr(
                field, 'geom_type', None) == 'POINT':
        ...
        if reverse_type == "DecimalField":
            ...
            kwargs['decimal_places'] = description[5] and abs(
                description[5]) or description[5]
        ...
        if not tablespace:
            tablespace = "public"
        if (tablespace, table_name,
                field.column) in self.unsigned and self.unsigned_suffix not in field_db_type:
    ...
    def get_indexes(self, meta):
        indexes_normalized = list(normalize_together(meta.indexes))
        ...
        for field in all_local_fields(meta):
            ...
            if field.unique and meta.managed:
                ...
                if not db_field_unique and table_constraints:
                    db_field_unique = any(
                        constraint['unique'] for contraint_name,
                        constraint in table_constraints.items() if [attname] == constraint['columns'])
    ...
    def find_unique_missing_in_model(
        ...
        fields = dict([(field.column, field)
                      for field in all_local_fields(meta)])
        unique_together = self.get_unique_together(meta)
    ...
    def find_index_missing_in_db(
        ...
        indexes = self.get_indexes(meta)
        db_indexes = normalize_together(
            [v['columns'] for v in table_constraints.values() if v['index'] and not v['unique']])
        for columns in indexes:
            if columns in db_indexes:
                continue
    ...
    def find_index_missing_in_model(
        ...
        for constraint_name, constraint in table_constraints.items():
            ...
            if constraint['unique'] and not constraint['index']:
                # unique constraints are handled by
                # find_unique_missing_in_model
                continue
            ...
            if (constraint['unique'] and constraint['index']) or field is None:
                ...
                if constraint['primary_key'] and field.primary_key:
                    continue
                if constraint['foreign_key'] and isinstance(
                        field, models.ForeignKey) and field.db_constraint:
                    continue
                if constraint['unique'] and field.unique:
                    continue
                if constraint['index'] and constraint['type'] == 'idx' and constraint.get(
                        'orders') and field.unique:
                    ...
                    # note: mysql does not have and/or introspect and fill the
                    # 'orders' attribute of constraint information
                    continue
                if constraint['index'] and field.db_index:
                    continue
                if constraint['check'] and field.db_check(
                        connection=connection):
                    continue
                if getattr(field, 'spatial_index', False):
                    continue
            else:
                if constraint['index'] and tuple(columns) in indexes:
                    continue
    ...
    def find_field_missing_in_model(
        ...
        for row in table_description:
            if row[0] not in fieldmap:
                self.add_difference(
                    'field-missing-in-model', table_name, row[0])
    ...
    def find_field_type_differ(
        ...
        for field in all_local_fields(meta):
            ...
            if not self.strip_parameters(
                    db_type) == self.strip_parameters(model_type):
                self.add_difference(
                    'field-type-differ',
                    table_name,
                    field.name,
    ...
    def find_field_parameter_differ(
        ...
        for field in all_local_fields(meta):
            ...
            if not self.strip_parameters(
                    model_type) == self.strip_parameters(db_type):
                continue
    ...
    def find_field_notnull_differ(self, meta, table_description, table_name):
        ...
        for field in all_local_fields(meta):
            ...
            if field.null != null:
                ...
                self.add_difference(
                    'notnull-differ', table_name, attname, action)
    ...
    def find_differences(self):
        ...
        for app_model in self.app_models:
            ...
            self.find_field_missing_in_model(
                fieldmap, table_description, table_name)
            ...
            self.find_field_parameter_differ(
                meta, table_description, table_name)
    ...
    def print_diff_text(self, style):
        ...
        if not self.can_detect_unsigned_differ:
            self.stdout.write(style.NOTICE(
                "# Detecting unsigned changes not implemented for this database backend"))
            self.stdout.write("")
        ...
        for app_label, model_name, diffs in self.differences:
            ...
            for diff in diffs:
                ...
                text = self.DIFF_TEXTS[diff_type] % dict(
                    (str(i), style.SQL_TABLE(', '.join(e)
                     if isinstance(e, (list, tuple)) else e))
                    for i, e in enumerate(diff_args)
                )
                text = "'".join(
                    i %
                    2 == 0 and style.ERROR(e) or e for i,
                    e in enumerate(
                        text.split("'")))
                if not self.dense:
...
class MySQLDiff(SQLDiff):
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        ...
        if field:
            ...
            if self.strip_parameters(field_type) == 'char' and self.strip_parameters(
                    db_type) == 'varchar':
    ...
    def find_index_missing_in_model(
        ...
        for constraint_name, constraint in table_constraints.items():
            ...
            # extra check removed from superclass here, otherwise function is
            # the same
            if len(columns) == 1:
                if not field:
                ...
                if constraint['index'] and tuple(columns) in indexes:
                    continue
                if constraint['index'] and constraint['unique'] and tuple(
                        columns) in unique_together:
                    continue
    ...
    def find_unique_missing_in_db(
        ...
        # This comparison changed from superclass - otherwise function is the
        # same
        db_unique_columns = normalize_together(
...
class SqliteSQLDiff(SQLDiff):
    ...
    def find_unique_missing_in_db(
        ...
        unique_columns = [field.db_column or field.attname for field in all_local_fields(
            meta) if field.unique]
        ...
        super().find_unique_missing_in_db(meta, table_indexes,
                                          table_constraints, table_name, skip_list=skip_list)
...
class PostgresqlSQLDiff(SQLDiff):
    ...
    def SQL_FIELD_TYPE_DIFFER(self,
                              style,
                              qn,
                              args): return "%s %s\n\t%s %s %s %s;" % (style.SQL_KEYWORD('ALTER TABLE'),
                                                                       style.SQL_TABLE(
                                                                           qn(args[0])),
                                                                       style.SQL_KEYWORD(
                                                                           'ALTER'),
    ...
    def SQL_FIELD_PARAMETER_DIFFER(self,
                                   style,
                                   qn,
                                   args): return "%s %s\n\t%s %s %s %s;" % (style.SQL_KEYWORD('ALTER TABLE'),
                                                                            style.SQL_TABLE(
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        ...
        if field:
            if db_type.endswith("[]"):
                ...
                # process to compare to whatever django spits out as the
            ...
            if table_name:
                ...
                check_constraint = self.check_constraints.get(
                    (tablespace, table_name, attname), {}).get(
                    'pg_get_constraintdef', None)
                if check_constraint:
                    check_constraint = check_constraint.replace("((", "(")
                    check_constraint = check_constraint.replace("))", ")")
                    check_constraint = '("'.join([')' in e and '" '.join(p.strip(
                        '"') for p in e.split(" ", 1)) or e for e in check_constraint.split("(")])
