...
try:
    ...
    try:
        ...
        def get_installed_distributions(
            ...
            return [cast(pkg_resources.Distribution,
                         dist)._dist for dist in dists]
except ImportError:
    # pip < 10
    try:
...
class Command(BaseCommand):
        ...
        with PipSession() as session:
            for filename in req_files:
                for req in parse_requirements(filename, session=session):
                    ...
                    # url attribute changed to link in pip version 6.1.0 and
                    # above
        ...
        for name, req in list(self.reqs.items()):
            if req["url"]:
                ...
                pkg_info = self.style.BOLD(
                    "{dist.project_name} {dist.version}".format(
                        dist=dist))
            else:
                msg = "not installed"
                pkg_info = name
            self.stdout.write(
                "{pkg_info:40} {msg}".format(
                    pkg_info=pkg_info, msg=msg))
            del self.reqs[name]
    ...
    def check_github(self):
        ...
        for name, req in list(self.reqs.items()):
            ...
            try:
                path_parts = urlparse(req_url).path.split(
                    "#", 1)[0].strip("/").rstrip("/").split("/")
            ...
            if frozen_commit_sha:
                ...
                frozen_commit_data = requests.get(
                    frozen_commit_url, headers=headers).json()

                if "message" in frozen_commit_data and frozen_commit_data["message"] == "Not Found":
                    msg = self.style.ERROR("{0} not found in {1}. Repo may be private.".format(
                    ...
                    msg = self.style.INFO("{0} is not the head of any branch".format(
