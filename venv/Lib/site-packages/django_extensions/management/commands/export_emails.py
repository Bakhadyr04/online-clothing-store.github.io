...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if group and not Group.objects.filter(name=group).count() == 1:
            names = "', '".join(g['name']
                                for g in Group.objects.values('name'))
            if names:
                names = "'" + names + "'."
            raise CommandError(
                "Unknown group '" +
        ...
        order_by = getattr(
            settings, 'EXPORT_EMAILS_ORDER_BY', [
                'last_name', 'first_name', 'username', 'email'])
        fields = getattr(
            settings, 'EXPORT_EMAILS_FIELDS', [
                'last_name', 'first_name', 'username', 'email'])
    ...
    def emails(self, qs):
        ...
        """
        self.stdout.write(",\n".join(ent['email']
                          for ent in qs if ent.get('email')))
        self.stdout.write("\n")
    ...
    def linkedin(self, qs):
        ...
        for ent in qs:
            csvf.writerow([ent.get('first_name', ''), ent.get(
                'last_name', ''), ent.get('email', '')])
    ...
    def outlook(self, qs):
        ...
        for ent in qs:
            csvf.writerow(
                [self.full_name(**ent), ent.get('email', '')] + empty)
