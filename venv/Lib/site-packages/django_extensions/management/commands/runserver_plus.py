            ...
            def extra_files(self):
                ...
                if len(self._template_files) != len(template_files):
                    ...
                    for filename in changed:
                        _log(
                            "info", f" * New file {filename} added, reset template loaders")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        setup_logger(logger, self.stderr, filename=options['output_file'])
        ...
        try:
            ...
            if (pdb_option or ipdb_option or settings.DEBUG):
                ...
                settings_middleware = getattr(
                    settings, 'MIDDLEWARE', None) or settings.MIDDLEWARE_CLASSES
            ...
            def postmortem(request, exc_type, exc_value, tb):
                if has_ipdb():
                    ...
                    p = pdb
                print(
                    "Exception occured: %s, %s" %
                    (exc_type, exc_value), file=sys.stderr)
                p.post_mortem(tb)
    ...
    def inner_run(self, options):
        if not HAS_WERKZEUG:
            raise CommandError(
                "Werkzeug is required to use runserver_plus. Please visit https://werkzeug.palletsprojects.com/ or install via pip. (pip install Werkzeug)")
        ...
        if use_reloader and settings.USE_I18N:
            self.extra_files |= set(
                filter(
                    lambda filename: str(filename).endswith('.mo'),
                    gen_filenames()))
        ...
        # See also:
        # https://github.com/django-extensions/django-extensions/issues/832
    ...
    def _determine_path_for_file(
        ...
        file_name = cls._get_file_name(
            current_file_path) or cls._get_file_name(other_file_path)
        extension = cls._get_extension(current_file_path) or expected_extension
        return os.path.join(directory, file_name + extension)

    @classmethod
    def _get_directory_basing_on_file_paths(
            cls, current_file_path, other_file_path):
        return cls._get_directory(current_file_path) or cls._get_directory(
            other_file_path) or os.getcwd()
