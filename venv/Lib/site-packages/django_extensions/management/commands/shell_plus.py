...
class Command(BaseCommand):
    ...
    def load_base_kernel_spec(self, app):
        ...
        if not ks:
            raise CommandError(
                "No notebook (Python) kernel specs found. Tried %r" %
                try_spec_names)
    ...
    def run_notebookapp(self, app_init, options,
                        use_kernel_specs=True, history=True):
        ...
        if not history and not any(arg.startswith(
                '--HistoryManager') for arg in ipython_arguments):
...
""",
    ...
    def set_application_name(self, options):
        ...
        for connection in connections.all():
            ...
            if any(klass.__module__.startswith(supported_backends)
                   for klass in mro):
    ...
    def handle(self, *args, **options):
        ...
        with monkey_patch_cursordebugwrapper(print_sql=options["print_sql"] or print_sql, truncate=truncate, print_sql_location=options["print_sql_location"], confprefix="SHELL_PLUS"):
            ...
            if get_runner:
                ...
                # try the runners that are least unexpected (normal shell
                # runners)
                preferred_runners = [
                    'ptipython',
                    'ptpython',
