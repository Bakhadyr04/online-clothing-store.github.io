    ...
    for directive in import_directives:
        ...
        try:
            if isinstance(directive, str) and directive.startswith(
                    ...
                    if not quiet_load:
                        print(
                            style.ERROR(
                                ...
                                (directive, exc)))
                        continue
                if not all(isinstance(body, (ast.Import, ast.ImportFrom))
                           for body in node.body):
                    if not quiet_load:
                        print(
                            style.ERROR(
                                "Only specify import statements: %r" %
                                directive))
                    continue
                ...
                warnings.warn(
                    "Old style import definitions are deprecated. You should use the new style which is similar to normal Python imports. ",
                ...
                if isinstance(directive, str):
                    ...
                    if isinstance(directive[1], (list, tuple)) and all(
                        ...
                        # Try the ('module.submodule', ('classname1',
                        # 'classname2')) form
...
def import_objects(options, style):
    ...
    def import_subclasses():
        base_classes_to_import = getattr(
            settings,
            'SHELL_PLUS_SUBCLASSES_IMPORT',
            [])  # type: List[Union[str, type]]
        if base_classes_to_import:
            if not quiet_load:
                print(style.SQL_TABLE("# Shell Plus Subclasses Imports"))
            perform_automatic_imports(SubclassesFinder(
                base_classes_to_import).collect_subclasses())
    ...
    def perform_automatic_imports(modules_to_classes):
        """
    ...
    if mongoengine and not dont_load_any_models:
        for name, mod in _document_registry.items():
            ...
            if app_name in dont_load or (
                    "%s.%s" % (app_name, name)) in dont_load:
                continue
    ...
    if not dont_load_any_models:
        for app_mod, app_models in get_apps_and_models():
            ...
            for mod in app_models:
                ...
                if mod.__module__:
                    # Only add the module to the dict if `__module__` is not
                    # empty.
                    load_models.setdefault(mod.__module__, [])
    ...
    import_subclasses()
    if not quiet_load:
        print(style.SQL_TABLE("# Shell Plus Model Imports%s") %
