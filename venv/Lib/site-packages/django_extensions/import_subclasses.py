...
class SubclassesFinder:
    ...
    # type: (Tuple[str, type]) -> bool
    def _should_be_imported(self, candidate_to_import):
    ...
    def collect_subclasses(self):
        ...
        for loader, module_name, is_pkg in walk_packages(
            ...
            subclasses_from_module = self._collect_classes_from_module(
                module_name)
    ...
    def _collect_classes_from_module(self, module_name):
        ...
        classes_to_import = getmembers(
            imported_module, lambda element: isclass(
                element) and element.__module__ == imported_module.__name__
        )
        classes_to_import = list(
            filter(
                self._should_be_imported,
                classes_to_import))
