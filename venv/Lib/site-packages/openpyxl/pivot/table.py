...
class ConditionalFormatList(Serialisable):
    ...
    def _dedupe(self):
        ...
        * Data fields are referenced by the 0-index reference.x.v value
        ...
        fmts = {
            field: fmt for (
                field, priority), fmt in sorted(
                fmts.items(), reverse=True)}
...
class TableDefinition(Serialisable):
    ...
    conditionalFormats = Typed(
        expected_type=ConditionalFormatList,
        allow_none=True)
    ...
    rowHierarchiesUsage = Typed(
        expected_type=RowHierarchiesUsage,
        allow_none=True)
    colHierarchiesUsage = Typed(
        expected_type=ColHierarchiesUsage,
        allow_none=True)
    extLst = Typed(expected_type=ExtensionList, allow_none=True)
    id = Relation()
