...
class WorkbookParser:
    ...
    def rels(self):
        if self._rels is None:
            self._rels = get_dependents(
                self.archive, get_rels_path(
                    self.workbook_part_name)).to_dict()
        return self._rels
    ...
    def find_sheets(self):
        ...
        for sheet in self.sheets:
            if not sheet.id:
                msg = f"File contains an invalid specification for {0}. This will be removed".format(
                    sheet.name)
                warn(msg)
                continue
            ...
            for name, defn in names.items():
                ...
                if reserved is None:
                    ...
                    try:
                        ...
                        warn(
                            f"Print area cannot be set to Defined name: {defn.value}.")
                        continue
    ...
    def pivot_caches(self):
        ...
        for c in self.caches:
            ...
            if cache.deps:
                records = get_rel(
                    self.archive, cache.deps, cache.id, RecordList)
