...
class Worksheet(_WorkbookChild):
    ...
    def _get_cell(self, row, column):
        ...
        if not 0 < row < 1048577:
            raise ValueError(
                f"Row numbers must be between 1 and 1048576. Row number supplied was {row}")
    ...
    def iter_rows(self, min_row=None, max_row=None,
        ...
        return self._cells_by_row(
            min_col, min_row, max_col, max_row, values_only)

    def _cells_by_row(self, min_col, min_row, max_col,
                      max_row, values_only=False):
    ...
    def iter_cols(self, min_col=None, max_col=None,
                  min_row=None, max_row=None, values_only=False):
        """
    ...
    def _cells_by_col(self, min_col, min_row, max_col,
                      max_row, values_only=False):
        """
        Get cells by column
        """
    ...
    def column_groups(self):
        ...
        """
        return [cd.range for cd in self.column_dimensions.values()
    ...
    def add_table(self, table):
        ...
        if self.parent._duplicate_name(table.name):
            raise ValueError(
                "Table with name {0} already exists".format(
                    table.name))
    ...
    def unmerge_cells(self, range_string=None, start_row=None,
                      start_column=None, end_row=None, end_column=None):
    ...
    def _move_cells(self, min_row=None, min_col=None,
        ...
        for row, column in sorted(
                self._cells, key=itemgetter(key), reverse=reverse):
    ...
    def _move_cell(self, row, column, row_offset, col_offset, translate=False):
        ...
        if translate and cell.data_type == "f":
            ...
            cell.value = t.translate_formula(
