...
def dataframe_to_rows(df, index=True, header=True):
    ...
    # Using the expanded index is preferable to df.itertuples(index=True) so
    # that we have 'None' inserted where applicable
...
def expand_index(index, header=False):
    ...
    # or if an earlier member was different, all later members should be added
    # to the row
    ...
    for value in values:
        ...
        # Once there's a difference in member of an index with the prior index,
        # we need to store all subsequent members in the row
        prior_change = False
        for idx, (current_index_member, previous_index_member) in enumerate(
        ...
        # If this is for a row index, we're already returning a row so just
        # yield
        if not header:
            yield row
        else:
