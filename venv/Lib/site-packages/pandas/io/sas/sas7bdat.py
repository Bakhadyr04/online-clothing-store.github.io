...
def _convert_datetimes(sas_datetimes: pd.Series, unit: str) -> pd.Series:
    ...
    if unit == "s":
        ...
        return pd.Series(vals, dtype="M8[s]",
                         index=sas_datetimes.index, copy=False)
...
class SAS7BDATReader(ReaderBase, abc.Iterator):
    ...
    def _get_properties(self) -> None:
        ...
        #  expected "Sized"
        if len(
                self._cached_page) != self.header_length:  # type: ignore[arg-type]
    ...
    def _read_and_convert_header_text(
            self, offset: int, length: int) -> str | bytes:
    ...
    def _read_page_header(self) -> None:
        ...
        self._current_page_block_count = self._read_uint(
    ...
    def _process_page_metadata(self) -> None:
        ...
        for i in range(self._current_page_subheaders_count):
            ...
            subheader_signature = self._read_bytes(
                subheader_offset, self._int_length)
            ...
            if subheader_processor is None:
                f1 = subheader_compression in (
                    const.compressed_subheader_id, 0)
    ...
    def _process_columntext_subheader(self, offset: int, length: int) -> None:
        ...
        if len(self.column_names_raw) == 1:
            ...
            if hasattr(self, "creator_proc"):
                self.creator_proc = self._convert_header_text(
                    self.creator_proc)
    ...
    def _process_columnattributes_subheader(
            self, offset: int, length: int) -> None:
        int_len = self._int_length
        column_attributes_vectors_count = (
            length - 2 * int_len - 12) // (int_len + 8)
        for i in range(column_attributes_vectors_count):
            col_data_offset = (
                offset + int_len + const.column_data_offset_offset +
                i * (int_len + 8)
            )
            col_data_len = (
                offset
    ...
    def _chunk_to_dataframe(self) -> DataFrame:
        ...
        for j in range(self.column_count):
            ...
            if self._column_types[j] == b"d":
                col_arr = self._byte_chunk[jb, :].view(
                    dtype=self.byte_order + "d")
                rslt[name] = pd.Series(
                    col_arr, dtype=np.float64, index=ix, copy=False)
