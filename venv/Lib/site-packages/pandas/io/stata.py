...
def _stata_elapsed_date_to_datetime_vec(dates: Series, fmt: str) -> Series:
    ...
    def convert_year_month_safe(year, month) -> Series:
        ...
        if year.max() < MAX_YEAR and year.min() > MIN_YEAR:
            ...
            return Series([datetime(y, m, 1)
                          for y, m in zip(year, month)], index=index)
    ...
    def parse_dates_safe(
        ...
        if lib.is_np_dtype(dates.dtype, "M"):
            if delta:
                ...
                d["delta"] = time_delta._values.view(
...
def _cast_to_stata_types(data: DataFrame) -> DataFrame:
    ...
    for col in data:
        ...
        if is_nullable_int:
            ...
            data[col] = data[col].fillna(fv).astype(
                data[col].dtype.numpy_dtype)
...
class StataMissingValue:
    ...
    for i in range(27):
        ...
        int_value = struct.unpack("<i", struct.pack("<f", key))[
            0] + increment_32
    ...
    for i in range(27):
        ...
        if i > 0:
            MISSING_VALUES[key] += chr(96 + i)
        int_value = struct.unpack("q", struct.pack("<d", key))[
            0] + increment_64
...
class StataReader(StataParser, abc.Iterator):
    ...
    def _read_uint16(self) -> int:
        return struct.unpack(f"{self._byteorder}H",
                             self._path_or_buf.read(2))[0]

    def _read_uint32(self) -> int:
        return struct.unpack(f"{self._byteorder}I",
                             self._path_or_buf.read(4))[0]

    def _read_uint64(self) -> int:
        return struct.unpack(f"{self._byteorder}Q",
                             self._path_or_buf.read(8))[0]
        ...
        for _ in range(self._nvar):
            ...
            if typ <= 2045:
                ...
                try:
                    ...
                    raise ValueError(
                        f"cannot convert stata types [{typ}]") from err
    ...
    def _get_varlist(self) -> list[str]:
        ...
        return [self._decode(self._path_or_buf.read(b))
                for _ in range(self._nvar)]
    ...
    def _read_old_header(self, first_char: bytes) -> None:
        ...
        try:
            ...
            raise ValueError(
                f"cannot convert stata types [{invalid_types}]") from err
        try:
            self._dtyplist = [self.DTYPE_MAP[typ] for typ in typlist]
        except ValueError as err:
            invalid_dtypes = ",".join([str(x) for x in typlist])
            raise ValueError(
                f"cannot convert stata dtypes [{invalid_dtypes}]") from err
    ...
    def _setup_dtype(self) -> np.dtype:
        ...
        for i, typ in enumerate(self._typlist):
            if typ in self.NUMPY_TYPE_MAP:
                ...
                dtypes.append(
                    (f"s{i}", f"{self._byteorder}{self.NUMPY_TYPE_MAP[typ]}"))
            else:
...
the string values returned are correct."""
    ...
    def read(
        ...
        valid_dtypes = [
            i for i, dtyp in enumerate(
                self._dtyplist) if dtyp is not None]
        ...
        if convert_dates:
            for i, fmt in enumerate(self._fmtlist):
                if any(fmt.startswith(date_fmt) for date_fmt in _date_formats):
                    data.isetitem(
                        i, _stata_elapsed_date_to_datetime_vec(
                            data.iloc[:, i], fmt)
                    )
    ...
    def _do_convert_missing(self, data: DataFrame,
                            convert_missing: bool) -> DataFrame:
        ...
        for i in range(len(data.columns)):
            ...
            if convert_missing:  # Replacement follows Stata notation
                ...
                umissing, umissing_loc = np.unique(
                    series[missing], return_inverse=True)
    ...
    def _do_select_columns(self, data: DataFrame,
        ...
        for col, label in zip(data, lbllist):
            if label in value_label_dict:
                ...
                if initial_categories is None:
                    # If None here, then we need to match the cats in the
                    # Categorical
                    categories = []
...
def _maybe_convert_to_int_keys(
        convert_dates: dict, varlist: list[Hashable]) -> dict:
    new_dict = {}
    for key in convert_dates:
        ...
        if key in varlist:
            ...
            if not isinstance(key, int):
                raise ValueError(
                    "convert_dates key must be a column or an integer")
    ...
    def write_file(self) -> None:
        ...
        with get_handle(
            ...
            if self.handles.compression["method"] is not None:
                ...
                # Write it first into a buffer and then write the buffer to the
                # ZipFile.
...
def _dtype_to_stata_type_117(
        dtype: np.dtype, column: Series, force_strl: bool) -> int:
    """
...
class StataWriter117(StataWriter):
    ...
    def _write_sortlist(self) -> None:
        ...
        self._write_bytes(self._tag(b"\x00" * sort_size *
                          (self.nvar + 1), "sortlist"))
    ...
    def _write_value_label_names(self) -> None:
        ...
        for i in range(self.nvar):
            ...
            if self._has_value_labels[i]:
                name = self.varlist[i]
            name = self._null_terminate_str(name)
            encoded_name = _pad_bytes_new(
                name[:32].encode(self._encoding), vl_len + 1)
    ...
    def _convert_strls(self, data: DataFrame) -> DataFrame:
        ...
        if convert_cols:
            ssw = StataStrLWriter(
                data, convert_cols, version=self._dta_version)
