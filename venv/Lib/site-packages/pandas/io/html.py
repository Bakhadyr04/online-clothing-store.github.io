...
def _get_skiprows(
        skiprows: int | Sequence[int] | slice | None) -> int | Sequence[int]:
    """
...
class _BeautifulSoupHtml5LibFrameParser(_HtmlFrameParser):
    ...
    def _parse_tables(self, document, match, attrs):
        ...
        for table in tables:
            ...
            if table not in unique_tables and table.find(
                    string=match) is not None:
                result.append(table)
    ...
    if head:
        ...
        if header is None:
            if len(head) == 1:
                ...
                header = [
                    i for i, row in enumerate(head) if any(
