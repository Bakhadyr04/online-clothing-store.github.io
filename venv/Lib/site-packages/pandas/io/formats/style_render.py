...
class StylerRenderer:
    ...
    def _translate_latex(self, d: dict, clines: str | None) -> None:
        ...
        d["head"] = [
            [
                {**col,
                 "cellstyle": self.ctx_columns[r,
                                               c - visible_index_level_n]}
    ...
    def relabel_index(
        ...
        for ai, i in enumerate(
                [i for i in range(len(obj)) if i not in hidden_labels]):
            if len(levels_) == 1:
    ...
    for i, lvl in enumerate(levels):
        ...
        for j, row in enumerate(lvl):
            ...
            if not sparsify:
                ...
                # this element has not been sparsified so must be the start of
                # section
                last_label = j
                ...
                # then element must be part of sparsified section and is
                # visible
                visible_row_count += 1
                if visible_row_count > max_index:
                    break  # do not add a length since the render trim limit reached
                if lengths[(i, last_label)] == 0:
                    # if previous iteration was first-of-section but hidden
                    # then offset
                    last_label = j
                    lengths[(i, last_label)] = 1
                else:
...
def _maybe_wrap_formatter(
    ...
    available.
    """
    # Get initial func from input string, input callable, or from default
    # factory
...
def _parse_latex_table_wrapping(
        table_styles: CSSStyles, caption: str | None) -> bool:
    """
...
def _parse_latex_table_styles(
        table_styles: CSSStyles, selector: str) -> str | None:
    """
...
def _parse_latex_cell_styles(
    ...
    for command, options in latex_styles[::-
                                         1]:  # in reverse for most recent style
        formatter = {
...
def _parse_latex_options_strip(value: str | float, arg: str) -> str:
    ...
    """
    return str(value).replace(arg, "").replace(
        "/*", "").replace("*/", "").strip()
