    ...
    *{"row": "MY_ROW_CLASS", "col_trim": "", "row_trim": ""}*.
    ...
    def set_tooltips(
        ...
        if not self.cell_ids:
            ...
            # redesign and more extensive code for a feature that might be
            # rarely used.
    ...
    def to_latex(
        ...
        **CSS Conversion**
        ...
        if column_format is not None:
            ...
            # create a default: set float, complex, int cols to 'r' ('S'),
            # index to 'l'
    ...
    def _map(self, func: Callable, subset: Subset |
             None = None, **kwargs) -> Styler:
    ...
    @Substitution(subset=subset_args)
    def map(self, func: Callable, subset: Subset |
            None = None, **kwargs) -> Styler:
        """
    ...
    def set_sticky(
        ...
        if not isinstance(obj, pd.MultiIndex):
            ...
            if axis == 1:
                ...
                # if self._hide_columns then no <thead><tr> here will exist: no
                # conflict
                styles: CSSStyles = [
                    {
    ...
    def hide(
        ...
        if subset is None:
            if level is None and names:
                # this combination implies user shows the index and hides just
                # names
    ...
    def highlight_quantile(
        ...
        # applying the calculated quantile to alternate axis, e.g. to each
        # column
...
def _highlight_value(data: DataFrame | Series, op: str,
    ...
    if isinstance(
            data, DataFrame):  # min/max must be done twice to return scalar
...
def _bar(
    ...
    def css_calc(x, left: float, right: float, align: str,
        ...
        if align == "left":
            # all proportions are measured from the left side between left and
            # right
            end = (x - left) / (right - left)

        elif align == "right":
            # all proportions are measured from the right side between left and
            # right
            ...
            if align == "zero":
                ...
                # bars drawn from zero either leftwards or rightwards with
                # center at mid
    ...
    # A tricky way to address the issue where np.nanmin/np.nanmax fail to
    # handle pd.NA.
