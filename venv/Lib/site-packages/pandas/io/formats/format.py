...
class DataFrameFormatter:
    ...
    def is_truncated_horizontally(self) -> bool:
        return bool(self.max_cols_fitted and (
            len(self.columns) > self.max_cols_fitted))

    @property
    def is_truncated_vertically(self) -> bool:
        return bool(self.max_rows_fitted and (
            len(self.frame) > self.max_rows_fitted))
    ...
    def _initialize_colspace(
            self, col_space: ColspaceArgType | None) -> ColspaceType:
        result: ColspaceType
    ...
    def _get_formatted_column_labels(
            self, frame: DataFrame) -> list[list[str]]:
    ...
    def _get_formatted_index(self, frame: DataFrame) -> list[str]:
        ...
        if isinstance(index, MultiIndex):
            ...
            fmt_index = [
                index._format_flat(
                    include_name=self.show_row_idx_names,
                    formatter=fmt)
            ]
...
class _GenericArrayFormatter:
    ...
    def _format_strings(self) -> list[str]:
        ...
        for i, v in enumerate(vals):
            if (not is_float_type[i]
                    or self.formatter is not None) and leading_space:
...
def _format_datetime64_dateonly(
    ...
    if date_format:
        ...
        # Timestamp._date_repr relies on string formatting (faster than
        # strftime)
        return x._date_repr
...
def _trim_zeros_complex(str_complexes: ArrayLike,
                        decimal: str = ".") -> list[str]:
    """
...
def set_eng_float_format(accuracy: int = 3,
                         use_eng_prefix: bool = False) -> None:
    """
