...
class PythonParser(ParserBase):
    ...
    def _infer_columns(
        ...
        if self.header is not None:
            ...
            for level, hr in enumerate(header):
                try:
                    ...
                    if 0 < self.line_pos <= hr and (
                        ...
                        # we have mi columns, the last line is not part of the
                        # header
                        joi = list(
                ...
                if not have_mi_columns:
                    ...
                    # TODO: Use pandas.io.common.dedup_names instead (see
                    # #50371)
                    ...
                    if hr == header[-1]:
                        ...
                        if (lc != unnamed_count and lc -
                                ic > unnamed_count) or ic == 0:
                            clear_buffer = False
            ...
            if names is not None:
                ...
                if len(names) > len(columns[0]) and len(
                        names) > len_first_data_row:
                    raise ValueError(
    ...
    def _next_line(self) -> list[Scalar]:
        if isinstance(self.data, list):
            ...
            while self.skipfunc(self.pos):
                self.pos += 1
                # assert for mypy, data is Iterator[str] or None, would error
                # in next
    ...
    def _next_iter_line(self, row_num: int) -> list[Scalar] | None:
        ...
        """
        try:
            # assert for mypy, data is Iterator[str] or None, would error in
            # next
            assert self.data is not None
            line = next(self.data)
            # for mypy
    ...
    def _remove_empty_lines(
            self, lines: list[list[Scalar]]) -> list[list[Scalar]]:
        """
...
class FixedWidthReader(abc.Iterator):
    ...
    def get_rows(self, infer_nrows: int,
    ...
    def __next__(self) -> list[str]:
        ...
        return [line[from_:to].strip(self.delimiter)
...
class FixedWidthFieldParser(PythonParser):
    ...
    def _make_reader(self, f: IO[str] |
                     ReadCsvBuffer[str]) -> FixedWidthReader:
...
def _validate_skipfooter_arg(skipfooter: int) -> int:
    ...
    # Incompatible return value type (got "Union[int, integer[Any]]", expected
    # "int")
