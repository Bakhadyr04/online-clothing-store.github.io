...
class ParserBase:
    ...
    def __init__(self, kwds) -> None:
        ...
        self.on_bad_lines = kwds.get(
            "on_bad_lines", self.BadLineHandleMethod.ERROR)

    def _validate_parse_dates_presence(
            self, columns: Sequence[Hashable]) -> Iterable:
        """
        ...
        if is_dict_like(self.parse_dates):
            ...
            cols_needed = itertools.chain.from_iterable(
                col if is_list_like(col) and not isinstance(
                    col, tuple) else [col]
    ...
    def _extract_multi_indexer_columns(
        ...
        if not all(len(header_iter) ==
                   field_count for header_iter in header[1:]):
            raise ParserError(
    ...
    def _get_complex_date_index(self, data, col_names):
        def _get_name(icol):
            ...
            if col_names is None:
                raise ValueError(
                    f"Must supply column order to use {icol!s} as index")
        ...
        for i, arr in enumerate(index):
            ...
            if self.index_names is not None:
                ...
                if isinstance(converters, dict):
                    index_converter = converters.get(
                        self.index_names[i]) is not None
    ...
    def _infer_types(
        ...
        if issubclass(values.dtype.type, (np.number, np.bool_)):
            # If our array has numeric dtype, we don't have to check for
            # strings in isin
            na_values = np.array(
            ...
            try:
                ...
                if non_default_dtype_backend:
                    ...
                    if result_mask.all():
                        result = IntegerArray(
                            np.ones(
                                result_mask.shape, dtype=np.int64), result_mask
                        )
        ...
        if dtype_backend == "pyarrow":
            ...
            if isinstance(result, np.ndarray):
                result = ArrowExtensionArray(
                    pa.array(result, from_pandas=True))
            elif isinstance(result, BaseMaskedArray):
                if result._mask.all():
                    # We want an arrow null array here
                    result = ArrowExtensionArray(
                        pa.array([None] * len(result)))
                else:
                    result = ArrowExtensionArray(
    ...
    @final
    def _cast_types(self, values: ArrayLike,
                    cast_type: DtypeObj, column) -> ArrayLike:
        """
    ...
    def _check_data_length(
        ...
        if not self.index_col and len(columns) != len(data) and columns:
            ...
            empty_str_or_na = empty_str | isna(
                data[-1])  # type: ignore[operator]
    ...
    def _clean_index_names(
            self, columns, index_col) -> tuple[list | None, list, list]:
    ...
    def _get_empty_meta(self, columns, dtype: DtypeArg | None = None):
        ...
        if not is_dict_like(dtype):
            ...
            dtype_dict = defaultdict(
                lambda: object,
                {columns[k] if is_integer(
                    k) else k: v for k, v in dtype.items()},
            )
...
def _make_date_converter(
    ...
    def converter(*date_cols, col: Hashable):
        ...
        if date_parser is lib.no_default:
            ...
            date_fmt = (
                date_format.get(col) if isinstance(
                    date_format, dict) else date_format
            )
...
def _process_date_conversion(
        ...
        for colspec in parse_spec:
            if is_scalar(colspec) or isinstance(colspec, tuple):
                ...
                if new_name in data_dict:
                    raise ValueError(
                        f"New date column already in dict {new_name}")
    ...
    if isinstance(data_dict, DataFrame):
        data_dict = concat(
            [DataFrame(new_data), data_dict], axis=1, copy=False)
    else:
...
def _try_convert_dates(
    ...
    new_col = parser(
        *to_parse,
        col=new_name if target_name is None else target_name)
