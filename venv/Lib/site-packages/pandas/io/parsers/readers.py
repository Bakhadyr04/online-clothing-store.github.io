...
@overload
def validate_integer(name: str, val: int | None,
...
def read_csv(
    ...
    *,
    ...
    if lib.is_list_like(parse_dates):
        ...
        if not all(is_hashable(x)
                   for x in parse_dates):  # type: ignore[union-attr]
            depr = True
...
def read_table(
    ...
    *,
    ...
    # error: Item "bool" of "bool | Sequence[Hashable]" has no attribute
    # "__iter__"
    if lib.is_list_like(parse_dates) and not all(is_hashable(x)
                                                 for x in parse_dates):  # type: ignore[union-attr]
        # GH#55569
        warnings.warn(
