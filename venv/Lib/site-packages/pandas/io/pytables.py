    ...
    if isinstance(where, (list, tuple)):
        where = [
            Term(term, scope_level=level +
                 1) if maybe_expression(term) else term
            for term in where
            if term is not None
        ]
...
def to_hdf(
    ...
    if append:
        def f(store): return store.append(
            key,
            value,
            format=format,
            index=index,
    ...
    **kwargs, the HDF5 file is loaded or created in-memory and will only be
        ...
        for k, v in d.items():
            ...
            filtered = (
                {key: value for (key, value)
                 in min_itemsize.items() if key in v}
    ...
    def _write_to_group(
        ...
        if append:
            ...
            if not s.is_table or (s.is_table and format ==
                                  "fixed" and s.is_exists):
...
class DataCol(IndexCol):
    ...
    def __repr__(self) -> str:
        temp = tuple(
            map(
                pprint_thing, (self.name, self.cname,
                               self.dtype, self.kind, self.shape)
            )
        )
        return ",".join(
            [
...
class Fixed:
    ...
    def pandas_type(self):
        return _ensure_decoded(
            getattr(self.group._v_attrs, "pandas_type", None))
...
class GenericFixed(Fixed):
    ...
    def read_array(self, key: str, start: int | None = None,
                   stop: int | None = None):
    ...
    def write_index(self, key: str, index: Index) -> None:
        if isinstance(index, MultiIndex):
            ...
            converted = _convert_index(
                "index", index, self.encoding, self.errors)
    ...
    def write_multi_index(self, key: str, index: MultiIndex) -> None:
        ...
        for i, (lev, level_codes, name) in enumerate(
            ...
            conv_level = _convert_index(
                level_key, lev, self.encoding, self.errors)
    ...
    def read(
        ...
        for i in range(self.nblocks):
            ...
            values = self.read_array(
                f"block{i}_values", start=_start, stop=_stop)
...
class Table(Fixed):
    ...
    def indexables(self):
        ...
        #  `c` below is a str.
        _indexables.extend([f(i, c)
                           for i, c in enumerate(self.attrs.values_cols)])
    ...
    def validate_data_columns(
            self, data_columns, min_itemsize, non_index_axes):
        """
        ...
        for i, (blk, b_items) in enumerate(zip(blocks, blk_items)):
            ...
            if data_columns and len(
                ...
                if not (name is None or isinstance(name, str)):
                    # TODO: should the message here be more specifically
                    # non-str?
                    raise ValueError(
    ...
    def process_axes(self, obj, selection: Selection,
    ...
    def read_coordinates(
        ...
        if selection.filter is not None:
            for field, op, filt in selection.filter.format():
                ...
                coords = coords[op(
                    data.iloc[coords - coords.min()], filt).values]
    ...
    def read_column(
        ...
        for a in self.axes:
            if column == a.name:
                ...
                return Series(
                    _set_tz(col_values[1], a.tz), name=column, copy=False)
...
class AppendableTable(Table):
    ...
    def delete(self, where=None, start: int |
               None = None, stop: int | None = None):
...
class AppendableFrameTable(AppendableTable):
    ...
    def read(
        ...
        inds = [
            i for i, ax in enumerate(
                self.axes) if ax is self.index_axes[0]]
        assert len(inds) == 1
        ind = inds[0]
...
class AppendableMultiFrameTable(AppendableFrameTable):
    ...
    def read(
        ...
        df.index = df.index.set_names(
            [None if self._re_levels.search(
                name) else name for name in df.index.names]
        )
...
@overload
def _set_tz(values: np.ndarray | Index, tz: None,
            coerce: bool = False) -> np.ndarray:
    ...
...
def _unconvert_index(data, kind: str, encoding: str,
    ...
    if kind.startswith("datetime64"):
        ...
        try:
            index = np.asarray([date.fromordinal(v)
                               for v in data], dtype=object)
        except ValueError:
            index = np.asarray([date.fromtimestamp(v)
                               for v in data], dtype=object)
...
def _maybe_convert_for_string_atom(
    ...
    if isinstance(min_itemsize, dict):
        min_itemsize = int(min_itemsize.get(
            name) or min_itemsize.get("values") or 0)
...
def _unconvert_string_array(
    ...
    if len(data):
        ...
        if isinstance(data[0], bytes):
            data = Series(
                data, copy=False).str.decode(
                encoding, errors=errors)._values
        else:
...
def _maybe_convert(values: np.ndarray, val_kind: str,
...
class Selection:
    ...
    def generate(self, where):
        ...
        try:
            return PyTablesExpr(where, queryables=q,
