...
def assert_numpy_array_equal(
    ...
    def _raise(left, right, err_msg) -> NoReturn:
        if err_msg is None:
            ...
            msg = f"{obj} values are different ({np.round(diff, 5)} %)"
            raise_assert_detail(
                obj, msg, left, right, index_values=index_values)
...
def is_extension_array_dtype_and_needs_i8_conversion(
    ...
    """
    return isinstance(
        left_dtype, ExtensionDtype) and needs_i8_conversion(right_dtype)


def assert_indexing_slices_equivalent(
        ser: Series, l_slc: slice, i_slc: slice) -> None:
    """
