...
class TablePlotter:
    ...
    def _get_cells(self, left, right, vertical) -> tuple[int, int]:
        ...
        if vertical:
            ...
            vcells = max(sum(self._shape(df)[0]
                         for df in left), self._shape(right)[0])
            hcells = max(self._shape(df)[1]
                         for df in left) + self._shape(right)[1]
        else:
            vcells = max([self._shape(df)[0]
                         for df in left] + [self._shape(right)[0]])
            hcells = sum([self._shape(df)[1]
                         for df in left] + [self._shape(right)[1]])
        return hcells, vcells

    def plot(self, left, right,
             labels: Iterable[str] = (), vertical: bool = True):
        """
    ...
    def _make_table(self, ax, df, title: str,
...
def main() -> None:
    ...
    df3 = pd.DataFrame({"v1": [1, 2, 3, 4, 5, 6], "v2": [
                       5, 6, 7, 8, 9, 10]}, index=idx)
    df3.columns = column
