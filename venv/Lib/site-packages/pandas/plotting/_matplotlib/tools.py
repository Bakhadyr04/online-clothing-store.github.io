...
def _has_externally_shared_axis(ax1: Axes, compare_axis: str) -> bool:
    ...
    # Retain ax1 and any of its siblings which aren't in the same position as
    # it
...
def handle_shared_axes(
    ...
    if nplots > 1:
        def row_num(x): return x.get_subplotspec().rowspan.start
        def col_num(x): return x.get_subplotspec().colspan.start

        def is_first_col(x): return x.get_subplotspec().is_first_col()
        ...
        if nrows > 1:
            try:
                ...
                def is_last_row(x): return x.get_subplotspec().is_last_row()
                for ax in axarr:
                    if is_last_row(ax):
                        continue
                    if sharex or _has_externally_shared_axis(ax, "x"):
