...
def deregister() -> None:
    ...
    for unit, formatter in _mpl_units.items():
        if type(formatter) not in {DatetimeConverter,
                                   PeriodConverter, TimeConverter}:
...
class TimeConverter(munits.ConversionInterface):
    ...
    def convert(value, unit, axis):
        ...
        if isinstance(value, valid_types) or is_integer(
                value) or is_float(value):
...
def _period_break_mask(dates: PeriodIndex,
                       period: str) -> npt.NDArray[np.bool_]:
...
def _daily_finder(vmin: float, vmax: float, freq: BaseOffset) -> np.ndarray:
    ...
    def first_label(label_flags):
        if (label_flags[0] == 0) and (
                label_flags.size > 1) and ((vmin_orig % 1) > 0.0):
...
@functools.cache
def _quarterly_finder(vmin: float, vmax: float,
                      freq: BaseOffset) -> np.ndarray:
    _, _, periodsperyear = _get_periods_per_ymd(freq)
    vmin_orig = vmin
    (vmin, vmax) = (int(vmin), int(vmax))
    span = vmax - vmin + 1
