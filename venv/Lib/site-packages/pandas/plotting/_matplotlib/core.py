...
class MPLPlot(ABC):
    ...
    def _validate_color_args(self, color, colormap):
        ...
        if isinstance(color, tuple) and self.nseries == 1 and len(
                color) in (3, 4):
    ...
    def _has_plotted_object(ax: Axes) -> bool:
        ...
        return len(ax.lines) != 0 or len(
            ax.artists) != 0 or len(ax.containers) != 0
    ...
    def _append_legend_handles_labels(
            self, handle: Artist, label: str) -> None:
        """
    ...
    def _col_idx_to_axis_idx(self, col_idx: int) -> int:
        ...
        if isinstance(self.subplots, list):
            # Subplots is a list: some columns will be grouped together in the
            # same ax
            return next(
                group_idx
...
class ScatterPlot(PlanePlot):
    ...
    def _get_c_values(self, color, color_by_categorical: bool,
                      c_is_column: bool):
        c = self.c
...
class HexBinPlot(PlanePlot):
    ...
    def __init__(self, data, x, y, C=None, *,
                 colorbar: bool = True, **kwargs) -> None:
        super().__init__(data, x, y, **kwargs)
        if is_integer(C) and not self.data.columns._holds_integer():
            C = self.data.columns[C]
        self.C = C
...
class LinePlot(MPLPlot):
    ...
    def _make_plot(self, fig: Figure) -> None:
        ...
        for i, (label, y) in enumerate(it):
            ...
            if self._is_ts_plot():
                ...
                # TODO: GH28021, should find a way to change view limit on
                # xaxis
    ...
    @final
    @classmethod
    def _update_stacker(cls, ax: Axes, stacking_id: int |
                        None, values) -> None:
        if stacking_id is None:
            return
        if (values >= 0).all():
    ...
    def _post_plot_logic(self, ax: Axes, data) -> None:
        ...
        if self._need_to_set_index:
            ...
            ax.xaxis.set_major_locator(
                FixedLocator(xticks))  # type: ignore[arg-type]
