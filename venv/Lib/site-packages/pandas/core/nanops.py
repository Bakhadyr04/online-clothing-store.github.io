...
def _has_infs(result) -> bool:
    if isinstance(result, np.ndarray):
        if result.dtype in ("f8", "f4"):
            ...
            # result.ndim == 1, so there is no risk of this ravel making a
            # copy.
...
def _na_for_min_count(values: np.ndarray, axis: AxisInt |
...
@bottleneck_switch()
def nanmedian(values, *, axis: AxisInt | None = None,
              skipna: bool = True, mask=None):
    """
    Parameters
    ----------
    values : ndarray
            ...
            if not skipna:
                ...
                with warnings.catch_warnings():
                    ...
                    if (values.shape[1] == 1 and axis == 0) or (
                        ...
                        # GH52788: fastpath when squeezable, nanmedian for 2D
                        # array slow
