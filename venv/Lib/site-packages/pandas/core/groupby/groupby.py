...
class BaseGroupBy(
        PandasObject, SelectionMixin[NDFrameT], GroupByIndexingMixin):
    ...
    def __iter__(self) -> Iterator[tuple[Hashable, NDFrameT]]:
        ...
        # error: Argument 1 to "len" has incompatible type "Hashable"; expected
        # "Sized"
        ...
        if isinstance(keys, list) and len(keys) == 1:
            # GH#42795 - when keys is a list, return tuples even when length is
            # 1
    ...
    **Attributes**
    ...
    def _op_via_apply(self, name: str, *args, **kwargs):
        ...
        if "axis" in sig.parameters:
            if kwargs.get("axis", None) is None or kwargs.get(
                    "axis") is lib.no_default:
    ...
    def _set_result_index_ordered(
        ...
        if self._grouper.has_dropped_na:
            ...
            # with values referring to the row of the input so can use
            # RangeIndex
    ...
    def apply(self, func, *args, include_groups: bool = True,
              **kwargs) -> NDFrameT:
        orig_func = func
    ...
    def _agg_py_fallback(
        ...
        try:
            res_values = self._grouper.agg_series(
                ser, alt, preserve_dtype=True)
        ...
        **kwargs,
        ...
        def array_func(values: ArrayLike) -> ArrayLike:
            ...
            result = self._agg_py_fallback(
                how, values, ndim=data.ndim, alt=alt)
            return result
    ...
    @final
    def _transform(self, func, *args, engine=None,
        ...
        if not isinstance(func, str):
            return self._transform_general(
                func, engine, engine_kwargs, *args, **kwargs)
    ...
    def _value_counts(
        ...
        if isinstance(obj, Series):
            ...
            keys = [
                # Can't use .values because the column label needs to be
                # preserved
                obj.iloc[:, idx]
        ...
        if any(
            isinstance(grouping.grouping_vector,
        ...
        if normalize:
            ...
            levels = list(
                range(len(self._grouper.groupings),
    ...
    def sem(self, ddof: int = 1, numeric_only: bool = False) -> NDFrameT:
        ...
        Freq: MS, dtype: float64
        """
        if numeric_only and self.obj.ndim == 1 and not is_numeric_dtype(
                self.obj.dtype):
            raise TypeError(
    ...
    def size(self) -> DataFrame | Series:
        ...
        with com.temp_setattr(self, "as_index", True):
            ...
            # as_index=False path of _reindex_output fail on categorical
            # groupers.
    ...
    @final
    def resample(self, rule, *args, include_groups: bool = True,
                 **kwargs) -> Resampler:
        """
    ...
    @final
    def _fill(self, direction: Literal["ffill",
              "bfill"], limit: int | None = None):
        """
    ...
    def _nth(
        ...
        if not is_integer(n):
            raise ValueError(
                "dropna option only supported for an integer argument")
        ...
        if len(dropped) == len(self._selected_obj):
            ...
            if self._grouper.has_dropped_na:
                # Null groups need to still be encoded as -1 when passed to
                # groupby
                nulls = grouper == -1
    ...
    def quantile(
        ...
        """
        mgr = self._get_data_to_aggregate(
            numeric_only=numeric_only, name="quantile")
        ...
        def pre_processor(
                vals: ArrayLike) -> tuple[np.ndarray, DtypeObj | None]:
            ...
            if isinstance(vals, BaseMaskedArray) and is_numeric_dtype(
                    vals.dtype):
            ...
            if inference:
                ...
                if isinstance(orig_vals, BaseMaskedArray):
                    ...
                    if needs_i8_conversion(inference):
                        ...
                        # error: Item "ExtensionArray" of "Union[ExtensionArray,
                        # ndarray[Any, Any]]" has no attribute
                        # "_from_backing_data"
    ...
    def rank(
        ...
        if axis != 0:
            ...
            def f(x): return x.rank(axis=axis, numeric_only=False, **kwargs)
            result = self._python_apply_general(
    ...
    def cumprod(
        ...
        if axis != 0:
            def f(x): return x.cumprod(axis=axis, **kwargs)
    ...
    def cumsum(
        ...
        if axis != 0:
            def f(x): return x.cumsum(axis=axis, **kwargs)
            return self._python_apply_general(
                f, self._selected_obj, is_transform=True)
    ...
    def cummin(
        ...
        **kwargs,
        ...
        if axis != 0:
            def f(x): return np.minimum.accumulate(x, axis)
            obj = self._selected_obj
            if numeric_only:
    ...
    def cummax(
        ...
        **kwargs,
        ...
        if axis != 0:
            def f(x): return np.maximum.accumulate(x, axis)
            obj = self._selected_obj
            if numeric_only:
                obj = obj._get_numeric_data()
            return self._python_apply_general(f, obj, is_transform=True)
    ...
    def shift(
        ...
        for period in periods:
            ...
            if freq is not None or axis != 0:
                ...
                libgroupby.group_shift_indexer(
                    res_indexer, ids, ngroups, period)
    ...
    def diff(
        ...
        if obj.ndim == 1:
            ...
            to_coerce = [
                c for c,
                dtype in obj.dtypes.items() if dtype in dtypes_to_f32]
            if len(to_coerce):
    ...
    def pct_change(
        ...
        if self.axis == 0:
            ...
            fill_grp = filled.T.groupby(
                self._grouper.codes, group_keys=self.group_keys)
    ...
    def sample(
        ...
        group_iterator = self._grouper.get_iterator(
            self._selected_obj, self.axis)
    ...
    def _idxmax_idxmin(
        ...
        if not self.observed and any(
            ...
            if len(self._grouper.groupings) == 1:
                result_len = len(
                    self._grouper.groupings[0].grouping_vector.unique())
            else:
        ...
        if axis == 1:
            try:
                ...
                def func(df):
                    ...
                    return method(axis=axis, skipna=skipna,
                                  numeric_only=numeric_only)
    ...
    def _wrap_idxmax_idxmin(self, res: NDFrameT) -> NDFrameT:
        ...
        if res.size == 0:
            ...
            if isinstance(res, Series):
                # mypy: expression has type "Series", variable has type
                # "NDFrameT"
                result = res._constructor(  # type: ignore[assignment]
                    index.array.take(
                        values, allow_fill=True, fill_value=na_value),
                    index=res.index,
                    name=res.name,
                )
            else:
...
def _insert_quantile_level(
        idx: Index, qs: npt.NDArray[np.float64]) -> MultiIndex:
    """
    ...
    if idx._is_multi:
        ...
        codes = [np.repeat(x, nqs) for x in idx.codes] + \
            [np.tile(lev_codes, len(idx))]
