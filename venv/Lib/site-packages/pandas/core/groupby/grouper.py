...
class Grouper:
    ...
    def _set_grouper(
        ...
        if (self.sort or sort) and not ax.is_monotonic_increasing:
            ...
            # TODO: why does putting na_position="first" fix datetimelike
            # cases?
    ...
    def _group_index(self) -> Index:
        ...
        if not self._dropna and self._passed_categorical:
            ...
            if self._sort and (codes == len(uniques)).any():
                ...
                # Need to determine proper placement of NA value when not
                # sorting
                cat = self.grouping_vector
                na_idx = (cat.codes < 0).argmax()
                if cat.codes[na_idx] < 0:
                    # count number of unique codes that comes before the nan
                    # value
            ...
            if not self._dropna:
                ...
                if np.any(na_mask):
                    if self._sort:
                        ...
                        # A negative code must exist, no need to check
                        # codes[na_idx] < 0
                        na_idx = na_mask.argmax()
                        # count number of unique codes that comes before the
                        # nan value
