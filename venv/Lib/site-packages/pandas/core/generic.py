...
class NDFrame(PandasObject, indexing.IndexingMixin):
    ...
    def _as_manager(self, typ: str, copy: bool_t = True) -> Self:
        ...
        return self._constructor_from_mgr(
            new_mgr, axes=new_mgr.axes).__finalize__(self)
    ...
    def _construct_axes_dict(
            self, axes: Sequence[Axis] | None = None, **kwargs):
    ...
    def _get_index_resolvers(self) -> dict[Hashable, Series | MultiIndex]:
        ...
        return {clean_column_name(k): v for k,
                v in d.items() if not isinstance(k, int)}
    ...
    @final
    def swapaxes(self, axis1: Axis, axis2: Axis,
                 copy: bool_t | None = None) -> Self:
        """
    ...
    def squeeze(self, axis: Axis | None = None):
        ...
        """
        axes = range(
            self._AXIS_LEN) if axis is None else (
    ...
    def _rename(
        ...
        *,
        ...
        result = self if inplace else self.copy(
            deep=copy and not using_copy_on_write())
        ...
        for axis_no, replacements in enumerate((index, columns)):
            ...
            if not callable(replacements):
                if ax._is_multi and level is not None:
                    indexer = ax.get_level_values(
                        level).get_indexer_for(replacements)
                else:
            ...
            result._set_axis_nocheck(
                new_index, axis=axis_no, inplace=True, copy=False)
        ...
        if mapper is not lib.no_default:
            ...
            for axis in range(self._AXIS_LEN):
                ...
                result._set_axis_name(
                    newnames, axis=axis, inplace=True, copy=copy)
            if not inplace:
                return result
            return None
    ...
    @final
    def _is_label_or_level_reference(
            self, key: Level, axis: AxisInt = 0) -> bool_t:
        """
    ...
    def _check_label_or_level_ambiguity(
            self, key: Level, axis: Axis = 0) -> None:
        """
    ...
    def _get_label_or_level_values(
            self, key: Level, axis: AxisInt = 0) -> ArrayLike:
        """
    ...
    def _drop_labels_or_levels(self, keys, axis: AxisInt = 0):
        ...
        levels_to_drop = [
            k for k in keys if self._is_level_reference(
                k, axis=axis)]

        labels_to_drop = [
            k for k in keys if not self._is_level_reference(
                k, axis=axis)]
    ...
    def __setstate__(self, state) -> None:
        if isinstance(state, BlockManager):
            ...
            if typ is not None:
                ...
                raise NotImplementedError(
                    "Pre-0.12 pickles are no longer supported")
        elif len(state) == 2:
            raise NotImplementedError(
                "Pre-0.12 pickles are no longer supported")
    ...
    def to_latex(
        ...
        if longtable is None:
            longtable = config.get_option(
                "styler.latex.environment") == "longtable"
        if escape is None:
        ...
        if isinstance(header, (list, tuple)) and len(header) != length:
            raise ValueError(
                f"Writing {length} cols but got {len(header)} aliases")

        # Refactor formatters/float_format/decimal/na_rep/escape to Styler
        # structure
        base_format_ = {
            "na_rep": na_rep,
    ...
    def _take_with_is_copy(self, indices, axis: Axis = 0) -> Self:
        ...
        if self.ndim == 2 and not result._get_axis(
                axis).equals(self._get_axis(axis)):
    ...
    def xs(
        ...
        if isinstance(key, list):
            raise TypeError(
                "list keys are not supported in xs, pass a tuple instead")

        if level is not None:
            if not isinstance(labels, MultiIndex):
                raise TypeError("Index must be a MultiIndex")
            loc, new_ax = labels.get_loc_level(
                key, level=level, drop_level=drop_level)
    ...
    def _check_setitem_copy(self, t: str = "setting", force: bool_t = False):
        ...
        if self._is_copy is not None and not isinstance(self._is_copy, str):
            r = self._is_copy()
            if not gc.get_referents(r) or (
                    r is not None and r.shape == self.shape):
                self._is_copy = None
                return
    ...
    def reindex_like(
        self,
        other,
        method: Literal["backfill", "bfill", "pad",
                        "ffill", "nearest"] | None = None,
        copy: bool_t | None = None,
        limit: int | None = None,
        tolerance=None,
    ) -> Self:
    ...
    def _drop_axis(
        ...
        # Case for non-unique axis
            ...
            is_tuple_labels = is_nested_list_like(
                labels) or isinstance(labels, tuple)
            ...
            if level is not None:
                ...
                # In case of tuples we get dtype object but have to use isin
                # GH#42771
    ...
    def add_prefix(self, prefix: str, axis: Axis | None = None) -> Self:
        ...
        # type: ignore[return-value, arg-type, misc]
    ...
    def _needs_reindex_multi(self, axes, method,
                             level: Level | None) -> bool_t:
    ...
    def _reindex_with_indexers(
        ...
        for axis in sorted(reindexers.keys()):
            ...
            # TODO: speed up on homogeneous DataFrame objects (see
            # _reindex_multi)
    ...
    def __finalize__(self, other, method: str | None = None, **kwargs) -> Self:
        ...
        if method == "concat":
            ...
            if all(bool(obj.attrs) for obj in other.objs):
                ...
                have_same_attrs = all(
                    obj.attrs == attrs for obj in other.objs[1:])
                if have_same_attrs:
    ...
    def _consolidate(self):
        ...
        """
        def f(): return self._mgr.consolidate()
    ...
    def dtypes(self):
        ...
        return self._constructor_sliced(
            data, index=self._info_axis, dtype=np.object_)
    ...
    def astype(
        ...
        if is_dict_like(dtype):
            ...
            dtype_ser = dtype_ser.reindex(
                self.columns, fill_value=None, copy=False)
            ...
            for i, (col_name, col) in enumerate(self.items()):
                ...
                if isna(cdt):
                    ...
                    try:
                        res_col = col.astype(
                            dtype=cdt, copy=copy, errors=errors)
    ...
    def _pad_or_backfill(
        ...
        *,
        ...
        if not self._mgr.is_single_block and axis == 1:
            ...
            # in all axis=1 cases, and remove axis kward from
            # mgr.pad_or_backfill.
            if inplace:
                raise NotImplementedError()
                    ...
                    if not inplace:
                        ...
                        if isinstance(res_k, ABCSeries):
                            ...
                            if isinstance(locs, slice):
                                ...
                                isinstance(
                                    locs, np.ndarray) and locs.dtype.kind == "b"
                            ):
                                locs = locs.nonzero()[0]
                            elif not (
                                isinstance(
                                    locs, np.ndarray) and locs.dtype.kind == "i"
                            ):
                                # Should never be reached, but let's cover our
                                # bases
                                raise NotImplementedError(
    ...
    def ffill(
        ...
        *,
        ...
        return self._pad_or_backfill(
            ...
            # error: Argument "downcast" to "_fillna_with_method" of "NDFrame"
            # has incompatible type "Union[Dict[Any, Any], None,
            # Literal[_NoDefault.no_default]]"; expected "Optional[Dict[Any,
            # Any]]"
            downcast=downcast,  # type: ignore[arg-type]
        )
    ...
    def pad(
        ...
        *,
        ...
        return self.ffill(axis=axis, inplace=inplace,
    ...
    def backfill(
        ...
        *,
        ...
        return self.bfill(axis=axis, inplace=inplace,
                          limit=limit, downcast=downcast)
    ...
    def replace(
        ...
        *,
        ...
        limit: int | None = None,
        regex: bool_t = False,
        method: Literal["pad", "ffill",
                        "bfill"] | lib.NoDefault = lib.no_default,
    ) -> Self | None:
        ...
        if (
            ...
            # case that goes through _replace_single and defaults to
            # method="pad"
            warnings.warn(
                # GH#33302
    ...
    def interpolate(
        ...
        **kwargs,
        ...
        limit_direction = missing.infer_limit_direction(
    ...
    def asof(self, where, subset=None):
        ...
        if nulls.all():
            if is_series:
                ...
                self = cast("DataFrame", self)
                return self._constructor(
                    np.nan, index=where, columns=self.columns)
            else:
                self = cast("DataFrame", self)
                return self._constructor_sliced(
    ...
    @final
    def at_time(self, time, asof: bool_t = False,
                axis: Axis | None = None) -> Self:
        """
    ...
    def resample(
        ...
        convention: Literal["start", "end", "s",
                            "e"] | lib.NoDefault = lib.no_default,
        kind: Literal["timestamp",
                      "period"] | None | lib.NoDefault = lib.no_default,
        on: Level | None = None,
        level: Level | None = None,
    ...
    def compare(
        ...
        indices = (
            np.arange(diff.shape[axis]).reshape(
                [2, diff.shape[axis] // 2]).T.flatten()
        )
    ...
    def _align_series(
        ...
        if not axis:
            ...
            if is_series:
                ...
                new_mgr = self._mgr.reindex_indexer(
                    join_index, lidx, axis=1, copy=copy)
        ...
        if fill_na:
            ...
            if method is not None:
                left = left._pad_or_backfill(
                    method, limit=limit, axis=fill_axis)
    ...
    def _where(
        ...
        if isinstance(cond, NDFrame):
            ...
            cond = self._constructor(
                cond, **self._construct_axes_dict(), copy=False)
        ...
        if inplace:
            ...
            new_data = self._mgr.putmask(
                mask=cond, new=other, align=align, warn=warn)
            result = self._constructor_from_mgr(new_data, axes=new_data.axes)
            return self._update_inplace(result)
    ...
    def pct_change(
        ...
        **DataFrame**
        ...
        """
        # GH#53491
        if fill_method not in (lib.no_default,
                               None) or limit is not lib.no_default:
            warnings.warn(
    ...
    def _logical_func(
        ...
        **kwargs,
        ...
        if self.ndim > 1 and axis is None:
            # Reduce along one dimension then the other, to simplify
            # DataFrame._reduce
            res = self._logical_func(
                name, func, axis=0, bool_only=bool_only, skipna=skipna, **kwargs
            )
            # error: Item "bool" of "Series | bool" has no attribute
            # "_logical_func"
    ...
    def cummax(self, axis: Axis | None = None,
               skipna: bool_t = True, *args, **kwargs):
    ...
    def cumsum(self, axis: Axis | None = None,
               skipna: bool_t = True, *args, **kwargs):
        return self._accum_func("cumsum", np.cumsum,
                                axis, skipna, *args, **kwargs)

    def cumprod(self, axis: Axis | None = None,
                skipna: bool_t = True, *args, **kwargs):
        return self._accum_func("cumprod", np.cumprod,
                                axis, skipna, *args, **kwargs)
    ...
    def _inplace_method(self, other, op) -> Self:
        ...
        if not PYPY and warn_copy_on_write():
            if sys.getrefcount(self) <= REF_COUNT + 2:
                # we are probably in an inplace setitem context (e.g. df['a']
                # += 1)
                warn = False
        ...
        if (
            ...
            # BlockManager, SingleBlockManager]" has no attribute
            # "setitem_inplace"
    ...
    def __iadd__(self, other) -> Self:
        ...
        return self._inplace_method(other, type(
            self).__add__)  # type: ignore[operator]

    @final
    def __isub__(self, other) -> Self:
        # error: Unsupported left operand type for - ("Type[NDFrame]")
        return self._inplace_method(other, type(
            self).__sub__)  # type: ignore[operator]

    @final
    def __imul__(self, other) -> Self:
        # error: Unsupported left operand type for * ("Type[NDFrame]")
        return self._inplace_method(other, type(
            self).__mul__)  # type: ignore[operator]
    ...
    def __imod__(self, other) -> Self:
        # error: Unsupported left operand type for % ("Type[NDFrame]")
        return self._inplace_method(other, type(
            self).__mod__)  # type: ignore[operator]

    @final
    def __ipow__(self, other) -> Self:
        # error: Unsupported left operand type for ** ("Type[NDFrame]")
        return self._inplace_method(other, type(
            self).__pow__)  # type: ignore[operator]

    @final
    def __iand__(self, other) -> Self:
        # error: Unsupported left operand type for & ("Type[NDFrame]")
        return self._inplace_method(other, type(
            self).__and__)  # type: ignore[operator]
    ...
    def __ixor__(self, other) -> Self:
        # error: Unsupported left operand type for ^ ("Type[NDFrame]")
        return self._inplace_method(other, type(
            self).__xor__)  # type: ignore[operator]
