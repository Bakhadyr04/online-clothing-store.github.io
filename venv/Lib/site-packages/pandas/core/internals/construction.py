...
def ndarray_to_mgr(
    ...
    if is_1d_only_ea_dtype(vdtype) or is_1d_only_ea_dtype(dtype):
        ...
        if isinstance(values, (np.ndarray, ExtensionArray)
                      ) and values.ndim > 1:
...
def dict_to_mgr(
    ...
    *,
    ...
    return arrays_to_mgr(arrays, columns, index,
                         dtype=dtype, typ=typ, consolidate=copy)
...
def _homogenize(
    ...
    oindex = None
    homogenized = []
    # if the original array-like in `data` is a Series, keep track of this
    # Series' refs
    refs: list[Any] = []
...
def _validate_or_indexify_columns(
    ...
    if columns is None:
        ...
        if is_mi_list:
            # check if nested list column, length of each sub-list should be
            # equal
            ...
            # if columns is not empty and length of sublist is not equal to
            # content
