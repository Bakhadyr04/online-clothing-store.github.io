    ...
    if len(mgrs_indexers) > 0 and mgrs_indexers[0][0].nblocks > 0:
        ...
        if first_dtype in [np.float64, np.float32]:
            ...
            if (
                ...
                # Fastpath!
                # Length restriction is just to avoid having to worry about
                # 'copy'
...
def _concat_homogeneous_fastpath(
    ...
    """
    # assumes
    # all(_is_homogeneous_mgr(mgr, first_dtype) for mgr, _ in in
    # mgrs_indexers)
...
class JoinUnit:
    ...
    def _is_valid_na_for(self, dtype: DtypeObj) -> bool:
        ...
        if blk.dtype == object:
            values = blk.values
            return all(is_valid_na_for_dtype(x, dtype)
...
def _concatenate_join_units(
        join_units: list[JoinUnit], copy: bool) -> ArrayLike:
    """
...
def _get_empty_dtype(
        join_units: Sequence[JoinUnit]) -> tuple[DtypeObj, DtypeObj]:
    """
...
def _is_uniform_join_units(join_units: list[JoinUnit]) -> bool:
    ...
    if first.dtype.kind == "V":
        return False
    return (
        # exclude cases where a) ju.block is None or b) we have e.g.
        # Int64+int64
