...
class Block(PandasObject, libinternals.Block):
    ...
    def _standardize_fill_value(self, value):
        ...
        if self.dtype != _dtype_obj and is_valid_na_for_dtype(
                value, self.dtype):
    ...
    def make_block(
        ...
        return new_block(values, placement=placement,
                         ndim=self.ndim, refs=refs)
    ...
    def make_block_same_class(
        ...
        return type(self)(values, placement=placement,
                          ndim=self.ndim, refs=refs)
    ...
    def take_block_columns(self, indices: npt.NDArray[np.intp]) -> Self:
        ...
        # that never happens with 1-column blocks, i.e. never for
        # ExtensionBlock.
    ...
    def reduce(self, func) -> list[Block]:
        ...
        # Block with the same mgr_locs; squeezing will be done at a higher
        # level
        assert self.ndim == 2
    ...
    def coerce_to_target_dtype(
            self, other, warn_on_upcast: bool = False) -> Block:
        """
        ...
        if (
            ...
            and not (
                isinstance(
                    other, (np.datetime64, np.timedelta64)) and np.isnat(other)
            )
        ):
    ...
    def _maybe_downcast(
        ...
        if self.dtype == _dtype_obj:
            ...
            if caller == "fillna" and get_option(
                    "future.no_silent_downcasting"):
                return blocks

            nbs = extend_blocks(
                [blk.convert(using_cow=using_cow, copy=not using_cow)
                 for blk in blocks]
            )
            if caller == "fillna":
            ...
            nbs = extend_blocks(
                [b._downcast_2d(downcast, using_cow) for b in blocks])
    ...
    def convert_dtypes(
        ...
        for blk in blks:
            ...
            sub_blks = [
                blk] if blk.ndim == 1 or self.shape[0] == 1 else blk._split()
            dtypes = [
                convert_dtypes(
                    b.values,
                    convert_string,
    ...
    def astype(
        ...
        refs = None
        if (using_cow or not copy) and astype_is_view(
                values.dtype, new_values.dtype):
            refs = self.refs
    ...
    def copy(self, deep: bool = True) -> Self:
        ...
        if deep:
            ...
            refs = self.refs
        return type(self)(values, placement=self._mgr_locs,
                          ndim=self.ndim, refs=refs)
            ...
            for blk_num, blk in enumerate(rb):
                ...
                if (
                    ...
                    for res_blk in result:
                        ...
                        if len(
                                converted) > 1 or converted[0].dtype != res_blk.dtype:
                            warnings.warn(
                                # GH#54710
    ...
    def take_nd(
        ...
        # Note: algos.take_nd has upcast logic similar to
    ...
    def setitem(self, indexer, value, using_cow: bool = False) -> Block:
        ...
        if self.dtype != _dtype_obj:
            # GH48933: extract_array would convert a pd.Series value to
            # np.ndarray
        ...
        try:
            ...
            if self.dtype == _dtype_obj:
                ...
                if lib.is_list_like(vi):
                    ...
                    casted = setitem_datetimelike_compat(
                        values, len(vi), casted)

            self = self._maybe_copy(using_cow, inplace=True)
            values = cast(np.ndarray, self.values.T)
            if isinstance(casted, np.ndarray) and casted.ndim == 1 and len(
                    casted) == 1:
                # NumPy 1.25 deprecation:
        ...
        try:
            ...
            if self.ndim == 1 or self.shape[0] == 1:
                ...
                if not is_list_like(new):
                    ...
                    nb = self.setitem(
                        indexer, new[indexer], using_cow=using_cow)
                    return [nb]
    ...
    def where(
        ...
        try:
            ...
            # without using 'casted', expressions.where may do unwanted
            # upcasts.
    ...
    def fillna(
        ...
        """
        # Caller is responsible for validating limit; if int it is strictly
        # positive
    ...
    def pad_or_backfill(
        ...
        *,
        ...
        # We know self.array_values is a NumpyExtensionArray bc EABlock
        # overrides
    ...
    def interpolate(
        ...
        **kwargs,
        ...
        nb = self.make_block_same_class(data, refs=refs)
        return nb._maybe_downcast(
            [nb], downcast, using_cow, caller="interpolate")
    ...
    def quantile(
        ...
        Block
        """
        # We should always have ndim == 2 because Series dispatches to
        # DataFrame
        assert self.ndim == 2
        assert is_list_like(qs)  # caller is responsible for this

        result = quantile_compat(
            self.values, np.asarray(
                qs._values), interpolation)
...
class EABackedBlock(Block):
    ...
    def shift(self, periods: int, fill_value: Any = None) -> list[Block]:
        ...
        #  along rows.
        new_values = self.values.T.shift(
            periods=periods, fill_value=fill_value).T
        return [self.make_block_same_class(new_values)]
    ...
    def setitem(self, indexer, value, using_cow: bool = False):
        ...
        try:
            ...
            if isinstance(self.dtype, IntervalDtype):
                ...
                nb = self.coerce_to_target_dtype(
                    orig_value, warn_on_upcast=True)
    ...
    def where(
        ...
        if noop:
            # GH#44181, GH#45135
            # Avoid a) raising for Interval/PeriodDtype and b) unnecessary
            # object upcast
            if using_cow:
                return [self.copy(deep=False)]
            return [self.copy()]
    ...
    def pad_or_backfill(
        ...
        *,
        ...
        if "limit_area" in inspect.signature(
                values._pad_or_backfill).parameters:
...
class ExtensionBlock(EABackedBlock):
    ...
    def fillna(
        ...
        if using_cow and self._can_hold_na and not self.values._hasna:
            ...
            try:
                ...
                refs = None
                new_values = self.values.fillna(
                    value=value, method=None, limit=limit)
        ...
        return nb._maybe_downcast(
            [nb], downcast, using_cow=using_cow, caller="fillna")
    ...
    def _unwrap_setitem_indexer(self, indexer):
        ...
        if isinstance(indexer, tuple) and len(indexer) == 2:
            ...
            if all(isinstance(x, np.ndarray) and x.ndim == 2 for x in indexer):
                ...
                if not (
                    second.size == 1 and (
                        second == 0).all() and first.shape[1] == 1
                ):
                    raise NotImplementedError(
    ...
    def slice_block_rows(self, slicer: slice) -> Self:
        ...
        return type(self)(new_values, self._mgr_locs,
                          ndim=self.ndim, refs=self.refs)
    ...
    def _unstack(
        ...
        # Caller is responsible for ensuring self.shape[-1] ==
        # len(unstacker.index)
...
def maybe_coerce_values(values: ArrayLike) -> ArrayLike:
    ...
    values : np.ndarray or ExtensionArray
    """
    # Caller is responsible for ensuring NumpyExtensionArray is already
    # extracted.
    ...
    if isinstance(values, (DatetimeArray, TimedeltaArray)
        ...
        # freq is only stored in DatetimeIndex/TimedeltaIndex, not in
        # Series/DataFrame
...
def get_block_type(dtype: DtypeObj) -> type[Block]:
    ...
    if isinstance(dtype, DatetimeTZDtype):
        ...
        # Note: need to be sure NumpyExtensionArray is unwrapped before we get
        # here
...
def ensure_block_shape(values: ArrayLike, ndim: int = 1) -> ArrayLike:
    ...
    if values.ndim < ndim:
        if not is_1d_only_ea_dtype(values.dtype):
            ...
            values = cast(
                "np.ndarray | DatetimeArray | TimedeltaArray", values)
