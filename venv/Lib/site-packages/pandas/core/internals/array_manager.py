...
class BaseArrayManager(DataManager):
    ...
    def apply_with_block(self, f, align_keys=None, **kwargs) -> Self:
        ...
        for i, arr in enumerate(self.arrays):
            ...
            if isinstance(arr.dtype, np.dtype) and not isinstance(
                    arr, np.ndarray):
    ...
    def astype(self, dtype, copy: bool | None = False,
        ...
        return self.apply(astype_array_safe, dtype=dtype,
...
class ArrayManager(BaseArrayManager):
    ...
    def fast_xs(self, loc: int) -> SingleArrayManager:
        ...
        # for datetime64/timedelta64, the np.ndarray constructor cannot handle
        # pd.NaT
    ...
    def unstack(self, unstacker, fill_value) -> ArrayManager:
        ...
        for arr in self.arrays:
            for i in range(unstacker.full_shape[1]):
                if allow_fill:
                    # error: Value of type "Optional[Any]" is not indexable
                    # [index]
                    new_arr = take_1d(
                        arr,
                        new_indexer2D[:, i],
    ...
    def concat_horizontal(cls, mgrs: list[Self], axes: list[Index]) -> Self:
        ...
        """
        # concatting along the columns -> combine reindexed arrays in a single
        # manager
...
class SingleArrayManager(BaseArrayManager, SingleDataManager):
    ...
    def get_rows_with_mask(
            self, indexer: npt.NDArray[np.bool_]) -> SingleArrayManager:
        new_array = self.array[indexer]
        new_index = self.index[indexer]
        return type(self)([new_array], [new_index])

    # error: Signature of "apply" incompatible with supertype
    # "BaseArrayManager"
...
class NullArrayProxy:
    ...
    def to_array(self, dtype: DtypeObj) -> ArrayLike:
        ...
        if isinstance(dtype, ExtensionDtype):
            ...
            # when introducing missing values, int becomes float, bool becomes
            # object
...
def concat_arrays(to_concat: list) -> ArrayLike:
    ...
    to_concat_no_proxy = [
        x for x in to_concat if not isinstance(
            x, NullArrayProxy)]
