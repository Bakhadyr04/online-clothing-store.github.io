...
class BaseBlockManager(DataManager):
    ...
    def references_same_values(
            self, mgr: BaseBlockManager, blkno: int) -> bool:
        """
        Checks if two blocks from two different block managers reference the
        same underlying values.
        """
        blk = self.blocks[blkno]
        return any(blk is ref()
                   for ref in mgr.blocks[blkno].refs.referenced_blocks)
    ...
    def apply(
        ...
        **kwargs
        ...
        # fillna: Series/DataFrame is responsible for making sure value is
        # aligned
    ...
    def astype(self, dtype, copy: bool | None = False,
               errors: str = "raise") -> Self:
        if copy is None:
    ...
    def _combine(self, blocks: list[Block],
                 index: Index | None = None) -> Self:
...
class BlockManager(libinternals.BlockManager, BaseBlockManager):
    ...
    def iset(
        ...
        unfit_val_locs = []
        removed_blknos = []
        for blkno_l, val_locs in libinternals.get_blkno_placements(
                blknos, group=True):
    ...
    def column_setitem(
        ...
        # so don't track references, otherwise the `setitem` would perform CoW
        # again
    ...
    def insert(self, loc: int, item: Hashable,
               value: ArrayLike, refs=None) -> None:
        """
    ...
    def unstack(self, unstacker, fill_value) -> BlockManager:
        ...
        bm = BlockManager(
            new_blocks, [
                new_columns, new_index], verify_integrity=False)
        return bm
...
class SingleBlockManager(BaseBlockManager, SingleDataManager):
    ...
    def __setstate__(self, state) -> None:
        def unpickle_block(values, mgr_locs, ndim: int) -> Block:
            ...
            # older pickles may store e.g. DatetimeIndex instead of
            # DatetimeArray
        ...
        if isinstance(state, tuple) and len(
                state) >= 4 and "0.14.1" in state[3]:
    ...
    def get_rows_with_mask(self, indexer: npt.NDArray[np.bool_]) -> Self:
        ...
        if isinstance(indexer, np.ndarray) and indexer.dtype.kind == "b":
            ...
            # TODO(CoW) in theory only need to track reference if new_array is
            # a view
            refs = blk.refs
...
def _form_blocks(arrays: list[ArrayLike],
                 consolidate: bool, refs: list) -> list[Block]:
