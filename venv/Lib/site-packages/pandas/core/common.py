...
@overload
def asarray_tuplesafe(values: Iterable, dtype: NpDtype |
                      None = ...) -> ArrayLike:
    ...
...
def asarray_tuplesafe(values: Iterable, dtype: NpDtype |
    ...
    try:
        ...
        return construct_1d_object_array_from_listlike(
...
def random_state(state: RandomState | None = None):
    ...
    """
    if is_integer(state) or isinstance(
            state, (np.ndarray, np.random.BitGenerator)):
...
def convert_to_list_like(
    ...
    """
    if isinstance(values, (list, np.ndarray, ABCIndex,
...
def fill_missing_names(names: Sequence[Hashable | None]) -> list[Hashable]:
    ...
    """
    return [f"level_{i}" if name is None else name for i,
