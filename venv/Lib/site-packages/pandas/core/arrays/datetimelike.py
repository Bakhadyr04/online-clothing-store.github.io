...
class DatetimeLikeArrayMixin(  # type: ignore[misc]
    ...
    def __setitem__(
        ...
        # on invalid 'value' even if this is a no-op, e.g. wrong-dtype empty
        # array.
    ...
    def _validation_error_message(
            self, value, allow_listlike: bool = False) -> str:
        """
    ...
    @final
    def _unbox(
            self, other) -> np.int64 | np.datetime64 | np.timedelta64 | np.ndarray:
        """
    ...
    def _add_datetimelike_scalar(self, other) -> DatetimeArray:
        ...
        if isna(other):
            ...
            result = self._ndarray + \
                NaT.to_datetime64().astype(f"M8[{self.unit}]")
        ...
        return DatetimeArray._simple_new(
            res_values, dtype=dtype, freq=new_freq)
    ...
    def _sub_datetimelike_scalar(
        ...
        if self.dtype.kind != "M":
            raise TypeError(
                f"cannot subtract a datelike from a {type(self).__name__}")
    ...
    @final
    def _sub_datetimelike(self, other: Timestamp |
        ...
        return TimedeltaArray._simple_new(
            res_m8, dtype=res_m8.dtype, freq=new_freq)
    ...
    def _add_timedeltalike(self, other: Timedelta | TimedeltaArray):
        ...
        # dtype[timedelta64]]"; expected "Union[dtype[datetime64],
        # DatetimeTZDtype]"
    ...
    @final
    def _sub_periodlike(self, other: Period |
                        PeriodArray) -> npt.NDArray[np.object_]:
    ...
    def __add__(self, other):
        ...
        if other is NaT:
            ...
            result = obj._addsub_int_array_or_scalar(
                other * obj.dtype._n, operator.add)
        ...
        if isinstance(result, np.ndarray) and lib.is_np_dtype(
                result.dtype, "m"):
            from pandas.core.arrays import TimedeltaArray
    ...
    def __sub__(self, other):
        ...
        if other is NaT:
            ...
            obj = cast("PeriodArray", self)
            result = obj._addsub_int_array_or_scalar(
                other * obj.dtype._n, operator.sub)
    ...
    @_period_dispatch
    def min(self, *, axis: AxisInt | None = None,
            skipna: bool = True, **kwargs):
        """
    ...
    @_period_dispatch
    def max(self, *, axis: AxisInt | None = None,
            skipna: bool = True, **kwargs):
        """
    ...
    @_period_dispatch
    def median(self, *, axis: AxisInt | None = None,
               skipna: bool = True, **kwargs):
    ...
    def _groupby_op(
        ...
        **kwargs,
        ...
        if dtype.kind == "M":
            ...
            if how in ["sum", "prod", "cumsum", "cumprod", "var", "skew"]:
                raise TypeError(
                    f"Period type does not support {how} operations")
            if how in ["any", "all"]:
                # GH#34479
                warnings.warn(
        ...
        if how in ["std", "sem"]:
            ...
            return TimedeltaArray._simple_new(
                res_values, dtype=res_values.dtype)
...
class TimelikeOps(DatetimeLikeArrayMixin):
    ...
    def __init__(
        ...
        if isinstance(values, type(self)):
            ...
            if dtype is not None and dtype != values.dtype:
                # TODO: we only have tests for this for DTA, not TDA
                # (2022-07-01)
                raise TypeError(
    ...
    def as_unit(self, unit: str, round_ok: bool = True) -> Self:
        ...
        new_values = astype_overflowsafe(
            self._ndarray, dtype, round_ok=round_ok)
        ...
        return type(self)._simple_new(
            ...
            new_values, dtype=new_dtype, freq=self.freq
        )
    ...
    def _ensure_matching_resos(self, other):
        if self._creso != other._creso:
            # Just as with Timestamp/Timedelta, we cast to the higher
            # resolution
            if self._creso < other._creso:
    ...
    def any(self, *, axis: AxisInt | None = None, skipna: bool = True) -> bool:
        # GH#34479 the nanops call will issue a FutureWarning for non-td64
        # dtype
        return nanops.nanany(self._ndarray, axis=axis,
                             skipna=skipna, mask=self.isna())

    def all(self, *, axis: AxisInt | None = None, skipna: bool = True) -> bool:
        # GH#34479 the nanops call will issue a FutureWarning for non-td64
        # dtype

        return nanops.nanall(self._ndarray, axis=axis,
                             skipna=skipna, mask=self.isna())
    ...
    def _with_freq(self, freq) -> Self:
        ...
        if freq is None:
            ...
            # Always valid.  In the TimedeltaArray case, we require a Tick
            # offset
    ...
    def _concat_same_type(
        ...
        if axis == 0:
            ...
            if obj.freq is not None and all(
                    x.freq == obj.freq for x in to_concat):
