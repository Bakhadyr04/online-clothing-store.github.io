...
class IntervalArray(IntervalMixin, ExtensionArray):
    ...
    def _ensure_simple_new_inputs(
        ...
        if isinstance(left.dtype, CategoricalDtype) or is_string_dtype(
                left.dtype):
            # GH 19016
            msg = (
        ...
        if isinstance(left, ABCPeriodIndex):
            ...
            raise ValueError(msg)
        if isinstance(left, ABCDatetimeIndex) and str(
                left.tz) != str(right.tz):
            msg = (
    ...
    def _cmp_method(self, other, op):
        ...
        if isinstance(other, Interval):
            ...
            # extract intervals if we have interval categories with matching
            # closed
    ...
    def shift(self, periods: int = 1,
              fill_value: object = None) -> IntervalArray:
    ...
    def __arrow_array__(self, type=None):
        ...
        mask = self.isna()
        if mask.any():
            # if there are missing values, set validity bitmap also on the
            # array level
        ...
        return pyarrow.ExtensionArray.from_storage(
            interval_type, storage_array)
    ...
    def unique(self) -> IntervalArray:
        ...
        nc = unique(
            self._combined.view("complex128")[
