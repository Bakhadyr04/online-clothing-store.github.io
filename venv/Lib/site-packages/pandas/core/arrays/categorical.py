...
def _cat_compare_op(op):
    ...
    def func(self, other):
        ...
        if hashable:
            ...
            if isinstance(other, ExtensionArray) and needs_i8_conversion(
                    other.dtype):
...
class Categorical(NDArrayBackedExtensionArray, PandasObject,
        ...
        if isinstance(vdtype, CategoricalDtype):
            ...
            if isinstance(values, list) and len(values) == 0:
                ...
                if null_mask.any():
                    ...
                    # GH#44900 Do not cast to float if we have only missing
                    # values
    ...
    def _from_scalars(cls, scalars, *, dtype: DtypeObj) -> Self:
        if dtype is None:
            # The _from_scalars strictness doesn't make much sense in this
            # case.
            raise NotImplementedError
    ...
    @overload
    def astype(self, dtype: ExtensionDtype,
    ...
    def _from_inferred_categories(
        ...
        known_categories = (
            isinstance(
                dtype, CategoricalDtype) and dtype.categories is not None
        )
    ...
    def set_categories(self, new_categories, ordered=None,
                       rename: bool = False):
        """
    ...
    def remove_categories(self, removals) -> Self:
        ...
        return self.set_categories(
            new_categories, ordered=self.ordered, rename=False)
    ...
    def map(
        ...
        if na_action is None and has_nans:
            na_val = mapper(
                np.nan) if callable(mapper) else mapper.get(
                np.nan, np.nan)
    ...
    def _validate_codes_for_dtype(
            cls, codes, *, dtype: CategoricalDtype) -> np.ndarray:
        ...
        if len(codes) and (codes.max() >= len(
                dtype.categories) or codes.min() < -1):
            raise ValueError(
    ...
    def memory_usage(self, deep: bool = False) -> int:
        ...
        return self._codes.nbytes + \
            self.dtype.categories.memory_usage(deep=deep)
    ...
    def _concat_same_type(
            cls, to_concat: Sequence[Self], axis: AxisInt = 0) -> Self:
    ...
    def _groupby_op(
        ...
        **kwargs,
        ...
        if how in ["min", "max", "rank", "idxmin",
                   "idxmax"] and not dtype.ordered:
...
def factorize_from_iterables(
        iterables) -> tuple[list[np.ndarray], list[Index]]:
    """
