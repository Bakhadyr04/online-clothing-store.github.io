...
class SparseArray(OpsMixin, PandasObject, ExtensionArray):
    ...
    def isna(self) -> Self:  # type: ignore[override]
        ...
        if self._null_fill_value:
            return type(self)._simple_new(
                isna(self.sp_values), self.sp_index, dtype)
    ...
    def __getitem__(
        ...
        if is_integer(key):
            ...
            if key.step is None or key.step == 1:
                ...
                return type(self)._simple_new(
                    sp_vals, new_sp_index, self.dtype)
            else:
    ...
    def take(self, indices, *, allow_fill: bool = False,
             fill_value=None) -> Self:
        ...
        if self.sp_index.npoints == 0 and old_fill_indices.all():
            ...
            if m0.any():
                result_type = np.result_type(
                    result_type, type(self.fill_value))
    ...
    def _take_without_fill(self, indices) -> Self:
        ...
        new_sp_index = make_sparse_index(
            len(indices), value_indices, kind=self.kind)
        return type(self)._simple_new(
            new_sp_values, new_sp_index, dtype=self.dtype)
    ...
    def map(self, mapper, na_action=None) -> Self:
        ...
        if na_action is None or notna(fill_val):
            fill_val = mapper.get(
                fill_val, fill_val) if is_map else mapper(fill_val)

        def func(sp_val):
            new_sp_val = mapper.get(sp_val, None) if is_map else mapper(sp_val)
            # check identity and equality because nans are not equal to each
            # other
    ...
    def sum(
        ...
        *args, **kwargs
        ...
        if self._null_fill_value:
            ...
            if check_below_min_count(
                    valid_vals.shape, None, min_count - nsparse):
                return na_value_for_dtype(self.dtype.subtype, compat=False)
    ...
    def _min_max(self, kind: Literal["min", "max"], skipna: bool) -> Scalar:
        ...
        if len(valid_vals) > 0:
            ...
            # If a non-null fill value is currently present, it might be the
            # min/max
    ...
    def __array_ufunc__(self, ufunc: np.ufunc, method: str, *inputs, **kwargs):
        ...
        if len(inputs) == 1:
            ...
            if ufunc.nout > 1:
                ...
                arrays = tuple(
                    self._simple_new(
                        sp_value, self.sp_index, SparseDtype(
                            sp_value.dtype, fv)
                    )
            ...
            return self._simple_new(
                sp_values, self.sp_index, SparseDtype(
                    sp_values.dtype, fill_value)
            )
...
@overload
def make_sparse_index(length: int, indices,
                      kind: Literal["block"]) -> BlockIndex:
    ...


@overload
def make_sparse_index(length: int, indices,
                      kind: Literal["integer"]) -> IntIndex:
    ...


def make_sparse_index(length: int, indices,
                      kind: SparseIndexKind) -> SparseIndex:
    index: SparseIndex
    if kind == "block":
