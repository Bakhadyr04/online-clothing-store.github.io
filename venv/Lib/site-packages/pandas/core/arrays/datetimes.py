...
class DatetimeArray(dtl.TimelikeOps, dtl.DatelikeOps):  # type: ignore[misc]
    ...
    # error: Signature of "_simple_new" incompatible with supertype
    # "NDArrayBacked"
    @classmethod
        ...
        *,
        ...
        if freq is not None:
            ...
            if i8values.dtype != "i8":
                ...
                # to overflow and cast to e.g. f8, but if it does we need to
                # cast
        ...
        if start == end:
            ...
            if not left_inclusive or not right_inclusive:
                if not left_inclusive and len(
                        i8values) and i8values[0] == start_i8:
                    i8values = i8values[1:]
                if not right_inclusive and len(
                        i8values) and i8values[-1] == end_i8:
                    i8values = i8values[:-1]
        ...
        if dtype == self.dtype:
            ...
            if not isinstance(dtype, DatetimeTZDtype):
                ...
                return type(self)._simple_new(
                    res_values, dtype=dtype, freq=self.freq)
...
default 'raise'
        ...
        freq = None
        if timezones.is_utc(tz) or (
                len(self) == 1 and not isna(new_dates_dt64[0])):
    ...
    def std(
        ...
        return tda.std(axis=axis, out=out, ddof=ddof,
                       keepdims=keepdims, skipna=skipna)
...
def _maybe_infer_tz(tz: tzinfo | None, inferred_tz: tzinfo |
                    None) -> tzinfo | None:
    """
