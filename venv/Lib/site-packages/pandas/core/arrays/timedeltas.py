...
def _field_accessor(name: str, alias: str, docstring: str):
    def f(self) -> np.ndarray:
        ...
        if alias == "days":
            ...
            result = get_timedelta_field(
                values, alias, reso=self._creso)  # type: ignore[assignment]
        if self._hasna:
...
class TimedeltaArray(dtl.TimelikeOps):
    ...
    def _is_recognized_dtype(x): return lib.is_np_dtype(x, "m")
    ...
    _datetimelike_ops: list[str] = _field_ops + \
        _object_ops + _bool_ops + ["unit"]
    ...
    def __neg__(self) -> TimedeltaArray:
        ...
        if self.freq is not None:
            freq = -self.freq
        return type(self)._simple_new(-self._ndarray,
...
def _objects_to_td64ns(data, unit=None,
                       errors: DateTimeErrorChoices = "raise"):
    """
