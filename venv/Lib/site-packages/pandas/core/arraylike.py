...
def array_ufunc(self, ufunc: np.ufunc, method: str,
                *inputs: Any, **kwargs: Any):
    """
    ...
    if self.ndim > 1 and (len(inputs) > 1 or ufunc.nout > 1):
        ...
        if method == "__call__" and not kwargs:
            ...
            # Those can have an axis keyword and thus can't be called
            # block-by-block
            result = default_array_ufunc(
                inputs[0], ufunc, method, *inputs, **kwargs)
            # e.g. np.negative (only one reached), with "where" and "out" in
            # kwargs
...
def dispatch_ufunc_with_out(self, ufunc: np.ufunc,
                            method: str, *inputs, **kwargs):
    """
...
def dispatch_reduction_ufunc(
    ...
    if self.ndim > 1:
        ...
        if "axis" not in kwargs:
            ...
            # so we set axis=0 to match the behaviorof
            # np.minimum.reduce(df.values)
            kwargs["axis"] = 0
