...
class IndexModel(models.StructModel):
    def __init__(self, dmm, fe_type) -> None:
        ...
        members = [
            ...
            # typed dict
            # It maps from values in the index to their integer positions in
            # the array
...
def box_index(typ, val, c):
    ...
    with c.builder.if_else(cgutils.is_not_null(c.builder, index.parent)) as (
        ...
        with otherwise:
            ...
            # this is basically Index._simple_new(array_obj, name_obj) in
            # python
        ...
        if isinstance(series1, SeriesType):
            if isinstance(value, SeriesType):
                ...
                def series_binop_impl(series1, value):
                    return Series(
                        binop(
                            series1.values, value), series1.index, series1.name
                    )
...
def iloc_constructor(context, builder, sig, args):
    (obj,) = args
    iloc_indexer = cgutils.create_struct_proxy(
        sig.return_type)(
        context, builder)
