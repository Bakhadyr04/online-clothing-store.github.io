...
def maybe_box_datetimelike(
        value: Scalar, dtype: Dtype | None = None) -> Scalar:
    """
...
@overload
def maybe_downcast_to_dtype(result: np.ndarray,
                            dtype: str | np.dtype) -> np.ndarray:
    ...


@overload
def maybe_downcast_to_dtype(result: ExtensionArray,
                            dtype: str | np.dtype) -> ArrayLike:
    ...


def maybe_downcast_to_dtype(result: ArrayLike,
                            dtype: str | np.dtype) -> ArrayLike:
    """
...
def maybe_downcast_numeric(
    ...
    """
    if not isinstance(dtype, np.dtype) or not isinstance(
    ...
    if dtype.kind in "biu":
        ...
        if not isinstance(
                element, (np.integer, np.floating, int, float, bool)):
        ...
        if np.allclose(new_result, result, equal_nan=True,
                       rtol=0.0, atol=atol):
            return new_result
...
def maybe_promote(dtype: np.dtype, fill_value=np.nan):
    ...
    # of non-hashable arguments), so we fallback to the actual implementation
    # if needed
    try:
...
def _maybe_promote(dtype: np.dtype, fill_value=np.nan):
    ...
    if issubclass(dtype.type, np.datetime64):
        ...
        if issubclass(dtype.type, np.bool_):
            ...
            if not np_can_cast_scalar(
                    fill_value, dtype):  # type: ignore[arg-type]
...
def infer_dtype_from_scalar(val) -> tuple[DtypeObj, Any]:
    ...
    if isinstance(val, np.ndarray):
        ...
        if val is NaT or val.tz is None:
            ...
            dtype = val.dtype
            # TODO: test with datetime(2920, 10, 1) based on
            # test_replace_dtypes
        else:
...
def maybe_cast_to_datetime(
    ...
    if lib.is_np_dtype(dtype, "m"):
        ...
        try:
            ...
            if "cannot supply both a tz and a timezone-naive dtype" in str(
                    err):
                raise ValueError(
...
def _ensure_nanosecond_dtype(dtype: DtypeObj) -> None:
    ...
    if not isinstance(dtype, np.dtype):
        ...
        if not is_supported_dtype(dtype):
            ...
            # test_constructor_generic_timestamp_bad_frequency expects
            # TypeError
            raise TypeError(
...
def find_result_type(left_dtype: DtypeObj, right: Any) -> DtypeObj:
    ...
    if (
        ...
        if lib.is_float(right) and right.is_integer(
        ) and left_dtype.kind != "f":
            right = int(right)
        ...
        if isinstance(right, int) and not isinstance(right, np.integer):
            ...
            if right == 0:
                ...
                # If left dtype isn't unsigned, check if it fits in the signed
                # dtype
...
def maybe_cast_to_integer_array(
        arr: list | np.ndarray, dtype: np.dtype) -> np.ndarray:
    """
...
def np_can_hold_element(dtype: np.dtype, element: Any) -> Any:
    ...
    if dtype.kind in "iu":
        ...
        if tipo is not None:
            if tipo.kind not in "iu":
                if isinstance(
                        element, np.ndarray) and element.dtype.kind == "f":
                    # If all can be losslessly cast to integers, then we can
                    # hold them
    ...
    if dtype.kind == "c":
        if lib.is_integer(element) or lib.is_complex(
                element) or lib.is_float(element):
            if np.isnan(element):
...
def _dtype_can_hold_range(rng: range, dtype: np.dtype) -> bool:
    ...
    if not len(rng):
        return True
    return np_can_cast_scalar(
        rng.start, dtype) and np_can_cast_scalar(rng.stop, dtype)
