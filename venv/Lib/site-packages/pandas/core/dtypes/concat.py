...
def concat_compat(
    ...
    """
    if len(to_concat) and lib.dtypes_all_equal(
            [obj.dtype for obj in to_concat]):
        # fastpath!
        obj = to_concat[0]
...
def _get_result_dtype(
    ...
    if any_ea:
        ...
        # cast this to float)
        if len(kinds) != 1:
            if not len(kinds - {"i", "u", "f"}
                       ) or not len(kinds - {"b", "i", "u"}):
                # let numpy coerce
                pass
            else:
                # coerce to object
...
def union_categoricals(
    ...
    ordered = False
    if all(first._categories_match_up_to_permutation(other)
