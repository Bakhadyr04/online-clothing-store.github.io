...
class CategoricalDtype(PandasExtensionDtype, ExtensionDtype):
    ...
    def _finalize(self, categories, ordered: Ordered,
    ...
    def _hash_categories(self) -> int:
        ...
        if len(categories) and isinstance(categories[0], tuple):
            ...
            if categories.dtype == "O" and len(
                    {type(x) for x in categories}) != 1:
    ...
    def update_dtype(self, dtype: str_type |
        ...
        # update categories/ordered unless they've been explicitly passed as
        # None
        new_categories = (
...
class DatetimeTZDtype(PandasExtensionDtype):
    ...
    **This is not an actual numpy dtype**, but a duck type.
    ...
    # error: Signature of "str" incompatible with supertype
    # "PandasExtensionDtype"
    @cache_readonly
    def str(self) -> str:  # type: ignore[override]
        return f"|M8[{self.unit}]"

    def __init__(self, unit: str_type |
                 DatetimeTZDtype = "ns", tz=None) -> None:
    ...
    def __from_arrow__(self, array: pa.Array |
                       pa.ChunkedArray) -> DatetimeArray:
        """
    ...
    def _get_common_dtype(self, dtypes: list[DtypeObj]) -> DtypeObj | None:
        if all(isinstance(t, DatetimeTZDtype)
               and t.tz == self.tz for t in dtypes):
            np_dtype = np.max(
...
class BaseMaskedDtype(ExtensionDtype):
    ...
    def _get_common_dtype(self, dtypes: list[DtypeObj]) -> DtypeObj | None:
        ...
        if not isinstance(new_dtype, np.dtype):
            # If we ever support e.g. Masked[DatetimeArray] then this will
            # change
            return None
        try:
...
class SparseDtype(ExtensionDtype):
    ...
    def __init__(self, dtype: Dtype = np.float64,
    ...
    def _get_common_dtype(self, dtypes: list[DtypeObj]) -> DtypeObj | None:
        ...
        fill_values = [
            x.fill_value for x in dtypes if isinstance(
                x, SparseDtype)]
        ...
        np_dtypes = (
            x.subtype if isinstance(
                x, SparseDtype) else x for x in dtypes)
        return SparseDtype(np_find_common_type(
