...
class BaseWindow(SelectionMixin):
    ...
    def _slice_axis_for_step(self, index: Index,
                             result: Sized | None = None) -> Index:
        """
    ...
    def _create_data(self, obj: NDFrameT,
                     numeric_only: bool = False) -> NDFrameT:
        """
        Split data into blocks & return conformed data.
        """
        # filter out the on from the object
        if self.on is not None and not isinstance(
                self.on, Index) and obj.ndim == 2:
            obj = obj.reindex(
...
class BaseWindowGroupby(BaseWindow):
    ...
    def _apply_pairwise(
        ...
        if other is not None and not all(
            ...
            # the groupby labels by the number of columns in the original
            # object
        ...
        # 2) Determine the levels + codes of the result from
        # super()._apply_pairwise
...
class Window(BaseWindow):
    ...
    **on**
    ...
    def var(self, ddof: int = 1, numeric_only: bool = False, **kwargs):
        ...
        return self._apply(window_func, name="var",
...
class RollingAndExpandingMixin(BaseWindow):
        ...
        if maybe_use_numba(engine):
            if self.method == "table":
                ...
                return self._numba_apply(
                    sliding_min_max, engine_kwargs, is_max=True)
    ...
    def min(
        ...
        if maybe_use_numba(engine):
            if self.method == "table":
                ...
                return self._numba_apply(
                    sliding_min_max, engine_kwargs, is_max=False)
    ...
    def median(
        ...
        return self._apply(window_func, name="median",
                           numeric_only=numeric_only)
    ...
    def std(
        ...
        if maybe_use_numba(engine):
            ...
            return zsqrt(self._numba_apply(
                sliding_var, engine_kwargs, ddof=ddof))
        window_func = window_aggregations.roll_var

        def zsqrt_func(values, begin, end, min_periods):
            return zsqrt(window_func(
                values, begin, end, min_periods, ddof=ddof))
    ...
    def quantile(
        ...
        return self._apply(window_func, name="quantile",
                           numeric_only=numeric_only)
    ...
    def _validate(self):
        ...
        if (
            ...
            if isinstance(self._on, PeriodIndex):
                ...
                try:
                    unit = dtype_to_unit(
                        self._on.dtype)  # type: ignore[arg-type]
                except TypeError:
            ...
            # Passed BaseIndexer subclass should handle all other rolling
            # kwargs
            pass
