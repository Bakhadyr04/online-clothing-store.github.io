...
class _LocationIndexer(NDFrameIndexerBase):
    ...
    def _getitem_lowerdim(self, tup: tuple):
        ...
        if (
            ...
            with suppress(IndexingError):
                return cast(
                    _LocIndexer, self)._handle_lowerdim_multi_index_axis0(tup)
    ...
    def _getitem_nested_tuple(self, tup: tuple):
        ...
        # which selects only necessary blocks which avoids dtype conversion if
        # possible
        axis = len(tup) - 1
        for key in tup[::-1]:
            if com.is_null_slice(key):
    ...
    def _check_deprecated_callable_usage(
            self, key: Any, maybe_callable: T) -> T:
        # GH53533
        if self.name == "iloc" and callable(
                key) and isinstance(maybe_callable, tuple):
            warnings.warn(
    ...
    def __getitem__(self, key):
        ...
        if type(key) is tuple:
            ...
            maybe_callable = com.apply_if_callable(key, self.obj)
            maybe_callable = self._check_deprecated_callable_usage(
                key, maybe_callable)
...
class _LocIndexer(_LocationIndexer):
    ...
    def _validate_key(self, key, axis: Axis):
        ...
        if isinstance(key, slice) and (
            ...
            raise TypeError(
                f"{key}: boolean values can not be used in a slice")
    ...
    def _getitem_axis(self, key, axis: AxisInt):
        ...
        if isinstance(key, slice):
            ...
            if is_nested_tuple(key, labels):
                ...
                indexer: list[slice | npt.NDArray[np.intp]] = [
    ...
    def _convert_to_indexer(self, key, axis: AxisInt):
        ...
        if is_scalar(key) or (
            isinstance(labels, MultiIndex) and is_hashable(
                key) and not contains_slice
        ):
...
class _iLocIndexer(_LocationIndexer):
    ...
    def _get_list_axis(self, key, axis: AxisInt):
        ...
        try:
            ...
            # re-raise with different error message, e.g.
    ...
    def _setitem_with_indexer_split_path(self, indexer, value, name: str):
        ...
        if (isinstance(value, ABCSeries) and name !=
                "iloc") or isinstance(value, dict):
        ...
        if is_list_like_indexer(value) and getattr(value, "ndim", 1) > 0:
            if isinstance(value, ABCDataFrame):
                ...
                if len(value) == 1 and not is_integer(info_axis):
                    ...
                    return self._setitem_with_indexer(
                        (pi, info_axis[0]), value[0])
                ...
                # We are setting nested data, only possible for object dtype
                # data
    ...
    def _setitem_with_indexer_2d_value(self, indexer, value):
        ...
        for i, loc in enumerate(ilocs):
            ...
            if is_object_dtype(value_col.dtype):
                # casting to list so that we do type inference in
    ...
    def _setitem_single_column(self, loc: int, value, plane_indexer) -> None:
        ...
        is_full_setter = com.is_null_slice(
            pi) or com.is_full_slice(pi, len(self.obj))

        is_null_setter = com.is_empty_slice(
            pi) or is_array_like(pi) and len(pi) == 0
        ...
        if self.ndim == 1:
            ...
            if not is_scalar(value):
                ...
                if not is_object_dtype(self.obj.dtype):
                    # Every NA value is suitable for object, no conversion
                    # needed
    ...
    def _ensure_iterable_column_indexer(self, column_indexer):
        ...
        if is_integer(column_indexer):
            ...
            isinstance(
                column_indexer,
                np.ndarray) and column_indexer.dtype.kind == "b"
        ):
    ...
    def _align_series(
        ...
        if isinstance(indexer, tuple):
            ...
            if sum_aligners == self.ndim and all(
                    is_sequence(_) for _ in indexer):
                ser_values = ser.reindex(
                    obj.axes[0][indexer[0]], copy=True)._values
                ...
                if len(indexer) > 1 and not multiindex_indexer:
                    ...
                    ser_values = (
                        np.tile(
                            ser_values, len_indexer).reshape(
                            len_indexer, -1).T
                    )
...
class _ScalarAccessIndexer(NDFrameIndexerBase):
    ...
    def __setitem__(self, key, value) -> None:
        ...
        if len(key) != self.ndim:
            raise ValueError(
                "Not enough indexers for scalar access (setting)!")
...
class _AtIndexer(_ScalarAccessIndexer):
    ...
    def __getitem__(self, key):
        if self.ndim == 2 and not self._axes_are_unique:
            ...
            if not isinstance(key, tuple) or not all(is_scalar(x)
                                                     for x in key):
                raise ValueError("Invalid call for scalar access (getting)!")
...
def convert_from_missing_indexer_tuple(indexer, axes):
    ...
    def get_indexer(_i, _idx):
        return axes[_i].get_loc(_idx["key"]) if isinstance(
            _idx, dict) else _idx
