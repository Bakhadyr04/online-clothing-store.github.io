...
class MultiIndex(Index):
    ...
    def dtypes(self) -> Series:
        ...
        return Series([level.dtype for level in self.levels],
                      index=Index(names))
    ...
    def levels(self) -> FrozenList:
        ...
        result = [
            x._rename(
                name=name) for x, name in zip(
                self._levels, self._names)]
        for level in result:
    ...
    def _shallow_copy(self, values: np.ndarray,
                      name=lib.no_default) -> MultiIndex:
    ...
    def _get_values_for_csv(
        ...
        if len(new_levels) == 1:
            ...
            return Index(new_levels[0].take(
                new_codes[0]))._get_values_for_csv()
        else:
    ...
    def format(
        ...
        for lev, level_codes in zip(self.levels, self.codes):
            ...
            if len(lev) > 0:
                ...
                formatted = [
                    pprint_thing(
                        na if isna(x) else x, escape_chars=(
                            "\t", "\r", "\n"))
    ...
    def is_monotonic_increasing(self) -> bool:
        ...
        if all(level.is_monotonic_increasing for level in self.levels):
            # If each level is sorted, we can operate on the codes directly.
            # GH27495
    ...
    def _get_level_values(self, level: int, unique: bool = False) -> Index:
        ...
        **this is an internal method**
        ...
        filled = algos.take_nd(
            lev._values,
            level_codes,
            fill_value=lev._na_value)
        return lev._shallow_copy(filled, name=name)

    # error: Signature of "get_level_values" incompatible with supertype
    # "Index"
    ...
    def to_frame(
        ...
        # Guarantee resulting column order - PY36+ dict maintains insertion
        # order
        result = DataFrame(
    ...
    def _sort_levels_monotonic(
            self, raise_if_incomparable: bool = False) -> MultiIndex:
        """
    ...
    def _maybe_preserve_names(self, target: Index,
                              preserve_names: bool) -> Index:
        if (
            preserve_names
    ...
    def _raise_if_missing(self, key, indexer, axis_name: str) -> None:
        ...
        if len(keyarr) and not isinstance(keyarr[0], tuple):
            # i.e. same condition for special case in
            # MultiIndex._get_indexer_strict
    ...
    def _get_loc_single_level_index(
            self, level_index: Index, key: Hashable) -> int:
        """
    ...
    def get_loc(self, key):
        ...
        if keylen == self.nlevels and self.is_unique:
            ...
            try:
                ...
                # e.g. test_partial_slicing_with_multiindex partial string
                # slicing
        ...
        return _maybe_to_slice(loc) if len(
            loc) != stop - start else slice(start, stop)

    def get_loc_level(self, key, level: IndexLabel = 0,
    ...
    def _get_level_indexer(
        ...
        def convert_indexer(start, stop, step,
                            indexer=indexer, codes=level_codes):
    ...
    def get_locs(self, seq) -> npt.NDArray[np.intp]:
        ...
        for i, k in enumerate(seq):
            ...
            if com.is_bool_indexer(k):
                ...
                # a collection of labels to include from this level (these are
                # or'd)
                ...
                try:
                    ...
                    for x in k:
                        ...
                        # TODO: how to handle IntervalIndex level? (no test
                        # cases)
    ...
    def _reorder_indexer(
        ...
        for i, k in enumerate(seq):
            if is_scalar(k):
                # GH#34603 we want to treat a scalar the same as an all equal
                # list
                k = [k]
            if com.is_bool_indexer(k):
                ...
                if not isinstance(
                        k, (np.ndarray, ExtensionArray, Index, ABCSeries)):
    ...
    # base class "Index" defined the type as "Callable[[Index, Any, bool],
    # Any]")
...
def _coerce_indexer_frozen(array_like, categories,
