...
def _maybe_return_indexers(meth: F) -> F:
    ...
    def join(
        ...
        *,
        ...
        join_index, lidx, ridx = meth(
            self, other, how=how, level=level, sort=sort)
...
class Index(IndexOpsMixin, PandasObject):
    ...
    def __new__(
        ...
        try:
            ...
            if "index must be specified when data is not list-like" in str(
                    err):
    ...
    def _engine(
        ...
        if isinstance(self._values,
                      ArrowExtensionArray) and self.dtype.kind in "Mm":
    ...
    def __array_ufunc__(self, ufunc: np.ufunc,
                        method: str_t, *inputs, **kwargs):
        if any(isinstance(other, (ABCSeries, ABCDataFrame))
               for other in inputs):
    ...
    @final
    def _maybe_disallow_fill(self, allow_fill: bool,
                             fill_value, indices) -> bool:
        """
    ...
    def _format_flat(
        ...
        *,
        ...
        return self._format_with_header(
            header=header, na_rep=self._default_na_rep)

    def _format_with_header(
            self, *, header: list[str_t], na_rep: str_t) -> list[str_t]:
    ...
    def name(self, value: Hashable) -> None:
        if self._no_setting_name:
            # Used in MultiIndex.levels to avoid silently ignoring name
            # updates.
            raise RuntimeError(
    ...
    def _set_names(self, values, *, level=None) -> None:
        ...
        validate_all_hashable(
            *values, error_name=f"{type(self).__name__}.name")
    ...
    @overload
    def set_names(self, names, *, level=...,
    ...
    def set_names(self, names, *, level=None,
        ...
        if level is not None and not is_list_like(
                level) and is_list_like(names):
            raise TypeError(
        ...
        if isinstance(self, ABCMultiIndex) and is_dict_like(
                names) and level is None:
        ...
        if len(new_levels) == 1:
            ...
            if len(lev) == 0:
                ...
                if len(new_codes[0]) == 0:
                    ...
                    res_values = algos.take(
                        lev._values, new_codes[0], allow_fill=True)
                    # _constructor instead of type(lev) for RangeIndex compat
                    # GH#35230
    ...
    def is_floating(self) -> bool:
        ...
        return self.inferred_type in ["floating",
                                      "mixed-integer-float", "integer-na"]
    ...
    @final
    def _dti_setop_align_tzs(
            self, other: Index, setop: str_t) -> tuple[Index, Index]:
        """
    ...
    def union(self, other, sort=None):
        ...
        if self.dtype != other.dtype:
            ...
            # or after the dtype equality check above affects the returned
            # dtype
    ...
    def _union(self, other: Index, sort: bool | None):
        ...
        # The rest of this method is analogous to
        # Index._intersection_via_get_indexer
    ...
    def _difference(self, other, sort):
        ...
        this = self
        if isinstance(
                self, ABCCategoricalIndex) and self.hasnans and other.hasnans:
            this = this.dropna()
        other = other.unique()
    ...
    def symmetric_difference(self, other, result_name=None, sort=None):
        ...
        if self.dtype != other.dtype:
            self, other = self._dti_setop_align_tzs(
                other, "symmetric_difference")
        ...
        if not self._should_compare(other):
            ...
            return this.symmetric_difference(
                that, sort=sort).rename(result_name)
    ...
    def get_indexer(
        ...
        if not self._should_compare(
                target) and not self._should_partial_index(target):
            # IntervalIndex get special treatment bc numeric scalars can be
            #  matched to Interval scalars
            return self._get_indexer_non_comparable(
                target, method=method, unique=True)
        ...
        if isinstance(target.dtype, CategoricalDtype):
            ...
            indexer = algos.take_nd(
                categories_indexer, target.codes, fill_value=-1)
        ...
        if self.dtype != target.dtype and not self._should_partial_index(
                target):
    ...
    def _check_indexing_method(
        ...
        if isinstance(self.dtype, (IntervalDtype, CategoricalDtype)):
            # GH#37871 for now this is only for IntervalIndex and
            # CategoricalIndex
    ...
    def _get_fill_indexer(
        ...
        if self.is_monotonic_increasing and target.is_monotonic_increasing:
            ...
            if method == "pad":
                ...
                # i.e. "backfill"
                indexer = libalgos.backfill(
                    own_values, target_values, limit=limit)
        else:
    ...
    def _difference_compat(
        ...
        if isinstance(self.dtype, PeriodDtype):
            ...
            diff = self._values[indexer] - \
                target._values  # type: ignore[operator]
        return abs(diff)
    ...
    def reindex(
        ...
        if level is not None and (
            isinstance(
                self, ABCMultiIndex) or isinstance(
    ...
    def _join_level(
        ...
        right_indexer = (
            None if right_indexer is None else ensure_platform_int(
                right_indexer)
        )
    ...
    def _wrap_joined_index(
        ...
        if isinstance(self, ABCMultiIndex):
            ...
            return self._constructor._with_infer(
                joined, name=name, dtype=self.dtype)
    ...
    def _getitem_slice(self, slobj: slice) -> Self:
        ...
        result = type(self)._simple_new(
            res, name=self._name, refs=self._references)
        if "_engine" in self._cache:
            reverse = slobj.step is not None and slobj.step < 0
            result._engine._update_from_sliced(
                self._engine, reverse=reverse)  # type: ignore[union-attr]
    ...
    def get_indexer_non_unique(
        ...
        if not self._should_compare(
            ...
            #  that can be matched to Interval scalars.
            return self._get_indexer_non_comparable(
                target, method=None, unique=False)
    ...
    def _get_indexer_strict(
            self, key, axis_name: str_t) -> tuple[Index, np.ndarray]:
        """
    ...
    def _get_indexer_non_comparable(
        ...
        if method is not None:
            ...
            raise TypeError(
                f"Cannot compare dtypes {self.dtype} and {other_dtype}")
    ...
    @final
    def _maybe_downcast_for_indexing(
            self, other: Index) -> tuple[Index, Index]:
        """
    ...
    def _should_compare(self, other: Index) -> bool:
        ...
        if (
            other.inferred_type == "boolean" and is_any_real_numeric_dtype(
                self.dtype)
        ) or (
            self.inferred_type == "boolean" and is_any_real_numeric_dtype(
                other.dtype)
        ):
    ...
    def map(self, mapper, na_action: Literal["ignore"] | None = None):
        ...
        same_dtype = lib.infer_dtype(
            new_values, skipna=False) == self.inferred_type
        if same_dtype:
            new_values = maybe_cast_pointwise_result(
                new_values, self.dtype, same_dtype=same_dtype
            )

        return Index._with_infer(
    ...
    def _maybe_cast_slice_bound(self, label, side: str_t):
        ...
        # wish to have special treatment for floats/ints, e.g. datetimelike
        # Indexes
    ...
    def slice_locs(self, start=None, end=None, step=None) -> tuple[int, int]:
        ...
        if isinstance(start, (str, datetime)) and isinstance(
                end, (str, datetime)):
            try:
...
def _unpack_nested_dtype(other: Index) -> DtypeObj:
    ...
    if isinstance(dtype, CategoricalDtype):
        ...
        if pa.types.is_dictionary(dtype.pyarrow_dtype):
            other = other[:0].astype(ArrowDtype(
...
def get_values_for_csv(
    ...
    *,
    ...
    """
    if isinstance(
            values, Categorical) and values.categories.dtype.kind in "Mm":
    ...
    if isinstance(values, (DatetimeArray, TimedeltaArray)):
        ...
        # GH#21734 Process every column separately, they might have different
        # formats
        results_converted = []
        ...
        res = values._format_native_types(
            na_rep=na_rep, date_format=date_format)
        return res
