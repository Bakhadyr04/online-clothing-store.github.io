...
def _groupby_and_merge(
    ...
    for key, lhs in lby._grouper.get_iterator(
            lby._selected_obj, axis=lby.axis):
        if rby is None:
            rhs = right
        else:
            try:
...
class _MergeOperation:
    ...
    def __init__(
        ...
        # GH 40993: raise when merging between different levels; enforced in
        # 2.0
    ...
    def _get_merge_keys(
        ...
        def is_lkey(x): return isinstance(x, _known) and len(x) == len(left)
        def is_rkey(x): return isinstance(x, _known) and len(x) == len(right)
        ...
        for lk, rk, name in zip(
            ...
            if is_numeric_dtype(lk.dtype) and is_numeric_dtype(rk.dtype):
                ...
                if is_integer_dtype(rk.dtype) and is_float_dtype(lk.dtype):
                    # GH 47391 numpy > 1.24 will raise a RuntimeError for nan
                    # -> int
            ...
            if name in self.left.columns:
                typ = cast(
                    Categorical,
                    lk).categories.dtype if lk_is_cat else object
                self.left = self.left.copy()
                self.left[name] = self.left[name].astype(typ)
            if name in self.right.columns:
                typ = cast(
                    Categorical,
                    rk).categories.dtype if rk_is_cat else object
        ...
        if self.on is None and left_on is None and right_on is None:
            if self.left_index and self.right_index:
                ...
                if (
                    ...
                    raise MergeError(
                        f"Data columns not unique: {repr(common_cols)}")
    ...
    def _validate_validate_kwd(self, validate: str) -> None:
        ...
        if self.right_index:
            ...
            right_unique = MultiIndex.from_arrays(
                self.right_join_keys).is_unique
...
class _AsOfMerge(_OrderedMerge):
    ...
    def _get_join_indexers(
            self) -> tuple[npt.NDArray[np.intp], npt.NDArray[np.intp]]:
        """return the join indexers"""
...
def _get_multiindex_indexer(
    ...
    if sort:
        ...
        def i8copy(a): return a.astype("i8", subok=False, copy=True)
...
def _factorize_keys(
    ...
    """
    # TODO: if either is a RangeIndex, we can likely factorize more
    # efficiently?

    if (
        isinstance(
            lk.dtype,
            DatetimeTZDtype) and isinstance(
            rk.dtype,
            DatetimeTZDtype)
    ) or (lib.is_np_dtype(lk.dtype, "M") and lib.is_np_dtype(rk.dtype, "M")):
        # Extract the ndarray (UTC-localized) values
        # Note: we dont need the dtypes to match, as these can still be
        # compared
        ...
        if (isinstance(lk.dtype, ArrowDtype) and is_string_dtype(lk.dtype)) or (
            ...
            dc = (
                ...
                pa.chunked_array(lk.chunks + rk.chunks)
                .combine_chunks()
                .dictionary_encode()
            )
    ...
    if isinstance(lk, BaseMaskedArray):
        ...
        # ndarray[Any, dtype[object_]]]"; expected "ndarray[Any,
        # dtype[object_]]"
...
def _items_overlap_with_suffix(
    ...
    """
    if not is_list_like(
            suffixes, allow_sets=False) or isinstance(suffixes, dict):
        raise TypeError(
    ...
    if not rlabels.is_unique:
        dups.extend(rlabels[(rlabels.duplicated()) &
