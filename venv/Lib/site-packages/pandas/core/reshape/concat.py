    ...
    def _clean_keys_and_objs(
        ...
        if keys is None:
            ...
            if isinstance(keys, MultiIndex):
                ...
                keys = Index(
                    clean_keys, name=name, dtype=getattr(
                        keys, "dtype", None))
...
def _make_concat_multiindex(indexes, keys, levels=None,
    ...
    if not all_indexes_same(indexes) or not all(
            level.is_unique for level in levels):
        codes_list = []
        ...
        for hlevel, level in zip(zipped, levels):
            ...
            if isinstance(hlevel, Index) and hlevel.equals(level):
                ...
                for key, index in zip(hlevel, indexes):
                    # Find matching codes, include matching nan values as
                    # equal.
