...
class _Unstacker:
    ...
    def __init__(
        ...
        # GH 26314: Previous ValueError raised was too restrictive for many
        # users.
    ...
    @cache_readonly
    def arange_result(
            self) -> tuple[npt.NDArray[np.intp], npt.NDArray[np.bool_]]:
...
def unstack(obj: Series | DataFrame, level,
...
def stack_multiple(frame: DataFrame, level,
                   dropna: bool = True, sort: bool = True):
...
def stack_v3(frame: DataFrame, level: list[int]) -> Series | DataFrame:
    ...
    # If we need to drop `level` from columns, it needs to be in descending
    # order
    ...
    if len(level) > 1:
        # Arrange columns in the order we want to take them, e.g. level=[2, 0,
        # 1]
    ...
    # sort result, but faster than calling sort_index since we know the order
    # we need
    len_df = len(frame)
    n_uniques = len(ordered_stack_cols_unique)
    indexer = np.arange(n_uniques)
    idxs = np.tile(len_df * indexer, len_df) + \
