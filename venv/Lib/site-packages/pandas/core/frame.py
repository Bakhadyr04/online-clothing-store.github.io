...
class DataFrame(NDFrame, OpsMixin):
    ...
    def __init__(
            ...
            if len(data) > 0:
                ...
                if not isinstance(data, np.ndarray) and treat_as_nested(data):
                    ...
                    arrays, columns, index = nested_data_to_arrays(
                        ...
                        # type "Optional[Collection[Any]]"; expected
                        # "Optional[Index]"
                        data,
                        columns,
        ...
        # For data is scalar
            ...
            if isinstance(dtype, ExtensionDtype):
                ...
                mgr = arrays_to_mgr(
                    values, columns, index, dtype=None, typ=manager)
            else:
    ...
    def __dataframe_consortium_standard__(
        ...
        return convert_to_standard_compliant_dataframe(
    ...
    def __arrow_c_stream__(self, requested_schema=None):
        ...
        if requested_schema is not None:
            requested_schema = pa.Schema._import_from_c_capsule(
                requested_schema)
    ...
    def _get_values_for_csv(
        ...
        *,
        ...
        return self._constructor_from_mgr(
            mgr, axes=mgr.axes)  # type: ignore[return-value]
    ...
    def itertuples(
        ...
        if name is not None:
            ...
            # error: namedtuple() expects a string literal as the first
            # argument
    ...
    def dot(self, other: AnyArrayLike | DataFrame) -> DataFrame | Series:
        ...
        if isinstance(other, (Series, DataFrame)):
            ...
            if len(common) > len(self.columns) or len(
                    common) > len(other.index):
        ...
        if isinstance(other, DataFrame):
            common_type = find_common_type(
                list(self.dtypes) + list(other.dtypes))
            ...
            if result.ndim == 2:
                ...
                return self._constructor_sliced(
                    result, index=left.index, copy=False)
        ...
        if index is not None:
            if isinstance(index, str) or not hasattr(index, "__iter__"):
                ...
                try:
                    ...
                    result_index = ensure_index_from_sequences(
                        index_data, names=index)
    ...
    def to_records(
        ...
        if index:
            ...
            names = [
                str(name) for name in itertools.chain(
                    index_names, self.columns)]
        else:
            arrays = [np.asarray(self.iloc[:, i])
    ...
    def to_feather(self, path: FilePath |
                   WriteBuffer[bytes], **kwargs) -> None:
        """
    ...
    def transpose(self, *args, copy: bool = False) -> DataFrame:
        ...
        **Non-square DataFrame with mixed dtypes**
        ...
        if self._can_fast_transpose:
            # Note: tests pass without this, but this improves perf quite a
            # bit.
            ...
            if isinstance(dtypes[0], BaseMaskedDtype):
                # We have masked arrays with the same dtype. We can transpose
                # faster.
                ...
                # We have arrow EAs with the same dtype. We can transpose
                # faster.
                ...
                new_values = transpose_homogeneous_pyarrow(
                    cast(
                        Sequence[ArrowExtensionArray],
                        self._iter_column_arrays())
                )
            else:
                # We have other EAs with the same dtype. We preserve dtype in
                # transpose.
                dtyp = dtypes[0]
                arr_typ = dtyp.construct_array_type()
                values = self.values
                new_values = [
                    arr_typ._from_sequence(
                        row, dtype=dtyp) for row in values]
    ...
    def _ixs(self, i: int, axis: AxisInt = 0) -> Series:
        ...
        if axis == 0:
            ...
            copy = isinstance(new_mgr.array,
                              np.ndarray) and new_mgr.array.base is None
            result = self._constructor_sliced_from_mgr(
    ...
    def _setitem_frame(self, key, value):
        ...
        if isinstance(key, np.ndarray):
            ...
            key = self._constructor(
                key, **self._construct_axes_dict(), copy=False)
    ...
    def _set_item_frame_value(self, key, value: DataFrame) -> None:
        ...
        if key in self.columns:
            ...
            len_cols = 1 if is_scalar(cols) or isinstance(
                cols, tuple) else len(cols)
            ...
            if isinstance(self.columns, MultiIndex) and isinstance(
                ...
                if len(cols_droplevel) and not cols_droplevel.equals(
                        value.columns):
    ...
    def _iset_item_mgr(
        ...
        # when called from _set_item_mgr loc can be anything returned from
        # get_loc
    ...
    def _iset_item(self, loc: int, value: Series,
    ...
    def _ensure_valid_index(self, value) -> None:
        ...
        if not len(self.index) and is_list_like(value) and len(value):
            ...
            self._mgr = self._mgr.reindex_axis(
                index_copy, axis=1, fill_value=np.nan)
    ...
    def query(self, expr: str, *,
              inplace: Literal[False] = ..., **kwargs) -> DataFrame:
        ...
    ...
    def eval(self, expr: str, *,
             inplace: Literal[False] = ..., **kwargs) -> Any:
        ...
    ...
    def select_dtypes(self, include=None, exclude=None) -> Self:
        ...
        * To select Pandas datetimetz dtypes, use ``'datetimetz'``
        ...
        def check_int_infer_dtype(dtypes):
            ...
            for dtype in dtypes:
                ...
                if (isinstance(dtype, str) and dtype ==
                    ...
                    # GH#42452 : np.dtype("float") coerces to np.float64 from
                    # Numpy 1.20
        ...
        def dtype_predicate(dtype: DtypeObj, dtypes_set) -> bool:
            ...
            dtype = dtype if not isinstance(
                dtype, ArrowDtype) else dtype.numpy_dtype
        ...
        # error: Incompatible return value type (got "DataFrame", expected
        # "Self")
        return self._constructor_from_mgr(mgr, axes=mgr.axes).__finalize__(
    ...
    def _sanitize_column(
            self, value) -> tuple[ArrayLike, BlockValuesRefs | None]:
        """
    ...
    @property
    def _series(self):
        return {item: self._ixs(idx, axis=1)
    ...
    def shift(
        ...
        if axis == 1 and periods != 0 and ncols > 0 and freq is None:
            if fill_value is lib.no_default:
                ...
                # Use a column that we know is valid for our column's dtype
                # GH#38434
    ...
    def set_index(
        ...
        *,
        ...
        for col in keys:
            if isinstance(
                    col, (Index, Series, np.ndarray, list, abc.Iterator)):
        ...
        for col in keys:
            if isinstance(col, MultiIndex):
                arrays.extend(col._get_level_values(n)
    ...
    def reset_index(
        ...
        *,
        ...
        if allow_duplicates is not lib.no_default:
            allow_duplicates = validate_bool_kwarg(
        ...
        if not drop:
            ...
            for i, (lev, lab) in reversed(list(enumerate(to_insert))):
                ...
                name = names[i]
                if multi_col:
                    col_name = list(name) if isinstance(
                        name, tuple) else [name]
                    if col_fill is None:
    ...
    def dropna(
        ...
        *,
        ...
        if thresh is not lib.no_default:
            ...
            # faster equivalent to 'agg_obj.count(agg_axis) ==
            # self.shape[agg_axis]'
    ...
    def sort_values(
        ...
        *,
        ...
        if is_range_indexer(indexer, len(indexer)):
            result = self.copy(
                deep=(not inplace and not using_copy_on_write()))
            if ignore_index:
                result.index = default_index(len(result))
    ...
    def nlargest(
        ...
        return selectn.SelectNFrame(
            self, n=n, keep=keep, columns=columns).nlargest()
    ...
    def nsmallest(
        ...
        """
        return selectn.SelectNFrame(
            self, n=n, keep=keep, columns=columns).nsmallest()
    ...
    def swaplevel(self, i: Axis = -2, j: Axis = -1,
                  axis: Axis = 0) -> DataFrame:
        result = self.copy(deep=None)

        axis = self._get_axis_number(axis)

        if not isinstance(result._get_axis(
                axis), MultiIndex):  # pragma: no cover
    ...
    def reorder_levels(
            self, order: Sequence[int | str], axis: Axis = 0) -> DataFrame:
        """
        ...
        """
        axis = self._get_axis_number(axis)
        if not isinstance(self._get_axis(
                axis), MultiIndex):  # pragma: no cover
    ...
    def _should_reindex_frame_op(
        ...
        if fill_value is None and level is None and axis == 1:
            ...
            # Intersection is always unique so we have to check the unique
            # columns
            left_uniques = self.columns.unique()
            right_uniques = right.columns.unique()
            cols = left_uniques.intersection(right_uniques)
            if len(cols) and not (
                len(cols) == len(left_uniques) and len(
                    cols) == len(right_uniques)
            ):
    ...
    def _align_for_op(
        ...
        if isinstance(right, np.ndarray):
            ...
            # GH#36702. Raise when attempting arithmetic with list of
            # array-like.
    ...
    def _flex_cmp_method(self, other, op, *,
                         axis: Axis = "columns", level=None):
        axis = self._get_axis_number(axis) if axis is not None else 1
    ...
    def combine(
        ...
        # sorts if possible; otherwise align above ensures that these are
        # set-equal
    ...
    def explode(
        ...
        if len(columns) == 1:
            ...
            def mylen(x): return len(x) if (
                is_list_like(x) and len(x) > 0) else 1
    ...
    def unstack(self, level: IndexLabel = -1,
                fill_value=None, sort: bool = True):
        """
    ...
    def cov(
        ...
        **Minimum number of periods**
        ...
        result = self._constructor(
            base_cov, index=idx, columns=cols, copy=False)
    ...
    def corrwith(
        ...
        if isinstance(other, Series):
            return this.apply(lambda x: other.corr(
                x, method=method), axis=axis)
    ...
    def count(self, axis: Axis = 0, numeric_only: bool = False):
        ...
        if len(frame._get_axis(axis)) == 0:
            result = self._constructor_sliced(
                0, index=frame._get_agg_axis(axis))
        else:
            result = notna(frame).sum(axis=axis)

        return result.astype("int64", copy=False).__finalize__(
            self, method="count")
            ...
            if isinstance(values, ExtensionArray):
                ...
                if has_keepdims:
                    return values._reduce(
                        name, skipna=skipna, keepdims=True, **kwds)
                else:
                    warnings.warn(
            ...
            if df.shape[1] and name != "kurt":
                ...
                if isinstance(dtype, ExtensionDtype):
                    ...
                    if not skipna and name not in ("any", "all"):
                        ...
                        other = -1 if name in ("idxmax",
                                               "idxmin") else lib.no_default
    ...
    def _reduce_axis1(self, name: str, func, skipna: bool) -> Series:
        ...
        res_ser = self._constructor_sliced(
    ...
    def cummin(self, axis: Axis | None = None,
               skipna: bool = True, *args, **kwargs):
        return NDFrame.cummin(self, axis, skipna, *args, **kwargs)

    @doc(make_doc("cummax", ndim=2))
    def cummax(self, axis: Axis | None = None,
               skipna: bool = True, *args, **kwargs):
        return NDFrame.cummax(self, axis, skipna, *args, **kwargs)

    @doc(make_doc("cumsum", ndim=2))
    def cumsum(self, axis: Axis | None = None,
               skipna: bool = True, *args, **kwargs):
        return NDFrame.cumsum(self, axis, skipna, *args, **kwargs)

    @doc(make_doc("cumprod", 2))
    def cumprod(self, axis: Axis | None = None,
                skipna: bool = True, *args, **kwargs):
    ...
    def idxmin(
        ...
        final_result = data._constructor_sliced(
            result, index=data._get_agg_axis(axis))
    ...
    def quantile(
        ...
        if not is_list_like(q):
            ...
            # ndarray[Any, Any] | Index | Series | Sequence[float]"; expected
            # "float"
        ...
        if method == "single":
            ...
            if len(data) == 0:
                if data.ndim == 2:
                    ...
                    dtype = self.dtype
                return self._constructor(
                    [], index=q, columns=data.columns, dtype=dtype)
    ...
    def _to_dict_of_blocks(self):
        ...
        """
        mgr = self._mgr
        # convert to BlockManager if needed -> this way support ArrayManager as
        # well
