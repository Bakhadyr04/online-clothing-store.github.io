...
def _ensure_arraylike(values, func_name: str) -> ArrayLike:
    ...
    """
    if not isinstance(values, (ABCIndex, ABCSeries,
                      ABCExtensionArray, np.ndarray)):
        # GH#52986
...
def factorize(
    ...
    if (
        ...
        if not use_na_sentinel and values.dtype == object:
            ...
            # provided dtype. This may be revisited in the future, see
            # GH#48476.
...
def value_counts_internal(
    ...
    if bins is not None:
        ...
        if is_extension_array_dtype(values):
            ...
            result = Series(
                values, copy=False)._values.value_counts(
                dropna=dropna)
            result.name = name
...
def take(
    ...
    """
    if not isinstance(
            arr, (np.ndarray, ABCExtensionArray, ABCIndex, ABCSeries)):
        # GH#52981
        warnings.warn(
...
def diff(arr, n: int, axis: AxisInt = 0):
    ...
    if isinstance(dtype, NumpyEADtype):
        # NumpyExtensionArray cannot necessarily hold shifted versions of
        # itself.
        arr = arr.to_numpy()
        dtype = arr.dtype
...
def _sort_mixed(values) -> AnyArrayLike:
    ...
    # convert boolean arrays to positional indices, then order by underlying
    # values
...
def union_with_duplicates(
    ...
    with warnings.catch_warnings():
        ...
        # the index here, so the deprecation does not affect the end result
        # here.
