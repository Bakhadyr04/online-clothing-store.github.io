...
def data():
    ...
    return pd.array(
        [True, False] * 4 + [np.nan] + [True, False] *
        44 + [np.nan] + [True, False],
        dtype="boolean",
    )
...
def test_error_invalid_values(
        data, all_arithmetic_operators, using_infer_string):
    # invalid ops
    ...
    if op not in ("__mul__", "__rmul__"):
        # TODO(extension) numpy's mul with object array sees booleans as
        # numbers
        msg = "|".join(
            [
                r"unsupported operand type\(s\) for",
