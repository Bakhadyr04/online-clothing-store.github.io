...
class TestSparseIndexUnion:
    ...
    def test_index_make_union(self, xloc, xlen, yloc,
                              ylen, eloc, elen, test_length):
        # Case 1
        # x: ----
        # y:     ----
        # r: --------
        ...
        tm.assert_numpy_array_equal(
            bresult.blocs, np.array(
                eloc, dtype=np.int32))
        tm.assert_numpy_array_equal(
            bresult.blengths, np.array(
                elen, dtype=np.int32))
...
class TestSparseIndexCommon:
    def test_int_internal(self):
        idx = make_sparse_index(4, np.array(
            [2, 3], dtype=np.int32), kind="integer")
    ...
    def test_block_internal(self):
        ...
        idx = make_sparse_index(4, np.array(
            [0, 1, 2, 3], dtype=np.int32), kind="block")
        assert isinstance(idx, BlockIndex)
        assert idx.npoints == 4
        ...
        idx = make_sparse_index(4, np.array(
            [0, 2, 3], dtype=np.int32), kind="block")
        assert isinstance(idx, BlockIndex)
    ...
    def test_lookup(self, kind):
        ...
        idx = make_sparse_index(4, np.array(
            [0, 1, 2, 3], dtype=np.int32), kind=kind)
        assert idx.lookup(-1) == -1
        ...
        idx = make_sparse_index(4, np.array(
            [0, 2, 3], dtype=np.int32), kind=kind)
        assert idx.lookup(-1) == -1
        assert idx.lookup(0) == 0
        assert idx.lookup(1) == -1
...
class TestBlockIndex:
    ...
    def test_make_block_boundary(self, i):
        ...
        tm.assert_numpy_array_equal(
            idx.blengths, np.ones(
                len(exp), dtype=np.int32))
    ...
    def test_to_int_index(self):
        ...
        tm.assert_numpy_array_equal(
            dense.indices, np.array(
                exp_inds, dtype=np.int32))
