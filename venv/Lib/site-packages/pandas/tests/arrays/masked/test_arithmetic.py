...
# floating dtypes
arrays += [pd.array([0.1, 0.2, 0.3, None], dtype=dtype)
           for dtype in tm.FLOAT_EA_DTYPES]
scalars += [0.2, 0.2]
# boolean
...
@pytest.fixture(params=zip(arrays, scalars),
                ids=[a.dtype.name for a in arrays])
def data(request):
...
def test_array_scalar_like_equivalence(data, all_arithmetic_operators):
    ...
    # TODO also add len-1 array (np.array([scalar],
