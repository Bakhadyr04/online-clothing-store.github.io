    ...
    def test_setitem_listlike(self):
        ...
        cat = Categorical(
            np.random.default_rng(2).integers(
                0, 5, size=150000).astype(np.int8)
        ).add_categories([-1000])
...
class TestCategoricalIndexing:
    ...
    @pytest.mark.parametrize(
        "null_val",
        [None, np.nan, NaT, NA, math.nan, "NaT",
            "nat", "NAT", "nan", "NaN", "NAN"],
    )
    def test_periodindex_on_null_types(self, null_val):
        ...
        expected = PeriodIndex(
            ["2022-04-06", "2022-04-07", "NaT"], dtype="period[D]")
    ...
    def test_get_indexer_non_unique(
            self, idx_values, key_values, key_class, dtype):
        # GH 21448
    ...
    def test_where_unobserved_categories(self):
        ...
        expected = Series(Categorical(
            ["a", "b", "b"], categories=ser.cat.categories))
        tm.assert_series_equal(result, expected)
