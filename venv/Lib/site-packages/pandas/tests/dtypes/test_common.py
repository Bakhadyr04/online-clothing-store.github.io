...
@pytest.mark.parametrize("name1,dtype1",
                         list(dtypes.items()), ids=lambda x: str(x))
...
def get_is_dtype_funcs():
    ...
    """
    fnames = [
        f for f in dir(com) if (
            f.startswith("is_") and f.endswith("dtype"))]
...
@pytest.mark.parametrize("func", get_is_dtype_funcs(),
...
@pytest.mark.parametrize(
    "dtype", [np.int64, np.array(
        [1, 2], dtype=np.int64), "Int64", pd.Int64Dtype]
)
