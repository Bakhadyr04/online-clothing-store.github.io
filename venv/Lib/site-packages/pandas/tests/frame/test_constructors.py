...
class TestDataFrameConstructors:
    ...
    def test_constructor_dict_with_tzaware_scalar(self):
        # GH#42505
        dt = Timestamp(
            "2019-11-03 01:00:00-0700").tz_convert("America/Los_Angeles")
        dt = dt.as_unit("ns")
    ...
    def test_datetimelike_values_with_object_dtype(
            self, kind, frame_or_series):
        # with dtype=object, we should cast dt64 values to Timestamps, not
        # pydatetimes
        if kind == "M":
            dtype = "M8[ns]"
    ...
    def test_constructor_dict_nan_tuple_key(self, value):
        ...
        values = [[0, 3], [1, 4], [2, 5]]
        data = {cols[c]: Series(values[c], index=idx) for c in range(3)}
        result = DataFrame(data).sort_values(
            (11, 21)).sort_values(
            ("a", value), axis=1)
        expected = DataFrame(
            np.arange(6, dtype="int64").reshape(2, 3), index=idx, columns=cols
    ...
    def test_constructor_period_dict(self):
        ...
        # list of periods
        df = DataFrame({"a": a.astype(object).tolist(),
    ...
    def test_constructor_list_of_series_aligned_index(self):
        series = [Series(i, index=["b", "a", "c"], name=str(i))
                  for i in range(3)]
    ...
    def test_constructor_list_of_dict_order(self):
        ...
        data = [
            ...
            {"Second": 6, "First": 3, "Fourth": 12,
                "Third": 9, "YYY": 14, "XXX": 13},
        ]
        expected = DataFrame(
            {
                "First": [1, 2, 3],
    ...
    def test_constructor_single_value(self):
        ...
        tm.assert_frame_equal(
            df, DataFrame(np.zeros(df.shape).astype(
    ...
    def test_constructor_with_datetimes3(self):
        ...
        tm.assert_series_equal(
            df.dtypes, Series(
                {"End Date": "datetime64[us, US/Eastern]"}, dtype=object)
        )
        ...
        tm.assert_series_equal(
            df.dtypes, Series(
                {"End Date": "datetime64[ns, US/Eastern]"}, dtype=object)
        )
    ...
    def test_constructor_datetimes_non_ns(self, order, unit):
        ...
        if unit in ["M", "D", "h", "m"]:
            ...
            # instead the constructor casts to the closest supported reso, i.e.
            # "s"
    ...
    def test_constructor_categorical_series(self):
        ...
        expected = Series(
            Categorical(
                values=[
                    np.nan, np.nan, np.nan], categories=[
                    "a", "b", "c"])
        )
    ...
    def test_check_dtype_empty_numeric_column(self, dtype):
        ...
        # Empty DataFrame is generated via dictionary data with non-overlapping
        # columns.
    ...
    def test_check_dtype_empty_string_column(
            self, request, dtype, using_array_manager):
        # GH24386: Ensure dtypes are set correctly for an empty DataFrame.
        # Empty DataFrame is generated via dictionary data with non-overlapping
        # columns.
        data = DataFrame({"a": [1, 2]}, columns=["b"], dtype=dtype)
        ...
        if using_array_manager and dtype in tm.BYTES_DTYPES:
            # TODO(ArrayManager) astype to bytes dtypes does not yet give
            # object dtype
    ...
    def test_frame_ctor_datetime64_column(self):
        ...
        df = DataFrame(
            {"A": np.random.default_rng(
                2).standard_normal(len(rng)), "B": dates}
        )
    ...
    def test_dict_nocopy(
        ...
        if copy:
            ...
            # i.e. df.iloc[0, 2]=45 did *not* update c
            assert c[0] == c_orig[0]
...
class TestDataFrameConstructorIndexInference:
    ...
    def test_dict_data_arrow_column_expansion(
            self, key_val, col_vals, col_type):
        # GH 53617
        pa = pytest.importorskip("pyarrow")
        cols = pd.arrays.ArrowExtensionArray(
            pa.array(
                col_vals, type=pa.dictionary(
                    pa.int8(), getattr(
                        pa, col_type)()))
        )
...
class TestFromScalar:
    ...
    def test_tzaware_data_tznaive_dtype(
            self, constructor, box, frame_or_series):
        tz = "US/Eastern"
        ts = Timestamp("2019", tz=tz)
