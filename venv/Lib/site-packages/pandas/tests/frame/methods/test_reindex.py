...
class TestReindexSetIndex:
    ...
    def test_reindex_timestamp_with_fold(
            self, timezone, year, month, day, hour):
        # see gh-40817
...
class TestDataFrameSelectReindex:
    ...
    def test_reindex_methods_nearest_special(self):
        ...
        actual = df.reindex(
            target, method="nearest", tolerance=[
                0.5, 0.01, 0.4, 0.1])
        tm.assert_frame_equal(expected, actual)
    ...
    def test_reindex_frame_tz_ffill_bfill(
            self, frame_or_series, method, exp_values):
        # GH#38566
        obj = frame_or_series(
            [0, 1, 2, 3],
            index=date_range(
    ...
    def test_reindex_multiindex_ffill_added_rows(self):
        # GH#23693
        # reindex added rows with nan values even when fill method was
        # specified
