...
class TestFrameAsof:
    ...
    def test_subset(self, date_range_frame):
        N = 10
        # explicitly cast to float to avoid implicit upcast when setting to
        # np.nan
        ...
        expected.iloc[20:] = 9
        # no "missing", so "B" can retain int dtype (df["A"].dtype
        # platform-dependent)
    ...
    def test_is_copy(self, date_range_frame):
        ...
        # doesn't track the parent dataframe / doesn't give SettingWithCopy
        # warnings
        df = date_range_frame.astype({"A": "float"})
        N = 50
        df.loc[df.index[15:30], "A"] = np.nan
