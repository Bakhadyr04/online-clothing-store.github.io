...
class TestAstype:
    ...
    def test_astype_dict_like(self, dtype_class):
        ...
        dt7 = dtype_class(
            {}) if dtype_class is dict else dtype_class(
            {}, dtype=object)
        equiv = df.astype(dt7)
        tm.assert_frame_equal(df, equiv)
        tm.assert_frame_equal(df, original)
    ...
    @pytest.mark.parametrize("cls",
                             [CategoricalDtype, DatetimeTZDtype, IntervalDtype])
    ...
    def test_astype_tz_object_conversion(self, tz):
        ...
        # convert expected to object dtype from other tz str (independently
        # tested)
        result = expected.astype({"tz": f"datetime64[ns, {tz}]"})
        result = result.astype({"tz": "object"})

        # do real test: object dtype to a specified tz, different from
        # construction tz.
