...
class TestDataFramePctChange:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (-1, "ffill", None, [np.nan, np.nan, -
             0.5, -0.5, -0.6, 0, 0, np.nan]),
            (-1, "ffill", 1, [np.nan, np.nan, -
             0.5, -0.5, -0.6, 0, np.nan, np.nan]),
            (-1, "bfill", None, [0, 0, -0.5, -
             0.5, -0.6, np.nan, np.nan, np.nan]),
            (-1, "bfill", 1, [np.nan, 0, -0.5, -
             0.5, -0.6, np.nan, np.nan, np.nan]),
        ],
    )
    ...
    def test_pct_change_numeric(self):
        ...
        for axis in range(2):
            expected = pnl.ffill(axis=axis) / \
                pnl.ffill(axis=axis).shift(axis=axis) - 1
