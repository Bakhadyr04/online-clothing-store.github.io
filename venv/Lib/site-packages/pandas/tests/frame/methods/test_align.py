...
class TestDataFrameAlign:
    ...
    def test_align_float(self, float_frame, using_copy_on_write):
        ...
        with tm.assert_produces_warning(FutureWarning, match=msg):
            af, bf = float_frame.align(
                other, join="inner", axis=1, method="pad")
        tm.assert_index_equal(bf.columns, other.columns)
    ...
    def test_multiindex_align_to_series_with_common_index_level_missing_in_right(
        ...
        series = Series([1, 2], index=Index(
            [1, 2], name="bar"), name="foo_series")
        df = DataFrame(
            {"col": np.arange(12)},
            index=pd.MultiIndex.from_product([foo_index, bar_index]),
        )
    ...
    def test_multiindex_align_to_series_with_common_index_level_missing_in_both(
        ...
        expected_r = Series([1, np.nan, 3] *
                            3, index=df.index, name="foo_series")
        result_l, result_r = df.align(series, axis=0)
