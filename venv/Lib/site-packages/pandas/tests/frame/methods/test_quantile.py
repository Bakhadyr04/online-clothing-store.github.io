...
class TestDataFrameQuantile:
    ...
    def test_non_numeric_exclusion(
            self, interp_method, request, using_array_manager):
        interpolation, method = interp_method
    ...
    def test_quantile_box_nat(self):
        # DatetimeLikeBlock may be consolidated and contain NaT in different
        # loc
        df = DataFrame(
            {
                "A": [
    ...
    def test_quantile_nat(self, interp_method, request,
                          using_array_manager, unit):
        interpolation, method = interp_method
        if method == "table" and using_array_manager:
            request.applymarker(
                pytest.mark.xfail(
