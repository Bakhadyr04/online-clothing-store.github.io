...
def where_frame(request, float_string_frame,
...
class TestDataFrameIndexingWhere:
    ...
    def test_where_set(self, where_frame, float_string_frame, mixed_int_frame):
        ...
        def _check_set(df, cond, check_dtypes=True):
            dfi = df.copy()
            econd = cond.reindex_like(df).fillna(
    ...
    def test_where_axis_with_upcast(self):
        ...
        result = df.copy()
        with tm.assert_produces_warning(FutureWarning, match="incompatible dtype"):
            return_value = result.where(
                mask, ser, axis="columns", inplace=True)
        assert return_value is None
        tm.assert_frame_equal(result, expected)
    ...
    def test_where_tz_values(self, tz_naive_fixture, frame_or_series):
        obj1 = DataFrame(
            DatetimeIndex(["20150101", "20150102", "20150103"],
                          tz=tz_naive_fixture),
            columns=["date"],
        )
        obj2 = DataFrame(
            DatetimeIndex(["20150103", "20150104", "20150105"],
                          tz=tz_naive_fixture),
            columns=["date"],
        )
        mask = DataFrame([True, True, False], columns=["date"])
        exp = DataFrame(
            DatetimeIndex(["20150101", "20150102", "20150105"],
                          tz=tz_naive_fixture),
            columns=["date"],
        )
    ...
    def test_where_categorical_filtering(self):
        # GH#22609 Verify filtering operations on DataFrames with categorical
        # Series
