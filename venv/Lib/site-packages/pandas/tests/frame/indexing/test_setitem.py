...
class TestDataFrameSetItem:
    ...
    def test_setitem_different_dtype(self):
        ...
        result = df.dtypes
        expected = Series(
            [np.dtype("float64")] * 4 + [np.dtype("float32")] *
            2 + [np.dtype("int32")],
    ...
    def test_setitem_dt64_ndarray_with_NaT_and_diff_time_units(self):
        ...
        # OutOfBoundsDatetime error shouldn't occur; as of 2.0 we preserve
        # "M8[s]"
    ...
    def test_setitem_intervals(self):
        ...
        def c(x): return Index(np.array(x))
    ...
    def test_setitem_dtypes_bytes_type_to_object(self):
        ...
        result = df.dtypes
        expected = Series([np.uint32, object, object,
                          np.uint8], index=list("abcd"))
        tm.assert_series_equal(result, expected)
...
class TestDataFrameSetItemWithExpansion:
    ...
    def test_setitem_string_column_numpy_dtype_raising(self):
        ...
        df["0 - Name"] = [5, 6]
        expected = DataFrame([[1, 2, 5], [3, 4, 6]],
                             columns=[0, 1, "0 - Name"])
        tm.assert_frame_equal(df, expected)
    ...
    def test_setitem_with_expansion_categorical_dtype(self):
        ...
        result = df.dtypes
        expected = Series(
            [np.dtype("int32"), CategoricalDtype(
    ...
    def test_loc_expansion_with_timedelta_type(self):
        ...
        expected = DataFrame(
            [[pd.Timedelta("0 days 00:00:05"), pd.Timedelta(
                "0 days 00:01:12"), "23"]],
            index=Index([0]),
            columns=(["a", "b", "c"]),
        )
...
class TestDataFrameSetItemSlicing:
    ...
    def test_setitem_slice_indexer_broadcasting_rhs(self, n, box, indexer):
        ...
        expected = DataFrame([[1, 3, 5]] + [[10, 11, 12]]
                             * n, columns=["a", "b", "c"])
        tm.assert_frame_equal(df, expected)
...
class TestDataFrameSetItemBooleanMask:
    ...
    def test_setitem_boolean_indexing(self):
        ...
        df2 = DataFrame(
            index=idx, columns=cols, data=np.ones(
                (len(idx), len(cols))))

        expected = DataFrame(
            index=idx,
            columns=cols,
            data=np.array([[0.0, 0.5, 1.0], [1.5, 2.0, -1],
                          [-1, -1, -1]], dtype=float),
        )
...
class TestDataFrameSetitemCopyViewSemantics:
    ...
    def test_setitem_column_update_inplace(
        ...
        df = DataFrame({col: np.zeros(len(labels))
                       for col in labels}, index=labels)
    ...
    def test_setitem_frame_dup_cols_dtype(self):
        ...
        rhs = DataFrame([[0, 1.5], [2, 2.5]], columns=["a", "a"])
        df["a"] = rhs
        expected = DataFrame([[0, 1.5, 3], [2, 2.5, 6]],
                             columns=["a", "a", "b"])
        tm.assert_frame_equal(df, expected)
