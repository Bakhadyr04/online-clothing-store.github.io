...
class TestDataFrameIndexing:
    ...
    def test_getitem_numeric_should_not_fallback_to_positional(
        ...
        result = df[1]
        expected = DataFrame(
            [[1, 3], [4, 6]], columns=Index([1, 1], dtype=dtype))
    ...
    def test_getitem_boolean(self, mixed_float_frame,
    ...
    def test_boolean_index_empty_corner(self):
        # #2096
        blah = DataFrame(np.empty([0, 1]), columns=[
                         "A"], index=DatetimeIndex([]))
    ...
    def test_setitem_ambig(self, using_infer_string):
        ...
        coercable_series = Series([Decimal(1)
    ...
    # already covered in test_iloc_col_slice_view
    ...
    def test_setitem_frame_mixed(self, float_string_frame):
        ...
        tm.assert_almost_equal(
            f.loc[f.index[0:2], ["A", "B"]].values, piece.values)
    ...
    def test_setitem_frame_mixed_key_unaligned(self, float_string_frame):
        ...
        piece["B"] = np.nan
        tm.assert_almost_equal(
            f.loc[f.index[-2:], ["A", "B"]].values, piece.values)
    ...
    def test_loc_duplicates(self):
        ...
        trange = trange.insert(
            loc=5, item=Timestamp(
                year=2017, month=1, day=5))
    ...
    def test_loc_setitem_datetimelike_with_inference(self):
        ...
        df.loc[df.index[:3], "C"] = np.array(
            [3 * one_hour] * 3, dtype="m8[ns]")
        df.loc[:, "D"] = np.array([4 * one_hour] * 4, dtype="m8[ns]")
        df.loc[df.index[:3], "E"] = np.array(
            [5 * one_hour] * 3, dtype="m8[ns]")
        df["F"] = np.timedelta64("NaT")
        df.loc[df.index[:-1],
               "F"] = np.array([6 * one_hour] * 3, dtype="m8[ns]")
        df.loc[df.index[-3]:, "G"] = date_range("20130101", periods=3)
        df["H"] = np.datetime64("NaT")
        result = df.dtypes
        expected = Series(
            [np.dtype("timedelta64[ns]")] * 6 +
    ...
    def test_setitem_value_coercing_dtypes(self, indexer, idx):
        # GH#50467
        df = DataFrame([["1", np.nan], ["2", np.nan],
                       ["3", np.nan]], dtype=object)
        rhs = DataFrame([[1, np.nan], [2, np.nan]])
        indexer(df)[:idx, :] = rhs
        expected = DataFrame(
            [[1, np.nan], [2, np.nan], ["3", np.nan]], dtype=object)
        tm.assert_frame_equal(df, expected)
...
class TestDataFrameIndexingUInt64:
    def test_setitem(self):
        ...
        with tm.assert_produces_warning(FutureWarning, match="incompatible dtype"):
            ...
            df2.iloc[1, 2] = pd.NaT
        result = df2["B"]
        tm.assert_series_equal(notna(result), Series(
...
class TestLocILocDataFrameCategorical:
    ...
    def exp_single_row(self):
        ...
        cats1 = Categorical(["a", "a", "b", "a", "a", "a",
                            "a"], categories=["a", "b"])
        idx1 = Index(["h", "i", "j", "k", "l", "m", "n"])
        values1 = [1, 1, 2, 1, 1, 1, 1]
        exp_single_row = DataFrame(
            {"cats": cats1, "values": values1}, index=idx1)
    ...
    def test_loc_iloc_setitem_list_of_lists(
            self, orig, exp_multi_row, indexer):
    ...
    def test_loc_iloc_setitem_full_row_non_categorical_rhs(
        ...
        # not categorical dtype, but "b" _is_ among the categories for
        # df["cat"]
        indexer(df)[key, :] = ["b", 2]
    ...
    def test_getitem_preserve_object_index_with_dates(self, indexer):
        ...
        # from dataframe, don't try to infer object dtype index on Series
        # construction
...
def test_adding_new_conditional_column_with_string(
        dtype, infer_string) -> None:
...
class TestSetitemValidation:
    ...
    @pytest.mark.parametrize("invalid", _invalid_scalars +
                             [True, 1.5, np.float64(1.5)])
    @pytest.mark.parametrize("indexer", _indexers)
