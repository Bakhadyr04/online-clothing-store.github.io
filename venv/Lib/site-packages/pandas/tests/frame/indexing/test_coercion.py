...
class TestDataFrameSetitemCoercion:
    ...
    def test_loc_setitem_multiindex_columns(self, consolidate):
        ...
        # TODO: i think this isn't about MultiIndex and could be done with
        # iloc?
...
def test_6942(indexer_al):
    # check that the .at __setitem__ after setting "Live" actually sets the
    # data
...
def test_26395(indexer_al):
    ...
    indexer_al(df)["C", "D"] = 2
    expected = DataFrame({"D": [0, 0, 2]}, index=[
                         "A", "B", "C"], dtype=np.int64)
    tm.assert_frame_equal(df, expected)
    ...
    expected = DataFrame({"D": [0, 0, 44.5]}, index=[
    ...
    expected = DataFrame({"D": [0, 0, "hello"]}, index=[
                         "A", "B", "C"], dtype=object)
    tm.assert_frame_equal(df, expected)
...
def test_12499():
    ...
    expected = DataFrame(
        {"one": [0, np.nan], "two": Series(
            [ts, NaT], dtype="datetime64[ns, UTC]")}
    )
    tm.assert_frame_equal(df, expected)
...
def test_20476():
    ...
    filler = DataFrame([[1, 2, 3.0]] *
                       3, index=range(3), columns=["a", "b", "c"])
    df["A"] = filler
    ...
    expected.columns = mi
    exp_dtypes = Series(
        [np.dtype(np.int64)] * 2 + [np.dtype(np.float64)] +
        [np.dtype(np.int64)] * 3,
        index=mi,
    )
    tm.assert_series_equal(df.dtypes, exp_dtypes)
