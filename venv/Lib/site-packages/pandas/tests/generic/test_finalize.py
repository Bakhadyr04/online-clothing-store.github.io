...
def test_binops(request, args, annotate, all_binary_operators):
    ...
    if not (isinstance(left, int) or isinstance(
            right, int)) and annotate != "both":
    ...
    if is_cmp and isinstance(
            left, pd.DataFrame) and isinstance(right, pd.Series):
