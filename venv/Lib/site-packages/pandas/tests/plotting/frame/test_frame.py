...
class TestDataFramePlots:
    ...
    def test_plot_subplots(self):
        ...
        # _check_plot_works adds an ax so use default_axes=True to avoid
        # warning
    ...
    def test_unsorted_index_lims_x_y(self):
        df = DataFrame({"y": [0.0, 1.0, 2.0, 3.0],
                       "z": [91.0, 90.0, 93.0, 92.0]})
        ax = df.plot(x="z", y="y")
        xmin, xmax = ax.get_xlim()
        lines = ax.get_lines()
        assert xmin <= np.nanmin(lines[0].get_data()[0])
    ...
    def test_line_area_nan_df_stacked_area(self, idx, kwargs):
        ...
        if kwargs:
            ...
            tm.assert_numpy_array_equal(
                ax.lines[1].get_ydata(), expected1 + expected2)
    ...
    def test_plot_scatter_error(self):
        ...
        msg = re.escape(
            "scatter() missing 1 required positional argument: 'y'")
        with pytest.raises(TypeError, match=msg):
            df.plot.scatter(x="x")
        msg = re.escape(
            "scatter() missing 1 required positional argument: 'x'")
        with pytest.raises(TypeError, match=msg):
            df.plot.scatter(y="y")
    ...
    def test_raise_error_on_datetime_time_data(self):
        ...
        df = DataFrame(np.random.default_rng(
            2).standard_normal(10), columns=["a"])
    ...
    def test_scatterplot_color_by_categorical(self, ordered, categories):
        ...
        result_yticklabels = [i.get_text()
                              for i in colorbar.ax.get_ymajorticklabels()]
        assert all(
            i == j for i,
            j in zip(
                result_yticklabels,
    ...
    def test_plot_scatter_with_c_array(self):
        ...
        # identical.
        tm.assert_numpy_array_equal(
            ax.collections[0].get_facecolor(), rgba_array)
    ...
    def test_plot_scatter_with_s(self):
        ...
        tm.assert_numpy_array_equal(
            df["c"].values,
            right=ax.collections[0].get_sizes())
    ...
    def test_hist_weights(self, weights):
        ...
        df = DataFrame(
            dict(zip(["A", "B"], np.random.default_rng(
                2).standard_normal((2, 100))))
        )
        ...
        expected_patch_height = [0.1 * patch.get_height()
    ...
    def _check_box_coord(
        ...
        if expected_y is not None:
            tm.assert_numpy_array_equal(
                result_y, expected_y, check_dtype=False)
        if expected_h is not None:
            tm.assert_numpy_array_equal(
                result_height, expected_h, check_dtype=False)
        if expected_x is not None:
            tm.assert_numpy_array_equal(
                result_x, expected_x, check_dtype=False)
        if expected_w is not None:
            tm.assert_numpy_array_equal(
                result_width, expected_w, check_dtype=False)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            {
                "A": np.repeat(
                    np.array([np.nan, 1, 2, 3, 4, 5]), np.array(
                        [3, 10, 9, 8, 7, 6])
                ),
                "B": np.repeat(
                    np.array([1, np.nan, 2, 3, 4, 5]), np.array(
                        [8, 3, 8, 8, 8, 8])
                ),
                "C": np.repeat(
                    np.array([1, 2, 3, np.nan, 4, 5]), np.array(
                        [6, 7, 8, 3, 9, 10])
                ),
            },
        ],
    ...
    def test_specified_props_kwd_plot_box(self, props, expected):
        # GH 30346
        df = DataFrame({k: np.random.default_rng(2).random(100)
                       for k in "ABC"})
        kwd = {props: {"color": "C1"}}
    ...
    def test_unordered_ts(self):
        ...
        assert pd.to_datetime(
            xlabels, format="%Y-%m-%d").is_monotonic_increasing
    ...
    def test_errorbar_scatter_color(self):
        def _check_errorbar_color(containers, expected, has_err="has_xerr"):
            lines = []
            errs = next(
                c.lines for c in ax.containers if getattr(
                    c, has_err, False))
            for el in errs:
                if is_list_like(el):
                    lines.extend(el)
            ...
            _check_colors(
                err_lines, linecolors=np.array(
                    [expected] * len(err_lines)))
    ...
    def test_df_gridspec_patterns_vert_horiz(self):
        ...
        with tm.assert_produces_warning(UserWarning):
            axes = df.plot(
                subplots=True, ax=[
                    ax1, ax2], sharex=True, sharey=True)
    ...
    def test_group_subplot_multiindex_notimplemented(self):
        df = DataFrame(
            np.eye(2), columns=MultiIndex.from_tuples([(0, 1), (1, 2)]))
    ...
    def test_xlabel_ylabel_dataframe_single_plot(
        ...
        # old xlabel will be overridden and assigned ylabel will be used as
        # ylabel
