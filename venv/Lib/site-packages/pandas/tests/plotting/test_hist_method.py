...
class TestSeriesPlots:
    ...
    def test_hist_layout_with_by(
            self, hist_df, by, layout, axes_num, res_layout):
        df = hist_df
...
class TestDataFramePlots:
    ...
    @pytest.mark.slow
    @pytest.mark.parametrize(
        "kwargs", [{"sharex": True, "sharey": True}, {"figsize": (8, 10)}, {
            "bins": 5}]
    )
    ...
    def test_hist_with_nans_and_weights(self):
        ...
        rects = [
            x for x in ax0.get_children() if isinstance(
                x, mpl_patches.Rectangle)]
...
class TestDataFrameGroupByPlots:
    ...
    def test_grouped_hist_layout_error(
            self, hist_df, msg, plot_col, by_col, layout):
        df = hist_df
        with pytest.raises(ValueError, match=msg):
    ...
    def test_grouped_hist_layout_figsize(
            self, hist_df, layout, check_layout, figsize):
        df = hist_df
        axes = df.hist(
            column="height",
            by=df.category,
    ...
    def test_axis_share_xy(self, hist_df):
        df = hist_df
        ax1, ax2 = df.hist(column="height", by=df.gender,
