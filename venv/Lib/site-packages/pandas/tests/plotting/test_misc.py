...
class TestDataFramePlots:
    ...
    def test_dictionary_color(self, kind):
        ...
        df1 = DataFrame(
            np.random.default_rng(2).random(
                (2, 2)), columns=data_files)
        dic_color = {"b": (0.3, 0.7, 0.7), "a": (0.5, 0.24, 0.6)}

        ax = df1.plot(kind=kind, color=dic_color)
        if kind == "bar":
            colors = [rect.get_facecolor()[0:-1]
                      for rect in ax.get_children()[0:3:2]]
        else:
            colors = [rect.get_color() for rect in ax.get_lines()[0:2]]
        assert all(color == expected[index]
    ...
    def test_barh_plot_labels_mixed_integer_string(self):
        ...
        df = DataFrame([{"word": 1, "value": 0}, {
                       "word": "knowledge", "value": 2}])
    ...
    def test_has_externally_shared_axis_invalid_compare_axis(self):
        ...
        # Check that an invalid compare_axis value triggers the expected
        # exception
