...
class TestiLocBaseIndependent:
    ...
    def test_iloc_setitem_fullcol_categorical(
            self, indexer, key, using_array_manager):
        ...
        indexer(df)[key, 0] = cat
        expected = DataFrame(
            {0: Series(cat.astype(object), dtype=object), 1: range(3)})
        tm.assert_frame_equal(df, expected)

    @pytest.mark.parametrize("box", [array, Series])
    def test_iloc_setitem_ea_inplace(
            self, frame_or_series, box, using_copy_on_write):
    ...
    def test_iloc_exceeds_bounds(self):
        ...
        tm.assert_frame_equal(
            dfl.iloc[:, 2:3],
            DataFrame(
                index=dfl.index, columns=Index(
                    [], dtype=dfl.columns.dtype)),
        )
    ...
    def test_iloc_getitem_dups(self):
        ...
        result = df.iloc[0, :]
        expected = Series([np.nan, 1, 3, 3], index=[
                          "A", "B", "A", "B"], name=0)
        tm.assert_series_equal(result, expected)
    ...
    def test_iloc_setitem_dups(self):
        ...
        # reversed x 2
        df.iloc[[1, 0], [0, 1]] = df.iloc[[
            1, 0], [0, 1]].reset_index(drop=True)
        df.iloc[[1, 0], [0, 1]] = df.iloc[[
            1, 0], [0, 1]].reset_index(drop=True)
        tm.assert_frame_equal(df, expected)
    ...
    @pytest.mark.parametrize("indexer",
                             [[0], slice(None, 1, None), np.array([0])])
    ...
    def test_iloc_setitem_categorical_updates_inplace(self):
        # Mixed dtype ensures we go through take_split_path in
    ...
    def test_iloc_getitem_read_only_values(self, indexer):
        ...
        tm.assert_frame_equal(
            indexer(rw_df)[[1, 2, 3]], indexer(ro_df)[[1, 2, 3]])
    ...
    def test_iloc_assign_series_to_df_cell(self):
        ...
        expected = DataFrame(
            {"a": [Series([1, 2, 3])]}, columns=["a"], index=[0])
        tm.assert_frame_equal(df, expected)
    ...
    def test_iloc_setitem_2d_ndarray_into_ea_block(self):
        ...
        expected = DataFrame(
            {"status": ["a", "a", "c"]}, dtype=df["status"].dtype)
        tm.assert_frame_equal(df, expected)
    ...
    def test_iloc_setitem_multicolumn_to_datetime(self):
        # GH#20511
        df = DataFrame(
            {"A": ["2022-01-01", "2022-01-02"], "B": ["2021", "2022"]})
