...
class TestSetitemCoercion(CoercionBase):
    ...
    # tests.series.indexing.test_setitem in SetitemCastingEquivalents
    # subclasses.
    klasses: list[str] = []

    def test_setitem_series_no_coercion_from_values_list(self):
        # GH35865 - int casted to str when internally calling
        # np.array(ser.values)
        ser = pd.Series(["a", 1])
        ser[:] = list(ser.values)
...
class TestInsertIndexCoercion(CoercionBase):
    ...
    def _assert_insert_conversion(
            self, original, value, expected, expected_dtype):
    ...
    def test_insert_float_index(
        ...
        if np_version_gt2 and dtype == "float32" and coerced_val == 1.1:
            ...
            # the expected dtype will be float32 if the original dtype was
            # float32
...
class TestWhereCoercion(CoercionBase):
    ...
    def test_where_float64(self, index_or_series,
                           fill_val, exp_dtype, request):
        klass = index_or_series
...
class TestReplaceSeriesCoercion(CoercionBase):
    ...
    # Expected needs adjustment for the infer string option, seems to work as
    # expecetd
    @pytest.mark.skipif(using_pyarrow_string_dtype(),
                        reason="TODO: test is to complex")
    ...
    def test_replace_series_datetime_datetime(
            self, how, to_key, from_key, replacer):
        index = pd.Index([3, 4], name="xyz")
        obj = pd.Series(self.rep[from_key], index=index, name="yyy")
        assert obj.dtype == from_key
