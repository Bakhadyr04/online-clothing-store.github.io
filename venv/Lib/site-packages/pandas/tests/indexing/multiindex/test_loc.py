...
def frame_random_data_integer_multi_index():
    ...
    return DataFrame(np.random.default_rng(
        2).standard_normal((6, 2)), index=index)
...
class TestMultiIndexLoc:
    ...
    def test_loc_getitem_general(self, any_real_numpy_dtype):
        ...
        index = MultiIndex.from_arrays(
            [col_arr, year_arr], names=["col", "num"])
    ...
    @pytest.mark.parametrize("key, pos",
                             [([2, 4], [0, 1]), ([2], []), ([2, 3], [])])
    ...
    def test_loc_getitem_nested_indexer(self, indexer_type_1, indexer_type_2):
        ...
        # any list-like objects (see `is_list_like` (`pandas.api.types`)) or
        # slices
...
def test_loc_datetime_mask_slicing():
    ...
    result = df.loc[(
        dt_idx[0], (df.index.get_level_values(1) > "2017-05-04")), "C1"]
    expected = Series(
        [3],
        name="C1",
...
def test_loc_getitem_drops_levels_for_one_row_dataframe():
    # GH#10521 "x" and "z" are both scalar indexing, so those levels are
    # dropped
