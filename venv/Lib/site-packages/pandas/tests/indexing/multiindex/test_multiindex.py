...
class TestMultiIndexBasic:
    ...
    def test_indexing_over_hashtable_size_cutoff(self, monkeypatch, offset):
        ...
        with monkeypatch.context():
            ...
            s = Series(
                np.arange(n), MultiIndex.from_arrays(
                    (["a"] * n, np.arange(n))))
    ...
    def test_nested_tuples_duplicates(self):
        ...
        df = DataFrame(
            {"c1": [1, 2, 3], "c2": [np.nan, np.nan, np.nan]}, index=mi)

        expected = DataFrame(
            {"c1": df["c1"], "c2": [1.0, 1.0, np.nan]}, index=mi)
    ...
    def test_rename_multiindex_with_duplicates(self):
        ...
        mi2 = MultiIndex.from_tuples(
            [("Apple", "cat"), ("B", "cat"), ("B", "cat")])
    ...
    def test_series_align_multiindex_with_nan_overlap_only(self):
        ...
        mi = MultiIndex.from_arrays(
            [[81.0, 82.0, np.nan], [np.nan, np.nan, np.nan]])
    ...
    def test_multiindex_repeated_keys(self):
        ...
        tm.assert_series_equal(
            ...
            Series([1, 1, 2, 2], MultiIndex.from_arrays(
                [["a", "a", "b", "b"]])),
        )
