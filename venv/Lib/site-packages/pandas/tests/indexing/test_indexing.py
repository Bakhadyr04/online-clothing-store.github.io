...
class TestFancy:
    ...
    def test_setitem_dtype_upcast(self):
        ...
        with tm.assert_produces_warning(
            ...
            df.loc[0, "c"] = "foo"
        expected = DataFrame(
            {"a": [1, 3], "b": [np.nan, 2], "c": Series(
                ["foo", np.nan], dtype=object)}
        )
...
class TestMisc:
    ...
    @pytest.mark.parametrize(
        "idx", [_mklbl("A", 20), np.arange(20) + 100,
    ...
    def test_slice_with_zero_step_raises(
            self, index, indexer_sl, frame_or_series):
    ...
    def test_loc_setitem_indexing_assignment_dict_already_exists(self):
        ...
        df.loc[5] = rhs
        expected = DataFrame(
            {"x": [1, 2, 9], "y": [2.0, 2.0, 99.0]}, index=index)
        tm.assert_frame_equal(df, expected)
...
class TestDataframeNoneCoercion:
    ...
    def test_coercion_with_setitem_and_dataframe(self, expected):
        ...
        start_dataframe = DataFrame({"foo": start_data})
        start_dataframe[start_dataframe["foo"]
                        == start_dataframe["foo"][0]] = None
    ...
    def test_none_coercion_loc_and_dataframe(self, expected):
        ...
        start_dataframe = DataFrame({"foo": start_data})
        start_dataframe.loc[start_dataframe["foo"]
                            == start_dataframe["foo"][0]] = None
...
class TestDatetimelikeCoercion:
    ...
    @pytest.mark.parametrize("box",
                             [list, np.array, pd.array, pd.Categorical, Index])
    @pytest.mark.parametrize(
        "key", [[0, 1], slice(0, 2), np.array([True, True, False])]
    )
    def test_setitem_dt64_string_values(
            self, tz_naive_fixture, indexer_sli, key, box):
...
def test_extension_array_cross_section():
    ...
    expected = Series(pd.array([1, 3], dtype="Int64"),
                      index=["A", "B"], name="a")
    result = df.loc["a"]
    tm.assert_series_equal(result, expected)
...
def test_scalar_setitem_with_nested_value(value):
    ...
    df = DataFrame(
        {"A": [1, 2, 3], "B": np.array([1, "a", "b"], dtype=object)})
...
def test_scalar_setitem_with_nested_value_length1(value):
    ...
    # For numeric data, assigning length-1 array to scalar position gets
    # unpacked
    df = DataFrame({"A": [1, 2, 3]})
    df.loc[0, "B"] = value
