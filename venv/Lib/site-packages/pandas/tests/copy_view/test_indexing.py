...
def backend(request):
    if request.param == "numpy":
        ...
        def make_dataframe(*args, **kwargs):
            ...
            # copy final result to ensure we start with a fully self-owning
            # DataFrame
...
def test_subset_set_with_mask(
    ...
    # Case: setting values with a mask on a viewing subset: subset[mask] =
    # value
    _, DataFrame, _ = backend
    df = DataFrame({"a": [1, 2, 3, 4], "b": [4, 5, 6, 7],
                   "c": [0.1, 0.2, 0.3, 0.4]})
...
def test_subset_chained_single_block_row(
    ...
    # not parametrizing this for dtype backend, since this explicitly tests
    # single block
...
def test_series_getitem_ellipsis(using_copy_on_write, warn_copy_on_write):
    # Case: taking a view of a Series using Ellipsis + afterwards modifying
    # the subset
    s = Series([1, 2, 3])
    s_orig = s.copy()
...
def test_del_series(backend):
    ...
    # modifying s2 doesn't need copy on write (due to `del`, s2 is backed by
    # new array)
    values = s2.values
    s2.loc["b"] = 100
    assert values[0] == 100
