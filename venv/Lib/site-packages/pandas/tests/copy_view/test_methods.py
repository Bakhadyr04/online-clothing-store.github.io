...
def test_methods_series_copy_keyword(
        request, method, copy, using_copy_on_write):
    index = None
    if "to_timestamp" in request.node.callspec.id:
        index = period_range("2012-01-01", freq="D", periods=3)
    elif "to_period" in request.node.callspec.id:
...
def test_transpose_copy_keyword(
        using_copy_on_write, copy, using_array_manager):
    df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]})
...
def test_to_frame(using_copy_on_write, warn_copy_on_write):
    ...
    if using_copy_on_write:
        ...
        # but currently select_dtypes() actually returns a view -> mutates
        # parent
...
@pytest.mark.parametrize("obj",
                         [Series([1, 2], name="a"), DataFrame({"a": [1, 2]})])
def test_to_timestamp(using_copy_on_write, obj):
    obj.index = Index([Period("2012-1-1", freq="D"),
                      Period("2012-1-2", freq="D")])
...
def test_head_tail(method, using_copy_on_write, warn_copy_on_write):
    ...
    if using_copy_on_write:
        ...
        # without CoW enabled, head and tail return views. Mutating df2 also
        # mutates df.
        assert np.shares_memory(get_array(df2, "b"), get_array(df, "b"))
        with tm.assert_cow_warning(warn_copy_on_write):
            df2.iloc[0, 0] = 1
...
def test_sort_values_inplace(
        using_copy_on_write, obj, kwargs, warn_copy_on_write):
    obj_orig = obj.copy()
    view = obj[:]
...
@pytest.mark.parametrize("obj",
                         [Series([1, 2, 3]), DataFrame({"a": [1, 2, 3]})])
...
def test_putmask_dont_copy_some_blocks(
    ...
    if using_copy_on_write:
        ...
        # Without CoW the original will be modified, the other case upcasts,
        # e.g. copy
        assert np.shares_memory(get_array(view, "a"), get_array(df, "a"))
        assert np.shares_memory(get_array(view, "c"), get_array(df, "c"))
        assert view.iloc[0, 0] == 5
...
def test_where_mask_noop_on_single_column(
        using_copy_on_write, dtype, val, func):
...
def test_apply_modify_row(using_copy_on_write, warn_copy_on_write):
    ...
    # function mutates the row object (which needs to trigger CoW if row is a
    # view)
