    ...
    if using_copy_on_write:
        if (df_replaced["b"] == df["b"]).all():
            assert np.shares_memory(
                get_array(
                    df_replaced, "b"), get_array(
                    df, "b"))
        assert np.shares_memory(
            get_array(
                df_replaced, "c"), get_array(
                df, "c"))
...
def test_replace_coerce_single_column(
        using_copy_on_write, using_array_manager):
...
def test_replace_inplace_reference(
        using_copy_on_write, to_replace, warn_copy_on_write):
    df = DataFrame({"a": [1.5, 2, 3]})
    arr_a = get_array(df, "a")
    view = df[:]
    with tm.assert_cow_warning(warn_copy_on_write):
...
def test_replace_categorical_inplace_reference(
        using_copy_on_write, val, to_replace):
