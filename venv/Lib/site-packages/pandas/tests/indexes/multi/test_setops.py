...
def test_difference_keep_ea_dtypes(any_numeric_ea_dtype, val):
    ...
    expected = MultiIndex.from_arrays(
        [Series([1], dtype=any_numeric_ea_dtype), [2]])
    tm.assert_index_equal(result, expected)
...
def test_intersection_with_missing_values_on_both_sides(nulls_fixture):
    ...
    mi1 = MultiIndex.from_arrays(
        [[3, nulls_fixture, 4, nulls_fixture], [1, 2, 4, 2]])
...
def test_union_nan_got_duplicated(dtype, sort):
    # GH#38977, GH#49010
    mi1 = MultiIndex.from_arrays(
        [pd.array([1.0, np.nan], dtype=dtype), [2, 3]])
    mi2 = MultiIndex.from_arrays(
        [pd.array([1.0, np.nan, 3.0], dtype=dtype), [2, 3, 4]])
...
def test_intersection_keep_ea_dtypes(val, any_numeric_ea_dtype):
    ...
    expected = MultiIndex.from_arrays(
        [Series([2], dtype=any_numeric_ea_dtype), [1]])
    tm.assert_index_equal(result, expected)
...
def test_union_with_na_when_constructing_dataframe():
    ...
    series2 = Series(
        (10, 20), index=MultiIndex.from_tuples(
            ((None, None), ("a", "b"))))
    result = DataFrame([series1, series2])
    expected = DataFrame(
        {(np.nan, np.nan): [1.0, 10.0], ("a", "b"): [np.nan, 20.0]})
