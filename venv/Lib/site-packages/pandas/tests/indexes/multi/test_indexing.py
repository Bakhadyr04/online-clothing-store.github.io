...
class TestGetIndexer:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ([[1, np.nan, 2], [3, 4, 5]], [(np.nan, 4)],
             np.array([1], dtype=np.intp)),
            ([[1, 2, 3], [np.nan, 4, 5]], [(1, np.nan)],
    ...
    def test_get_indexer_three_or_more_levels(self):
        ...
        expected = np.array([0, 4, 5, 6, 6, 6, -1],
                            dtype=indexer_backfilled.dtype)
        ...
        expected = np.array([-1, 3, 5, 5, 5, 5, 11],
                            dtype=indexer_padded.dtype)
...
@pytest.mark.parametrize(
    "ind2",
    [[True, False, True, False, False], Index(
...
class TestGetLoc:
    ...
    def test_get_loc_cast_bool(self, dtype):
        ...
        levels = [Index([False, True], dtype=dtype),
        ...
        if dtype is bool:
            ...
            # We use python object comparisons, which treat 0 == False and 1 ==
            # True
            assert idx.get_loc((0, 1)) == 1
            assert idx.get_loc((1, 0)) == 2
