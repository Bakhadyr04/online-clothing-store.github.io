...
class TestWhere:
    def test_where_doesnt_retain_freq(self):
        ...
        expected = DatetimeIndex(
            [dti[0], dti[1], dti[0]], freq=None, name="idx")
...
class TestTake:
    ...
    def test_take_nan_first_datetime(self):
        index = DatetimeIndex(
            [pd.NaT, Timestamp("20130101"), Timestamp("20130102")])
...
class TestGetLoc:
    ...
    def test_get_loc_time_obj2(self, monkeypatch, offset):
        ...
        with monkeypatch.context():
            ...
            ts = pd.Series(np.random.default_rng(
                2).standard_normal(n), index=idx)
    ...
    def test_get_loc_time_nat(self):
        # GH#35114
        # Case where key's total microseconds happens to match iNaT % 1e6 //
        # 1000
    ...
    @pytest.mark.parametrize("key",
                             [pd.Timedelta(0), pd.Timedelta(1), timedelta(0)])
...
class TestGetIndexer:
    ...
    def test_get_indexer(self):
        ...
        target = idx[0] + \
            pd.to_timedelta(["-1 hour", "12 hours", "1 day 1 hour"])
