...
class TestGetIndexer:
    ...
    def test_get_indexer_requires_unique(self):
        ...
        for n in [1, 2, 5, len(ci)]:
            finder = oidx[np.random.default_rng(
                2).integers(0, len(ci), size=n)]
    ...
    def test_get_indexer_same_categories_same_order(self):
        ...
        result = ci.get_indexer(CategoricalIndex(
            ["b", "b"], categories=["a", "b"]))
...
class TestWhere:
    def test_where(self, listlike_box):
        ...
        expected = CategoricalIndex([np.nan] +
                                    i[1:].tolist(), categories=i.categories)
        result = i.where(klass(cond))
        tm.assert_index_equal(result, expected)
