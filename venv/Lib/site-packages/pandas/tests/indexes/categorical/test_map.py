...
def test_map_str(data, categories, ordered):
    # GH 31202 - override base class since we want to maintain
    # categorical/ordered
...
def test_map():
    ...
    result = ci.map(f)
    exp = CategoricalIndex([10, 20, 10, 20, 30], categories=[
                           20, 10, 30], ordered=False)
    tm.assert_index_equal(result, exp)
...
def test_map_with_dict_or_series():
    ...
    expected = CategoricalIndex(
        new_values, name="XXX", categories=[
            3.0, 2, "one"])
