...
class ConstructorTests:
    ...
    def test_constructor(
            self, constructor, breaks_and_expected_subtype, closed, name):
        ...
        tm.assert_index_equal(result.left, Index(
            breaks[:-1], dtype=expected_subtype))
        tm.assert_index_equal(result.right, Index(
            breaks[1:], dtype=expected_subtype))
    ...
    def test_constructor_pass_closed(self, constructor, breaks):
        ...
        for dtype in (iv_dtype, str(iv_dtype)):
            with tm.assert_produces_warning(None):
                result = constructor(
                    dtype=dtype, closed="left", **result_kwargs)
...
class TestFromTuples(ConstructorTests):
    ...
    def get_kwargs_from_breaks(self, breaks, closed="right"):
        ...
        if is_unsigned_integer_dtype(breaks):
            pytest.skip(
                f"{breaks.dtype} not relevant IntervalIndex.from_tuples tests")
...
class TestClassConstructors(ConstructorTests):
    ...
    def klass(self, request):
        # We use a separate fixture here to include Index.__new__ with dtype
        # kwarg
        return request.param
    ...
    def get_kwargs_from_breaks(self, breaks, closed="right"):
        ...
        if is_unsigned_integer_dtype(breaks):
            pytest.skip(
                f"{breaks.dtype} not relevant for class constructor tests")
...
@pytest.mark.parametrize(
    "dtype",
    ["Float64", pytest.param(
        "float64[pyarrow]", marks=td.skip_if_no("pyarrow"))],
)
def test_ea_dtype(dtype):
    ...
    expected = IntervalIndex.from_tuples(
        bins, closed="left").astype(interval_dtype)
    tm.assert_index_equal(result, expected)
