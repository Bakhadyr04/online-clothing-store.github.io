...
class TestTimedelta64ArrayLikeComparisons:
    ...
    def test_compare_timedelta64_zerodim(self, box_with_array):
        ...
        box = box_with_array
        xbox = box_with_array if box_with_array not in [
            Index, pd.array] else np.ndarray
...
class TestTimedelta64ArithmeticUnsorted:
    ...
    def test_ufunc_coercions(self):
        ...
        for result in [idx * 2, np.multiply(idx, 2)]:
            ...
            exp = TimedeltaIndex(
                ["4h", "8h", "12h", "16h", "20h"], freq="4h", name="x")
        ...
        idx = TimedeltaIndex(
            ["-2h", "-1h", "0h", "1h", "2h"], freq="h", name="x")
        for result in [abs(idx), np.absolute(idx)]:
            assert isinstance(result, TimedeltaIndex)
            exp = TimedeltaIndex(
                ["2h", "1h", "0h", "1h", "2h"], freq=None, name="x")
            tm.assert_index_equal(result, exp)
...
class TestTimedeltaArraylikeAddSubOps:
    ...
    def test_operators_timedelta64(self):
        ...
        expected = Series([timedelta(days=4017 + i)
                          for i in range(3)], name="A")
        tm.assert_series_equal(result, expected)
        assert result.dtype == "m8[ns]"
    ...
    def test_timedelta64_ops_nat(self):
        ...
        tm.assert_series_equal(timedelta_series *
                               1.5, Series([NaT, Timedelta("1.5s")]))
        tm.assert_series_equal(1.5 * timedelta_series,
                               Series([NaT, Timedelta("1.5s")]))

        tm.assert_series_equal(
            timedelta_series * np.nan,
            nat_series_dtype_timedelta)
        tm.assert_series_equal(
            np.nan * timedelta_series,
            nat_series_dtype_timedelta)

        # division
        tm.assert_series_equal(timedelta_series /
                               2, Series([NaT, Timedelta("0.5s")]))
        tm.assert_series_equal(timedelta_series /
                               2.0, Series([NaT, Timedelta("0.5s")]))
        tm.assert_series_equal(
            timedelta_series / np.nan,
            nat_series_dtype_timedelta)
    ...
    def test_td64arr_addsub_numeric_scalar_invalid(
            self, box_with_array, other):
        # vector-like others are tested in
    ...
    def test_td64arr_add_sub_offset_index(self, names, box_with_array):
        ...
        tdi = TimedeltaIndex(
            ["1 days 00:00:00", "3 days 04:00:00"], name=names[0])
        ...
        expected_sub = tm.box_expected(
            expected_sub, box).astype(
            object, copy=False)
    ...
    def test_td64arr_add_sub_offset_array(self, box_with_array):
        ...
        expected_sub = tm.box_expected(
            expected_sub, box_with_array).astype(object)
        with tm.assert_produces_warning(PerformanceWarning):
            res_sub = tdi - other
        tm.assert_equal(res_sub, expected_sub)
    ...
    def test_td64arr_with_offset_series(self, names, box_with_array):
        ...
        tdi = TimedeltaIndex(
            ["1 days 00:00:00", "3 days 04:00:00"], name=names[0])
        other = Series(
            [offsets.Hour(n=1), offsets.Minute(n=-2)], name=names[1])
    ...
    def test_td64arr_add_sub_object_array(self, box_with_array):
        ...
        other = np.array(
            [Timedelta(days=1), offsets.Day(2), Timestamp("2000-01-04")])
...
class TestTimedeltaArraylikeMulDivOps:
    ...
    def test_td64arr_div_td64_scalar(self, m, unit, box_with_array):
        ...
        # reverse op
        expected = Series(
            [Timedelta(np.timedelta64(m, unit)) / x for x in flat])
        expected = tm.box_expected(expected, xbox)
        result = np.timedelta64(m, unit) / ser
        tm.assert_equal(result, expected)

    def test_td64arr_div_tdlike_scalar_with_nat(
            self, two_hours, box_with_array):
        box = box_with_array
        xbox = np.ndarray if box is pd.array else box
    ...
    def test_td64_div_object_mixed_result(self, box_with_array):
        ...
        expected = Index([1.0, np.timedelta64("NaT", "ns"),
                         orig[0], 1.5], dtype=object)
        ...
        expected = Index(
            [1, np.timedelta64("NaT", "ns"), orig[0], 1], dtype=object)
        expected = tm.box_expected(expected, box_with_array, transpose=False)
        if isinstance(expected, NumpyExtensionArray):
            expected = expected.to_numpy()
        tm.assert_equal(res, expected)
