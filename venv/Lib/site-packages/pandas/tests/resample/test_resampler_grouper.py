...
def test_groupby_resample_on_api_with_getitem():
    # GH 17813
    df = DataFrame(
        {"id": list("aabbb"), "date": date_range(
            "1-1-2016", periods=5), "data": 1}
    )
...
def test_apply_with_mutated_index():
    ...
    # A case for series
    expected = df["col1"].groupby(
        pd.Grouper(
            freq="ME"),
        group_keys=False).apply(f)
...
def test_apply_non_naive_index():
    ...
    result = data.resample("D").apply(
        weighted_quantile, weights=weights, q=0.5)
    ind = date_range(
...
def test_resample_groupby_agg_listlike():
    ...
    df = DataFrame({"class": ["beta"], "value": [69]},
                   index=Index([ts], name="date"))
    resampled = df.groupby("class").resample("ME")["value"]
    result = resampled.agg(["sum", "size"])
    expected = DataFrame(
        [[69, 1]],
        index=pd.MultiIndex.from_tuples(
            [("beta", ts)], names=["class", "date"]),
        columns=["sum", "size"],
    )
    tm.assert_frame_equal(result, expected)
...
def test_empty(keys):
    ...
    msg = "DataFrameGroupBy.resample operated on the grouping columns"
    with tm.assert_produces_warning(DeprecationWarning, match=msg):
        result = df.groupby(keys).resample(
            rule=pd.to_timedelta("00:00:01")).mean()
    expected = (
...
def test_resample_no_columns():
    ...
    result = df.groupby([0, 0, 1]).resample(
        rule=pd.to_timedelta("06:00:00")).mean()
    ...
    tm.assert_frame_equal(
        result,
        expected,
        check_index_type=not is_platform_windows())
...
def test_groupby_resample_on_index_with_list_of_keys_multi_columns():
    ...
    result = df.groupby("group").resample(
        "2D")[["first_val", "second_val"]].mean()
    ...
    df = DataFrame(
        {
            "datetime": pd.to_datetime(
                ["20181101 1100", "20181101 1200",
                    "20181102 1300", "20181102 1400"]
            ),
            "group": ["A", "B", "A", "B"],
            "value": [1, 2, 3, 4],
        }
