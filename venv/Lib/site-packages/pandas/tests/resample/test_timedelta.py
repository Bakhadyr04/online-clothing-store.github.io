...
def test_resample_as_freq_with_subperiod():
    ...
    expected = DataFrame(
        data=expected_data, index=timedelta_range(
...
def test_resample_single_period_timedelta():
    s = Series(
        list(
            range(5)), index=timedelta_range(
            "1 day", freq="s", periods=5))
    result = s.resample("2s").sum()
    expected = Series([1, 5, 4], index=timedelta_range(
...
def test_resample_categorical_data_with_timedeltaindex():
    # GH #12169
    df = DataFrame({"Group_obj": "A"}, index=pd.to_timedelta(
