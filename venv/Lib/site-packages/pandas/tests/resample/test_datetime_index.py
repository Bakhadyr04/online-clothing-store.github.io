...
def simple_date_range_series():
    ...
    def _simple_date_range_series(start, end, freq="D"):
        ...
        return Series(np.random.default_rng(
...
def test_resample_ohlc_result_odd_period(unit):
    ...
    expected = df.reindex(index=date_range(
        rng[0], rng[-1], freq="B").as_unit(unit))
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    "origin", ["invalid_value", "epch", "startday",
...
def test_resample_origin_epoch_with_tz_day_vs_24h(unit):
    ...
    # check that we have the same behavior with epoch even if we are not
    # timezone aware
    ...
    tm.assert_series_equal(
        result_1, result_3.tz_localize(
            rng.tz), check_freq=False)
    tm.assert_series_equal(
        result_1, result_4.tz_localize(
            rng.tz), check_freq=False)

    # check that we have the similar results with two different timezones (+2H
    # and +5H)
...
def test_resample_dtype_coercion(unit):
    ...
    df = DataFrame(
        ...
        index=date_range(
            "2017-01-01",
            "2017-01-04").as_unit(unit))

    expected = df.astype("float64").resample(
        "h").mean()["a"].interpolate("cubic")
...
def test_resample_anchored_intraday(unit):
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        expected = df.resample(
            "ME", kind="period").mean().to_timestamp(
            how="end")
...
def test_resample_anchored_intraday2(unit):
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        expected = df.resample(
            "QE", kind="period").mean().to_timestamp(
            how="end")
    expected.index += Timedelta(1, "ns") - Timedelta(1, "D")
...
def test_resample_timegrouper2(dates, unit):
    ...
    df = DataFrame({"A": dates, "B": np.arange(
        len(dates)), "C": np.arange(len(dates))})
    result = df.set_index("A").resample("ME").count()
...
def test_resample_size(unit):
    ...
    val = np.bincount(bins, minlength=len(
        ix) + 1)[1:].astype("int64", copy=False)
...
def test_resample_dst_anchor(unit):
    # 5172
    dti = DatetimeIndex([datetime(2012, 11, 4, 23)],
                        tz="US/Eastern").as_unit(unit)
...
def test_resample_dst_anchor2(unit):
    ...
    result_d = rs_d.agg({"a": "min", "b": "max", "c": "count"})[
        ["a", "b", "c"]]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("19910905 06:00", "19920406 06:00", "h",
         "19910905 06:00", "19920406 07:00"),
...
def test_get_timestamp_range_edges(
        first, last, freq, exp_first, exp_last, unit):
    first = Period(first)
...
def test_resample_apply_product(duplicates, unit):
    ...
    expected = DataFrame(
        np.array([[0, 24], [60, 210], [336, 720],
                 [990, 1716]], dtype=np.int64),
        index=DatetimeIndex(
...
def test_resample_calendar_day_with_dst(
    ...
    # GH 35219
    ts = Series(
        1.0, date_range(first, last, freq=freq_in,
                        tz="Europe/Amsterdam").as_unit(unit)
    )
...
def test_long_rule_non_nano():
    ...
    ser = Series([1, 4, 2, 8, 5, 7, 1, 4, 2, 8,
                 5, 7, 1, 4, 2, 8, 5], index=idx)
    ...
    expected = Series([1.0, 3.0, 6.5, 4.0, 3.0, 6.5,
                      4.0, 3.0, 6.5], index=expected_idx)
    tm.assert_series_equal(result, expected)
