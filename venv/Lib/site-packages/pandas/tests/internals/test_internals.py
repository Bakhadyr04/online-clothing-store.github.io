...
def create_block(typestr, placement, item_shape=None,
                 num_offset=0, maker=new_block):
    """
    Supported typestr:
...
class TestBlock:
    ...
    def test_delete_datetimelike(self):
        # dont use np.delete on values, as that will coerce from DTA/TDA to
        # ndarray
...
class TestBlockManager:
    ...
    def test_set(self):
        ...
        tm.assert_numpy_array_equal(
            ...
            np.array(
                [0] * 3))
        tm.assert_numpy_array_equal(
            mgr.iget(1).internal_values(), np.array(
                ["bar"] * 3, dtype=np.object_)
        )
        tm.assert_numpy_array_equal(
            ...
            np.array(
                [2] * 3))
        tm.assert_numpy_array_equal(
            mgr.iget(3).internal_values(), np.array(
                ["foo"] * 3, dtype=np.object_)
        )
    ...
    def test_set_change_dtype(self, mgr):
        ...
        mgr2.iset(
            mgr2.items.get_loc("quux"), np.random.default_rng(
                2).standard_normal(N)
        )
    ...
    def test_copy(self, mgr):
        ...
        for blk, cp_blk in zip(mgr.blocks, cp.blocks):
            ...
            if isinstance(cpvals, DatetimeArray):
                assert (
                    lbase is None and rbase is None) or (
                    lbase is not rbase)
    ...
    @pytest.mark.parametrize("t", ["float16",
    ...
    def test_interleave(self):
        # self
        for dtype in ["f8", "i8", "object",
                      "bool", "complex", "M8[ns]", "m8[ns]"]:
    ...
    def test_consolidate_ordering_issues(self, mgr):
        ...
        tm.assert_numpy_array_equal(
            cons.blocks[0].mgr_locs.as_array, np.arange(
                len(cons.items), dtype=np.intp)
        )
    ...
    def test_get_numeric_data(self, using_copy_on_write):
        ...
        tm.assert_index_equal(numeric.items, Index(
    ...
    def test_iset_split_block(self):
        ...
        # First indexer currently does not have a block associated with it in
        # case
        tm.assert_numpy_array_equal(
            bm.blknos, np.array(
    ...
    def test_iset_split_block_values(self):
        ...
        tm.assert_numpy_array_equal(
            bm.blknos, np.array(
                [0, 2, 2, 1], dtype="int64" if IS64 else "int32")
        )
...
class TestIndexing:
    ...
    def test_take(self, mgr):
        ...
        for ax in range(mgr.ndim):
            ...
            assert_take_ok(
                mgr, ax, indexer=np.array(
                    list(range(mgr.shape[ax])), dtype=np.intp)
            )

            if mgr.shape[ax] >= 3:
                assert_take_ok(mgr, ax, indexer=np.array(
                    [0, 1, 2], dtype=np.intp))
                assert_take_ok(mgr, ax, indexer=np.array(
                    [-1, -2, -3], dtype=np.intp))
    ...
    def test_reindex_axis(self, fill_value, mgr):
        def assert_reindex_axis_is_ok(mgr, axis, new_labels, fill_value):
            ...
            reindexed = mgr.reindex_axis(
                new_labels, axis, fill_value=fill_value)
            tm.assert_numpy_array_equal(
        ...
        for ax in range(mgr.ndim):
            ...
            assert_reindex_axis_is_ok(mgr, ax, Index(
    ...
    def test_reindex_indexer(self, fill_value, mgr):
        def assert_reindex_indexer_is_ok(
                mgr, axis, new_labels, indexer, fill_value):
            mat = _as_array(mgr)
            reindexed_mat = algos.take_nd(
                mat, indexer, axis, fill_value=fill_value)
        ...
        for ax in range(mgr.ndim):
            ...
            assert_reindex_indexer_is_ok(
                mgr, ax, mgr.axes[ax][::-
                                      1], np.arange(mgr.shape[ax]), fill_value
            )
            assert_reindex_indexer_is_ok(
                mgr, ax, mgr.axes[ax], np.arange(mgr.shape[ax])[
                    ::-1], fill_value
            )
            assert_reindex_indexer_is_ok(
                mgr, ax, Index(["foo", "bar", "baz"]), np.array(
                    [0, 0, 0]), fill_value
            )
            assert_reindex_indexer_is_ok(
                mgr, ax, Index(["foo", "bar", "baz"]), np.array(
...
class TestCanHoldElement:
    ...
    def test_interval_can_hold_element(self, dtype, element):
        ...
        ii3 = IntervalIndex.from_breaks(
            [Timestamp(1), Timestamp(3), Timestamp(4)])
        elem = element(ii3)
        with tm.assert_produces_warning(FutureWarning):
            self.check_series_setitem(elem, ii, False)
        assert not blk._can_hold_element(elem)
        ...
        ii4 = IntervalIndex.from_breaks(
            [Timedelta(1), Timedelta(3), Timedelta(4)])
        elem = element(ii4)
        with tm.assert_produces_warning(FutureWarning):
            self.check_series_setitem(elem, ii, False)
        assert not blk._can_hold_element(elem)
