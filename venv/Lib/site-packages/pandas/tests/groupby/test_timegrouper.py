...
class TestGroupBy:
    def test_groupby_with_timegrouper(self):
        ...
        for df in [df_original, df_reordered]:
            ...
            # Cast to object to avoid implicit cast when setting entry to
            # "CarlCarlCarl"
    ...
    def test_timegrouper_with_reg_groups(self):
        ...
        for df in [df_original, df_sorted]:
            ...
            result = df.groupby([Grouper(freq="YE"), "Buyer"]).sum(
                numeric_only=True)
        ...
        for df in [df_original, df_sorted]:
            ...
            result = df.groupby([Grouper(freq="1ME"), "Buyer"]).sum(
                numeric_only=True)
            expected = DataFrame(
                {
                    "Buyer": "Carl Joe Mark".split(),
    ...
    def test_timegrouper_with_reg_groups_freq(self, freq):
        ...
        result1 = (
            df.sort_index().groupby([Grouper(freq=freq), "user_id"])[
                "whole_cost"].sum()
        )
        tm.assert_series_equal(result1, expected)

        result2 = df.groupby([Grouper(freq=freq), "user_id"])[
            "whole_cost"].sum()
    ...
    def test_timegrouper_apply_return_type_series(self):
        ...
        # Issue #11742
        df = DataFrame(
            {"date": ["10/10/2000", "11/10/2000"], "value": [10, 13]})
        df_dt = df.copy()
    ...
    def test_groupby_groups_datetimeindex2(self):
        ...
        df = DataFrame({"A": [5, 6, 7, 8, 9], "B": [
                       1, 2, 3, 4, 5]}, index=index)
        ...
        for date in dates:
            ...
            expected = DataFrame(
                data, columns=list("AB"), index=expected_index)
            tm.assert_frame_equal(result, expected)
    ...
    def test_groupby_groups_datetimeindex_tz(self):
        ...
        df = DataFrame(
            {"value1": np.arange(6, dtype="int64"), "value2": [
                1, 2, 3, 1, 2, 3]},
            index=didx,
        )
    ...
    def test_groupby_max_datetime64(self):
        ...
        expected = df.groupby("A")["A"].apply(
            lambda x: x.max()).astype("M8[s]")
    ...
    def test_datetime_count(self):
        df = DataFrame(
            {"a": [1, 2, 3] *
             2, "dates": date_range("now", periods=6, freq="min")}
        )
        result = df.groupby("a").dates.count()
        expected = Series([2, 2, 2], index=Index(
            [1, 2, 3], name="a"), name="dates")
        tm.assert_series_equal(result, expected)
    ...
    def test_groupby_apply_timegrouper_with_nat_apply_squeeze(
        ...
        dti = Index([Timestamp("2013-12-31")],
                    dtype=df["Date"].dtype, name="Date")
        expected = DataFrame(
            [[36, 6, 6, 10, 2]],
            index=dti,
