...
class TestCounting:
    ...
    @pytest.mark.parametrize(
        "datetimelike",
        [
            [Timestamp(f"2016-05-{i:02d} 20:09:25+00:00")
             for i in range(1, 4)],
            [Timestamp(f"2016-05-{i:02d} 20:09:25") for i in range(1, 4)],
            [Timestamp(f"2016-05-{i:02d} 20:09:25", tz="UTC")
             for i in range(1, 4)],
...
def test_count_object():
    ...
    df = DataFrame({"a": ["a", np.nan, np.nan] +
                   ["b"] * 3, "c": [2] * 3 + [3] * 3})
    result = df.groupby("c").a.count()
...
def test_count_uses_size_on_exception():
    ...
    df = DataFrame({"a": [RaisingObject()
                   for _ in range(4)], "grp": list("ab" * 2)})
    result = df.groupby("grp").count()
