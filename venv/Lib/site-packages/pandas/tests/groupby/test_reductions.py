...
def test_masked_kleene_logic(bool_agg_func, skipna, data):
    ...
    # there is verified in
    # test_reductions.py::test_any_all_boolean_kleene_logic
...
def test_masked_bool_aggs_skipna(
        bool_agg_func, dtype, skipna, frame_or_series):
    # GH#40585
...
def test_empty(frame_or_series, bool_agg_func):
    # GH 45231
    kwargs = {
        "columns": ["a"]} if frame_or_series is DataFrame else {
        "name": "a"}
...
def test_groupby_non_arithmetic_agg_int_like_precision(method, data):
    ...
    expected = DataFrame({"b": [expected_value]},
...
def test_max_min_non_numeric():
    # #2700
    aa = DataFrame({"nn": [11, 11, 22, 22], "ii": [
                   1, 2, 3, 4], "ss": 4 * ["mama"]})
...
def test_aggregate_numeric_object_dtype():
    ...
    df = DataFrame(
        {"key": ["A", "A", "B", "B"], "col1": list("abcd"), "col2": [
            np.nan] * 4},
    ).astype(object)
...
def test_aggregate_categorical_lost_index(func: str):
    # GH: 28641 groupby drops index, when grouping over categorical column
    # with min/max
...
def test_nunique():
    df = DataFrame({"A": list("abbacc"), "B": list(
        "abxacc"), "C": list("abbacx")})
...
def test_groupby_sum_timedelta_with_nat():
    ...
    expected = Series([td3, pd.NaT], dtype="m8[ns]",
                      name="b", index=expected.index)
    tm.assert_series_equal(res, expected)


@pytest.mark.parametrize(
    "dtype", ["int8", "int16", "int32",
              "int64", "float32", "float64", "uint64"]
)
@pytest.mark.parametrize(
    "method,data",
    [
