...
def test_transform():
    ...
    result = df.groupby(key).transform(
    ...
    expected = people.groupby(key, group_keys=False).apply(
...
def test_transform_axis_1(request, transformation_func):
    ...
    msg = "DataFrameGroupBy.fillna is deprecated"
    with tm.assert_produces_warning(warn, match=msg):
        expected = df.T.groupby([0, 0, 1]).transform(
            transformation_func, *args).T
...
def test_transform_axis_1_reducer(request, reduction_func):
    ...
    if reduction_func in (
        ...
        marker = pytest.mark.xfail(
            reason="transform incorrectly fails - GH#45986")
...
def test_transform_datetime_to_numeric():
    # GH 10972
    # convert dt to float
    df = DataFrame({"a": 1, "b": date_range(
        "2015-01-01", periods=2, freq="D")})
...
def test_transform_casting():
    ...
    result = df[["ID3", "DATETIME"]].groupby(
        "ID3").transform(lambda x: x.diff())
...
def test_dispatch_transform(tsframe):
    ...
    def fillit(x): return x.fillna(method="pad")
    with tm.assert_produces_warning(FutureWarning, match=msg):
...
def test_transform_transformation_func(transformation_func):
    ...
    if transformation_func == "cumcount":
        def test_op(x): return x.transform("cumcount")
        def mock_op(x): return Series(range(len(x)), x.index)
    elif transformation_func == "fillna":
        def test_op(x): return x.transform("fillna", value=0)
        def mock_op(x): return x.fillna(value=0)
    elif transformation_func == "ngroup":
        def test_op(x): return x.transform("ngroup")
        counter = -1
        ...
        def test_op(x): return x.transform(transformation_func)
        def mock_op(x): return getattr(x, transformation_func)()
...
def test_groupby_transform_with_nan_group():
    ...
    expected = Series([1.0, 1.0, 2.0, 3.0, np.nan, 6.0,
                      6.0, 9.0, 9.0, 9.0], name="a")
    tm.assert_series_equal(result, expected)
...
def test_cython_transform_series(op, args, targop):
    ...
    for data in [s, s_missing]:
        ...
        tm.assert_series_equal(
            expected, data.groupby(labels).transform(
                op, *args))
        tm.assert_series_equal(
            expected, getattr(
                data.groupby(labels), op)(
                *args))
...
@pytest.mark.parametrize(
    "gb_target",
    [
        {"by": np.random.default_rng(2).integers(
            0, 50, size=10).astype(float)},
        {"level": 0},
        {"by": "string"},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("ffill", 1, [np.nan, np.nan, "val1",
         "val1", np.nan, "val2", "val2", np.nan]),
        (
            "bfill",
            None,
            ["val1", "val1", "val1", "val2", "val2", "val2", np.nan, np.nan],
        ),
        ("bfill", 1, [np.nan, "val1", "val1",
         np.nan, "val2", "val2", np.nan, np.nan]),
    ],
)
...
def test_groupby_transform_rename():
    ...
    def demean_rename(x):
        ...
        result = result.rename(
            columns={
                c: f"{c}_demeaned" for c in result.columns})
...
def test_groupby_transform_timezone_column(func):
    ...
    expected = DataFrame([[ts, 1, ts]], columns=[
...
def test_categorical_and_not_categorical_key(observed):
    ...
    # DataFrame case
    result = df_with_categorical.groupby(
        ["A", "C"], observed=observed).transform("sum")
...
def test_null_group_str_reducer(request, dropna, reduction_func):
    ...
    df = DataFrame({"A": [1, 1, np.nan, np.nan],
                   "B": [1, 2, 2, 3]}, index=index)
...
def test_null_group_str_transformer(request, dropna, transformation_func):
    ...
    if dropna:
        ...
        buffer.append(DataFrame([[np.nan]], index=[
                      3], dtype=dtype, columns=["B"]))
    expected = concat(buffer)

    if transformation_func in ("cumcount", "ngroup"):
        # ngroup/cumcount always returns a Series as it counts the groups, not
        # values
...
def test_transform_aligns(func, frame_or_series,
                          expected_values, keys, keys_in_index):
...
def test_idxmin_idxmax_transform_args(how, skipna, numeric_only):
    ...
    df = DataFrame({"a": [1, 1, 1, 2], "b": [
                   3.0, 4.0, np.nan, 6.0], "c": list("abcd")})
    gb = df.groupby("a")
