...
def test_groupby_dropna_multi_index_dataframe_nan_in_two_groups(
    ...
    # GH 3729 this is to test that NA in different groups with different
    # representations
    df_list = [
        ["A", "B", 12, 12, 12],
        ["A", nulls_fixture, 12.3, 233.0, 12],
...
@pytest.mark.parametrize(
    "dropna, idx, outputs",
    [
        (True, ["A", "B"], {"b": [123.23, 13.0],
         "c": [123.0, 13.0], "d": [1.0, 13.0]}),
        (
            False,
            ["A", "B", np.nan],
            {
...
def test_groupby_dropna_normal_index_dataframe(dropna, idx, outputs):
    ...
    expected = pd.DataFrame(
        outputs, index=pd.Index(
            idx, dtype="object", name="a"))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            False,
            pd.Series([210.0, 350.0, 20.0], index=[
                      "a", "b", np.nan], name="Max Speed"),
        ),
    ],
)
...
def test_groupby_dataframe_slice_then_transform(dropna, index):
    ...
    df = pd.DataFrame({"A": [0, 0, 1, None], "B": [
                      1, 2, 3, None]}, index=index)
    gb = df.groupby("A", dropna=dropna)
...
@pytest.mark.parametrize("dropna, values",
...
def test_groupby_dropna_datetime_like_data(
    ...
    expected = pd.DataFrame(
        {"values": values}, index=pd.Index(indexes, name="dt"))
...
def test_groupby_apply_with_dropna_for_multi_index(
    ...
    with tm.assert_produces_warning(DeprecationWarning, match=msg):
        result = gb.apply(lambda grp: pd.DataFrame(
            {"values": range(len(grp))}))
...
def test_no_sort_keep_na(sequence_index, dtype, test_series, as_index):
    ...
    sequence = "".join(
        [{0: "x", 1: "y", 2: "z"}[
            sequence_index // (3**k) % 3] for k in range(4)]
    )
    ...
    if dtype == "category":
        ...
        index = pd.Index([uniques[label]
                         for label in summed], dtype=dtype, name="key")
...
def test_categorical_reducers(
        reduction_func, observed, sort, as_index, index_kind):
    # Ensure there is at least one null value by appending to the end
    values = np.append(np.random.default_rng(
        2).choice([1, 2, None], size=19), None)
    df = pd.DataFrame(
    ...
    if reduction_func in ("idxmax", "idxmin") and index_kind != "range":
        # expected was computed with a RangeIndex; need to translate to index
        # values
...
def test_categorical_head_tail(method, observed, sort, as_index):
    ...
    df = pd.DataFrame(
        {"x": pd.Categorical(values, categories=[
                             1, 2, 3]), "y": range(len(values))}
    )
    gb = df.groupby(
        "x",
        dropna=False,
    ...
    mask = (
        ...
        # flake8 doesn't like the vectorized check for None, thinks we should
        # use `is`
