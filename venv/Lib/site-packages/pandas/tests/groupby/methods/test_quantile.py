...
def test_quantile(interpolation, a_vals, b_vals, q, request):
    ...
    df = DataFrame({"key": ["a"] * len(a_vals) + ["b"]
                   * len(b_vals), "val": all_vals})
...
def test_quantile_array_no_sort():
    ...
    expected = DataFrame(
        {"A": [0.5, 1.0, 1.5, 1.0, 1.0, 1.0],
            "B": [3.5, 4.0, 4.5, 4.0, 4.0, 4.0]},
...
def test_groupby_quantile_with_arraylike_q_and_int_columns(
        frame_size, groupby, q):
    # GH30289
    nrow, ncol = frame_size
    df = DataFrame(np.array([ncol * [_ % 4]
                   for _ in range(nrow)]), columns=range(ncol))
...
def test_quantile_raises():
    df = DataFrame([["foo", "a"], ["foo", "b"], ["foo", "c"]],
                   columns=["key", "val"])
...
def test_groupby_quantile_NA_float(any_float_dtype):
    ...
    expected = pd.Series(
        [0.2] * 2,
        index=pd.MultiIndex.from_product(
            (exp_index, [0.5, 0.75]), names=["x", None]),
        name="y",
        dtype=expected_dtype,
    )
    tm.assert_series_equal(result, expected)
...
@pytest.mark.parametrize(
    "interpolation, val1, val2", [
        ("lower", 2, 2), ("higher", 2, 3), ("nearest", 2, 2)]
)
def test_groupby_quantile_all_na_group_masked(
    ...
    result = df.groupby("a").quantile(
        q=[0.5, 0.7], interpolation=interpolation)
    expected = DataFrame(
...
def test_groupby_quantile_all_na_group_masked_interp(
    ...
    result = df.groupby("a").quantile(
        q=[0.5, 0.75], interpolation=interpolation)
...
def test_timestamp_groupby_quantile(unit):
    ...
    mi = pd.MultiIndex.from_product(
        [dti[::99], [0.2, 0.8]], names=("timestamp", None))
    expected = DataFrame(
        [
...
def test_groupby_quantile_nonmulti_levels_order():
    ...
    expected_levels = pd.core.indexes.frozen.FrozenList(
        [["B", "A"], [0.2, 0.8]])
