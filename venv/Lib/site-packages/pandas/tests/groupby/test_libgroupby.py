...
def test_cython_group_mean_datetimelike():
    ...
    data = (
        np.array(
            [np.timedelta64(2, "ns"), np.timedelta64(
                4, "ns"), np.timedelta64("NaT")],
            dtype="m8[ns]",
        )[:, None]
        .view("int64")
        .astype("float64")
...
def test_cython_group_mean_not_datetimelike_but_has_NaT_values():
    ...
    tm.assert_numpy_array_equal(
        actual[:, 0], np.array(
            np.divide(np.add(data[0], data[1]), 2), dtype="float64")
    )
...
def test_cython_group_mean_Inf_at_begining_and_end():
    ...
    data = np.array(
        [[np.inf, 1.0], [1.0, 2.0], [2.0, 3.0], [
            3.0, 4.0], [4.0, 5.0], [5, np.inf]],
        dtype="float64",
    )
