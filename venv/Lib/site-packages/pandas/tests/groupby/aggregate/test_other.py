...
def test_agg_datetimes_mixed():
    ...
    data = [
        [
            row[0],
            (dt.datetime.strptime(row[1],
                                  "%Y-%m-%d").date() if row[1] else None),
            row[2],
        ]
        for row in data
    ]
...
def test_agg_dict_parameter_cast_result_dtypes():
    ...
    # count
    exp = Series([2, 2, 2, 2], index=Index(
        list("ABCD"), name="class"), name="time")
    tm.assert_series_equal(grouped.time.agg(len), exp)
    tm.assert_series_equal(grouped.time.size(), exp)

    exp = Series([0, 1, 1, 2], index=Index(
        list("ABCD"), name="class"), name="time")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda x: tuple(x), Series(
            [(1, 1, 1), (3, 4, 4)], index=[1, 3], name="C")),
        (lambda x: list(x), Series(
            [[1, 1, 1], [3, 4, 4]], index=[1, 3], name="C")),
    ],
)
...
def test_agg_category_nansum(observed):
    categories = ["a", "b", "c"]
    df = DataFrame(
        {"A": pd.Categorical(["a", "a", "b"], categories=categories), "B": [
            1, 2, 3]}
    )
...
def test_agg_list_like_func():
    # GH 18473
    df = DataFrame({"A": [str(x) for x in range(3)],
