...
class TestMergeMulti:
    ...
    def test_left_join_multi_index(self, sort, infer_string):
        with option_context("future.infer_string", infer_string):
            ...
            def bind_cols(df):
                def iord(a): return 0 if a != a else ord(a)
                def f(ts): return ts.map(iord) - ord("a")
                return f(df["1st"]) + f(df["3rd"]) * \
                    1e2 + df["2nd"].fillna(0) * 10
    ...
    def test_merge_multiple_cols_with_mixed_cols_index(self):
        ...
        df = DataFrame({"lev1": list("AAABBB"), "lev2": [
                       1, 2, 3, 1, 2, 3], "col": 0})
    ...
    def test_left_join_index_multi_match_multiindex(self):
        ...
        result = left.join(
            right, on=[
                "cola", "colb", "colc"], how="left", sort=True)
