...
class TestCrosstab:
    ...
    def test_crosstab_multiple(self, df):
        ...
        expected = expected.unstack("B").unstack(
            "C").fillna(0).astype(np.int64)
    ...
    def test_crosstab_margins(self):
        ...
        result = crosstab(a, [b, c], rownames=["a"],
                          colnames=("b", "c"), margins=True)
        ...
        exp_rows = pd.concat(
            [exp_rows, Series([len(df)], index=[("All", "")])])
        exp_rows.name = "All"
    ...
    def test_crosstab_margins_set_margin_name(self):
        ...
        exp_rows = df.groupby(["b", "c"]).size().astype("i8")
        exp_rows = pd.concat(
            [exp_rows, Series([len(df)], index=[("TOTAL", "")])])
        exp_rows.name = "TOTAL"
    ...
    def test_margin_dropna4(self):
        ...
        expected = DataFrame(
            [[1, 0, 1.0], [1, 3, 4.0], [0, 1, np.nan], [2, 4, 6.0]])
    ...
    def test_margin_dropna6(self):
        ...
        a = np.array(["foo", "foo", "foo", "bar",
                     "bar", "foo", "foo"], dtype=object)
        b = np.array(["one", "one", "two", "one", "two",
                     np.nan, "two"], dtype=object)
        c = np.array(
            ["dull", "dull", "dull", "dull", "dull", "shiny", "shiny"], dtype=object
        )
    ...
    def test_crosstab_normalize(self):
        # Issue 12578
        df = DataFrame(
            {"a": [1, 2, 2, 2, 2], "b": [3, 3, 4, 4, 4],
                "c": [1, 1, np.nan, 1, 1]}
        )

        rindex = Index([1, 2], name="a")
        cindex = Index([3, 4], name="b")
        full_normal = DataFrame([[0.2, 0], [0.2, 0.6]],
                                index=rindex, columns=cindex)
        row_normal = DataFrame([[1.0, 0], [0.25, 0.75]],
                               index=rindex, columns=cindex)
        col_normal = DataFrame([[0.5, 0], [0.5, 1.0]],
                               index=rindex, columns=cindex)
        ...
        tm.assert_frame_equal(
            crosstab(
                df.a, df.b, normalize=0), crosstab(
                df.a, df.b, normalize="index")
        )
        ...
        tm.assert_frame_equal(
            crosstab(df.a, df.b, normalize="index",
        ...
        tm.assert_frame_equal(
            crosstab(df.a, df.b, normalize=True,
    ...
    def test_crosstab_normalize_arrays(self):
        ...
        # Test arrays
        crosstab(
            [np.array([1, 1, 2, 2]), np.array(
                [1, 2, 1, 2])], np.array([1, 2, 1, 2])
        )
    ...
    def test_crosstab_with_categorial_columns(self):
        ...
        df["MODEL"] = df["MODEL"].astype(
            "category").cat.set_categories(categories)
    ...
    def test_crosstab_duplicate_names(self):
        ...
        expected = crosstab(
            s3, [
                s1, s2_bar]).rename_axis(
            columns=mapper, axis=1)
    ...
    def test_crosstab_unsorted_order(self):
        ...
        e_columns = MultiIndex.from_tuples(
    ...
    def test_margin_normalize(self):
        ...
        expected = DataFrame(
            [[0.5, 0.5], [0.5, 0.5], [0.666667, 0.333333],
                [0, 1], [0.444444, 0.555556]]
        )
    ...
    def test_margin_with_ordered_categorical_column(self):
        ...
        df["First"] = df["First"].cat.reorder_categories(
            customized_categories_order)
        ...
        expected_data = [[1, 1, 0, 2], [
            0, 1, 0, 1], [0, 1, 2, 3], [1, 3, 2, 6]]
        expected = DataFrame(
            expected_data, index=expected_index, columns=expected_columns
        )
        tm.assert_frame_equal(result, expected)
