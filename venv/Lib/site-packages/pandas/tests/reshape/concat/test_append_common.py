...
class TestConcatAppendCommon:
    ...
    def test_concatlike_common_period(self):
        ...
        exp = pd.PeriodIndex(
            ["2011-01", "2011-02", "2012-01", "2012-02"], freq="M")
    ...
    def test_concat_categorical_3elem_coercion(self):
        ...
        exp = Series([1, 2, np.nan, 2, 1, 2, 1, 2,
                     1, 2, np.nan], dtype="float")
        tm.assert_series_equal(pd.concat([s1, s2, s3], ignore_index=True), exp)
        tm.assert_series_equal(s1._append([s2, s3], ignore_index=True), exp)

        exp = Series([1, 2, 1, 2, np.nan, 1, 2,
                     np.nan, 2, 1, 2], dtype="float")
    ...
    def test_concat_categorical_ordered(self):
        ...
        exp = Series(Categorical(
            [1, 2, np.nan, 2, 1, 2, 1, 2, np.nan], ordered=True))
    ...
    def test_concat_categorical_coercion_nan(self):
        ...
        # some edge cases
        # category + not-category => not category
        s1 = Series(np.array([np.nan, np.nan],
                    dtype=np.float64), dtype="category")
        s2 = Series([np.nan, 1])
