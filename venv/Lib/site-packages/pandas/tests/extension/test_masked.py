...
def make_data():
    return list(range(1, 9)) + [pd.NA] + \
        list(range(10, 98)) + [pd.NA] + [99, 100]
...
def make_bool_data():
    return [True, False] * 4 + [np.nan] + \
        [True, False] * 44 + [np.nan] + [True, False]
...
class TestMaskedArrays(base.ExtensionTests):
    ...
    def _get_expected_exception(self, op_name, obj, other):
        ...
        if dtype.kind == "b":
            if op_name.strip("_").lstrip("r") in [
                    "pow", "truediv", "floordiv"]:
    ...
    def check_reduce(self, ser: pd.Series, op_name: str, skipna: bool):
        ...
        if op_name == "count":
            ...
            if not skipna and ser.isna().any() and op_name not in [
                    "any", "all"]:
                expected = pd.NA
