...
class TestArrowArray(base.ExtensionTests):
    ...
    def test_from_sequence_of_strings_pa_array(self, data, request):
        pa_dtype = data.dtype.pyarrow_dtype
        if pa.types.is_time64(pa_dtype) and pa_dtype.equals(
                "time64[ns]") and not PY311:
            request.applymarker(
                pytest.mark.xfail(
    ...
    def test_reduce_series_numeric(
            self, data, all_numeric_reductions, skipna, request):
        dtype = data.dtype
        pa_dtype = dtype.pyarrow_dtype
    ...
    def test_construct_from_string_own_name(self, dtype, request):
        ...
        if pa.types.is_string(pa_dtype):
            # We still support StringDtype('pyarrow') over
...
def test_quantile(data, interpolation, quantile, request):
    ...
    if pa.types.is_timestamp(pa_dtype) and interpolation not in [
            "lower", "higher"]:
...
def test_arrow_dtype_type(arrow_dtype, expected_type):
    # GH 51845
    # TODO: Redundant with test_getitem_scalar once arrow_dtype exists in data
    # fixture
...
@pytest.mark.parametrize("pa_type",
...
def test_to_numpy_temporal(pa_type, dtype):
    ...
    if dtype == object or (pa.types.is_timestamp(
            pa_type) and pa_type.tz is not None):
        if dtype == object:
            na = pd.NA
        else:
            na = pd.NaT
