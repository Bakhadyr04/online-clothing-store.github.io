...
def test_select_dtypes(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        # floats w/o NaN
        df = DataFrame(
            {"cols": range(11), "values": range(11)}, dtype="float64")
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        results = list(
            store.select_as_multiple(
                ["df1", "df2"], selector="df1", chunksize=2)
        )
        result = concat(results)
        tm.assert_frame_equal(expected, result)
...
def test_query_compare_column_type(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        for op in ["<", ">", "=="]:
            ...
            for v in [2.1, True, Timestamp(
                    "2014-01-01"), pd.Timedelta(1, "s")]:
                query = f"date {op} v"
...
def test_select_large_integer(tmp_path):
    ...
    with HDFStore(path) as s:
        ...
        result = s.select(
            "data", where="y==-9223372036854775801").get("y").get(0)
    expected = df["y"][0]
