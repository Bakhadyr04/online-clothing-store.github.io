...
class TestPandasContainer:
    ...
    def test_frame_non_unique_columns(self, orient, data):
        ...
        if orient == "values":
            ...
            if expected.iloc[:, 0].dtype == "datetime64[ns]":
                ...
                expected.isetitem(
                    0, expected.iloc[:, 0].astype(np.int64) // 1000000)
        elif orient == "split":
            expected = df
    ...
    def test_roundtrip_mixed(self, orient, convert_axes):
        ...
        expected = expected.assign(
            **expected.select_dtypes("number").astype(np.int64))
    ...
    def test_series_roundtrip_simple(
            self, orient, string_series, using_infer_string):
    ...
    def test_timedelta(self):
        def converter(x): return pd.to_timedelta(x, unit="ms")
    ...
    def test_mixed_timedelta_datetime(self):
        ...
        expected = DataFrame(
            {"a": [pd.Timedelta(td).as_unit("ns")._value,
                   ts.as_unit("ns")._value]}
        )
        data = StringIO(frame.to_json(date_unit="ns"))
    ...
    @pytest.mark.parametrize("as_object", [True, False])
    @pytest.mark.parametrize("timedelta_typ", [pd.Timedelta, timedelta])
    def test_timedelta_to_json_fractional_precision(
            self, as_object, timedelta_typ):
    ...
    @pytest.mark.xfail(reason="GH#13774 encoding kwarg not supported",
                       raises=TypeError)
    @pytest.mark.parametrize(
        "val",
        [
    ...
    @pytest.mark.xfail(using_pyarrow_string_dtype(),
                       reason="incorrect na conversion")
...
{spaces}}}
    ...
    def test_frame_int_overflow(self):
        ...
        encoded_json = json.dumps(
            [{"col": "31900441201190696999"}, {"col": "Text"}])
    ...
    def test_to_s3(self, s3_public_bucket, s3so):
        ...
        while True:
            if target_file in (
                    obj.key for obj in s3_public_bucket.objects.all()):
                break
            time.sleep(0.1)
            timeout -= 0.1
    ...
    def test_to_json_series_of_objects(self):
        ...
        # JSON keys should be all non-callable non-underscore attributes, see
        # GH-42768
    ...
    def test_json_uint64(self):
        ...
        df = DataFrame(
            data={
                "col1": [
                    13342205958987758245,
                    12388075603347835679]})
    ...
    def test_read_json_dtype_backend(
        ...
        if using_infer_string:
            string_array = ArrowStringArrayNumpySemantics(
                pa.array(["a", "b", "c"]))
            string_array_na = ArrowStringArrayNumpySemantics(
                pa.array(["a", "b", None]))
    ...
    def test_read_json_nullable_series(
            self, string_storage, dtype_backend, orient):
        # GH#50750
        pa = pytest.importorskip("pyarrow")
