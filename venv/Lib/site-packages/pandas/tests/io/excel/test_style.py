...
def test_styler_to_excel_unstyled(engine):
    ...
    with tm.ensure_clean(".xlsx") as path:
        ...
        with contextlib.closing(openpyxl.load_workbook(path)) as wb:
            for col1, col2 in zip(wb["dataframe"].columns,
                                  wb["unstyled"].columns):
...
def test_styler_to_excel_basic(engine, css, attrs, expected):
    ...
    with tm.ensure_clean(".xlsx") as path:
        ...
        with contextlib.closing(openpyxl.load_workbook(path)) as wb:
            ...
            u_cell, s_cell = wb["dataframe"].cell(
                2, 2), wb["styled"].cell(2, 2)
        for attr in attrs:
...
def test_styler_to_excel_basic_indexes(engine, css, attrs, expected):
    ...
    with tm.ensure_clean(".xlsx") as path:
        ...
        with contextlib.closing(openpyxl.load_workbook(path)) as wb:
            ...
            # test styled cell has expected styles
            ui_cell, si_cell = wb["null_styled"].cell(
                2, 1), wb["styled"].cell(2, 1)
            uc_cell, sc_cell = wb["null_styled"].cell(
                1, 2), wb["styled"].cell(1, 2)
        for attr in attrs:
            ui_cell, si_cell = getattr(
                ui_cell, attr, None), getattr(
                si_cell, attr)
            uc_cell, sc_cell = getattr(
                uc_cell, attr, None), getattr(
                sc_cell, attr)
