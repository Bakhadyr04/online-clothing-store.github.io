...
@pytest.fixture(
    params=[
        ...
        datetime.datetime.strptime(
            "2019-01-04T16:41:24+0200",
            "%Y-%m-%dT%H:%M:%S%z"),
        datetime.datetime.strptime(
            "2019-01-04T16:41:24+0215",
            "%Y-%m-%dT%H:%M:%S%z"),
        datetime.datetime.strptime(
            "2019-01-04T16:41:24-0200",
            "%Y-%m-%dT%H:%M:%S%z"),
...
class Base:
    ...
    def test_parquet_read_from_url(
            self, httpserver, datapath, df_compat, engine):
        if engine != "auto":
...
class TestBasic(Base):
    ...
    def test_dtype_backend(self, engine, request):
        ...
        with tm.ensure_clean() as path:
            ...
            result2 = read_parquet(
                path, engine=engine, dtype_backend="numpy_nullable")
...
class TestParquetPyArrow(Base):
    ...
    def test_s3_roundtrip_explicit_fs(
            self, df_compat, s3_public_bucket, pa, s3so):
    ...
    def test_pyarrow_backed_string_array(self, pa, string_storage):
        ...
        with pd.option_context("string_storage", string_storage):
            check_round_trip(
                df, pa, expected=df.astype(f"string[{string_storage}]"))
    ...
    def test_timestamp_nanoseconds(self, pa):
        ...
        # Note in previous pyarrows(<7.0.0), only the pseudo-version 2.0 was
        # available
        ver = "2.6"
        df = pd.DataFrame(
    ...
    def test_read_parquet_manager(self, pa, using_array_manager):
        # ensure that read_parquet honors the pandas.options.mode.data_manager
        # option
        df = pd.DataFrame(
    ...
    @pytest.mark.skipif(pa_version_under11p0,
                        reason="not supported before 11.0")
...
class TestParquetFastParquet(Base):
    ...
    @pytest.mark.skipif(using_copy_on_write(),
                        reason="fastparquet writes into Index")
    ...
    def test_close_file_handle_on_read_error(self):
        with tm.ensure_clean("test.parquet") as path:
            ...
            # The next line raises an error on Windows if the file is still
            # open
