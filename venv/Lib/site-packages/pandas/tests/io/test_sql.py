...
def iris_table_metadata():
    ...
    dtype = Double if Version(
        sqlalchemy.__version__) >= Version("2.0.0") else Float
    metadata = MetaData()
    iris = Table(
        "iris",
        metadata,
...
def create_and_load_iris_postgresql(conn, iris_file: Path):
    ...
    with conn.cursor() as cur:
        ...
        with iris_file.open(newline=None, encoding="utf-8") as csvfile:
            ...
            # ADBC requires explicit types - no implicit str -> float
            # conversion
            records = [
                (
                    float(row[0]),
...
def create_and_load_postgres_datetz(conn):
    ...
    metadata = MetaData()
    datetz = Table(
        "datetz", metadata, Column(
            "DateColWithTz", DateTime(
                timezone=True)))
    datetz_data = [
        {
...
def check_iris_frame(frame: DataFrame):
    ...
    tm.assert_series_equal(
        row, Series([5.1, 3.5, 1.4, 0.2, "Iris-setosa"],
...
def sqlite_engine(sqlite_str):
    sqlalchemy = pytest.importorskip("sqlalchemy")
    engine = sqlalchemy.create_engine(
        sqlite_str, poolclass=sqlalchemy.pool.NullPool)
    yield engine
    for view in get_all_views(engine):
        drop_view(view, engine)
    for tbl in get_all_tables(engine):
...
sqlalchemy_connectable = mysql_connectable + \
    postgresql_connectable + sqlite_connectable
...
all_connectable = sqlalchemy_connectable + \
    ["sqlite_buildin"] + adbc_connectable

all_connectable_iris = (
    sqlalchemy_connectable_iris +
    ["sqlite_buildin_iris"] + adbc_connectable_iris
)
...
def test_read_iris_table_chunksize(conn, request):
    if "adbc" in conn:
        request.node.add_marker(
            pytest.mark.xfail(
                reason="chunksize argument NotImplemented with ADBC")
        )
    conn = request.getfixturevalue(conn)
...
def test_copy_from_callable_insertion_method(conn, expected_count, request):
    ...
    conn = request.getfixturevalue(conn)
    expected = DataFrame(
        {"col1": [1, 2], "col2": [0.1, 0.2], "col3": ["a", "n"]})
...
def test_insertion_method_on_conflict_update(conn, request):
    ...
    def insert_on_conflict(table, conn, keys, data_iter):
        ...
        stmt = stmt.on_duplicate_key_update(
            b=stmt.inserted.b, c=stmt.inserted.c)
        result = conn.execute(stmt)
        return result.rowcount
...
def test_api_to_sql_index_label(
        conn, request, index_name, index_label, expected):
    if "adbc" in conn:
        request.node.add_marker(
            pytest.mark.xfail(
                reason="index_label argument NotImplemented with ADBC")
        )
    conn = request.getfixturevalue(conn)
...
def test_api_chunksize_read(conn, request):
    ...
    for chunk in sql.read_sql_query(
            "select * from test_chunksize", conn, chunksize=5):
...
def test_api_categorical(conn, request):
    if conn == "postgresql_adbc_conn":
        adbc = import_optional_dependency(
            "adbc_driver_postgresql", errors="ignore")
...
def test_sqlalchemy_type_mapping(conn, request):
    ...
    df = DataFrame(
        {"time": to_datetime(
            ["2014-12-12 01:54", "2014-12-11 02:54"], utc=True)}
    )
    with sql.SQLDatabase(conn) as db:
...
def test_sql_open_close(test_frame3):
    ...
    with tm.ensure_clean() as name:
        ...
        with closing(sqlite3.connect(name)) as conn:
            result = sql.read_sql_query(
                "SELECT * FROM test_frame3_legacy;", conn)
...
def test_sqlalchemy_read_table_columns(conn, request):
    ...
    tm.assert_index_equal(iris_frame.columns, Index(
        ["SepalLength", "SepalLength__1"]))
...
def test_naive_datetimeindex_roundtrip(conn, request):
    ...
    assert expected.to_sql(
        name="foo_table",
        con=conn,
        index_label="info_date") == 5
...
def test_get_schema_create_table(conn, request, test_frame3):
    ...
    if conn == "sqlite_str":
        request.applymarker(
            pytest.mark.xfail(
                reason="test does not support sqlite_str fixture")
        )
...
def test_to_sql_with_negative_npinf(conn, request, input):
    ...
    if "mysql" in conn_name:
        ...
        if Version(pymysql.__version__) < Version(
                "1.0.3") and "infe0" in df.columns:
...
def test_to_sql_with_sql_engine(conn, request, test_frame1):
    ...
    with pandasSQL_builder(conn) as pandasSQL:
        with pandasSQL.run_transaction():
            assert pandasSQL.to_sql(
                test_frame1, "test_frame1", engine="auto") == 4
...
def test_read_sql_dtype_backend(
    ...
    if "adbc" in conn_name:
        ...
        request.applymarker(
            pytest.mark.xfail(
                reason="adbc does not support chunksize argument")
        )
    ...
    with pd.option_context("mode.string_storage", string_storage):
        ...
        expected = dtype_backend_expected(
...
def dtype_backend_expected():
    def func(storage, dtype_backend, conn_name) -> DataFrame:
        ...
        if storage == "python":
            ...
            string_array = ArrowExtensionArray(
                pa.array(["a", "b", "c"]))  # type: ignore[assignment]
            string_array_na = ArrowExtensionArray(
                pa.array(["a", "b", None]))  # type: ignore[assignment]
