...
class TestStata:
    ...
    def test_read_write_reread_dta14(
            self, file, parsed_114, version, datapath):
        file = datapath("io", "data", "stata", f"{file}.dta")
        parsed = self.read_dta(file)
    ...
    def test_no_index(self):
        ...
        original = DataFrame(
            np.reshape(
                np.arange(10.0), (5, 2)), columns=columns)
    ...
    def test_large_value_conversion(self):
        ...
        tm.assert_frame_equal(
            written_and_read_again.set_index("index"), modified)
        ...
        with tm.ensure_clean() as path:
            ...
            with StataReader(path) as sr:
                ...
                for variable, fmt, typ in zip(
                        sr._varlist, sr._fmtlist, sr._typlist):
    ...
    @pytest.mark.parametrize("file",
                             ["stata8_113", "stata8_115", "stata8_117"])
    ...
    def test_big_dates(self, datapath):
        ...
        parsed_115 = read_stata(
            datapath(
                "io",
                "data",
                "stata",
                "stata9_115.dta"))
        parsed_117 = read_stata(
            datapath(
                "io",
                "data",
                "stata",
    ...
    def test_categorical_writing(self, version):
        ...
        expected["incompletely_labeled"] = expected["incompletely_labeled"].apply(
            str)
    ...
    def test_categorical_order(self, file, datapath):
        ...
        expected = [
            ...
            (True, "noorder", ["a", "b", "c", "d", "e"],
             np.array([2, 1, 4, 0, 3])),
            (True, "floating", ["a", "b", "c", "d", "e"], np.arange(0, 5)),
            (True, "float_missing", ["a", "d", "e"],
             np.array([0, 1, 2, -1, -1])),
    ...
    def test_write_variable_label_errors(self, mixed_frame):
        ...
        with pytest.raises(ValueError, match=msg):
            with tm.ensure_clean() as path:
                mixed_frame.to_stata(
                    path, variable_labels=variable_labels_utf8)
        ...
        with pytest.raises(ValueError, match=msg):
            with tm.ensure_clean() as path:
                mixed_frame.to_stata(
                    path, variable_labels=variable_labels_long)
...
"""
    ...
    def test_mixed_string_strl(self):
        # GH 23633
        output = [{"mixed": "string" * 500, "number": 0},
                  {"mixed": None, "number": 1}]
...
the string values returned are correct."""
        ...
        with tm.assert_produces_warning(UnicodeWarning, filter_level="once") as w:
            ...
            # with filter_level="always", produces 151 warnings which can be
            # slow
            assert len(w) == 1
    ...
    def test_utf8_writer(self, version):
        ...
        with tm.ensure_clean() as path:
            ...
            data["β"] = (
                data["β"].replace(value_labels["β"]).astype(
    ...
    @pytest.mark.parametrize(
        "dtype_backend",
        ["numpy_nullable", pytest.param(
            "pyarrow", marks=td.skip_if_no("pyarrow"))],
    )
...
def test_compression(compression, version, use_dict,
                     infer, compression_to_extension):
...
def test_chunked_categorical(version):
    df = DataFrame(
        {"cats": Series(["a", "b", "a", "b", "c"], dtype="category")})
    df.index.name = "index"
...
def test_iterator_value_labels():
    ...
    df = DataFrame({f"col{k}": pd.Categorical(values, ordered=True)
                   for k in range(2)})
    with tm.ensure_clean() as path:
        df.to_stata(path, write_index=False)
        ...
        with read_stata(path, chunksize=100) as reader:
            for j, chunk in enumerate(reader):
                for i in range(2):
                    tm.assert_index_equal(
                        chunk.dtypes.iloc[i].categories, expected)
