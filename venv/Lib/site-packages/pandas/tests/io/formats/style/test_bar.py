...
def test_vmin_vmax_clipping(df_pos, df_neg, df_mix,
                            values, vmin, vmax, nullify, align):
...
@pytest.mark.parametrize("nullify", [None, "vmin", "vmax"])
@pytest.mark.parametrize("align", ["left", "right", "zero", "mid"])
def test_vmin_vmax_widening(df_pos, df_neg, df_mix,
                            values, vmin, vmax, nullify, align):
...
def test_bar_align_height():
    # test when keyword height is used 'no-repeat center' and
    # 'background-size' present
    data = DataFrame([[1], [2]])
...
def test_bar_invalid_color_type_error_raises():
    ...
    # Test that providing a color list with more than two elements raises a
    # ValueError
    with pytest.raises(ValueError, match=msg):
