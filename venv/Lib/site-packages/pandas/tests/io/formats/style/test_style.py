...
def df():
    df = DataFrame(
        {"A": [0, 1], "B": np.random.default_rng(2).standard_normal(2)})
    return df
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # dynamically reduce rows to 250, keep cols
        (1000, 3, 750, None, None, 250, 3),
        # dynamically reduce cols to 125, keep rows
        (4, 1000, 500, None, None, 4, 125),
        # overwrite above dynamics with max_row
        (1000, 3, 750, 10, None, 10, 3),
        (4, 1000, 500, None, 5, 4, 5),  # overwrite above dynamics with max_col
        # rows cols below given maxes so < 700 elmts
...
def test_render_trimming_mi():
    ...
    assert {"class": "data row_trim col_trim"}.items(
    ) <= ctx["body"][2][4].items()
...
def test_copy(comprehensive, render, deepcopy, mi_styler, mi_styler_comp):
    ...
    s2 = copy.deepcopy(styler) if deepcopy else copy.copy(
        styler)  # make copy and check
    ...
    if not deepcopy:  # check memory locations are equal for all included attributes
        for attr in [a for a in styler.__dict__ if (
                not callable(a) and a not in excl)]:
...
def test_clear(mi_styler_comp):
    ...
    # tests vars are not same vals on obj and clean copy before clear (except
    # for excl)
...
def test_export(mi_styler_comp, mi_styler):
    ...
    for attr in exp_attrs:
        ...
        assert not (
            all(check) if (hasattr(check, "__iter__")
                           and len(check) > 0) else check
        )
    ...
    for attr in exp_attrs:
        ...
        assert all(check) if (hasattr(check, "__iter__")
                              and len(check) > 0) else check
...
def test_hide_index_level(mi_styler, level):
    mi_styler.index.names, mi_styler.columns.names = [
        "zero", "one"], [
        "zero", "one"]
...
class TestStyler:
    ...
    def test_render(self):
        df = DataFrame({"A": [0, 1]})
        def style(x): return Series(["color: red", "color: blue"], name=x.name)
    ...
    def test_set_properties(self):
        ...
        result = df.style.set_properties(
            color="white", size="10px")._compute().ctx
    ...
    def test_apply_axis(self):
        ...
        def f(x): return [f"val: {x.max()}" for v in x]
    ...
    def test_apply_series_return(self, axis):
        ...
        # test Series return where len(Series) < df.index or df.columns but
        # labels OK
        def func(s): return Series(["color: red;"], index=["Y"])
        ...
        def func(s): return Series(
            ["color: red;", "color: blue;"], index=["Y", "X"])
        result = df.style.apply(func, axis=axis)._compute().ctx
    ...
    def test_apply_dataframe_return(self, index, columns):
        ...
        # (Y,Y) styles always present
        assert result[(1, 1)] == [("color", "red")]
        assert (result[(0, 1)] == [("color", "red")]
                ) is index  # (X,Y) only if index
        assert (result[(1, 0)] == [("color", "red")]
                ) is columns  # (Y,X) only if cols
        assert (result[(0, 0)] == [("color", "red")]) is (
            index and columns)  # (X,X)
    ...
    def test_apply_subset(self, slice_, axis, df):
        ...
        result = df.style.apply(
            h,
            axis=axis,
            subset=slice_,
            color="baz")._compute().ctx
        expected = {
            (r, c): [("color", "baz")]
    ...
    def test_apply_none(self):
        ...
        result = DataFrame([[1, 2], [3, 4]]).style.apply(
            f, axis=None)._compute().ctx
        assert result[(1, 1)] == [("color", "red")]
    ...
    def test_export(self, df, styler):
        def f(x): return "color: red" if x > 0 else "color: blue"
        def g(x, z): return f"color: {z}" if x > 0 else f"color: {z}"
        style1 = styler
    ...
    def test_bad_apply_shape(self):
        ...
        with pytest.raises(ValueError, match=msg.format("index")):
            df.style._apply(lambda x: Series(
                ["a:v;", ""], index=["A", "C"]), axis=0)
    ...
    def test_mi_sparse_index_names(self, blank_value):
        # Test the class names and displayed value are correct on rendering MI
        # names
        df = DataFrame(
            {"A": [1, 2]},
    ...
    def test_hide_single_index(self, df):
        ...
        ctx4 = df.set_index("A").style.hide(
            axis="index")._translate(
            True, True)
    ...
    def test_hide_multiindex(self):
        ...
        # two visible indexes and two data columns
        ...
        # check for blank header rows
        # one hidden (col name) and two data columns
    ...
    def test_hide_columns_index_mult_levels(self):
        ...
        ctx = df.style.hide([("b", 1)], axis=1).hide(
            axis=0)._translate(True, True)
    ...
    def test_set_data_classes(self, classes):
        # GH 36159
        df = DataFrame(data=[[0, 1], [2, 3]], columns=[
                       "A", "B"], index=["a", "b"])
        s = Styler(df, uuid_len=0, cell_ids=False).set_td_classes(
            classes).to_html()
        ...
        # GH 39317
        s = Styler(df, uuid_len=0, cell_ids=True).set_td_classes(
            classes).to_html()
            ...
            if isinstance(key, tuple):
                for subkey in key:
                    if isinstance(subkey, list) and "-" in subkey:
                        # not present in the index level, raises KeyError since
                        # 2.0
...
def test_hidden_index_names(mi_df):
    ...
    for i in range(4):
        # 2 index levels + 2 data values visible
        assert ctx["body"][0][i]["is_visible"]
...
def test_hidden_column_names(mi_df):
    ...
    ctx = mi_styler._translate(True, True)
    # no index names and only one visible column headers
    assert len(ctx["head"]) == 1
    assert ctx["head"][0][1]["display_value"] == "&nbsp;"
...
def test_get_level_lengths_mi_hidden():
    ...
    result = _get_level_lengths(
        ...
        sparsify=False,
        max_index=100,
        # hidden element can repeat if duplicated index
...
def test_row_trimming_hide_index_mi():
    ...
    with option_context("styler.render.max_rows", 2):
        ctx = df.style.hide(
            [(0, 0), (0, 1)], axis="index")._translate(True, True)
    assert len(ctx["body"]) == 3
    ...
    assert {"value": "...", "is_visible": True}.items(
    ) <= ctx["body"][2][0].items()

    for r, val in enumerate(["2", "3", "..."]):
        # level 1 index headers
        assert ctx["body"][r][1]["display_value"] == val
    for r, val in enumerate(["3", "4", "..."]):
