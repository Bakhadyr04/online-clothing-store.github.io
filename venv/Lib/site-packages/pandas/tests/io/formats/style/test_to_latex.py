...
def test_column_format(styler):
    ...
    styler.set_table_styles(
        [{"selector": "column_format", "props": ":r|r|cc"}])
...
@pytest.mark.parametrize("column_format",
                         [(None, ""), ("rcrl", "{tabular}{rcrl}")])
@pytest.mark.parametrize("position_float",
                         [(None, ""), ("centering", "\\centering")])
...
def test_comprehensive(df_ext, environment):
    ...
    stlr.highlight_max(axis=None, props="Huge:--wrap;",
                       subset=[("Z", "a"), ("Z", "b")])
...
def test_parse_latex_table_styles(styler):
    ...
    assert _parse_latex_table_styles(
        styler.table_styles,
        "label") == "{fig:item}"
...
def test_parse_latex_table_wrapping(styler):
    ...
    assert _parse_latex_table_wrapping(
        styler.table_styles, styler.caption) is False
    assert _parse_latex_table_wrapping(
        styler.table_styles, "some caption") is True
    styler.set_table_styles(
        [
...
@pytest.mark.parametrize(
    "css, expected",
    [
        # test color and input format types
        ([("color", "red")], [("color", "{red}")]),
        (
        ...
        # test font-weight and types
        ([("font-weight", "bold")], [("bfseries", "")]),
        ([("font-weight", "bolder")], [("bfseries", "")]),
        ([("font-weight", "normal")], []),
        ([("background-color", "red")], [("cellcolor", "{red}--lwrap")]),
        (
            # test background-color command and wrap
            [("background-color", "#FF00FF")],
            [("cellcolor", "[HTML]{FF00FF}--lwrap")],
        ),
        # test font-style and types
        ([("font-style", "italic")], [("itshape", "")]),
        ([("font-style", "oblique")], [("slshape", "")]),
        ([("font-style", "normal")], []),
        ([("color", "red /*--dwrap*/")],
         [("color", "{red}--dwrap")]),  # css comments
        ([("background-color", "red /* --dwrap */")],
         [("cellcolor", "{red}--dwrap")]),
    ],
)
...
@pytest.mark.parametrize("label, lab_exp",
                         [(None, ""), ("tab:A", " \\label{tab:A}")])
...
def test_apply_map_header_render_mi(df_ext, index, columns, siunitx):
    ...
    def func(
        v): return "bfseries: --rwrap" if "A" in v or "Z" in v or "c" in v else None
...
def test_bool_options(styler, option):
    ...
    # options are reactive under to_latex(*no_args)
...
@pytest.mark.parametrize("clines",
                         ["bad", "index", "skip-last", "all", "data"])
...
def test_concat_recursion():
    ...
    styler2 = DataFrame([[9], [2]]).style.hide(
        [0]).highlight_min(color="green")
...
@pytest.mark.parametrize(
    "clines", [None, "all;data", "all;index",
               "skip-last;data", "skip-last;index"]
)
