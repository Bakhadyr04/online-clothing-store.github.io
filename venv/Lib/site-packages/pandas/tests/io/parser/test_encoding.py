...
def test_utf8_bom(all_parsers, data, kwargs, expected, request):
    ...
    if (
        ...
        # CSV parse error: Empty CSV file or block: cannot infer number of
        # columns
...
def test_binary_mode_file_buffers(all_parsers, file_path, encoding, datapath):
    ...
    # gh-31575: Python csv engine shouldn't error on files opened in raw
    # binary.
    parser = all_parsers
...
def test_encoding_temp_file(all_parsers, utf_value,
                            encoding_fmt, pass_encoding):
    # see gh-24130
    parser = all_parsers
    encoding = encoding_fmt.format(utf_value)

    if parser.engine == "pyarrow" and pass_encoding is True and utf_value in [
            16, 32]:
        # FIXME: this is bad!
...
@pytest.mark.parametrize("encoding",
                         ["utf-8", None, "utf-16", "cp1255", "latin-1"])
