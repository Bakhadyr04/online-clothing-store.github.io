...
class TestRolling:
    ...
    def test_rolling_apply(self, raw, roll_frame):
        ...
        with tm.assert_produces_warning(DeprecationWarning, match=msg):
            expected = g.apply(
                lambda x: x.rolling(4).apply(
                    lambda y: y.sum(), raw=raw))
        # groupby.apply doesn't drop the grouped-by column
        expected = expected.drop("A", axis=1)
        # GH 39732
        expected_index = MultiIndex.from_arrays([roll_frame["A"], range(40)])
    ...
    @pytest.mark.parametrize("expected_value,raw_value",
                             [[1.0, True], [0.0, False]])
    ...
    def test_groupby_rolling_subset_with_closed(self):
        ...
        result = (
            df.groupby("group").rolling(
                "1D", on="date", closed="left")["column1"].sum()
        )
        expected = Series(
    ...
    def test_groupby_rolling_resulting_multiindex3(self):
        # grouping with 1 level on dataframe with 2-level MI -> 3-level MI as
        # result
