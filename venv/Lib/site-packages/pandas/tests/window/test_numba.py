...
# Filter warnings when parallel=True and the function can't be
# parallelized by Numba
class TestEngine:
...
class TestEWM:
    ...
    def test_cython_vs_numba_times(
            self, grouper, nogil, parallel, nopython, ignore_na):
        # GH 40951
...
class TestTableMethod:
    ...
    def test_table_method_rolling_apply(
            self, axis, nogil, parallel, nopython, step):
        engine_kwargs = {
            "nogil": nogil,
            "parallel": parallel,
            "nopython": nopython}
    ...
    def test_table_method_expanding_apply(
            self, axis, nogil, parallel, nopython):
        engine_kwargs = {
            "nogil": nogil,
            "parallel": parallel,
            "nopython": nopython}
    ...
    @pytest.mark.parametrize("data",
                             [np.eye(3), np.ones((2, 3)), np.ones((3, 2))])
    @pytest.mark.parametrize("method", ["mean", "sum"])
    def test_table_method_ewm(self, data, method, axis,
                              nogil, parallel, nopython):
        engine_kwargs = {
            "nogil": nogil,
            "parallel": parallel,
            "nopython": nopython}
