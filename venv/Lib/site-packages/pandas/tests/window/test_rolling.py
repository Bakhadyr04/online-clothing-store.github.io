...
@pytest.mark.parametrize("window",
                         [timedelta(days=3), Timedelta(days=3), "3D"])
def test_constructor_timedelta_window_and_minperiods(window, raw):
    ...
    result_roll_generic = df.rolling(
        window=window, min_periods=2).apply(
        sum, raw=raw)
...
def test_datetimelike_centered_selections(
    ...
    expected = DataFrame(
        {"A": [getattr(df_time["A"].iloc[s], func_name)()
               for s in window_selections]},
        index=date_range("2020", periods=5),
    )
...
def test_missing_minp_zero_variable():
    ...
    x = Series(
        [np.nan] * 4,
        index=DatetimeIndex(["2017-01-01", "2017-01-04",
...
def test_rolling_datetime(axis_frame, tz_naive_fixture):
    ...
    df = DataFrame(
        {i: [1] *
         2 for i in date_range("2019-8-01", "2019-08-03", freq="D", tz=tz)}
    )
...
def test_rolling_positional_argument(grouping, _index, raw):
    ...
    result = df.groupby(
        **grouping).rolling(1).apply(scaled_sum, raw=raw, args=(2,))
    tm.assert_frame_equal(result, expected)
...
def test_rolling_numerical_accuracy_kahan_mean(add, unit):
    ...
    result = (
        df.resample("1s").ffill().rolling(
            "3s", closed="left", min_periods=3).mean()
    )
...
def test_rolling_on_df_transposed():
    ...
    expected = DataFrame(
        {"A": [1.0, np.nan], "B": [5.0, 5.0], "C": [11.0, 13.0]})
    msg = "Support for axis=1 in DataFrame.rolling is deprecated"
    with tm.assert_produces_warning(FutureWarning, match=msg):
...
@pytest.mark.parametrize("method",
                         ["var", "sum", "mean", "skew", "kurt", "min", "max"])
def test_rolling_decreasing_indices(method):
    ...
    df_reverse = DataFrame(
        {"values": df["values"][::-1]}, index=df.index[::-1])

    increasing = getattr(df.rolling(window=5), method)()
    decreasing = getattr(df_reverse.rolling(window=5), method)()

    assert np.abs(decreasing.values[::-1][:-4] -
                  increasing.values[4:]).max() < 1e-12
...
def test_rolling_float_dtype(float_numpy_dtype):
    # GH#42452
    df = DataFrame({"A": range(5), "B": range(10, 15)},
...
def test_numeric_only_corr_cov_frame(kernel, numeric_only, use_arg):
    ...
    # Compare result to op using float dtypes, dropping c when numeric_only is
    # True
    columns = ["a", "b"] if numeric_only else ["a", "b", "c"]
