...
@pytest.fixture(params=["python", pytest.param("numba",
                marks=pytest.mark.single_cpu)])
def engine(request):
...
def test_apply(float_frame, engine, request):
    if engine == "numba":
        mark = pytest.mark.xfail(
            reason="numba engine not supporting numpy ufunc yet")
...
def test_apply_reduce_to_dict():
    # GH 25196 37544
    data = DataFrame([[1, 2], [3, 4]], columns=[
                     "c0", "c1"], index=["i0", "i1"])

    result = data.apply(dict, axis=0)
    expected = Series(
        [{"i0": 1, "i1": 3}, {"i0": 2, "i1": 4}], index=data.columns)
    tm.assert_series_equal(result, expected)

    result = data.apply(dict, axis=1)
    expected = Series(
        [{"c0": 1, "c1": 2}, {"c0": 3, "c1": 4}], index=data.index)
    tm.assert_series_equal(result, expected)
...
def test_apply_differently_indexed():
    ...
    expected = DataFrame({i: v.describe()
                         for i, v in df.items()}, columns=df.columns)
    tm.assert_frame_equal(result, expected)

    result = df.apply(Series.describe, axis=1)
    expected = DataFrame({i: v.describe()
                         for i, v in df.T.items()}, columns=df.index).T
    tm.assert_frame_equal(result, expected)
...
def test_apply_with_byte_string():
    ...
    expected = DataFrame(np.array([b"abcd", b"efgh"]), columns=[
                         "col"], dtype=object)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([lambda x: x.sum()], "compat", DataFrame(
            {"a": [3]}, index=["<lambda>"])),
        ([lambda x: x.sum()], False, DataFrame(
            {"a": [3]}, index=["<lambda>"])),
        (
...
def test_with_listlike_columns_returning_list():
    ...
    result = df.apply(
        lambda row: [
            el for el in row["x"] if el in row["y"]],
        axis=1)
...
def test_consistent_names(int_frame_const_col):
    ...
    result = df.apply(lambda x: Series(
        [1, 2], index=["test", "other"]), axis=1)
...
def test_result_type_broadcast(int_frame_const_col, request, engine):
    ...
    if engine == "numba":
        mark = pytest.mark.xfail(
            reason="numba engine doesn't support list return")
        request.node.add_marker(mark)
    df = int_frame_const_col
    # broadcast result
    result = df.apply(
...
def test_result_type_broadcast_series_func(
        int_frame_const_col, engine, request):
    # result_type should be consistent no matter which
    # path we take in the code
    if engine == "numba":
        mark = pytest.mark.xfail(
...
def test_result_type_series_result(int_frame_const_col, engine, request):
    ...
    # series result
    result = df.apply(lambda x: Series(
        [1, 2, 3], index=x.index), axis=1, engine=engine)
    expected = df.copy()
    tm.assert_frame_equal(result, expected)
...
def test_result_type_series_result_other_index(
    ...
    columns = ["other", "col", "names"]
    result = df.apply(lambda x: Series(
        [1, 2, 3], index=columns), axis=1, engine=engine)
    expected = df.copy()
...
@pytest.mark.parametrize(
    "box",
    [lambda x: list(x), lambda x: tuple(
        x), lambda x: np.array(x, dtype="int64")],
...
def test_frequency_is_original(num_cols, engine, request):
    ...
    if engine == "numba":
        mark = pytest.mark.xfail(
            reason="numba engine only supports numeric indices")
        request.node.add_marker(mark)
...
def test_apply_mutating(using_array_manager,
    ...
    if using_copy_on_write or using_array_manager:
        ...
        # with ArrayManager the row is not a view, and thus not mutated in
        # place
...
def test_apply_no_suffix_index(engine, request):
    ...
    result = pdf.apply(
        ["sum", lambda x: x.sum(), lambda x: x.sum()], engine=engine)
    expected = DataFrame(
...
def test_apply_type():
    # GH 46719
    df = DataFrame(
        {"col1": [3, "string", float], "col2": [
            0.25, datetime(2020, 1, 1), np.nan]},
        index=["a", "b", "c"],
    )
