...
def test_apply_box_dt64():
    ...
    res = ser.apply(
        lambda x: f"{type(x).__name__}_{x.day}_{x.tz}",
        by_row="compat")
...
def test_apply_box_period():
    ...
    res = ser.apply(
        lambda x: f"{type(x).__name__}_{x.freqstr}",
        by_row="compat")
...
def test_apply_datetimetz(by_row):
    ...
    exp = Series(
        ...
        [0],
        name="XX",
        dtype="int64" if by_row else "int32")
    tm.assert_series_equal(result, exp)
...
def test_transform(string_series, by_row):
    ...
    with np.errstate(all="ignore"):
        ...
        result = string_series.apply(
            {"foo": np.sqrt, "bar": np.abs}, by_row=by_row)
...
def test_non_callable_aggregates(how, kwds):
    ...
    result = getattr(s, how)(
        {"size": "size", "count": "count", "mean": "mean"}, **kwds)
    tm.assert_series_equal(result, expected)
...
def test_series_apply_no_suffix_index(by_row):
    ...
    s = Series([4] * 3)
    result = s.apply(
        ["sum", lambda x: x.sum(), lambda x: x.sum()], by_row=by_row)
...
@pytest.mark.parametrize(
    "by_row, expected", [
        ("compat", Series(np.ones(10), dtype="int64")), (False, 1)]
)
...
def test_apply_listlike_reducer(string_series, ops, names, how, kwargs):
    # GH 39140
    expected = Series({name: op(string_series)
...
def test_apply_dictlike_transformer(string_series, ops, by_row):
    # GH 39140
    with np.errstate(all="ignore"):
        expected = concat({name: op(string_series)
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"a": lambda x: x},
            Series([1, 2, 3], index=MultiIndex.from_arrays(
                [["a"] * 3, range(3)])),
        ),
...
def test_series_apply_unpack_nested_data():
    ...
    expected = DataFrame(
        {0: [1.0, 4.0], 1: [2.0, 5.0], 2: [3.0, 6.0], 3: [np.nan, 7]})
    tm.assert_frame_equal(result, expected)
