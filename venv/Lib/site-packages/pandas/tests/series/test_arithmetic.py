...
class TestSeriesArithmetic:
    ...
    def test_alignment_doesnt_change_tz(self):
        ...
        # we don't care about the result, just that original indexes are
        # unchanged
        ser * ser_utc
...
class TestSeriesComparison:
    ...
    def test_comp_ops_df_compat(self, left, right, frame_or_series):
        # GH 1134
        # GH 50083 to clarify that index and columns must be identically
        # labeled
...
class TestNamePreservation:
    @pytest.mark.parametrize("box",
                             [list, tuple, np.array, Index, Series, pd.array])
    @pytest.mark.parametrize("flex", [True, False])
    def test_series_ops_name_retention(
            self, flex, box, names, all_binary_operators):
...
class TestInplaceOperations:
    ...
    def test_series_inplace_ops(
            self, dtype1, dtype2, dtype_expected, dtype_mul):
        # GH 37910
...
def test_none_comparison(request, series_with_simple_index):
    ...
    if lib.is_np_dtype(series.dtype, "M") or isinstance(
            series.dtype, DatetimeTZDtype):
