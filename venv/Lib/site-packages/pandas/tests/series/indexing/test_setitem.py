...
class TestSetitemWithExpansion:
    ...
    def test_setitem_enlargement_object_none(
            self, nulls_fixture, using_infer_string):
        # GH#48665
        ser = Series(["a", "b"])
        ser[3] = nulls_fixture
        dtype = (
...
class TestSetitemCasting:
    ...
    def test_setitem_non_bool_into_bool(self, val, indexer_sli, unique):
        ...
        assert isinstance(ser.iloc[1], type(val))
...
class SetitemCastingEquivalents:
    ...
    def test_int_key(self, obj, key, expected, warn,
                     val, indexer_sli, is_inplace):
        ...
        if indexer_sli is not tm.loc:
            ...
            with tm.assert_produces_warning(warn, match="incompatible dtype"):
                self.check_indexer(
                    obj, slc, expected, val, indexer_sli, is_inplace)
    ...
    def test_slice_key(self, obj, key, expected, warn,
                       val, indexer_sli, is_inplace):
        ...
        if indexer_sli is not tm.loc:
            ...
            with tm.assert_produces_warning(warn, match="incompatible dtype"):
                self.check_indexer(
                    obj, key, expected, val, indexer_sli, is_inplace)
    ...
    def test_index_where(self, obj, key, expected,
                         warn, val, using_infer_string):
        mask = np.zeros(obj.shape, dtype=bool)
        mask[key] = True
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            # TODO: maybe go to float64 since we are changing the _whole_
            # Series?
            0,
            FutureWarning,
...
class TestCoercionDatetime64TZ(CoercionTest):
    # previously test_setitem_series_datetime64tz in
    # tests.indexing.test_coercion
    @pytest.fixture
    def obj(self):
        tz = "US/Eastern"
...
class TestCoercionTimedelta64(CoercionTest):
    # previously test_setitem_series_timedelta64 in
    # tests.indexing.test_coercion
    @pytest.fixture
    def obj(self):
