...
class TestTimeConversionFormats:
    ...
    @pytest.mark.parametrize(
        ...
        [
            [
                "%d/%m/%Y",
                [Timestamp("20000101"), Timestamp(
                    "20000201"), Timestamp("20000301")],
            ],
            [
                "%m/%d/%Y",
                [Timestamp("20000101"), Timestamp(
                    "20000102"), Timestamp("20000103")],
            ],
        ],
    )
    def test_to_datetime_format(
            self, cache, index_or_series, format, expected):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ([None, np.nan, pd.NA], "%Y%m%d",
             DatetimeIndex(["NaT", "NaT", "NaT"])),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # long integer date partially parsed to datetime(2012,1,1), length
            # > 8
            [2012010101, 2012010101],
            # invalid date partially parsed to datetime(2012,9,9), length == 8
            [20129930, 20129930],
            # short integer date partially parsed to datetime(2012,9,9), length
            # < 8
            [2012993, 2012993],
    ...
    def test_non_exact_doesnt_parse_whole_string(
            self, cache, format, expected):
        # https://github.com/pandas-dev/pandas/issues/50412
        # the formats alternate between ISO8601 and non-ISO8601 to check both
        # paths
        result = to_datetime(
...
class TestToDatetime:
    ...
    def test_to_datetime_today(self, tz):
        ...
        with tm.set_timezone(tz):
            nptoday = np.datetime64("today").astype(
    ...
    def test_to_datetime_now_with_format(
            self, format, expected_ds, string, attribute):
    ...
    def test_to_datetime_array_of_dt64s(self, cache, unit):
        ...
        if cache:
            ...
            expected = DatetimeIndex(
                [Timestamp(x).asm8 for x in dts], dtype="M8[s]")
        else:
            expected = DatetimeIndex(
                [Timestamp(x).asm8 for x in dts], dtype="M8[ns]")
        ...
        if not cache:
            ...
            expected = DatetimeIndex(
                [Timestamp(dts_with_oob[0]).asm8,
                 Timestamp(dts_with_oob[1]).asm8] * 30
                + [NaT],
            )
        else:
    ...
    def test_datetime_bool_arrays_mixed(self, cache):
        ...
        tm.assert_index_equal(
            ...
            DatetimeIndex(
                [to_datetime(0, cache=cache), NaT, NaT,
    ...
    @pytest.mark.parametrize("constructor",
                             [list, tuple, np.array, Index, deque])
    ...
    def test_non_iso_strings_with_tz_offset(self):
        ...
        expected = DatetimeIndex(
            [datetime(2018, 3, 1, 12, tzinfo=timezone(
                timedelta(minutes=240)))] * 2
        )
        tm.assert_index_equal(result, expected)
    ...
    @pytest.mark.parametrize("dt_str",
                             ["00010101", "13000101", "30000101", "99990101"])
...
class TestToDatetimeUnit:
    ...
    def test_to_datetime_month_or_year_unit_int(
            self, cache, unit, item, request):
        # GH#50870 Note we have separate tests that pd.Timestamp gets these
        # right
    ...
    def test_unit_array_mixed_nans_large_int(self, cache):
        ...
        expected = Index([1420043460000000000000000, NaT,
                         NaT, NaT, NaT], dtype=object)
        tm.assert_index_equal(result, expected)

        result = to_datetime(values, errors="coerce", unit="s", cache=cache)
        expected = DatetimeIndex(
            ["NaT", "NaT", "NaT", "NaT", "NaT"], dtype="M8[ns]")
        tm.assert_index_equal(result, expected)
    ...
    def test_to_datetime_unit_fractional_seconds(self):
        ...
        # GH20455 argument will incur floating point errors but no premature
        # rounding
...
class TestToDatetimeMisc:
    ...
    @pytest.mark.parametrize("result",
                             [Timestamp("2012"), to_datetime("2012")])
    ...
    def test_dayfirst(self, cache):
        ...
        expected = DatetimeIndex(
            [datetime(2014, 2, 10), datetime(
                2014, 2, 11), datetime(2014, 2, 12)]
        )
    ...
    @pytest.mark.parametrize("klass",
                             [DatetimeIndex, DatetimeArray._from_sequence])
...
class TestDatetimeParsingWrappers:
    ...
    @pytest.mark.parametrize(
        "date_str, exp_def",
        [["10:15", datetime(1, 1, 1, 10, 15)], [
            "9:05", datetime(1, 1, 1, 9, 5)]],
    )
...
class TestShouldCache:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (0.5,
             11,
             r"check_count must be in next bounds: \[0; len\(arg\)\]"),
...
def test_empty_string_datetime(errors, args, format):
    ...
    expected = Series(["2016-03-24", "2016-03-25", NaT],
                      dtype="datetime64[ns]")
...
def test_empty_string_datetime_coerce__unit():
    ...
    expected = DatetimeIndex(
        ["1970-01-01 00:00:01", "NaT"], dtype="datetime64[ns]")
...
def test_to_datetime_mixed_iso8601():
    ...
    result = to_datetime(
        ["2020-01-01", "2020-01-01 05:00:00"], format="ISO8601")
...
def test_to_datetime_mixed_not_necessarily_iso8601_coerce(errors, expected):
    # https://github.com/pandas-dev/pandas/issues/50411
    result = to_datetime(["2020-01-01", "01-01-2000"],
                         format="ISO8601", errors=errors)
    tm.assert_index_equal(result, expected)
...
def test_to_datetime_with_empty_str_utc_false_format_mixed():
    ...
    result = to_datetime(vals, format="mixed")
    expected = Index([Timestamp("2020-01-01 00:00+00:00"),
                     "NaT"], dtype="M8[ns, UTC]")
    tm.assert_index_equal(result, expected)
...
def test_to_datetime_mixed_awareness_mixed_types(
        aware_val, naive_val, naive_first):
    # GH#55793, GH#55693
