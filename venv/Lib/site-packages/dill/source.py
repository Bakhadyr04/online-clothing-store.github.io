...
__all__ = [
    ...
    'dumpsource', 'getname', '_namespace', 'getimport', '_importable',
    'importable', 'isdynamic', 'isfrommain'
]
...
def _matchlambda(func, line):
    ...
    def dummy():
        return '__this_is_a_big_dummy_function__'
    ...
    _ = dict(
        re.match(r'([\W\D\S])(.*)', _[i]).groups() for i in range(1, len(_)))
    _f = dict(
        re.match(r'([\W\D\S])(.*)', _f[i]).groups() for i in range(1, len(_f)))
...
def findsource(object):
    ...
    if IS_IPYTHON and is_module_main:
        ...
        try:
            ...
            if isclass(object):
                for object_method in filter(isfunction,
        ...
        if file:
            ...
            history = '\n'.join(
                get_ipython().history_manager.input_hist_parsed)
...
def getsource(object,
    ...
    try:  # fails for builtins, and other assorted object types
        lines, lnum = getsourcelines(object, enclosing=enclosing)
            ...
            IOError):  # failed to get source, resort to import hooks
...
def _hascode(object):
    ...
    return getattr(object, '__code__', None) or getattr(
        object, 'func_code', None)
...
#       return False
    ...
    if not repr(type(object)).startswith(_types):  # FIXME: weak hack
        return False
    if not getmodule(object) or object.__module__ in [
...
def dumpsource(object, alias='', new=False, enclose=True):
    ...
    if not new or not _isinstance(object):
        ...
        # should have a module (no builtins here)
        mod = repr(object.__module__)
        code += pre + \
            'dill.loads(%s.replace(b%s,bytes(__name__,"UTF-8")))\n' % (pik, mod)
    # code += 'del %s' % object.__class__.__name__ #NOTE: kills any
    # existing!
...
def getname(obj,
            force=False,
            fqn=False):  # XXX: throw(?) to raise error on fail?
...
def _namespace(obj):
    ...
    try:  # XXX: needs some work and testing on different types
        module = qual = str(
            getmodule(obj)).split()[1].strip('>').strip('"').strip("'")
        qual = qual.split('.')
        if ismodule(obj):
            return qual
    ...
    if str(obj) in ['inf', 'nan', 'Inf',
                    'NaN']:  # is more, but are they needed?
...
def _closuredimport(func, alias='', builtin=False):
    ...
    for name, obj in list(free_vars.items()):
        ...
        if src.lstrip().startswith('@'):  # we have a decorator
            ...
            candidate = [
                ...
                if getname(encl) in line and re.match(pat, line)
            ]
            if not candidate:
    ...
    if not func_vars:
        ...
        if not mod or name is func:  # then it can be handled by getimport
            ...
            candidate = [
                line for line in lines if getname(name) in line
...
def importable(obj, alias='', source=None, builtin=True):
    ...
    while True:
        ...
        try:
            ...
            # XXX: don't worry about alias? recurse? etc?
            obj = globalvars(obj)
            obj = list(
                getsource(_obj, name, force=True)
                for (name, _obj) in obj.items() if not isbuiltin(_obj))
...
# return outdent(_importable(obj,alias,source=(not
# byname),builtin=explicit))
