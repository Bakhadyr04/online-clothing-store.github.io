...
def _module_map():
    ...
    for modname, module in sys.modules.items():
        if modname in ('__main__',
                       '__mp_main__') or not isinstance(module, ModuleType):
            continue
        if '.' not in modname:
...
IMPORTED_AS_TYPES = (ModuleType, TypeType, FunctionType, MethodType,
                     BuiltinMethodType)
...
def _lookup_module(modmap, name, obj, main_module):
    ...
    if isinstance(obj, IMPORTED_AS_TYPES) or (__module__ is not None and any(
            regex.fullmatch(__module__) for regex in IMPORTED_AS_MODULES)):
...
def _stash_modules(main_module):
    ...
    for name, obj in main_module.__dict__.items():
        ...
        # Avoid incorrectly matching a singleton value in another package (ex.:
        # __doc__).
            ...
            source_module, objname = _lookup_module(modmap, name, obj,
                                                    main_module)
            if source_module is not None:
                ...
                try:
                    imported_top_level.append(
                        (modmap.top_level[id(obj)], name))
                except KeyError:
                    original[name] = obj
    ...
    if len(original) < len(main_module.__dict__):
        ...
        if getattr(newmod, '__loader__',
                   None) is None and _is_imported_module(main_module):
            # Trick _is_imported_module() to force saving as an imported
            # module.
...
def _restore_modules(unpickler, main_module):
    try:
        ...
        for modname, objname, name in main_module.__dict__.pop(
                '__dill_imported_as'):
            main_module.__dict__[name] = unpickler.find_class(modname, objname)
        for modname, name in main_module.__dict__.pop(
                '__dill_imported_top_level'):
...
def dump_module(filename: Union[str, os.PathLike] = None,
    ...
    for old_par, par in [('main', 'module'), ('byref', 'refimported')]:
        if old_par in kwds:
            ...
            if locals()[par]:  # the defaults are None and False
                raise TypeError("both %r and %r arguments were used" %
                                (par, old_par))
...
def dump_session(filename=None, main=None, byref=False, **kwds):
    warnings.warn("dump_session() has been renamed dump_module()",
...
def _identify_module(file, main=None):
    ...
    try:
        ...
        # ValueError occours when the end of the chunk is reached (without a
        # STOP).
...
def load_module(filename: Union[str, os.PathLike] = None,
                module: Optional[Union[ModuleType, str]] = None,
                **kwds) -> Optional[ModuleType]:
    ...
    *Changed in version 0.3.6:* Function ``load_session()`` was renamed to
    ...
    """
    if 'main' in kwds:
        warnings.warn("The argument 'main' has been renamed 'module'.",
                      PendingDeprecationWarning)
...
def load_session(filename=None, main=None, **kwds):
    warnings.warn("load_session() has been renamed load_module().",
                  PendingDeprecationWarning)
...
def load_module_asdict(filename: Union[str, os.PathLike] = None,
                       update: bool = False,
                       **kwds) -> dict:
    """
