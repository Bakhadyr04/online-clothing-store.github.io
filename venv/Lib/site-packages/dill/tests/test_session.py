...
def _error_line(error, obj, refimported):
    ...
    )[-2].replace('[obj]', '[' + repr(obj) + ']')
    return "while testing (with refimported=%s):  %s" % (
        refimported, line.lstrip())
    ...
    with TestNamespace(**extra_objects) as ns:
        try:
            ...
            dill.dump_module(
                session_file %
                refimported,
                refimported=refimported)
            from dill.tests.__main__ import python, shell, sp
            error = sp.call([python, __file__, '--child',
                            str(refimported)], shell=shell)
            if error:
                sys.exit(error)
        finally:
            with suppress(OSError):
...
def test_session_other():
    ...
    dict_objects = [obj for obj in module.__dict__.keys()
