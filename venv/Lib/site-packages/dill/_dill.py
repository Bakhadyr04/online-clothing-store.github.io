...
if NumpyArrayType:  # then has numpy
    ...
    def ndarraysubclassinstance(obj_type):
        if all((c.__module__, c.__name__) != ('numpy', 'ndarray')
    ...
    def numpyufunc(obj_type):
        return any((c.__module__, c.__name__) == ('numpy', 'ufunc')
                   for c in obj_type.__mro__)

    def numpydtype(obj_type):
        if all((c.__module__, c.__name__) != ('numpy', 'dtype')
               for c in obj_type.__mro__):
            return False
...
try:
    ...
    PyBufferedRandomType = None if IS_PYODIDE else get_file_type(
        'r+b', buffering=-1, open=_open)
...
class Sentinel(object):
    ...
    def __init__(self, name, module_name=None):
        ...
        if module_name is None:
            ...
            self.__module__ = inspect.currentframe(
...
class Pickler(StockPickler):
    ...
    def save(self, obj, save_persistent_id=True):
        # numpy hack
        obj_type = type(obj)
        if NumpyArrayType and not (obj_type is type
                                   or obj_type in Pickler.dispatch):
            ...
            if numpydtype(obj_type):
                ...
                def save_numpy_dtype(pickler, obj):
                    ...
                    pickler.save_reduce(_create_dtypemeta, (obj.type, ),
                                        obj=obj)
            ...
            if ndarraysubclassinstance(obj_type):
                ...
                def save_numpy_array(pickler, obj):
                    ...
                    pickler.save_reduce(_create_array,
                                        (f, args, state, npdict),
                                        obj=obj)
                    logger.trace(pickler, "# Nu")
                    return
...
ENCODE_PARAMS = set(CODE_PARAMS).intersection([
    'code', 'lnotab', 'linetable', 'endlinetable', 'columntable',
    'exceptiontable'
])
...
def _create_code(*args):
        ...
        if m.case((
            ...
            if CODE_VERSION == (3, 11):
                return CodeType(
                    *args[:6],
                    args[6].encode()
                    if hasattr(args[6], 'encode') else args[6],  # code
                    *args[7:14],
                    args[14].encode()
                    if hasattr(args[14], 'encode') else args[14],  # linetable
                    args[15].encode() if hasattr(args[15], 'encode') else
                    args[15],  # exceptiontable
                    args[16],
                    args[17],
                )
            fields = m.fields
            ...
            if CODE_VERSION == (3, 10) or CODE_VERSION == (3, 8):
                return CodeType(
                    *args[:6],
                    ...
                    if hasattr(args[6], 'encode') else args[6],  # code
                    *args[7:13],
                    args[13].encode() if hasattr(args[13], 'encode') else
                    args[13],  # lnotab/linetable
                    args[14],
                    args[15],
                )
            fields = m.fields
        ...
        # Python 3.7 (15 args)
            ...
            if CODE_VERSION == (3, 7):
                return CodeType(
                    *args[:5],
                    args[5].encode()
                    if hasattr(args[5], 'encode') else args[5],  # code
                    *args[6:12],
                    args[12].encode()
                    if hasattr(args[12], 'encode') else args[12],  # lnotab
                    args[13],
                    args[14],
                )
            fields = m.fields
        # Python 3.11a (20 members)
            ...
            if CODE_VERSION == (3, 11, 'a'):
                return CodeType(
                    ...
                    *(a.encode() if hasattr(a, 'encode') else a
                      for a in args[14:18]),
                    args[18],
                    args[19],
                )
    ...
    args = (fields[k].encode() if k in ENCODE_PARAMS
            and hasattr(fields[k], 'encode') else fields[k]
...
try:
    ...
    MAPPING_PROXY_TRICK = __d is (DictProxyType(__d)
...
def _create_capsule(pointer, name, context, destructor):
    ...
    try:
        # based on
        # https://github.com/python/cpython/blob/f4095e53ab708d95e019c909d5928502775ba68f/Objects/capsule.c#L209-L231
...
def _getattribute(obj, name):
    for subpath in name.split('.'):
        ...
        try:
            ...
            raise AttributeError("Can't get attribute {!r} on {!r}".format(
                name, obj))
    return obj, parent
...
def _save_with_postproc(pickler,
                        reduction,
                        is_pickler_dill=None,
                        obj=Getattr.NO_DEFAULT,
                        postproc_list=None):
...
def save_code(pickler, obj):
    ...
    if hasattr(obj, "co_endlinetable"):  # python 3.11a (20 args)
        ...
        args = (obj.co_argcount, obj.co_posonlyargcount, obj.co_kwonlyargcount,
...
def save_module_dict(pickler, obj):
    ...
    elif (not is_dill(pickler, child=False)) and (obj
...
if not OLD310 and MAPPING_PROXY_TRICK:
    ...
    def save_dict_view(dicttype):
        ...
        return [(funcname,
                 save_dict_view_for_function(getattr(dicttype, funcname)))
    ...
    def save_dict_view(dicttype):
        ...
        return (('keys', save_dict_keys), ('values', save_dict_values),
...
def save_classobj(pickler, obj):  # FIXME: enable pickler._byref
    if not _locate_function(obj, pickler):
        ...
        pickler.save_reduce(ClassType,
                            (obj.__name__, obj.__bases__, obj.__dict__),
                            obj=obj)
...
def save_rlock(pickler, obj):
    ...
    pickler.save_reduce(_create_rlock, (
        count,
        owner,
    ), obj=obj)
...
def _save_file(pickler, obj, open_):
    ...
    pickler.save_reduce(_create_filehandle,
                        (obj.name, obj.mode, position, obj.closed, open_,
                         strictio, fmode, fdata),
                        obj=obj)
    return
...
if InputType:
    ...
    def save_stringi(pickler, obj):
        ...
        if obj.closed:
            ...
            value = obj.getvalue()
            position = obj.tell()
        pickler.save_reduce(_create_stringi, (value, position, obj.closed),
                            obj=obj)
    ...
    def save_stringo(pickler, obj):
        ...
        if obj.closed:
            ...
            value = obj.getvalue()
            position = obj.tell()
        pickler.save_reduce(_create_stringo, (value, position, obj.closed),
                            obj=obj)
        logger.trace(pickler, "# Io")
        return
...
if not IS_PYPY:
    ...
    def save_wrapper_descriptor(pickler, obj):
        ...
        pickler.save_reduce(_getattr,
                            (obj.__objclass__, obj.__name__, obj.__repr__()),
                            obj=obj)
...
def _is_builtin_module(module):
    ...
    names = [
        "base_prefix", "base_exec_prefix", "exec_prefix", "prefix",
        "real_prefix"
    ]
    rp = os.path.realpath
...
def _is_imported_module(module):
    return getattr(module, '__loader__',
                   None) is not None or module in sys.modules.values()
    ...
    if False:  # _use_diff:
        if obj.__name__.split('.', 1)[0] != "dill":
            try:
                ...
                pickler.save_reduce(_import_module, (obj.__name__, ),
                                    obj=obj,
                                    state=changed)
        ...
        if (obj.__name__ not in ("builtins", "dill", "dill._dill")
                and not builtin_mod or is_session_main):
            logger.trace(pickler, "M1: %s", obj)
            # Hack for handling module-type objects in load_module().
            mod_name = obj.__name__ if _is_imported_module(
                obj) else '__runtime__.%s' % obj.__name__
            # Second references are saved as __builtin__.__main__ in
            # save_module_dict().
            ...
            for item in IPYTHON_SINGLETONS:  # pragma: no cover
                if getattr(main_dict.get(item), '__module__',
                           '').startswith('IPython'):
                    del main_dict[item]
            pickler.save_reduce(_import_module, (mod_name, ),
                                obj=obj,
                                state=main_dict)
            logger.trace(pickler, "# M1")
...
def save_type(pickler, obj, postproc_list=None):
    if obj in _typemap:
        ...
        if not obj._field_defaults:
            _save_with_postproc(pickler,
                                (_create_namedtuple,
                                 (obj.__name__, obj._fields, obj.__module__)),
                                obj=obj,
                                postproc_list=postproc_list)
        else:
            defaults = [
                obj._field_defaults[field] for field in obj._fields
    ...
    # special caes: NoneType, NotImplementedType, EllipsisType, EnumMeta, etc
        ...
        if not _byref and not obj_recursive and incorrectly_named:  # not a function, but the name was held over
            ...
            _dict, attrs = _get_typedict_type(
                obj, obj.__dict__.copy(), None,
                postproc_list)  # copy dict proxy to a dict
            ...
            if isinstance(obj, abc.ABCMeta):
                ...
                _dict, attrs = _get_typedict_abc(obj, _dict, attrs,
                                                 postproc_list)
            ...
            if not hasattr(obj, '__orig_bases__'):
                ...
                _save_with_postproc(pickler,
                                    (new_class,
                                     (obj.__name__, obj.__orig_bases__,
                                      _metadict, _dict_update)),
                                    obj=obj,
                                    postproc_list=postproc_list)
    ...
    if not _locate_function(obj, pickler):
        ...
        if _recurse:
            ...
            if _main_modified and globs_copy is _original_main.__dict__:
                ...
                globs = globs_copy
            # If the globals is a module __dict__, do not save it in the
            # pickle.
        ...
        _save_with_postproc(pickler, (_create_function,
                                      (obj.__code__, globs, obj.__name__,
                                       obj.__defaults__, closure), state),
                            obj=obj,
                            postproc_list=postproc_list)
...
if HAS_CTYPES and hasattr(ctypes, 'pythonapi'):
    ...
    _testcapsule = _PyCapsule_New(ctypes.cast(_PyCapsule_New, ctypes.c_void_p),
                                  ctypes.c_char_p(_testcapsule_name), None)
    ...
    def save_capsule(pickler, obj):
        ...
        pickler.save_reduce(_create_capsule,
                            (pointer, name, context, destructor),
                            obj=obj)
...
def pickles(obj, exact=False, safe=False, **kwds):
    ...
    if safe:
        ...
        exceptions = (TypeError, AssertionError, NotImplementedError,
                      PicklingError, UnpicklingError)
    try:
        pik = copy(obj, **kwds)
        # FIXME: should check types match first, then check content if "exact"
        try:
            # FIXME: should be "(pik == obj).all()" for numpy comparison,
            # though that'll fail if shapes differ
        ...
        if result:
            return True
        if not exact:
            result = isinstance(pik, type(obj))
            if result:
                return result
...
def check(obj, *args, **kwds):
    ...
    *python* is the path to the python interpreter (defaults to sys.executable)
    ...
    msg = "SUCCESS" if not subprocess.call(msg.split(None,
                                                     2)) else "LOAD FAILED"
    if verbose:
        print(msg)
    return
